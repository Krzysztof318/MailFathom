...
internal sealed record FolderCopyFreshness
{
    ...
    [Description("The configured MailFathom account identifier the folder belongs to, unique within that account's user rather than across the deployment.")]
    public required string AccountId { get; init; }

    /// <summary>Gets the name that account is published under, within its user.</summary>
    [Description("The display name the account is published under, which is the operator's own name for the mailbox. It is unique within that account's user in the same way the identifier is.")]
