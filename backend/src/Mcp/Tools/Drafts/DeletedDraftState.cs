...
/// <summary>Publishes what giving up a draft did to the copy of it in the user's mailbox.</summary>
/// <remarks>
/// The draft is given up in every one of these — MailFathom holds nothing for it afterwards or is on its way to holding
/// nothing — and what they differ in is whether the user will still see a message in their drafts folder. That is the
...
internal enum DeletedDraftState
{
    ...
    [Description("The draft is gone and nothing is left in the user's drafts folder for it.")]
    Deleted = 0,

    /// <summary>The draft is gone and a copy the mail server would not give up stands in the mailbox.</summary>
    [Description("The draft is gone here, and one copy of it could not be taken out of the mailbox — the mail server refused it, or the folder it was put in is no longer the one this account means by drafts. The user may still see that message and can delete it in their own mail client; nothing here will touch it again.")]
    CopyLeftBehind = 1,

    /// <summary>The draft is marked as given up and the mailbox has not been settled yet.</summary>
    [Description("The draft is marked as given up and this deployment holds nothing further for it, but the mailbox could not be reached to take the copy out. A later pass finishes that, so the user may see the message in their drafts folder until it does.")]
    Pending = 2,
}
