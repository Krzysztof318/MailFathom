...
/// widened one <c>send_email</c> uses: the message the user wrote is gone and no call here brings it back. It is
...
/// copies that record names are the only messages the removal ever reaches — so a draft the user wrote in their own
...
internal sealed class DeleteDraftTool(MailDraftBook drafts)
{
    ...
    /// <remarks>The drafting grant, for the reason the contact book's erasure travels with its writing grant: a caller that may put a message into the user's drafts folder is the one that has to be able to take it back out again.</remarks>
    ...
    /// <returns>What giving the draft up did to the copy in the user's mailbox.</returns>
    ...
    [Description(
        "Gives up a draft this deployment holds and takes the copy of it back out of the user's Drafts folder. The "
        + "message the user wrote is GONE and no call here brings it back, so ask the person you are acting for "
        ...
        + "the user drafted in their own mail client is not one of them and is never touched. A draft that has "
        ...
        + "message is left as the user's to delete themselves, and nothing here touches it again.")]
