    ...
    ICallerMailAccountCatalog accountCatalog)
{
    ...
    /// <param name="accounts">The accounts to search, named by identifier or display name, or none to search every account the caller's user owns.</param>
    ...
    [Description(
        ...
        + "whose meaning is close and combines the two rankings. Where this server reads attachments, the text of "
        + "documents attached to a message is searched on the same terms its body is — by the words themselves and, "
        + "under hybrid, by meaning — and what an attachment contributed is reported in that result's attachmentMatches, "
        + "naming the file and the page, slide, or sheet, rather than folded into the message's own snippets. A picture "
        + "is different: where this server describes attached images, a picture is reachable only by meaning, never by "
        + "matching a word, and a result reached that way is marked isDepictedMatch and always sits below every result "
        + "any query word or any written text reached — so a photograph never displaces a message that contains the "
        + "word you searched for. Narrows by account, folder, sender address, recipient address, subject text, "
        ...
        + "a mail server, never marks mail as read, and never returns whole bodies, raw MIME, whole attachments, or a "
        + "whole attachment's text — every extract it returns is bounded. "
    ...
    public async Task<SearchEmailsToolResult> SearchEmailsAsync(
        [Description("The text to search for, up to 512 characters. Quoted phrases, OR, and a leading - to exclude a word are understood; every other punctuation mark is ordinary text. Write the words the mail itself is likely to contain, in the language it was written in rather than the language of your request: matching compares words rather than translating them, so a mailbox holding several languages is reached by a search per language. " + EmailSearchQueryText.MatchingDescription + " A search that finds nothing is worth repeating with fewer words before concluding that no mail matches. Required: a search with no text is a listing, which list_emails answers in a stable order and with a cursor.")]
        string queryText,
        [Description("MailFathom accounts to search, each named by the identifier the deployment generated for it or by the display name it is published under; the identifier is unique across the deployment and the display name within your own accounts. Omit to search every account you may read; call list_accounts to see what they are. At most 64 may be named, and a name that resolves to none of them is refused rather than answered with an empty window — the same refusal whether nothing carries that name or it names a mailbox that is not yours.")]
