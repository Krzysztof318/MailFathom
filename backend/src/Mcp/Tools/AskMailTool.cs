    ...
    ICallerMailAccountCatalog accountCatalog)
{
    ...
    /// <param name="accounts">The accounts the answer may be drawn from, named by identifier or display name, or none for every account the caller's user owns.</param>
    ...
    public async Task<AskMailToolResult> AskMailAsync(
        ...
        string question,
        [Description("MailFathom accounts the answer may be drawn from, each named by the identifier the deployment generated for it or by the display name it is published under; the identifier is unique across the deployment and the display name within your own accounts. Omit to draw on every account you may read; call list_accounts to see what they are. At most 64 may be named, and a name that resolves to none of them is refused rather than answered from the rest — the same refusal whether nothing carries that name or it names a mailbox that is not yours.")]
