...
internal sealed partial class ServedUsersStartupGate : IHostedService
{
    ...
    private readonly ILogger<ServedUsersStartupGate> logger;
    ...
    public ServedUsersStartupGate(
        ...
        ILogger<ServedUsersStartupGate> logger)
    {
    ...
    /// <inheritdoc />
    /// <exception cref="DeploymentUserUnresolvedException">Thrown when the roster this deployment holds is not a set of users it may serve.</exception>
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        {
            throw DeploymentUserUnresolvedException.TooManyUsers(ServedUsers.MaximumUsers);
        }

        var composed = await this.ServeEveryHeldUserAsync(scope, held, cancellationToken);
        IReadOnlyList<ServedUser> served = [.. composed.Select(entry => entry.User)];
        ...
        this.startupGates.MarkCompleted(HostStartupGate.ServedUsers);
    }
    ...
    /// </remarks>
    private async Task<IReadOnlyList<(ServedUser User, long Version)>> ServeEveryHeldUserAsync(
        AsyncServiceScope scope,
        IReadOnlyList<UserRecord> held,
        CancellationToken cancellationToken)
    {
        var served = new List<(ServedUser User, long Version)>(held.Count);
    ...
    /// </para>
    /// </remarks>
    private async Task<(ServedUser User, long Version)?> ServeFromTheOwnDocumentAsync(
        AsyncServiceScope scope,
        UserRecord record,
        CancellationToken cancellationToken)
    {
    ...
    /// <see cref="IDeploymentUserSource.User" />.
    /// </para>
    /// </remarks>
    private void RefuseSeveralUsersOnAUserFacingSurface(IReadOnlyList<ServedUser> served)
    {
        if (served.Count > 1 && this.admission.AdmitsACallerNamingNoUser)
        {
            throw DeploymentUserUnresolvedException.SeveralUsersOnAUserFacingSurface(this.admission.Refusal);
        }
    }
    ...
    /// roster decides.
    /// </remarks>
    private void ReportARuleSetTheRosterCannotAnswerFor(IReadOnlyList<ServedUser> served)
    {
    ...
    /// </remarks>
    private void Report(IReadOnlyList<ServedUser> served)
    {
        if (served.Count == 0)
        {
            this.LogNoUserHeld();
