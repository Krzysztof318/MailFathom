...
internal sealed partial class ClientTransportSecurityWarning : IHostedService
{
    ...
    [LoggerMessage(
        ...
        Message = "The client endpoint is enabled on {ClientRoutePrefix} with no authentication method configured, so "
            ...
            + "ClientEndpoint:Authentication naming a 'Method' of 'password', 'api-key', 'public-key', or "
            + "'oauth-subject', and provision a credential for each owner with 'mfctl credential create', unless the "
            + "address is reachable only from this machine or from a network you control.")]
