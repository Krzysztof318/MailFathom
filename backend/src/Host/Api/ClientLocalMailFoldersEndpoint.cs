...
internal static class ClientLocalMailFoldersEndpoint
{
    ...
    internal static ProblemHttpResult Refused(LocalMailFolderRefusal refusal)
    {
        var (status, detail) = refusal switch
        {
            ...
            LocalMailFolderRefusal.NameInvalid => (StatusCodes.Status400BadRequest, $"A folder name is 1 to {LocalMailFolderName.MaximumLength} characters, with no control character and no '{LocalMailFolderName.HierarchyDelimiter}'."),
            ...
            LocalMailFolderRefusal.AccountNotHeld => (StatusCodes.Status409Conflict, "MailFathom does not hold this account's mailbox, so its folders are the source server's."),
