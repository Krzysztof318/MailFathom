...
/// <summary>Authenticates a request against the keys this deployment provisioned for its users.</summary>
/// <remarks>
/// <para>
/// The handler is the adapter and nothing more: it lifts the credential out of the request, hands it to
/// <see cref="UserApiKeyAuthenticator" />, and turns the answer into the framework's own vocabulary. Every rule worth
...
internal sealed class UserApiKeyAuthenticationHandler : AuthenticationHandler<UserApiKeyAuthenticationSchemeOptions>
{
    private readonly UserApiKeyAuthenticator authenticator;

    /// <summary>Initializes a new user API key authentication handler.</summary>
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="authenticator" /> is <see langword="null" />.</exception>
    public UserApiKeyAuthenticationHandler(
        IOptionsMonitor<UserApiKeyAuthenticationSchemeOptions> schemeOptions,
        ILoggerFactory loggerFactory,
        UrlEncoder urlEncoder,
        UserApiKeyAuthenticator authenticator)
    ...
    protected override async Task<AuthenticateResult> HandleAuthenticateAsync()
    {
        ...
        // this request outlives it, and disabling the credential is what ends that session.
