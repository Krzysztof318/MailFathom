...
internal readonly record struct TransportSurface
{
    ...
    /// <remarks>
    /// It serves the attachment download route beneath its prefix as well as the protocol route. The two carry opposite
    /// credential policies — one requires whatever the endpoint configured, the other admits a signed capability and
    /// nothing else — and they are still one surface, because what a surface bounds is the process's own capacity rather
    /// than a caller's authority: both read the same mailbox, hold the same response streams open, and are enabled and
    /// disabled together.
    /// </remarks>
    ...
    /// <returns><see langword="true" /> when the path is beneath this surface's prefix.</returns>
    /// <exception cref="InvalidOperationException">Thrown when the value is the struct default rather than a surface.</exception>
    /// <remarks>
    /// Matched by segment, so a path merely beginning with the same characters is not claimed. It matters beyond
    /// isolation: the process-wide rate limiter rides on one application-wide limiter and gives whatever it does not
    /// recognize no limiter at all, so a route this method failed to claim would be served with no concurrency bound
    /// rather than with a wrong one — which is why every route a surface serves sits beneath its one prefix.
    /// </remarks>
    internal bool Serves(PathString path) => path.StartsWithSegments(this.RoutePrefix);
