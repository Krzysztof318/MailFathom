...
/// answers one indistinguishable refusal rather than reporting that a row was missing. The policy asks for the user
...
internal static partial class TransportSecurityExtensions
{
        ...
        bool exchangesCredentialsForSessions)
    {
        ...
        var acceptsApiKey = UserFacingAuthenticationConfiguration.Accepts(methods, UserCredentialMethod.ApiKey);
        var acceptsPublicKey = UserFacingAuthenticationConfiguration.Accepts(methods, UserCredentialMethod.PublicKey);
        var basicMethod = UserFacingAuthenticationConfiguration.BasicMethodIn(methods);
        ...
        if (acceptsApiKey)
        {
            authentication.AddScheme<UserApiKeyAuthenticationSchemeOptions, UserApiKeyAuthenticationHandler>(
        ...
        AddUserFacingAuthorizationPolicy(services, surface, oauthMethods);
    ...
    /// reaches by reading the user claim — so the refusal here is the same refusal an unknown API key meets, arrived
        ...
        bool narrowedByTokenScopes)
    {
        ...
        var admitted = await context.HttpContext.RequestServices
            .GetRequiredService<UserOAuthSubjectResolver>()
        ...
        services.AddAuthorization(authorizationOptions => authorizationOptions.AddPolicy(
            ...
            policy => policy
                ...
                .RequireAssertion(context =>
                    TransportAccessPolicy.IsUserAuthorized(context.User, requiredScopesByIssuer))));
    }
}
