...
/// <summary>Judges the assertion a request presented against the public keys this deployment holds for its users.</summary>
/// <remarks>
/// <para>
...
/// naming a key registered for another user is refused by that key's own signature check rather than admitted as that
/// user, and one naming nothing this deployment holds is refused for the same reason a key nobody holds is.
/// </para>
/// <para>
...
/// </para>
/// </remarks>
internal sealed partial class UserClientAssertionAuthenticator
{
    private readonly IUserCredentialStore credentials;
    private readonly ClientAssertionReplayStore replayStore;
    private readonly TimeProvider timeProvider;
    private readonly ILogger<UserClientAssertionAuthenticator> logger;

    /// <summary>Initializes a new user client assertion authenticator.</summary>
    /// <param name="credentials">Where the users' credentials are kept.</param>
    ...
    public UserClientAssertionAuthenticator(
        IUserCredentialStore credentials,
        ClientAssertionReplayStore replayStore,
        TimeProvider timeProvider,
        ILogger<UserClientAssertionAuthenticator> logger)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return UserClientAssertionAuthenticationResult.Rejected(
        ...
        {
            return UserClientAssertionAuthenticationResult.Rejected(ClientAssertionRejection.NotAnAssertion);
        }

        if (!UnverifiedJsonWebToken.TryReadKeyId(presentedAssertion, out var keyId)
            || !UserCredentialLookup.TryCreate(keyId, out var fingerprint))
        {
    ...
    private async Task<UserClientAssertionAuthenticationResult> VerifyAsync(
        string audience,
        ...
        CancellationToken cancellationToken)
    {
        ...
        return UserClientAssertionAuthenticationResult.Rejected(ClientAssertionRejection.IdentifierAlreadySpent);
    }
...
/// <summary>The outcome of judging one presented assertion against the public keys a user registered.</summary>
/// <remarks>
...
/// establishes: the credential that matched, the user the request acts for, and what that request may do.
/// </remarks>
internal sealed record UserClientAssertionAuthenticationResult
{
    private UserClientAssertionAuthenticationResult(
    ...
    public AdmittedUserCredential? Admitted { get; }
    ...
    /// <param name="admitted">The credential that matched, the user it names, and what it grants.</param>
    /// <returns>The successful result.</returns>
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="admitted" /> is <see langword="null" />.</exception>
    public static UserClientAssertionAuthenticationResult Authenticated(AdmittedUserCredential admitted)
    {
        ArgumentNullException.ThrowIfNull(admitted);

        return new UserClientAssertionAuthenticationResult(admitted, rejection: null);
    }
    ...
    public static UserClientAssertionAuthenticationResult Rejected(ClientAssertionRejection rejection) =>
