...
/// <summary>How one user asked their own mail to be classified, and what is to happen to their junk.</summary>
/// <remarks>
/// <para>
/// The user's half of <see cref="SpamClassificationOptions" />, and deliberately only that half. Junk is a judgement
...
/// in the deployment's own section, and there is no key here that shadows one — a user record layers over nothing.
/// </para>
/// <para>
/// Every switch is off when the block is absent, so a user whose record states none of it is classified as nothing,
/// which is the same answer a deployment that configured nothing gives. That is what the marker on their row decides
/// rather than this type: a user still read from configuration takes the deployment's section, and one whose document
...
internal sealed class UserSpamClassificationOptions
{
    /// <summary>The property a user's record holds this block under.</summary>
    internal const string RecordProperty = "SpamClassification";

    /// <summary>The path a user's record holds the junk actions under, as a refusal names it.</summary>
    internal const string ActionsPath = $"{RecordProperty}:{nameof(Actions)}";

    /// <summary>Gets or sets whether this user's mail is classified at all.</summary>
    ...
    /// rather than choosing it. A user who switches it on where the deployment registered none is answered by the
    /// deterministic stage alone, exactly as the deployment's own section is.
    /// </remarks>
    public bool UseScanner { get; set; }

    /// <summary>Gets or sets the folder aliases this user's mail is classified over, or <see langword="null" /> to take each of their accounts' inbox.</summary>
    /// <remarks>
    /// Nullable so that leaving the key out and writing an empty list stay distinguishable, which an <c>IList</c>-typed
    /// property could not express. Each alias is resolved within this user's own mail accounts, so one only somebody
    ...
    /// <summary>Gets or sets the score at or above which a scanner's verdict is spam for this user, or <see langword="null" /> to take the scanner's own.</summary>
    public double? ScannerThreshold { get; set; }

    /// <summary>Gets or sets what happens to mail a verdict calls this user's junk.</summary>
    /// <remarks>
    ...
    /// </remarks>
    public UserSpamActionOptions Actions { get; set; } = new();

    /// <summary>Finds everything about this block that would otherwise be discovered on the user's own mail.</summary>
    /// <param name="accounts">The user's own mail accounts, which are the only ones a folder may resolve within.</param>
    ...
    private IEnumerable<ValidationResult> FindPostureRefusals()
    {
        if (this.UseScanner && !this.Enabled)
        {
            yield return new ValidationResult(
                $"The user record asks for a scanner while {nameof(this.Enabled)} is false, and a scanner is only consulted where classification runs. Set it to true, or remove {nameof(this.UseScanner)}.",
        ...
        {
            yield return new ValidationResult(
                $"The user record names scanned folder '{alias}', which is not a usable folder alias.",
