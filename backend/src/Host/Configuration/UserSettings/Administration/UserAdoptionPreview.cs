...
/// <summary>What adopting one user would move out of this deployment's files and into their record.</summary>
/// <param name="User">The user the adoption is for.</param>
...
/// setting: it is what stops deciding this user's mailboxes once the adoption commits, and it is where somebody would
/// otherwise go on editing them with no effect. A user whose record is already their own previews nothing and is
...
internal sealed record UserAdoptionPreview(
    ...
    IReadOnlyList<UserAdoptableMailAccount> MailAccounts,
    IReadOnlyList<UserAdoptableRecordSetting> Classification,
    IReadOnlyList<UserAdoptableRecordSetting> SensitiveContent)
{
    /// <summary>Gets whether there is an adoption to perform at all.</summary>
    /// <remarks>False for a user whose record is already their own, which is the repeat of an adoption that has run.</remarks>
    public bool HasSomethingToAdopt => this.Source != MailUserAccountSource.UserDocument;
}

/// <summary>One mail account an adoption would move into the user's record.</summary>
...
/// </remarks>
internal sealed record UserAdoptableMailAccount(string AccountId, string DisplayName);

/// <summary>One setting an adoption would commit into the user's record beside their mailboxes.</summary>
...
/// its value reads the same whether it is about to be switched on or off, two of these settings write to that user's
...
/// </remarks>
internal sealed record UserAdoptableRecordSetting(string Path, string Value)
{
    ...
    internal static UserAdoptableRecordSetting For(ConfigurationEdit edit)
    {
        ArgumentNullException.ThrowIfNull(edit);

        return new UserAdoptableRecordSetting(
            edit.Path,
