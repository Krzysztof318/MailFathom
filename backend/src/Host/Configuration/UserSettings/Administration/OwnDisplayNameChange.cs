...
/// <param name="UserHeld">Whether this deployment holds a record for the person the change was made for.</param>
...
/// <para>
/// The absence of a record is apart from a refusal for the reason <see cref="UserRelabelOutcome" /> keeps them apart:
/// a person this deployment does not hold is the same answer as at every other user-scoped route, while a name
/// somebody else carries is about the name rather than about the person.
/// </para>
/// </remarks>
internal readonly record struct OwnDisplayNameChange(bool UserHeld, string? Recorded, string? RefusalMessage)
{
    /// <summary>Gets the outcome of a person this deployment holds no record for.</summary>
    internal static OwnDisplayNameChange NoSuchUser { get; } = new(UserHeld: false, Recorded: null, RefusalMessage: null);
    ...
    internal static OwnDisplayNameChange Recording(string recorded)
    {
        ...
        return new OwnDisplayNameChange(UserHeld: true, recorded, RefusalMessage: null);
    }
    ...
    internal static OwnDisplayNameChange Refused(string refusalMessage)
    {
        ...
        return new OwnDisplayNameChange(UserHeld: true, Recorded: null, refusalMessage);
    }
}
