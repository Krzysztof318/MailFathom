...
/// <summary>Turns the document one user's row holds into their typed record, or says why it is not one.</summary>
/// <remarks>
/// <para>
...
/// no value in it shadows a setting the deployment made, and a user-level setting is only ever a property the record
...
internal sealed class UserAccountDocumentBinder(
    ...
    IOptions<SensitiveContentOptions> sensitiveContent)
{
    ...
    public UserAccountBinding Bind(string json, UserRecordArrival arrival)
    {
        ...
        if (writtenOctets > UserSettingsDocument.MaximumOctets)
        {
            return UserAccountBinding.Refused(
            [
                $"The user record is {writtenOctets} octets as it was written, past the {UserSettingsDocument.MaximumOctets} MailFathom binds a user's document from — and the database stores it larger still. A user record is a page of declarations rather than a payload: check what wrote the settings_accounts row.",
            ]);
        }
        ...
        {
            return UserAccountBinding.Refused([NotADocument]);
        }

        if (persistedOctets > UserSettingsDocument.MaximumOctets)
        {
            return UserAccountBinding.Refused([PastTheCeiling(persistedOctets)]);
        }
    ...
    private const string NotADocument =
        "The user record is not a JSON object MailFathom can read. A user record is a document of settings, so check what wrote the settings_accounts row rather than the settings themselves.";
    ...
    private static string PastTheCeiling(int octets) =>
        $"The user record is {octets} octets as the database stores it, past the {UserSettingsDocument.MaximumOctets} MailFathom binds a user's document from. A user record is a page of declarations rather than a payload: check what wrote the settings_accounts row.";
    ...
    private UserAccountBinding Judge(IConfiguration document, UserRecordArrival arrival)
    {
        UserAccountOptions user;

        try
        {
            user = document.Get<UserAccountOptions>(binder => binder.ErrorOnUnknownConfiguration = true)
                ?? new UserAccountOptions();
        }
        catch (InvalidOperationException refusal)
        {
            return UserAccountBinding.Refused([BindingRefusalFor(refusal)]);
        }

        var refusals = new List<ValidationResult>();

        Validator.TryValidateObject(user, new ValidationContext(user), refusals, validateAllProperties: true);
        ...
        return refusals.Count > 0
            ? UserAccountBinding.Refused([.. refusals.Select(refusal => refusal.ErrorMessage ?? "The user record is invalid.")])
            : UserAccountBinding.Bound(user);
    }

    /// <summary>Finds every setting of the document carrying a secret's material where a reference belongs.</summary>
    /// <remarks>
    /// Every one of them is reported rather than the first, because a user correcting one at a time would learn
    ...
    /// </para>
    /// <para>
    /// A key that does name something is still a user's own text, and the rule this scan asks reads the last
