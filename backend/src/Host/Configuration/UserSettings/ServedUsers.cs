...
internal sealed class ServedUsers : IDeploymentUserSource
{
    ...
    private readonly Dictionary<UserId, long> publishedDocumentVersions = [];
    ...
    /// </remarks>
    private IReadOnlyList<ServedUser>? resolvedUsers;
    ...
    internal IReadOnlyList<ServedUser>? TryGetUsers()
    {
        lock (this.mutex)
        {
    ...
    /// <exception cref="DeploymentUserUnresolvedException">Thrown when this deployment serves nobody, or serves more than one user, and there is therefore no sole user to name.</exception>
    /// <remarks>
    /// <para>
        ...
        get
        {
            lock (this.mutex)
            {
                return (this.EstablishedRoster(), this.withheldUser) switch
                {
                    ...
                    ([], { } withheld) => withheld,
                    ([], null) => throw DeploymentUserUnresolvedException.NoUserToActFor(),
    ...
    /// </remarks>
    public (UserId User, MailSynchronizationAccountOptions Account)? FindAccount(MailAccountId accountId) =>
        this.Users
            ...
            .Where(entry => StringComparer.Ordinal.Equals(
                ...
                accountId.Value))
            .Cast<(UserId User, MailSynchronizationAccountOptions Account)?>()
            .FirstOrDefault();
    ...
    internal void Resolved(
        IReadOnlyList<ServedUser> users,
        IReadOnlyDictionary<UserId, long>? documentVersions = null)
    {
        ...
        lock (this.mutex)
        {
            ...
            foreach (var (user, version) in documentVersions ?? new Dictionary<UserId, long>())
            {
    ...
    internal long? PublishedVersionOf(UserId user)
    {
        lock (this.mutex)
        {
    ...
    internal Withholding Withhold(UserId user)
    {
        ...
        IReadOnlyList<ServedUser> withheldRoster;
    ...
    /// </remarks>
    private void Restore(UserId user, IReadOnlyList<ServedUser> withheldRoster, long? withheldVersion)
    {
        lock (this.mutex)
        {
    ...
    /// <exception cref="InvalidOperationException">Thrown when the startup gate that establishes the roster has not yet run.</exception>
    private IReadOnlyList<ServedUser> EstablishedRoster() =>
        this.resolvedUsers
            ?? throw new InvalidOperationException(
