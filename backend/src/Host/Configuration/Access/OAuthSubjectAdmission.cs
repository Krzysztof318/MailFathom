...
/// administrative endpoint serves the deployment administrator, who is nobody's user and holds no record, so the
/// profile's own list is the whole of who may sign in and a profile naming none would serve whoever the authorization
/// server's tenant holds. A mail-serving endpoint serves users, and a subject there resolves one user's credential
...
internal enum OAuthSubjectAdmission
{
    ...
    /// <summary>A subject resolves a user's credential record, so the profile names none and one written there is refused as a retired setting.</summary>
    ResolvedUserCredentials = 1,
}
