...
internal static class UserFacingAuthenticationConfiguration
{
    ...
    /// upgrading into the user axis would come up admitting nobody it used to admit and reporting nothing about it.
    ...
    {
        ["ApiKey"] =
            "a key a user's client presents is provisioned for that user rather than configured here, because it "
        ...
        ["PublicKey"] =
            "a client public key is registered against the user whose mail its assertions reach. Write an entry "
    ...
    internal static UserFacingAuthenticationOptions? BasicMethodIn(
        IEnumerable<UserFacingAuthenticationOptions> methods)
    {
        ArgumentNullException.ThrowIfNull(methods);

        return methods.FirstOrDefault(method => method.AcceptedMethod == UserCredentialMethod.Password);
    }
    ...
    internal static string SettingPathOf(
        string sectionName,
        UserFacingAuthenticationOptions method,
        int boundPosition) =>
    ...
    private static IEnumerable<string> FindRepeatedMethodErrors(
        ...
        IReadOnlyList<UserFacingAuthenticationOptions> methods)
    {
        var accepted = new HashSet<UserCredentialMethod>();

        foreach (var (index, method) in methods.Index())
        {
            var acceptedMethod = method.AcceptedMethod;

            if (acceptedMethod == UserCredentialMethod.OAuthSubject)
            {
                continue;
            }
            ...
            {
                yield return $"{SettingPathOf(sectionName, method, index)}:{nameof(UserFacingAuthenticationOptions.Method)} — an earlier entry already accepts '{acceptedMethod.Name}', and this endpoint accepts each method once. The credentials are rows rather than entries, so a second one is provisioned through the administrative endpoint rather than written here.";
            }
        }
    }
        ...
        IReadOnlyList<UserFacingAuthenticationOptions> methods)
    {
        ...
        foreach (var (index, method) in methods.Index())
        {
            ...
            {
                var settingPath =
                    $"{SettingPathOf(sectionName, method, index)}:{nameof(UserFacingAuthenticationOptions.OAuth)}:{nameof(OAuthValidationOptions.AuthorizationServers)}:{serverIndex}";
