...
/// </para>
/// <para>
/// The published names are the ones <see cref="UserCredentialMethod" /> publishes, which are the same names
...
internal sealed class UserFacingAuthenticationOptions
{
    ...
    /// <summary>Gets or sets how often a user's password may be tried, where this entry accepts one.</summary>
    /// <remarks>
    ...
    public UserCredentialMethod AcceptedMethod =>
        UserCredentialMethod.TryParse(this.Method, out var method) ? method : default;
    ...
    private IEnumerable<string> FindBlockErrors(string settingPath)
    {
        ...
        if (this.Basic is { } basic)
        {
            ...
            {
                yield return $"{settingPath}:{nameof(this.Basic)} — this entry accepts '{method.Name}', and the block bounds how often a password may be tried. Move it to an entry naming '{UserCredentialMethod.Password.Name}', or remove it.";
            }
            else
            {
        ...
        {
            yield return $"{settingPath}:{nameof(this.OAuth)} — this entry accepts '{method.Name}', which no authorization server is involved in. Move the block to an entry naming '{UserCredentialMethod.OAuthSubject.Name}', or remove it.";
        }
    }

    private IEnumerable<string> FindGrantErrors(string settingPath)
    {
        if (this.PermissionsFromTokenScopes && this.AcceptedMethod != UserCredentialMethod.OAuthSubject)
        {
            yield return $"{settingPath}:{nameof(this.PermissionsFromTokenScopes)} — a token's scopes can narrow a grant and a '{this.AcceptedMethod.Name}' credential carries none, so this entry cannot be read both ways. Write it on an entry naming '{UserCredentialMethod.OAuthSubject.Name}', or remove it.";
        }
    }
