...
/// <summary>Draws the keys a user's clients present, and reduces a presented one to its stored digest.</summary>
/// <remarks>
/// <para>
...
/// user axis needs — it is deterministic, so it can be an index, so one request resolves one row rather than being
...
/// </para>
/// </remarks>
internal sealed class UserApiKeyMinter : IUserApiKeyMinter
{
    ...
    public MintedUserApiKey Mint()
    {
        ...
        try
        {
            ...
            return TryDigestOf(key, out var lookup)
                ? new MintedUserApiKey(key, lookup)
    ...
    /// </remarks>
    private static bool TryDigestOf(ReadOnlySpan<char> key, out UserCredentialLookup lookup)
    {
        ...
        try
        {
            ...
            return UserCredentialLookup.TryCreate(Base64Url.EncodeToString(digest), out lookup);
        }
        finally
        {
