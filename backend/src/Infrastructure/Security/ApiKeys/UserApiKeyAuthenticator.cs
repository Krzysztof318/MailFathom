...
/// <summary>Judges the key a request presented against the credentials this deployment holds for its users.</summary>
/// <remarks>
/// <para>
...
/// </para>
/// </remarks>
public sealed class UserApiKeyAuthenticator
{
    private readonly IUserCredentialStore credentials;
    private readonly IUserApiKeyMinter minter;

    /// <summary>Initializes a new user API key authenticator.</summary>
    ...
    public UserApiKeyAuthenticator(IUserCredentialStore credentials, IUserApiKeyMinter minter)
    {
    ...
    public async Task<UserApiKeyAuthenticationResult> AuthenticateAsync(
        string? authorizationHeaderValue,
        CancellationToken cancellationToken)
    {
        if (!BearerCredentialHeader.TryRead(authorizationHeaderValue, out var presentedKey))
        {
            return UserApiKeyAuthenticationResult.Rejected(string.IsNullOrWhiteSpace(authorizationHeaderValue)
        ...
        {
            return UserApiKeyAuthenticationResult.Rejected(ApiKeyRejection.CredentialMalformed);
        }
        ...
        return credential is { Enabled: true }
            ? UserApiKeyAuthenticationResult.Authenticated(AdmittedUserCredential.For(credential))
            : UserApiKeyAuthenticationResult.Rejected(ApiKeyRejection.CredentialUnrecognized);
    }
}

/// <summary>The outcome of judging one presented key against the credentials a user holds.</summary>
/// <remarks>
...
/// </remarks>
public sealed record UserApiKeyAuthenticationResult
{
    private UserApiKeyAuthenticationResult(AdmittedUserCredential? admitted, ApiKeyRejection? rejection)
    {
    ...
    public static UserApiKeyAuthenticationResult Authenticated(AdmittedUserCredential admitted)
    {
        ArgumentNullException.ThrowIfNull(admitted);

        return new UserApiKeyAuthenticationResult(admitted, rejection: null);
    }
    ...
    public static UserApiKeyAuthenticationResult Rejected(ApiKeyRejection rejection) =>
