...
/// <summary>Judges the credential a request presented against the passwords this deployment stores for its users.</summary>
/// <remarks>
/// <para>
...
/// credential whose user record has since been removed all return
/// <see cref="UserPasswordRejection.CredentialUnrecognized" />, and each of them spends one key derivation before it
...
/// a user removes their credentials with them — and it is refused along the same path anyway rather than being
...
/// password paid into would bound the user's own traffic rather than anybody's guessing. Only a wrong password spends
...
/// </para>
/// </remarks>
public sealed partial class UserPasswordAuthenticator
{
    ...
    private readonly ILogger<UserPasswordAuthenticator> logger;

    /// <summary>Initializes a new user password authenticator.</summary>
    ...
    public UserPasswordAuthenticator(
        ...
        ILogger<UserPasswordAuthenticator> logger)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                return UserPasswordAuthenticationResult.Rejected(string.IsNullOrWhiteSpace(authorizationHeaderValue)
                    ? UserPasswordRejection.CredentialMissing
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return UserPasswordAuthenticationResult.Rejected(UserPasswordRejection.CredentialUnrecognized);
        }
