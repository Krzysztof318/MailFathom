...
/// </para>
/// <para>
/// What every line carries is the act, the credential's identifier, the user's identifier, the administrator the
...
/// </para>
/// </remarks>
internal sealed partial class LoggedUserCredentialAuditor(ILogger<LoggedUserCredentialAuditor> logger)
    : IUserCredentialAuditor
{
    /// <inheritdoc />
    public Task RecordCredentialChangeAsync(UserCredentialChange change, CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(change);

        switch (change.Act)
        {
            case UserCredentialAct.Provisioned:
            case UserCredentialAct.Enabled:
    ...
    [LoggerMessage(
        ...
        Message = "User credential {CredentialId} of user {UserId} was {CredentialAct} by {ActingAdministrator} at "
            + "{OccurredAt}. The username it carries is deliberately not recorded; read it from the user's credential "
            + "listing.")]
    private partial void LogCredentialOpened(
        UserCredentialAct credentialAct,
        Guid credentialId,
        Guid userId,
    ...
    [LoggerMessage(
        ...
        Message = "User credential {CredentialId} of user {UserId} was {CredentialAct} by {ActingAdministrator} at "
            ...
            + "deliberately not recorded; read it from the user's credential listing where the credential still "
            + "exists.")]
