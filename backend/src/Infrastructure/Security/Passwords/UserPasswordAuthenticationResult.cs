...
{
    private UserPasswordAuthenticationResult(
        AdmittedUserCredential? admitted,
        UserPasswordRejection? rejection)
    {
    ...
    public UserPasswordRejection? Rejection { get; }
        ...
        IReadOnlyList<MailFathomPermission> permissions)
    {
        ...
        {
            throw new ArgumentException(
                "An authenticated result must name the user the request acts for.",
                nameof(user));
        }
    ...
    public static UserPasswordAuthenticationResult Rejected(UserPasswordRejection rejection) =>
