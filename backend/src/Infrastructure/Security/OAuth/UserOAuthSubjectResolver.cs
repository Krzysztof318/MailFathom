...
/// <summary>Resolves the user a validated access token acts for, from the subject its issuer signed.</summary>
/// <remarks>
/// <para>
/// A token proves which person an authorization server signed in and can prove nothing about whose mail that person
/// reaches, because the server knows nothing about this deployment's users. The mapping is what supplies the missing
/// half, and it is a credential row like every other: a user, a lookup composed from the issuer and the subject
...
/// admitted for nobody in particular is the arrangement the user axis exists to remove.
/// </para>
/// </remarks>
public sealed class UserOAuthSubjectResolver
{
    ...
    public UserOAuthSubjectResolver(IUserCredentialStore credentials)
    {
    ...
    /// <summary>Reports which user one validated subject acts for.</summary>
    ...
    public async Task<AdmittedUserCredential?> ResolveAsync(
        string? issuer,
        string? subject,
        CancellationToken cancellationToken)
    {
        ...
        return credential is { Enabled: true } ? AdmittedUserCredential.For(credential) : null;
    }
}
