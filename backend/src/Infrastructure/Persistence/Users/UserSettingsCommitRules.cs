...
/// <summary>Decides whether a user's record may be handed to a statement at all.</summary>
/// <remarks>
...
/// this document's.
/// </remarks>
internal static class UserSettingsCommitRules
{
    ...
    internal static void RefuseWhatCannotBeCommitted(string json, long expectedVersion)
    {
        ...
        try
        {
            if (!RootSettingsCommitRules.RootIsAnObject(json))
            {
                throw new ArgumentException(
                    "The candidate user record is JSON whose root is not an object, so it carries no settings at all. The column would store it, and the next read would refuse it: only an object binds to a user's record.",
                    nameof(json));
            }
        ...
        {
            throw new ArgumentException(
                "The candidate user record is not JSON, so no statement is issued for it. The column would take it no further: its jsonb cast refuses exactly this, and the refusal belongs on the side that composed the document.",
                nameof(json),
                exception);
        }
        ...
        {
            throw new ArgumentException(
                $"The candidate user record occupies {persistedOctets} octets as the database stores it, past the {UserSettingsDocument.MaximumOctets} this build binds a user's record from, so persisting it would leave a row the next read refuses.",
                nameof(json));
        }
    }
}
