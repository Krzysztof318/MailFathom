...
/// <summary>One user's persisted record: the envelope the deployment keys them by, and the document they configured.</summary>
/// <param name="User">The user the row belongs to.</param>
    ...
    bool WrittenAtRuntime)
{
    ...
    /// dictionary the binder reads. A ceiling a user's declarations could plausibly reach would be the wrong
    /// ceiling; this one is far past every mail account and user-level setting one person configures, and far below
    ...
    /// bind accepts would persist a record the next read refuses, and the user would be locked out by a change that
