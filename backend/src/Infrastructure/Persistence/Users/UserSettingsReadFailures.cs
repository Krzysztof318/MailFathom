...
/// <summary>Turns the driver's answer to a failed user-record read into the operator's diagnosis.</summary>
/// <remarks>
/// <para>
...
/// carries the user's document either, which is that person's configuration rather than a diagnostic.
/// </para>
/// <para>
...
/// </para>
/// </remarks>
internal static class UserSettingsReadFailures
{
    ...
    private const string Unreachable =
        "The database holding the user records could not be reached, so the request naming that user was refused rather than answered from a record MailFathom does not have.";
    ...
    internal static string DiagnoseWhileReading(NpgsqlException exception) =>
        ...
        ?? exception switch
        {
            ...
            { InnerException: TimeoutException } =>
                "The statement reading a user's record did not finish within the configured command timeout. The database server was reached and accepted the connection, so what to look at is Persistence:CommandTimeoutSeconds and whatever is holding the settings_accounts row, rather than the network.",
    ...
    private static string? WhatTheServerAnswered(NpgsqlException exception) => exception switch
    {
        ...
        PostgresException { SqlState: PostgresErrorCodes.InvalidCatalogName } =>
            "The database server carries no database of the configured name, so no user record could be read. Create it, or correct the name in the connection settings: the server was reached and answered, so neither the network nor the credential is what refused MailFathom.",

        // How a database missing this build's migration answers.
        PostgresException { SqlState: PostgresErrorCodes.UndefinedTable } =>
            "The database does not carry the settings_accounts table this build reads a user's record from. Apply the migrations this build defines: until they are applied the request is refused while the deployment goes on running.",
        ...
        PostgresException { SqlState: PostgresErrorCodes.InvalidPassword } =>
            "The database holding the user records refused the configured credential. Check the Persistence secret block rather than the network: the server answered, and what it rejected is the password MailFathom composed for it.",
        ...
        PostgresException { SqlState: PostgresErrorCodes.InvalidAuthorizationSpecification } =>
            "The database server admits no connection for the configured role, host, and database, so no user record could be read. Its authorization rules are what refused MailFathom, so the credential and the network are both beside the point.",
        ...
        PostgresException { SqlState: PostgresErrorCodes.InsufficientPrivilege } =>
            "The serving role holds no privilege on settings_accounts. Grant it on the table a user's record lives in, the way the schema documentation describes for a schema applied by one role and served by another.",
