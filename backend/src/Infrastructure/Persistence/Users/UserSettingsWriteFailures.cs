...
/// <summary>Turns the driver's answer to a refused user-record write into the operator's diagnosis.</summary>
/// <remarks>
/// <para>
/// The sibling of <see cref="UserSettingsReadFailures" /> and separate from it for the reason the two exceptions are:
...
/// </para>
/// </remarks>
internal static class UserSettingsWriteFailures
{
    private const string Unreachable =
        "The database holding the user records could not be reached, so nothing was written. The user's record is exactly what it was, and the write is safe to attempt again.";
    ...
    private const string WhetherItApplied =
        "whether it applied is not known from here. Read the user's record to find out which version is now in force, and attempt the write again over the version it was composed on — which the version guard refuses if the first attempt did commit.";
    ...
    internal static string Diagnose(NpgsqlException exception) =>
        ...
        ?? exception switch
        {
            ...
            { InnerException: TimeoutException } =>
                $"The statement writing a user's record did not finish within the configured command timeout, so {WhetherItApplied} What to look at is Persistence:CommandTimeoutSeconds and whatever is holding the settings_accounts row, rather than the network.",
            ...
            _ =>
                $"The connection to the database was lost while the statement writing a user's record was in flight, so {WhetherItApplied} What to look at is the network and the database's own log, rather than anything MailFathom composed.",
        };
    ...
    private static string? WhatTheServerAnswered(NpgsqlException exception) => exception switch
    {
        ...
        PostgresException { SqlState: PostgresErrorCodes.UndefinedTable } =>
            "The database does not carry the settings_accounts table a user's record is written to, so nothing was written. Apply the migrations this build defines.",
        ...
        PostgresException { SqlState: PostgresErrorCodes.InvalidPassword } =>
            "The database holding the user records refused the configured credential, so nothing was written. Check the Persistence secret block rather than the network: the server answered, and what it rejected is the password MailFathom composed for it.",
        ...
        PostgresException { SqlState: PostgresErrorCodes.InsufficientPrivilege } =>
            "The serving role holds no privilege to update settings_accounts, so nothing was written. Grant UPDATE on the table a user's record lives in; a role granted only SELECT serves every start and refuses every write, which is why this is the first failure a deployment meets here and never meets on a read.",
        ...
        PostgresException { SqlState: { Length: > 0 } sqlState } =>
            $"The database refused the statement that would have written the user's record, answering SQLSTATE {sqlState}, so nothing was written. The server was reached and answered, so what to correct is the statement's own subject rather than the network; the provider's own text is the inner exception.",
