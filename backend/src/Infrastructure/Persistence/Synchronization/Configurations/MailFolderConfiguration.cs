...
internal sealed class MailFolderConfiguration : IEntityTypeConfiguration<MailFolderEntity>
{
    ...
    public void Configure(EntityTypeBuilder<MailFolderEntity> entity)
    {
        ...
        // The user leads it for the reason it leads every other account-narrowed structure: an account identifier
        // names one account within its user, so uniqueness is a statement about one user's mailbox rather than about
        // a deployment-wide namespace. Nothing that bound before this change is refused after it, the user being
        ...
        // would point at one mailbox in one user's hands and at a different one in another's. The unique index
