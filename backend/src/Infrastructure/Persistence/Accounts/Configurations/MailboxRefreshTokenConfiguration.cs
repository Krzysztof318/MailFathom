...
internal sealed class MailboxRefreshTokenConfiguration : IEntityTypeConfiguration<MailboxRefreshTokenEntity>
{
    ...
    public void Configure(EntityTypeBuilder<MailboxRefreshTokenEntity> entity)
    {
        ...
        // One token per account, and an account is the user and the identifier together — so the key is the pair
        // rather than the identifier, which names one mailbox within its user and another within the next.
        entity.HasKey(token => new { token.UserId, token.MailboxAccountId });
