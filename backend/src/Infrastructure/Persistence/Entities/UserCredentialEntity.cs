...
/// <summary>One credential a user is admitted by, whichever method presents it.</summary>
/// <remarks>
/// <para>
/// A row of its own rather than a field of the user's settings document or a block of the deployment's configuration,
...
/// that something projects, exports, or renders as configuration — which is precisely what the user's document and the
/// endpoint's section are for.
/// </para>
/// <para>
/// One table for all four methods, because the columns are the same columns: a user, a value the credential is
...
/// name, and no plaintext, and it is removed with its user by the cascade the foreign key declares.
/// </para>
/// </remarks>
...
internal sealed class UserCredentialEntity
{
    ...
    /// <remarks>Wider than every name <see cref="Domain.Access.UserCredentialMethod" /> publishes, so a method added later needs no migration to be storable, and narrow enough that the column is a name rather than a field.</remarks>
    ...
    /// <summary>The user a request authenticated by this credential acts for.</summary>
    /// <remarks>The whole of what the credential grants access to. It is a foreign key rather than a copied identifier, so removing a user removes their means of being reached with them rather than leaving a credential resolving nobody.</remarks>
    ...
    /// what that writes is a stronger record of the material already in use — moving it would report every user who
