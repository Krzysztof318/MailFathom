    ...
    : IStoredEmailExtractionBackfillStore
{
    ...
    /// something the user no longer runs, indefinitely, while the message's own document reported the new stamp.
    /// </para>
    /// <para>
    ...
    /// text a user cannot query and keep the one they can — and where attachment reading has since been switched off
    ...
    /// work where derived text was written under a configuration that message's own user no longer runs — its body,
