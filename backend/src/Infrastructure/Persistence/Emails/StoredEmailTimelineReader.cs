...
internal sealed class StoredEmailTimelineReader(MailFathomDbContext dbContext) : IStoredEmailTimelineReader
{
    ...
    /// nothing resolves to every account its user is assigned, so the branch count follows the deployment's configured
    ...
    /// user enough accounts for the fan-out to matter is a bound to place on the accounts rather than on the walk.
    /// </para>
    /// </remarks>
