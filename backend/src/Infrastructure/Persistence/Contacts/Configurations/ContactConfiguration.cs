...
internal sealed class ContactConfiguration : IEntityTypeConfiguration<ContactEntity>
{
    ...
    public void Configure(EntityTypeBuilder<ContactEntity> entity)
    {
        ...
        // adds no rule about contacts; what it adds is a key the dependent table can name the user through, which is
        // what makes an address filed under a different user than its contact impossible rather than unlikely.
        entity.HasAlternateKey(contact => new { contact.Id, contact.UserId });
        ...
        // The one order a book is walked in, and the one a keyset page continues from. The user leads it because a
        ...
        entity.HasIndex(contact => new { contact.UserId, contact.DisplayNameSortKey, contact.Id })
