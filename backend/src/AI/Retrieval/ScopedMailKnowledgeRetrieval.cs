...
internal sealed class ScopedMailKnowledgeRetrieval
{
    ...
    private const string SearchToolDescription =
        ...
        + "two rankings. A document attached to a message is searched on the same terms its body is, and an extract "
        + $"drawn from one arrives in an <{RetrievedMailContextFormatter.AttachmentExtractElementName}> element naming "
        + "the file and the page, slide, or sheet it was read from; cite those rather than the covering note. An "
        + $"<{RetrievedMailContextFormatter.ImageDescriptionElementName}> element is a model's account of a picture "
        + "that nobody wrote, reachable by meaning alone, so a claim resting on one is a claim about a guess and is "
        + "worth saying so. Narrow with the "
    ...
    private async Task<string> SearchAsync(
        [Description("The text to rank mail against, up to 512 characters. Quoted phrases, OR, and a leading - to exclude a word are understood; every other punctuation mark is ordinary text. Write the words you expect the mail itself to contain rather than the question you were asked, in the language that mail is likely written in: matching compares words rather than translating them, so a mailbox holding several languages is reached by a lookup per language. " + EmailSearchQueryText.MatchingDescription + " A search that finds nothing is worth repeating with fewer words before concluding that no mail matches.")]
        string queryText,
