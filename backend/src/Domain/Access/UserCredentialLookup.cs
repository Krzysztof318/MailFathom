...
/// <summary>The one value a presented credential is resolved to its user by, in the form the deployment indexes.</summary>
/// <remarks>
/// <para>
/// Every user-facing method resolves a user by exactly one indexed value, and this is that value whichever method
...
/// </para>
/// </remarks>
public readonly record struct UserCredentialLookup
{
    ...
    /// <see cref="UserCredentialUsername.MaximumLength" /> and a digest is 43 characters, while an issuer and a
    ...
    private UserCredentialLookup(string value) => this.value = value;
    ...
    public static UserCredentialLookup ForUsername(UserCredentialUsername username) => username.IsSpecified
        ? new UserCredentialLookup(username.Value)
    ...
    public static UserCredentialLookup ForDigest(string digest) =>
    ...
    /// alone would let one of them act for the other's user. An issuer carrying a space is refused, since that is what
    ...
    public static bool TryCreateForOAuthSubject(
        string? issuer,
        string? subject,
        out UserCredentialLookup lookup)
    {
        lookup = default;
    ...
    public static bool TryCreate(string? stored, out UserCredentialLookup lookup)
    {
        lookup = default;
        ...
        lookup = new UserCredentialLookup(stored);
