...
/// <summary>How one user-facing credential is presented, and therefore what a request is resolved to a user by.</summary>
/// <remarks>
/// <para>
...
/// </para>
/// <para>
/// The set is closed because a credential MailFathom cannot resolve to a user is not a credential it may admit. A
...
/// </para>
/// </remarks>
[JsonConverter(typeof(UserCredentialMethodJsonConverter))]
public readonly record struct UserCredentialMethod
{
    private readonly string? name;

    private UserCredentialMethod(
        string name,
        bool storesMaterial,
    ...
    /// <summary>Gets the method by which a user presents a username and a password over HTTP Basic.</summary>
    ...
    public static UserCredentialMethod Password { get; } =
    ...
    /// and nowhere else.
    /// </remarks>
    public static UserCredentialMethod ApiKey { get; } =
    ...
    public static UserCredentialMethod PublicKey { get; } =
    ...
    /// <summary>Gets the method by which an authorization server's validated subject names the user it stands for.</summary>
    /// <remarks>
    ...
    /// which user the subject acts for.
    /// </remarks>
    public static UserCredentialMethod OAuthSubject { get; } =
    ...
    public static IReadOnlyList<UserCredentialMethod> All { get; } = [Password, ApiKey, PublicKey, OAuthSubject];
    ...
    /// of which leaves the credential's identifier, its user, and its grant exactly where they were. A validated
    /// subject cannot, because there is nothing about it this deployment issued — pointing a user at a different
    ...
    /// publish theirs — a username is what a user types, a fingerprint is what the client's assertions must name, and
    ...
    public static bool TryParse(string? name, out UserCredentialMethod method)
    {
...
public sealed class UserCredentialMethodJsonConverter : JsonConverter<UserCredentialMethod>
{
    /// <inheritdoc />
    /// <exception cref="JsonException">Thrown when the token is not a string or does not name a published method.</exception>
    public override UserCredentialMethod Read(
    ...
    public override void Write(Utf8JsonWriter writer, UserCredentialMethod value, JsonSerializerOptions options)
    {
    ...
    public override UserCredentialMethod ReadAsPropertyName(
        ref Utf8JsonReader reader,
        Type typeToConvert,
    ...
    private static UserCredentialMethod ParseOrThrow(string? name)
    {
        if (!UserCredentialMethod.TryParse(name, out var method))
        {
    ...
    private static string NameOrThrow(UserCredentialMethod method) => method.IsSpecified
        ? method.Name
