...
/// is the line <see cref="UserLanguage" /> already draws. The second half needs no value of its own here: every
...
{
    private UserTimeZone(TimeZoneInfo zone) => this.Zone = zone;

    /// <summary>Gets the zone a person whose record states none is read in.</summary>
    public static UserTimeZone Coordinated { get; } = new(TimeZoneInfo.Utc);
    ...
    /// </remarks>
    public static bool TryRead(string? zoneId, [NotNullWhen(true)] out UserTimeZone? zone)
    {
        zone = null;
