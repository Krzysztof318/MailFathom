...
public sealed record RecurringSendRequest
{
    ...
    /// <summary>Gets the account every occurrence is submitted through and sent as, by its generated identifier.</summary>
    public MailAccountId Account { get; }

    /// <summary>Gets the user who declared the repetition, which is who it belongs to and who may stop it.</summary>
    /// <remarks>
    /// A standing instruction is one person's, so it stays theirs on a mailbox several people are assigned: two of
    ...
    /// <param name="account">The account every occurrence is sent as, by its generated identifier.</param>
    /// <param name="author">The user declaring the repetition.</param>
        ...
        string schedule)
    {
        ...
        var occurrence = OutgoingEmailRequest.Create(account, author, requester, recipients);
        ...
        return new RecurringSendRequest(account, author, requester, occurrence.Recipients, trimmedSchedule);
    }
}
