...
/// </para>
/// </remarks>
internal static class DeleteUserCredentialCommand
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var held = await ReadForTheOperatorAsync(context, deployment, profile.Token, user, credentialId, cancellationToken);

        if (held is null)
        {
            context.Console.WriteLine(
                $"User {user:D} lists no credential {credentialId:D}. The removal is sent anyway, and the deployment "
                + "answers for what it actually holds.");
        }
        else
        {
            UserCredentialOutput.WriteListing(context.Console, [held]);
        }
        ...
        await deployment.DeleteUserCredentialAsync(profile.Token, user, credentialId, cancellationToken);
    ...
    /// </remarks>
    private static async Task<UserCredential?> ReadForTheOperatorAsync(
        CliContext context,
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            var listing = await deployment.ReadUserCredentialsAsync(token, user, cancellationToken);
