...
/// <summary>The options the credential commands share, and how a method is settled on.</summary>
/// <remarks>
/// <para>
...
/// </para>
/// <para>
/// Which user a command acts for is <see cref="UserOptions" />'s, because both families of user-scoped commands
/// settle it the same way and a second reading of it would be a second answer to keep in step.
/// </para>
/// <para>
...
/// </para>
/// </remarks>
internal static class UserCredentialOptions
{
    ...
    /// <returns>The option.</returns>
    /// <remarks>An option rather than a prompt, because a username is not a secret: it is written down beside the address the user signs in to, and a deployment provisioning several from a script needs it to be an argument.</remarks>
    internal static Option<string> Username() => new("--username")
    {
        Description =
            "The name the user will sign in with, for '--method password'. Folded to its canonical lower-case form by the deployment.",
    };
    ...
    /// <summary>Builds the option naming the subject the mapped user acts as.</summary>
    /// <returns>The option.</returns>
    ...
    internal static UserCredentialMethod ResolveMethod(string? written) =>
        UserCredentialMethod.TryParse(written, out var method)
            ? method
