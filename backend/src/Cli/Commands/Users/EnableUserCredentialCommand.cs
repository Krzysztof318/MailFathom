...
internal static class EnableUserCredentialCommand
{
    /// <summary>Builds the <c>credential enable</c> command.</summary>
    /// <param name="context">What the command needs from its surroundings.</param>
    /// <returns>The command.</returns>
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="context" /> is <see langword="null" />.</exception>
    internal static Command Create(CliContext context) => UserCredentialEnablement.Build(context, enabled: true);
}
