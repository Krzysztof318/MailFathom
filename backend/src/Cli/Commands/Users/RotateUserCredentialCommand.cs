...
/// </para>
/// <para>
/// The credential keeps its identifier, its user, and its grant, so nothing an operator wrote down about it goes
...
/// </para>
/// <para>
/// A mapped subject cannot be rotated. There is nothing about it this deployment issued, so pointing a user at a
/// different subject is a credential to provision rather than a secret to replace, and the deployment says so.
/// </para>
/// </remarks>
internal static class RotateUserCredentialCommand
{
    ...
    internal static Command Create(CliContext context)
    {
        ...
        var credentialOption = UserCredentialOptions.Credential();
    ...
    {
        var method = UserCredentialOptions.ResolveMethod(requestedMethod);
        ...
        if (!method.MaterialIsReplaceable)
        {
            throw new CliFailure(
                ...
                + "nothing to rotate. Provision the credential the user should act under and delete this one.");
        }
        ...
        var rotated = await deployment.ReplaceUserCredentialMaterialAsync(
            profile.Token,
            user,
            credentialId,
            request,
        ...
        UserCredentialOutput.WriteRotated(context.Console, method, credentialId, rotated);
    ...
    private static UserCredentialMaterialRequest Compose(
        CliContext context,
        ...
        FileInfo? publicKeyFile)
    {
        if (method == UserCredentialMethod.Password)
        {
            ...
            return new UserCredentialMaterialRequest(
                method.Name,
                signIn,
                UserCredentialOutput.ReadPassword(context, $"New password for credential {credentialId:D}: "),
                PublicKey: null);
        }
        ...
        if (method == UserCredentialMethod.PublicKey)
        {
            return new UserCredentialMaterialRequest(
                ...
                Username: null,
                Password: null,
                UserCredentialOptions.ReadPublicKey(publicKeyFile));
        }

        return new UserCredentialMaterialRequest(method.Name, Username: null, Password: null, PublicKey: null);
    }
}
