...
/// <summary>Provisions a credential one user's clients present to reach that user's own mail.</summary>
/// <remarks>
/// <para>
/// The only way to create one: there is no self-service and no default. Whoever administers the deployment provisions
/// the credential and hands it over, which is what keeps a user from minting a way into anybody's mail, their own
/// included.
/// </para>
/// <para>
...
/// </para>
/// </remarks>
internal static class CreateUserCredentialCommand
{
    ...
    internal static Command Create(CliContext context)
    {
        ...
        var methodOption = UserCredentialOptions.Method();
        var usernameOption = UserCredentialOptions.Username();
        var publicKeyFileOption = UserCredentialOptions.PublicKeyFile();
        var issuerOption = UserCredentialOptions.Issuer();
    ...
    {
        var method = UserCredentialOptions.ResolveMethod(requested.Method);
        ...
        UserCredentialOutput.WriteProvisioned(context.Console, method, user, provisioned);

        return CliExitCode.Success;
    }

    /// <summary>Composes the request the named method asks for, reading whatever only this side can supply.</summary>
    /// <remarks>The password is read here rather than in the action, so it is asked for after the deployment has been reached and the user settled — a person typing one should not have it discarded because the endpoint was unreachable.</remarks>
    private static UserCredentialProvisioningRequest Compose(
        CliContext context,
        ...
        RequestedCredential requested)
    {
        if (method == UserCredentialMethod.Password)
        {
            var username = requested.Username is { Length: > 0 } written
                ? written
                : throw new CliFailure("Provisioning a password needs the name the user will sign in with. Pass '--username'.");

            return new UserCredentialProvisioningRequest(
                method.Name,
                username,
                UserCredentialOutput.ReadPassword(context, $"Password for '{username}': "),
                PublicKey: null,
                Issuer: null,
                Subject: null,
        ...
        if (method == UserCredentialMethod.PublicKey)
        {
            return new UserCredentialProvisioningRequest(
                ...
                Username: null,
                Password: null,
                UserCredentialOptions.ReadPublicKey(requested.PublicKeyFile),
                Issuer: null,
                Subject: null,
                requested.Permissions);
        }

        if (method == UserCredentialMethod.OAuthSubject)
        {
            return new UserCredentialProvisioningRequest(
                method.Name,
                Username: null,
