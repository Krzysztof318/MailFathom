...
/// still reaches that user.
/// </remarks>
public interface IMailUserDirectory
{
    ...
    /// observable rather than read in full. The order is stable so that a caller asking for one user twice is answered
    /// about the same user both times.
    /// </remarks>
    Task<IReadOnlyList<MailUserRecord>> ReadUsersAsync(int limit, CancellationToken cancellationToken);
    ...
    /// </remarks>
    Task<MailUserRecord?> ReadUserAsync(MailUserId user, CancellationToken cancellationToken);
}
