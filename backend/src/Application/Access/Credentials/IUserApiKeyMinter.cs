...
/// <summary>Draws the keys a user's clients present, and reduces a presented one to the value it is resolved by.</summary>
/// <remarks>
/// <para>
...
/// <see cref="MintedUserApiKey.Lookup" />, and a key nobody wrote down is a key nobody can recover.
/// </para>
/// <para>
...
/// </para>
/// </remarks>
public interface IUserApiKeyMinter
{
    ...
    /// </remarks>
    bool TryDigest(ReadOnlySpan<char> presentedKey, out UserCredentialLookup lookup);
}
...
/// <see cref="UserCredentialMethod.LookupIsDerivedFromTheSecret" /> gives — it verifies a presented key, so it is
...
/// refused.
/// </remarks>
public sealed record MintedUserApiKey(string Key, UserCredentialLookup Lookup)
{
    /// <inheritdoc />
    public override string ToString() => nameof(MintedUserApiKey);
}
