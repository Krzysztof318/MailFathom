...
/// <summary>What an administrator does to the credentials one user is admitted by.</summary>
/// <remarks>
/// <para>
/// Every act here names the user it is for, and that is the whole of what the user story asks: credentials are
/// provisioned by whoever administers the deployment rather than by whoever holds one, so a user cannot mint a
/// credential for another user because a user cannot mint one at all. The administrative principal acts for nobody's
/// mail — <see cref="AccessAuthorization.RequireUser" /> refuses it — so the user is stated in the request and
...
/// and every rule after that point is one rule: which user, which grant, which ceiling, and what is written down
...
/// user's long-lived credential, and for the same reason.
/// </para>
/// <para>
...
/// than answering: a boundary that means to report the rule calls <see cref="UserPasswordPolicy.FindRefusal" /> or
...
/// </para>
/// </remarks>
public sealed class UserCredentialAdministration
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!UserCredentialLookup.TryCreateForOAuthSubject(issuer, subject, out var lookup))
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        await this.RecordAsync(
            outcome,
            UserCredentialAct.MaterialRotated,
            credentialId,
            user,
            UserCredentialMethod.Password,
            cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        return new UserCredentialRotation(
            outcome,
            minted.Lookup,
            outcome == UserCredentialWriteOutcome.Written ? minted.Key : null);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        return new UserCredentialRotation(outcome, publicKey.Lookup, MintedKey: null);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        await this.RecordAsync(
            outcome,
            enabled ? UserCredentialAct.Enabled : UserCredentialAct.Disabled,
            credentialId,
            user,
            method: null,
            cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        await this.RecordAsync(outcome, UserCredentialAct.Deleted, credentialId, user, method: null, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        return new UserCredentialProvisioning(
            outcome,
            credentialId,
            lookup,
    ...
    {
        if (UserPasswordPolicy.FindRefusal(password) is { } refusal)
        {
            throw new ArgumentException(
...
/// </remarks>
public sealed record UserCredentialProvisioning(
    UserCredentialWriteOutcome Outcome,
    Guid CredentialId,
    UserCredentialLookup Lookup,
    string? MintedKey)
{
    ...
    public override string ToString() =>
        $"{nameof(UserCredentialProvisioning)} {{ {this.Outcome}, {this.CredentialId} }}";
}
