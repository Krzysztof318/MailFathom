...
/// <summary>Records that an administrator changed which credentials authenticate a user.</summary>
/// <remarks>
/// <para>
...
/// the act, the credential, the user, and who asked — which is everything a later question about "when did this
...
/// </para>
/// </remarks>
public interface IUserCredentialAuditor
{
    /// <summary>Records one change to a user's credentials.</summary>
    /// <param name="change">The act, the credential, the user, and the administrator who asked.</param>
    ...
    Task RecordCredentialChangeAsync(UserCredentialChange change, CancellationToken cancellationToken);
}

/// <summary>One administrative change to the credentials a user holds.</summary>
...
/// <param name="Method">How the credential is presented, which is what makes two records about one user readable apart, and <see langword="null" /> for the acts written against the identifier alone.</param>
...
/// unspecified <see cref="UserCredentialMethod" /> there instead would hand every sink a value whose published name
