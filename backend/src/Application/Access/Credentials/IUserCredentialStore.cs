...
/// <summary>Where the credentials that resolve a request to one user are kept.</summary>
/// <remarks>
/// <para>
/// The credentials live in relational columns of their own rather than in a user's settings document or in an
...
/// without rewriting a document, a grant has to be read beside the user it belongs to, and stored material must never
...
/// </para>
/// <para>
/// Every write names the user beside the credential, including the ones that could have been keyed by the credential
/// alone. That is deliberate: an administrator acts on a credential belonging to a selected user, so an identifier
/// copied from the wrong listing answers <see cref="UserCredentialWriteOutcome.UnknownCredential" /> rather than
...
public interface IUserCredentialStore
{
    ...
    /// <param name="cancellationToken">Cancels the write.</param>
    /// <returns>What the act did, or why it did nothing.</returns>
    /// <exception cref="ArgumentException">Thrown when <paramref name="user" /> names nobody, <paramref name="credentialId" /> is the empty identifier, or <paramref name="method" /> or <paramref name="lookup" /> is the unspecified struct default.</exception>
    /// <remarks>
    ...
    /// reports its material as having changed at must not move. Reporting every user who signed in after the work
    ...
    /// <see cref="UserCredentialWriteOutcome.UnknownCredential" /> so the rehash is dropped and the rotation stands.
    /// </para>
    /// </remarks>
