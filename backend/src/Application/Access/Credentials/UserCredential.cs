...
/// <summary>One credential a user is admitted by, as an administrator reads it back.</summary>
...
/// apart without the identifier being the only handle they have: a username is what a user types, a public key's
...
/// <see cref="UserCredentialMethod.LookupIsDerivedFromTheSecret" /> gives — the digest verifies a presented key, so
    ...
    DateTimeOffset MaterialChangedAt)
{
    ...
    /// per device or agent a user is reached by, plus whatever a rotation left standing — so a user reaching it has
