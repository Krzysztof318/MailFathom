...
/// <summary>Marks where one page of an answering record ended, so the next page continues from it.</summary>
/// <remarks>
/// <para>
/// The record is ordered newest first by completion instant, with the entry identifier breaking a tie, and this pairs
/// those two values with a fingerprint of the filters the page was read under. The pair is what makes pagination
/// keyset-based rather than offset-based: the next page asks for entries beyond a known boundary, so a question answered
/// between two requests neither shifts a window nor causes an entry to be skipped or repeated. The fingerprint is what
/// makes the boundary meaningful, because a position names a page edge only within the filtered set it was computed for.
/// </para>
/// <para>
...
public readonly record struct MailAnsweringAuditCursor
{
    ...
    /// <summary>Gets the identity of that entry, which breaks a tie between two runs that ended together.</summary>
    public MailAnsweringAuditEntryId EntryId { get; }

    /// <summary>Gets the fingerprint of the filters this cursor was issued for.</summary>
    public string FilterFingerprint { get; }

    /// <summary>Creates the cursor that continues a walk after one position in the record.</summary>
    ...
    /// </remarks>
    public static MailAnsweringAuditCursor After(
        DateTimeOffset completedAt,
        MailAnsweringAuditEntryId entryId,
        string filterFingerprint)
    {
        ...
        return new MailAnsweringAuditCursor(completedAt, entryId, filterFingerprint);
    }
