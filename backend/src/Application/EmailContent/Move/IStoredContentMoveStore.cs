...
public interface IStoredContentMoveStore
{
    ...
    /// <summary>Points one row at the verified object and releases the payload the database was holding.</summary>
    /// <param name="kind">The payload kind, which decides the table.</param>
    ...
    /// </para>
    /// <para>
    /// The payload column is emptied by the same statement because the schema requires it — an object-backed row carries
    /// a locator and no bytes, which is the check constraint the content tables declare. The recorded length and digest
    /// stay exactly as they were, so the object remains checkable against the row that names it.
    /// </para>
    /// </remarks>
