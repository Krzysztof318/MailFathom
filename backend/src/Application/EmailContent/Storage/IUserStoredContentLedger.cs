...
/// <summary>Answers how much stored mail content one user holds, without reading their mail to find out.</summary>
/// <remarks>
/// <para>
...
/// summing a user's payload lengths would put a scan of somebody's whole mailbox in front of every folder run. So the
/// figure is maintained as it changes — one counter per user, moved inside the transaction that stores or removes the
...
/// </para>
/// <para>
/// Both members refuse a user naming nobody. Nothing gates this port — a synchronization run calls it directly —
/// so the refusal is the port's own, and an implementation that answered such a user would be keeping a counter for
/// "nobody" that reads exactly like a working figure.
/// </para>
/// </remarks>
public interface IUserStoredContentLedger
{
    /// <summary>Reads what one user's stored mail content occupies, from the maintained counter.</summary>
    /// <param name="user">The user asked about.</param>
    /// <param name="cancellationToken">Cancels the read.</param>
    /// <returns>The bytes that user's stored payloads hold.</returns>
    /// <remarks>
    /// A user whose counter has never been written is derived once and adopted rather than answered as zero, so an
    ...
    Task<long> ReadStoredContentBytesAsync(MailUserId user, CancellationToken cancellationToken);

    /// <summary>Recomputes one user's figure from their stored payloads and adopts it.</summary>
    /// <param name="user">The user whose counter is re-derived.</param>
    ...
    /// left to race: it claims the user's row before it recomputes and holds it until the recomputation is written, so
    ...
    /// </remarks>
    /// <exception cref="ArgumentException">Thrown when <paramref name="user" /> names nobody.</exception>
    Task<long> RederiveStoredContentBytesAsync(MailUserId user, CancellationToken cancellationToken);
}
