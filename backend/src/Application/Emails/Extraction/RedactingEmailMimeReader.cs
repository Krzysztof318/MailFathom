...
/// <summary>Replaces what an account's switched-on scanner finds in a message's body before anything is derived from it.</summary>
/// <remarks>
/// <para>
...
/// </para>
/// <para>
/// <b>What is looked for is the account's, and the account arrives with the message.</b> A deployment serves several
/// mailboxes and one of them may have switched a scanner on that the deployment left off, so the body is redacted under
/// the posture of the mailbox the mail is in rather than under one answer for the whole store. Both paths that reach
/// here already hold it, which is why the port carries it rather than resolving it.
/// </para>
/// <para>
