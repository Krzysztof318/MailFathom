...
/// <remarks>
/// <para>
/// Every property is one of MailFathom's own identifiers: the user the mailbox belongs to, the deployment's
...
{
    /// <summary>Gets the user whose stored mail the work covers.</summary>
    /// <remarks>
    /// Named beside the identifier, because an identifier names one account within its user and this work writes rows
    /// about that account. The user is generated and names nobody outside this deployment, so carrying it discloses
    /// nothing an operator reading a queued job may not see.
    /// </remarks>
    public required Guid UserId { get; init; }

    /// <summary>Gets the account whose stored mail the work covers, within that user.</summary>
    ...
    /// <param name="account">The account whose stored mail the work covers, named by its user and its identifier.</param>
