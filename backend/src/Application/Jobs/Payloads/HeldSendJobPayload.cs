...
/// <remarks>
/// <para>
/// Every property is one of MailFathom's own identifiers: the account's generated identifier, which names one
/// mailbox across the deployment, and the surrogate its own outgoing record carries. Nothing about the
...
public sealed record HeldSendJobPayload : IJobPayload
{
    ...
    /// <param name="account">The account whose outbox holds the message, named by its generated identifier.</param>
    ...
    public static HeldSendJobPayload For(MailAccountId account, OutgoingEmailId outgoingEmailId) => new()
    {
