...
/// and one is a storage defect found while the source still has the mail. See
...
public enum MailboxDrainHoldBack
{
    ...
    /// <remarks>
    /// The one value here that is a defect rather than a bound, and the whole reason the gate reads the payload back
    /// instead of trusting that a committed row points at a readable one: the source still holds the message, so the
    /// damage is recoverable for exactly as long as the drain refuses to remove it.
    /// </remarks>
