...
/// <param name="User">The user the account belongs to, which is the other half of what identifies it.</param>
/// <param name="Id">The identifier every stored row, cursor, and log line names the account by, within its user.</param>
...
/// none of them may go. The user is a property of the mailbox rather than a connection detail, which is why it is here
    ...
    MailSynchronizationMode SynchronizationMode)
{
    ...
    public MailAccountIdentity Identity => MailAccountIdentity.Create(this.User, this.Id);
    ...
    /// asking it of another user's accounts a mistake rather than a refusal. Both names are unique within the user
    /// that gave them and nowhere wider, so two users may each answer to <c>work</c>: a caller-facing resolution asks
    /// this only of the accounts the caller's user owns, and one that asked it of the deployment's would resolve the
    /// caller's word to whichever user's account came first.
    /// </para>
    /// </remarks>
