...
{
    private OutgoingEmailClaimRequest(MailAccountId accountId, int batchSize, TimeSpan leaseDuration, Guid owner)
    {
    ...
    /// <summary>Gets the account whose queued sends this claim takes.</summary>
    ...
    /// <param name="accountId">The account whose queued sends the claim takes.</param>
    ...
    public static OutgoingEmailClaimRequest Create(MailAccountId accountId, int batchSize, TimeSpan leaseDuration)
    {
        ...
        return new OutgoingEmailClaimRequest(accountId, batchSize, leaseDuration, Guid.CreateVersion7());
    }
}
