...
/// <summary>Carries the page of a user's outbox a request named, or the reason it named none.</summary>
/// <remarks>
/// It is <see cref="OutboxQueryResult" /> after the query has been run rather than beside it, because a user-facing
/// reading resolves the account it may narrow to and reads the page in one act: splitting the two would put the
/// scoping in whatever called it, which is where a user-facing read must never decide.
/// </remarks>
...
public sealed record UserOutboxPageResult(OutboxPage? Page, OutboxQueryOutcome Outcome);
