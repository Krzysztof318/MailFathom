...
/// <summary>Serves one user their own outbox: what is queued, what became of one send, and the two decisions about it.</summary>
/// <remarks>
/// <para>
/// It is the user's counterpart of the administrative <see cref="OutboxOperations" /> and of the per-caller
...
/// names no recipient and no subject, because a page of an outbox would otherwise be an export of who this user
...
/// this user is sending, so a credential granted to read a mailbox learns nothing here — and withdrawing a send is
...
public sealed class UserOutbox(
    ICallerMailAccountCatalog accountCatalog,
    IOutgoingEmailStore outgoingEmails,
    IOutboxOperationStore outbox,
    AccessAuthorization authorization)
{
    /// <summary>Reads one page of what the named account of this user is sending, newest first.</summary>
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return new UserOutboxPageResult(null, queryResult.Outcome);
        }

        var page = await outbox.ReadPageAsync(query, cancellationToken);

        return new UserOutboxPageResult(page, OutboxQueryOutcome.Accepted);
    }

    /// <summary>Reads back one send of this user's, with what each of its recipients was told.</summary>
    ...
    /// <returns>What became of the send, which is <see cref="OutboxDecisionOutcome.RecordUnknown" /> for one this user does not hold.</returns>
    /// <exception cref="PrincipalNotAuthorizedException">Thrown when the caller does not hold <see cref="MailFathomPermission.MailSend" />, or is acting for no user.</exception>
    /// <remarks>
    /// The scoping runs before the withdrawal rather than beside it, so a record this user may not be told about is
    ...
    /// <summary>Offers one of this user's sends again, which is the decision this system will not take on its own.</summary>
