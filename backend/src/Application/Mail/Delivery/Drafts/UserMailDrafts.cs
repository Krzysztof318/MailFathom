...
/// <summary>Performs the acts a user takes on one of their own drafts: keeping it, giving it up, and sending it.</summary>
/// <remarks>
/// <para>
...
/// keeping one in the user's folder reaches their mail server, and sending one puts a message in somebody else's
...
{
    /// <summary>Gives up one of this user's drafts, and takes the copies of it back out of their folder.</summary>
    ...
    /// <exception cref="MailDraftRefusedException">Thrown when this user holds no draft under that identifier, or the draft has already been promoted to a send.</exception>
    ...
    /// <exception cref="MailDraftRefusedException">Thrown when this user holds no draft under that identifier, when the draft names nobody to send it to, or when the stored message exceeds what this deployment sends.</exception>
