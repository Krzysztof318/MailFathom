...
public interface INotificationStore
{
    ...
    /// <remarks>
    /// The comparison is against the user's unread notifications alone. A condition the person has already read is
    ...
    /// <summary>Reads one bounded page of a user's notifications, newest first.</summary>
    /// <param name="user">The user whose notifications are read.</param>
    ...
    /// shifts the window nor causes a row to be repeated or skipped. Whether the cursor was issued to this user is
    ...
    /// <summary>Counts one user's notifications that have not been read.</summary>
    /// <param name="user">The user whose notifications are counted.</param>
    /// <param name="cancellationToken">Cancels the read.</param>
    /// <returns>How many of the user's notifications stand unread.</returns>
    /// <remarks>
    ...
    /// <remarks>
    /// The user is part of the addressing rather than a filter applied after a lookup, so a notification another
    /// user holds is not found rather than found and refused — which is what keeps the answer identical for one that
    ...
    /// <summary>Marks every one of a user's unread notifications read.</summary>
    /// <param name="user">The user whose notifications are marked.</param>
    /// <param name="cancellationToken">Cancels the write.</param>
    /// <returns>How many notifications the request changed, which is zero where none stood unread.</returns>
    /// <remarks>
    /// It is unbounded on purpose, unlike the retention pass beside it: what it writes is one user's own unread rows,
