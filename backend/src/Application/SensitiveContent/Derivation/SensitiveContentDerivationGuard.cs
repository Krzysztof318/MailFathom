...
/// <see cref="SensitiveContentRedactor" /> behind both for any one user, which is what keeps a citation drawn from a
...
/// hold it — synchronization is running one user's account, and a re-derivation was enqueued for one user — so the
...
/// layers below whoever resolved the user, and here they are not.
/// </para>
/// <para>
...
/// "written under this scanner" two readable states rather than one absence. It is one user's stamp rather than the
/// deployment's, so a posture one user changed leaves nobody else's rows stale.
/// </para>
/// <para>
/// <b>With nothing switched on for a user this guard is inert.</b> It is registered whatever a deployment configured,
/// so no writer carries a null check or a second code path, and with no redaction behind that user's posture every
...
public sealed class SensitiveContentDerivationGuard
{
    ...
    /// <param name="postures">Answers what each user's mail is derived under.</param>
    ...
    /// <summary>Gets whether any user this deployment serves has what is derived from their mail redacted.</summary>
    /// <remarks>
    ...
    /// redact everybody's.
    /// </remarks>
    public bool IsActive => this.postures.IsActiveForAnyUser;

    /// <summary>Gets what every user this deployment serves has their mail derived under, ordered by user.</summary>
    /// <remarks>
    /// For the walk that judges rows belonging to several users in one query, which is the one consumer that cannot
    /// ask about the user in front of it. <see cref="ISensitiveContentPostures.Current" /> holds why.
    /// </remarks>
    public IReadOnlyList<UserSensitiveContentPosture> Current => this.postures.Current;
    ...
    /// <para>
    /// The unspecified identifier is what asks the question, because no roster carries one: a user is declared with a
    /// UUID that is refused if it is the all-zero one, so this reaches the fallback by the same route every user the
    /// roster does not name reaches it.
    /// </para>
    /// </remarks>
    public SensitiveContentDerivationStamp? StampForUnrostered => this.postures.ForUser(default).Stamp;

    /// <summary>Gets the configuration a row of one user's mail written now records, or nothing where it is not scanned.</summary>
    /// <param name="user">The user whose mail the row is derived from.</param>
    /// <returns>That user's stamp, or <see langword="null" /> where nothing scans their mail.</returns>
    public SensitiveContentDerivationStamp? StampFor(MailUserId user) => this.postures.ForUser(user).Stamp;

    /// <summary>Redacts one text about to be written into a derived store.</summary>
    /// <param name="user">The user whose mail the text was extracted from.</param>
    /// <param name="text">The text to redact, which must be a value rather than a document composed around one.</param>
    /// <param name="cancellationToken">Cancels the scan.</param>
    /// <returns>The text with every detected region replaced, or the text itself where nothing scans this user's mail.</returns>
    ...
    public async Task<string> GuardAsync(MailUserId user, string text, CancellationToken cancellationToken) =>
        (await this.GuardTextAsync(user, text, cancellationToken)).Text;

    /// <summary>Redacts one text and reports where every placeholder ended up.</summary>
    /// <param name="user">The user whose mail the text was extracted from.</param>
    /// <param name="text">The text to redact, which must be a value rather than a document composed around one.</param>
    /// <param name="cancellationToken">Cancels the scan.</param>
    /// <returns>The redaction, which is the text itself with no placement where nothing scans this user's mail.</returns>
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="text" /> is <see langword="null" />.</exception>
    /// <exception cref="SensitiveContentScannerUnavailableException">Thrown when a switched-on scanner could not establish what the text carries, which refuses the derived write.</exception>
    /// <exception cref="OperationCanceledException">Thrown when <paramref name="cancellationToken" /> is cancelled.</exception>
    /// <remarks>
    ...
    /// </remarks>
    public Task<RedactedText> GuardTextAsync(MailUserId user, string text, CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(text);

        return this.postures.ForUser(user).Redactor is { } active
            ? this.RedactAsync(active, text, cancellationToken)
            : Task.FromResult(RedactedText.Create(text, [], omittedCharacterCount: 0));
    }

    /// <summary>Runs the user's redaction and reports what it found, or reports the refusal and re-raises it.</summary>
    /// <remarks>
