...
/// <remarks>Everything here reads <see cref="UserCredentialMethod.All" /> rather than reflecting over the type, so a member declared and left out of that list fails as the missing coverage it is.</remarks>
public sealed class UserCredentialMethodTests
{
    [Fact]
    public void All_TheDeclaredMethods_PublishFourDistinctNames()
    {
        // Act
        var names = UserCredentialMethod.All.Select(method => method.Name).ToArray();
    ...
    {
        // Act
        var parsed = UserCredentialMethod.TryParse(name, out var method);
    ...
    {
        // Act
        var moving = UserCredentialMethod.All.Where(method => method.LookupMovesWithTheMaterial).ToArray();
    ...
    {
        // Act
        var parsed = UserCredentialMethod.TryParse(written, out var method);

        // Assert
        Assert.True(parsed);
        Assert.Equal(UserCredentialMethod.ApiKey, method);
    }
    ...
    {
        // Arrange
        var unspecified = default(UserCredentialMethod);
    ...
    {
        // Assert
        Assert.Equal("oauth-subject", UserCredentialMethod.OAuthSubject.ToString());
    }
    ...
    {
        // Act
        var roundTripped = UserCredentialMethod.All
            .Select(method => JsonSerializer.Deserialize<UserCredentialMethod>(JsonSerializer.Serialize(method)))
            .ToArray();

        // Assert
        Assert.Equal(UserCredentialMethod.All, roundTripped);
        Assert.Equal("\"public-key\"", JsonSerializer.Serialize(UserCredentialMethod.PublicKey));
    }
    ...
    {
        // Arrange
        var keyed = UserCredentialMethod.All.ToDictionary(method => method, method => method.Name);

        // Act
        var written = JsonSerializer.Serialize(keyed);
        var read = JsonSerializer.Deserialize<Dictionary<UserCredentialMethod, string>>(written);
    ...
    {
        // Act
        var read = Record.Exception(() => JsonSerializer.Deserialize<UserCredentialMethod>(written));
    ...
    {
        // Act
        var written = Record.Exception(() => JsonSerializer.Serialize(default(UserCredentialMethod)));
    ...
    {
        // Arrange
        var keyed = new Dictionary<UserCredentialMethod, string> { [default] = "nothing" };
