...
/// <summary>Covers the one value every user-facing method is resolved by: what composes it, what it refuses, and what a stored one reads back as.</summary>
public sealed class UserCredentialLookupTests
{
    [Fact]
    public void ForUsername_ACanonicalUsername_IsResolvedByThatFormAndNothingElse()
    {
        // Arrange
        Assert.True(UserCredentialUsername.TryCreate("  Ada.Lovelace@Example.Org ", out var username));
    ...
    {
        // Act
        var composing = Record.Exception(() => UserCredentialLookup.ForUsername(default));
    ...
    {
        // Arrange
        var overlong = new string('a', UserCredentialLookup.MaximumLength + 1);

        // Act
        var composing = Record.Exception(() => UserCredentialLookup.ForDigest(overlong));
    ...
    {
        // Act
        var read = UserCredentialLookup.TryCreate(new string('a', UserCredentialLookup.MaximumLength), out var lookup);

        // Assert
        Assert.True(read);
        Assert.Equal(UserCredentialLookup.MaximumLength, lookup.Value.Length);
    }
    ...
    {
        // Act
        var read = UserCredentialLookup.TryCreate(stored, out var lookup);
    ...
    {
        // Act
        var composed = UserCredentialLookup.TryCreateForOAuthSubject(
    ...
    {
        // Act
        var withSpaceInTheIssuer = UserCredentialLookup.TryCreateForOAuthSubject("https://a b", "c", out var collided);
        var withSpaceInTheSubject = UserCredentialLookup.TryCreateForOAuthSubject("https://a", "b c", out var stored);
    ...
    {
        // Act
        var composed = UserCredentialLookup.TryCreateForOAuthSubject(issuer, subject, out var lookup);
    ...
    {
        // Arrange
        var digest = UserCredentialLookup.ForDigest("Zm9vYmFyYmF6");
    ...
    {
        // Arrange
        var unspecified = default(UserCredentialLookup);
    ...
    {
        // Act
        Assert.True(UserCredentialLookup.TryCreate("ada@example.org", out var read));
        var composed = UserCredentialLookup.ForDigest("ada@example.org");
