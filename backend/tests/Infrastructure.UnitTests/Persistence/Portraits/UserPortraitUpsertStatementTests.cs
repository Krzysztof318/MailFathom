...
/// device replaces what the first supplied rather than colliding on the key, that a user this deployment no longer
...
/// </summary>
public sealed class UserPortraitUpsertStatementTests
{
    ...
    [Fact]
    public void Compose_TheWrite_ReplacesTheOctetsOnConflictWithTheUsersExistingRow()
    {
        // Act
    ...
    {
        // Assert
        Assert.Throws<ArgumentNullException>(() => UserPortraitUpsertStatement.Compose(null!));
    }
    ...
    private static string Composed()
    {
        ...
        return UserPortraitUpsertStatement.Compose(context.Model);
    }
}
