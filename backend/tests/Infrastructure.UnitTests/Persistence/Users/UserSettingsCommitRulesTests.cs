...
/// <summary>
/// Covers what a user's record has to satisfy before a statement is issued for it. The bound is the one the read
...
/// committed.
/// </summary>
public sealed class UserSettingsCommitRulesTests
{
    ...
    public void RefuseWhatCannotBeCommitted_JsonWhoseRootIsNotAnObject_IsRefused(string json)
    {
        ...
        var refusal = Assert.Throws<ArgumentException>(
            () => UserSettingsCommitRules.RefuseWhatCannotBeCommitted(json, expectedVersion: 1));
    ...
    public void RefuseWhatCannotBeCommitted_ACandidateThatIsNotJsonAtAll_IsRefusedBeforeAnyStatement()
    {
        // Act & Assert
        var refusal = Assert.Throws<ArgumentException>(
            () => UserSettingsCommitRules.RefuseWhatCannotBeCommitted("{\"MailAccounts\":", expectedVersion: 1));
    ...
    public void RefuseWhatCannotBeCommitted_ARecordThatOnlyFitsCompacted_IsRefused()
    {
        ...
        // Act & Assert
        Assert.True(
            compact.Length <= UserSettingsDocument.MaximumOctets,
            "The record has to fit compacted for the claim to mean anything.");

        var refusal = Assert.Throws<ArgumentException>(
            () => UserSettingsCommitRules.RefuseWhatCannotBeCommitted(compact, expectedVersion: 1));
    ...
    public void RefuseWhatCannotBeCommitted_ANegativeVersion_IsRefused()
    {
        ...
        Assert.Throws<ArgumentOutOfRangeException>(
            () => UserSettingsCommitRules.RefuseWhatCannotBeCommitted("{}", expectedVersion: -1));
    }
    ...
    {
        // Act & Assert
        UserSettingsCommitRules.RefuseWhatCannotBeCommitted(
