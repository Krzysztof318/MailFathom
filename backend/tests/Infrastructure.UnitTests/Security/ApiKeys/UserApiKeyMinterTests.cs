...
/// <summary>Covers the one shape a minted user key has and the digest a presented one is resolved by.</summary>
/// <remarks>
...
/// </remarks>
public sealed class UserApiKeyMinterTests
{
    ...
    {
        // Arrange
        var minter = new UserApiKeyMinter();

        // Act
        var minted = minter.Mint();

        // Assert
        Assert.StartsWith(UserApiKeyMinter.KeyPrefix, minted.Key, StringComparison.Ordinal);
    ...
    /// <summary>Two keys are two credentials, so a mint that repeated itself would hand one user's key to another.</summary>
    [Fact]
