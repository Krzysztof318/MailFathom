...
/// <summary>Covers which presented key admits a user and which of the several ways of failing are told apart.</summary>
/// <remarks>
/// A key nobody holds, a key whose row is disabled, and a key that resolves no user are one answer, because telling
...
public sealed class UserApiKeyAuthenticatorTests
{
    ...
    [Fact]
    public async Task AuthenticateAsync_AKeyTheDeploymentHolds_AdmitsTheUserItResolvesToWithTheGrantOnTheRow()
    {
        // Arrange
    ...
    public async Task AuthenticateAsync_AKeyTheDeploymentHolds_ResolvesTheRowByItsDigestAndNotByTheKey()
    {
        ...
        await harness.Credentials.Received(1).FindAsync(
            ...
            Arg.Is<UserCredentialLookup>(lookup => lookup.Value != harness.MintedKey),
    ...
    {
        private readonly UserApiKeyMinter minter = new();
        ...
        internal AuthenticatorHarness()
        {
            ...
            this.Authenticator = new UserApiKeyAuthenticator(this.Credentials, this.minter);
        }

        internal UserApiKeyAuthenticator Authenticator { get; }

        internal IUserCredentialStore Credentials { get; }

        internal string MintedKey { get; }

        private UserCredentialLookup MintedLookup { get; }
