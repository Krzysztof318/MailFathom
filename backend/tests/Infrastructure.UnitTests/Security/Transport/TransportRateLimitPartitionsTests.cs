...
public sealed class TransportRateLimitPartitionsTests
{
    private const string McpSurface = "Mcp";
    ...
    /// <summary>
    /// The two endpoints configure their key lists separately and neither consults the other's, so one name can
    /// legitimately be spelled under both. Were the surface not part of the key, that client would spend a single bucket
    /// from both endpoints and administering the service would consume the capacity for reading a mailbox.
    /// </summary>
    [Fact]
    ...
    /// <summary>The anonymous partition is the one every surface has, and the one a flood of bad credentials lands in, so it is the one that most has to stay per surface.</summary>
    [Fact]
    ...
    public void KeyFor_WithACertificateProfileAlone_CountsTheClientApplicationUnderIt()
    {
        ...
        // Assert
        Assert.NotEqual(first, second);
        Assert.NotEqual(anonymous, first);
    }
    ...
    /// </summary>
    [Fact]
    public void AnonymousIdentity_CannotBeSpelledAsAConfiguredName()
    {
        // Act
        var isAcceptedAsASecretName = SecretName.TryCreate(TransportRateLimitPartitions.AnonymousIdentity, out _);
    ...
    public void KeyFor_ACertificateProfilePartition_HoldsAnIdentityNoConfiguredNameCouldSpell()
    {
        ...
        Assert.False(SecretName.TryCreate("<profile:chatgpt-connector>", out _));
    }
    ...
    [InlineData("")]
    [InlineData("   ")]
    public void KeyFor_WithoutASurface_Throws(string? absentSurfaceName)
    {
        // Act, Assert
