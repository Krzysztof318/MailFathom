...
public sealed class UserPasswordAuthenticatorTests
{
    ...
    [Fact]
    public async Task AuthenticateAsync_TheStoredCredential_AuthenticatesNamingTheCredentialAndItsUser()
    {
        // Arrange
        ...
        // Act
        var result = await harness.AuthenticateAsync(Header("user", Password));

        // Assert
        Assert.True(result.Succeeded);
        Assert.NotNull(result.Admitted);
        Assert.Equal(CredentialId, result.Admitted.CredentialId);
        Assert.Equal(User, result.Admitted.User);
    ...
    public async Task AuthenticateAsync_TheWrongPassword_IsRefusedAsAnUnrecognizedCredential()
    {
        ...
        // Act
        var result = await harness.AuthenticateAsync(Header("user", "the-wrong-password"));
    ...
    public async Task AuthenticateAsync_AHeldUsernameAndOneNobodyHolds_SpendTheSameNumberOfVerifications()
    {
        ...
        // Act
        await held.AuthenticateAsync(Header("user", "the-wrong-password"));
    ...
    [Theory]
    [InlineData(null, UserPasswordRejection.CredentialMissing)]
    [InlineData("", UserPasswordRejection.CredentialMissing)]
    [InlineData("   ", UserPasswordRejection.CredentialMissing)]
    [InlineData("Bearer an-opaque-api-key", UserPasswordRejection.CredentialMalformed)]
    [InlineData("Basic not base64!", UserPasswordRejection.CredentialMalformed)]
    public async Task AuthenticateAsync_AHeaderCarryingNoBasicCredential_IsRefusedWithoutReachingTheStore(
        string? headerValue,
        UserPasswordRejection expected)
    {
        // Arrange
        using var harness = new AuthenticatorHarness();
    ...
    public async Task AuthenticateAsync_AUserIdThatIsNoUsername_IsRefusedWithoutVerifyingAnything()
    {
        ...
        // Act
        var result = await harness.AuthenticateAsync(Header("user name", Password));

        // Assert
        Assert.Equal(UserPasswordRejection.UsernameUnusable, result.Rejection);
        Assert.Equal(0, harness.PasswordHasher.VerificationCount);
    }
    ...
    public async Task AuthenticateAsync_MoreAttemptsThanTheSurfaceAllows_RefusesTheSurplusWithoutVerifyingThem()
    {
        ...
        // Act
        var results = new List<UserPasswordAuthenticationResult>();

        for (var attempt = 0; attempt < Allowed + 2; attempt++)
        {
            results.Add(await harness.AuthenticateAsync(Header("user", Password), attemptsPerMinute: Allowed));
        }

        // Assert
        Assert.Equal(Allowed, results.Count(result => result.Rejection == UserPasswordRejection.CredentialUnrecognized));
        Assert.Equal(2, results.Count(result => result.Rejection == UserPasswordRejection.TooManyAttempts));
        Assert.Equal(Allowed, harness.PasswordHasher.VerificationCount);
    }

    /// <summary>Basic re-presents the credential on every request and this deployment keeps no session, so a bucket a working password spent would bound a user's request rate rather than anybody's guessing.</summary>
    [Fact]
    public async Task AuthenticateAsync_MoreWorkingSignInsThanTheSurfaceAllowsAttempts_AreEveryOneServed()
    {
        ...
        var results = await Task.WhenAll(Enumerable
            .Range(0, Allowed + 3)
            .Select(_ => harness.AuthenticateAsync(Header("user", Password), attemptsPerMinute: Allowed)));
    ...
    public async Task AuthenticateAsync_AWorkingSignInBeforeTheGuessing_LeavesTheGuessersCapacityWhereItWas()
    {
        ...
        // Act
        var accepted = await harness.AuthenticateAsync(Header("user", Password), attemptsPerMinute: Allowed);
        var guessed = await harness.AuthenticateAsync(Header("user", "wrongpassword"), attemptsPerMinute: Allowed);
        var surplus = await harness.AuthenticateAsync(Header("user", "wrongpassword"), attemptsPerMinute: Allowed);

        // Assert
        Assert.True(accepted.Succeeded);
        Assert.Equal(UserPasswordRejection.CredentialUnrecognized, guessed.Rejection);
        Assert.Equal(UserPasswordRejection.TooManyAttempts, surplus.Rejection);
    }

    /// <summary>
    /// The guessing allowance must never cap a user's own parallelism. Basic re-presents the credential on every
    /// request and this deployment keeps no session, so a user whose client has more calls in flight than the surface
    ...
    public async Task AuthenticateAsync_MoreGuessesInFlightThanOneAxisAdmitsDerivations_RefusesTheSurplusWithoutVerifyingIt()
    {
        ...
        var inFlight = Enumerable
            .Range(0, admitted)
            .Select(_ => harness.AuthenticateAsync(Header("user", "wrongpassword")))
            .ToArray();
    ...
    public async Task AuthenticateAsync_AGuessAndThenTheWindow_AdmitsAnAttemptAgain()
    {
        ...
        // Act
        var spent = await harness.AuthenticateAsync(Header("user", Password), attemptsPerMinute: Allowed);
        var whileHeld = await harness.AuthenticateAsync(Header("user", Password), attemptsPerMinute: Allowed);
        harness.Clock.Advance(PasswordAttemptLimiter.SpentAttemptWindow);
        var afterTheWindow = await harness.AuthenticateAsync(Header("user", Password), attemptsPerMinute: Allowed);

        // Assert
        Assert.Equal(UserPasswordRejection.CredentialUnrecognized, spent.Rejection);
        Assert.Equal(UserPasswordRejection.TooManyAttempts, whileHeld.Rejection);
        Assert.Equal(UserPasswordRejection.CredentialUnrecognized, afterTheWindow.Rejection);
    }
    ...
    public async Task AuthenticateAsync_GuessesAtTwoUsernamesWithNoSourceToTellCallersApart_BoundsEachUsernameOnItsOwn()
    {
        ...
        // Act
        var firstUser = await harness.AuthenticateAsync(Header("user", Password), attemptsPerMinute: Allowed, source: null);
        var secondUser = await harness.AuthenticateAsync(Header("other", Password), attemptsPerMinute: Allowed, source: null);
        var firstUserAgain = await harness.AuthenticateAsync(Header("user", Password), attemptsPerMinute: Allowed, source: null);

        // Assert
        Assert.Equal(UserPasswordRejection.CredentialUnrecognized, firstUser.Rejection);
        Assert.Equal(UserPasswordRejection.CredentialUnrecognized, secondUser.Rejection);
        Assert.Equal(UserPasswordRejection.TooManyAttempts, firstUserAgain.Rejection);
    }
    ...
    public async Task AuthenticateAsync_GuessesAtTwoUsernamesFromOneSource_SpendsThatSourcesAllowance()
    {
        ...
        // Act
        var firstUser = await harness.AuthenticateAsync(Header("user", Password), attemptsPerMinute: Allowed);
        var secondUser = await harness.AuthenticateAsync(Header("other", Password), attemptsPerMinute: Allowed);

        // Assert
        Assert.Equal(UserPasswordRejection.CredentialUnrecognized, firstUser.Rejection);
        Assert.Equal(UserPasswordRejection.TooManyAttempts, secondUser.Rejection);
    }
    ...
    public async Task AuthenticateAsync_TheSameCallerOnASecondSurface_IsNotRefusedForTheFirstSurfacesSpentAttempts()
    {
        ...
        // Act
        var first = await harness.AuthenticateAsync(Header("user", Password), attemptsPerMinute: 1);
        var second = await harness.AuthenticateAsync(Header("user", Password), attemptsPerMinute: 1);
        ...
        // Assert
        Assert.Equal(UserPasswordRejection.CredentialUnrecognized, first.Rejection);
        Assert.Equal(UserPasswordRejection.TooManyAttempts, second.Rejection);
        Assert.Equal(UserPasswordRejection.CredentialUnrecognized, otherSurface.Rejection);
    }
    ...
    public async Task AuthenticateAsync_ARehashOfACredentialSomebodyRotatedMeanwhile_WritesOverTheVerifiedRecordOnly()
    {
        ...
        // Act
        await harness.AuthenticateAsync(Header("user", Password));
    ...
    public async Task AuthenticateAsync_ASurfaceAllowingNoAttempts_ThrowsBecauseThatWouldRefuseEveryRequestRatherThanBoundOne()
    {
        ...
        await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() =>
            harness.AuthenticateAsync(Header("user", Password), attemptsPerMinute: 0));
    }
    ...
    private sealed class CredentialReadGate
    {
        ...
        internal async Task<ResolvedUserCredential?> AnswerAsync(ResolvedUserCredential? credential)
        {
    ...
    private sealed class AuthenticatorHarness : IDisposable
    {
        ...
        internal UserPasswordAuthenticator Authenticator { get; }
        ...
        private static ResolvedUserCredential Credential(bool enabled) => new(
            CredentialId,
            User,
            UserCredentialMethod.Password,
            Grant,
