...
public sealed class MailKitImapSessionResilienceTests
{
    ...
    public async Task OpenReadOnlyAsync_ServerRejectsTheCredential_FailsOnTheFirstAttemptWithoutResolvingTheSecretAgain()
    {
        ...
        client.AuthenticateException = new global::MailKit.Security.AuthenticationException("the credential was rejected");
    ...
    public async Task OpenReadOnlyAsync_ServerNeverAnswersTheConnect_SpendsEveryAttemptAndReportsTheMailboxUnavailable()
    {
        ...
        Assert.Collection(
            resilience.Logs.Records.Where(
                record => record.Message.Contains("will retry as attempt", StringComparison.Ordinal)),
            record => Assert.Contains("will retry as attempt 2", record.Message, StringComparison.Ordinal),
            record => Assert.Contains("will retry as attempt 3", record.Message, StringComparison.Ordinal));
    }
