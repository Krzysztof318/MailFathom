...
/// <summary>Covers the three acts a user takes on one of their own drafts, and whose drafts each may reach.</summary>
/// <remarks>
/// The claim under test is the same one three times: an identifier becomes a draft the caller's own user holds before
/// anything acts on it, and a draft another user holds answers exactly as one nobody holds. What each act then does is
/// the book's and the promotion's own, and is covered where those are.
/// </remarks>
public sealed class UserMailDraftsTests
{
    ...
    /// <summary>Giving up one of this user's own drafts reaches the book and takes the copies out of the folder.</summary>
    [Fact]
    public async Task DiscardAsync_ADraftThisUserHolds_GivesItUp()
    {
        // Arrange
        var harness = Harness();
    ...
    /// <summary>Sending one of this user's drafts writes the ordinary outgoing record every send is written down as.</summary>
    [Fact]
    public async Task SendAsync_ADraftThisUserHolds_QueuesItAsAnOrdinarySend()
    {
        // Arrange
        var harness = Harness();
    ...
    /// <summary>Builds the user-facing acts over the harness a test arranged.</summary>
    private static UserMailDrafts UserDraftsOver(
