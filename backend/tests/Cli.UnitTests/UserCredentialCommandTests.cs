...
/// <summary>Covers the commands that provision, list, rotate, suspend, and remove a user's sign-in.</summary>
/// <remarks>
...
/// </remarks>
public sealed class UserCredentialCommandTests : IDisposable
{
    ...
    public async Task Create_AProvisionedCredential_ReportsTheIdentifierWithoutRepeatingThePassword()
    {
        ...
        // Act
        await this.RunAsync(deployment, "credential", "create", "--method", "password", "--username", "user", "--endpoint", Endpoint);
        ...
        Assert.Contains(
            ...
            line => line.Contains($"{FakeUserCredentialDeployment.ProvisionedCredentialId:D}", StringComparison.Ordinal));
    }
    ...
    public async Task Create_Always_AsksForThePasswordRatherThanAcceptingOneAsAnArgument()
    {
        ...
        Assert.Contains("user", this.harness.Console.LastPrompt, StringComparison.Ordinal);
    ...
    public async Task Create_NoPasswordSupplied_SendsNothingAndSaysWhatToDo()
    {
        ...
        Assert.Empty(deployment.RequestsTo(HttpMethod.Post, AdminEndpointRoutes.UserCredentialsPath(User)));
    ...
    {
        // Arrange
        using var deployment = FakeUserCredentialDeployment.Refusing(
            [User],
    ...
    public async Task Create_AUsernameAlreadyHeld_RepeatsWhatTheDeploymentSaid()
    {
        ...
        using var deployment = FakeUserCredentialDeployment.Refusing(
            ...
            "Another credential already signs in as 'user'.");
        this.harness.Console.SecretToSupply = Password;
    ...
    /// <summary>The deployment this serves holds one user, so the ordinary invocation names none and acts on the one there is.</summary>
    [Fact]
    public async Task Create_ADeploymentHoldingOneUser_ActsForThatUserWithoutBeingTold()
    {
        // Arrange
        using var deployment = FakeUserCredentialDeployment.Holding([User]);
        this.harness.Console.SecretToSupply = Password;
        ...
        // Assert
        Assert.Single(deployment.RequestsTo(HttpMethod.Post, AdminEndpointRoutes.UserCredentialsPath(User)));
    }

    /// <summary>Guessing which of several users a credential is for would provision a way into the wrong person's mail.</summary>
    [Fact]
    public async Task Create_ADeploymentHoldingSeveralUsers_RefusesAndNamesTheIdentifiersToChooseFrom()
    {
        // Arrange
        ...
        Assert.Contains(
            ...
            line => line.Contains("--user", StringComparison.Ordinal)
                && line.Contains($"{AnotherUser:D}", StringComparison.Ordinal));
    }
    ...
    public async Task Create_AnUserTheInvocationNamed_ActsForThatUserWithoutReadingTheRoster()
    {
        ...
        // Assert
        Assert.Equal(CliExitCode.Success, exitCode);
        Assert.Single(deployment.RequestsTo(HttpMethod.Post, AdminEndpointRoutes.UserCredentialsPath(AnotherUser)));
        Assert.Empty(deployment.RequestsTo(HttpMethod.Get, AdminEndpointRoutes.UsersPath));
    }
    ...
    public async Task List_AnUserHoldingCredentials_ReportsEachOneWithNothingDerivedFromItsPassword()
    {
        ...
        using var deployment = FakeUserCredentialDeployment.Holding(
            [User],
            FakeUserCredentialDeployment.Credential(CredentialId, "user"),
            FakeUserCredentialDeployment.Credential(Guid.Empty, "user.reader", enabled: false));
        ...
        // Assert
        Assert.Equal(CliExitCode.Success, exitCode);
        Assert.Contains(this.harness.Console.Lines, line => line.Contains("user.reader", StringComparison.Ordinal));
    ...
    public async Task Rotate_ANewPasswordTypedAtThePrompt_SendsItToTheCredentialsOwnPasswordRoute()
    {
        ...
        var rotation = Assert.Single(deployment.RequestsTo(
            HttpMethod.Put,
            AdminEndpointRoutes.UserCredentialMaterialPath(User, CredentialId)));
    ...
    public async Task SetEnabled_EitherCommand_SendsTheDecisionItsNameStates(string verb, bool written)
    {
        ...
        var decision = Assert.Single(deployment.RequestsTo(
            HttpMethod.Put,
            AdminEndpointRoutes.UserCredentialEnablementPath(User, CredentialId)));
    ...
    public async Task Delete_AnAgreedRemoval_ShowsTheCredentialBeforeRemovingIt()
    {
        ...
        Assert.Single(deployment.RequestsTo(
            HttpMethod.Delete,
            AdminEndpointRoutes.UserCredentialPath(User, CredentialId)));
    }
    ...
    public async Task Delete_AListingTheDeploymentRefuses_ReportsThatAndStillSendsTheRemoval()
    {
        ...
        using var deployment = FakeUserCredentialDeployment.RefusingTheListing([User], Refusal);
        this.harness.Console.AnswerToGive = true;
    ...
    {
        // Arrange
        const string Refusal = "User holds no such credential. List the user's credentials to read what it holds.";
        using var deployment = FakeUserCredentialDeployment.Refusing([User], HttpStatusCode.BadRequest, Refusal);
        this.harness.Console.AnswerToGive = true;
    ...
    public async Task Create_AnApiKey_SendsNoMaterialAndPrintsWhatTheDeploymentMintedOnce()
    {
        ...
        using var deployment = FakeUserCredentialDeployment.Provisioning([User], "mfk_not…", Minted);
    ...
    public async Task Create_APublicKey_SendsWhatTheFileHeldAndReportsTheFingerprintToNameItBy()
    {
        ...
        try
        {
            using var deployment = FakeUserCredentialDeployment.Provisioning([User], "SHA256:not-a-real-fingerprint", null);
    ...
    /// <summary>A mapped subject holds no material of its own: what it states is which token this deployment resolves onto which user.</summary>
    [Fact]
    public async Task Create_AMappedSubject_SendsTheIssuerAndSubjectAndNoMaterial()
    {
        ...
        using var deployment = FakeUserCredentialDeployment.Provisioning([User], $"{Issuer} {Subject}", null);
    ...
    /// <summary>What a credential grants is written where it is provisioned, so the invocation carries the grant and the deployment records it beside the user.</summary>
    [Fact]
    ...
    public async Task List_CredentialsOfSeveralMethods_ReportsEachMethodAndWhatResolvesIt()
    {
        ...
        using var deployment = FakeUserCredentialDeployment.Holding(
            [User],
            FakeUserCredentialDeployment.Credential(CredentialId, "user", permissions: "mailfathom.mail.read"),
            FakeUserCredentialDeployment.Credential(Guid.Empty, null, "api-key"));
    ...
    public async Task List_ACredentialWhoseLookupIsWithheld_SaysSoRatherThanLeavingTheCellEmpty()
    {
        ...
        using var deployment = FakeUserCredentialDeployment.Holding(
            [User],
            FakeUserCredentialDeployment.Credential(CredentialId, null, "api-key"));
