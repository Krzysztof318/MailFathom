...
/// <summary>A deployment answering the user roster and the five credential routes.</summary>
/// <remarks>
/// The roster is answered by every shape here rather than only by the tests about it, because every credential command
/// settles which user it acts for before doing anything else — a double serving only its own route would fail each of
/// them on a 404 that says nothing about what the command was doing.
/// </remarks>
internal static class FakeUserCredentialDeployment
{
    ...
    /// <summary>Builds a deployment holding the users named, and the credentials each of them holds.</summary>
    /// <param name="users">The users the roster reports, in the order it serves them.</param>
    /// <param name="credentials">What a listing of any user's credentials answers with.</param>
    /// <returns>The deployment.</returns>
    internal static FakeHttpMessageHandler Holding(IReadOnlyList<Guid> users, params string[] credentials) =>
    ...
    internal static FakeHttpMessageHandler Refusing(
        ...
        string detail) =>
        new((request, _) => Task.FromResult(Answer(request, users, [], status, detail, "user", mintedKey: null)));

    /// <summary>Answers every credential listing with a refusal while every write is accepted.</summary>
    /// <param name="users">The users the deployment holds records for.</param>
    /// <param name="detail">What the refusal says, as the problem document carries it.</param>
    /// <returns>The deployment.</returns>
    ...
    internal static FakeHttpMessageHandler RefusingTheListing(IReadOnlyList<Guid> users, string detail) =>
        new((request, _) => Task.FromResult(
            ...
            && (request.RequestUri?.AbsolutePath ?? string.Empty).EndsWith("/credentials", StringComparison.Ordinal)
                ...
                : Answer(request, users, [], HttpStatusCode.OK, string.Empty, "user", mintedKey: null)));
        ...
        if (path.EndsWith("/credentials", StringComparison.Ordinal))
        {
            return request.Method == HttpMethod.Get
                ? FakeAdminEndpoint.Json(
                    HttpStatusCode.OK,
                    $$"""{"user":"{{(users.Count > 0 ? users[0] : Guid.Empty):D}}","credentials":[{{string.Join(',', credentials)}}]}""")
                : Written(
                    writeStatus,
                    detail,
