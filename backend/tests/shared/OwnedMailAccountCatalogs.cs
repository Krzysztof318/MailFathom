...
/// is the comparison that type performs between the user a caller was admitted for and the user each served account
/// carries. A test that needs accounts served and says nothing about the user substitutes the port instead.
/// </remarks>
internal static class OwnedMailAccountCatalogs
{
    ...
    /// <param name="servedAccounts">The accounts this deployment serves, each carrying the user it belongs to.</param>
    /// <returns>The catalog, answering each user with the served accounts attributed to them and with none of anybody else's.</returns>
    /// <remarks>
