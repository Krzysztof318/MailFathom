...
/// <remarks>
/// Every use case over the book takes one, and composing it by hand means arranging a deployment-user source in each
/// suite that arranges a caller. What a test states here is the principal, and — where it matters — which user the
/// deployment serves, because those are the two facts the resolution reads.
/// </remarks>
internal static class ContactBookOwnerships
{
    /// <summary>Builds the resolution for work acting on the book of the user the deployment serves.</summary>
    ...
    /// <summary>Builds the resolution for a deployment serving <see cref="SyntheticMailUser.Deployment" />.</summary>
    ...
    internal static ContactBookOwnership For(AccessAuthorization authorization) =>
        For(authorization, SyntheticMailUser.Deployment);

    /// <summary>Builds the resolution for a deployment serving one named user.</summary>
    /// <param name="authorization">The authorization the use case beside it was given.</param>
    /// <param name="deploymentUser">The user a principal acting for none falls back to.</param>
    /// <returns>The resolution that use case consults.</returns>
    internal static ContactBookOwnership For(AccessAuthorization authorization, MailUserId deploymentUser) =>
        new(authorization, new StatedDeploymentUser(deploymentUser));

    /// <summary>Names the user a test says this deployment serves.</summary>
    private sealed class StatedDeploymentUser(MailUserId user) : IDeploymentMailUserSource
    {
