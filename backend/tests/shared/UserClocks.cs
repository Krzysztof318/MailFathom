...
/// </remarks>
internal static class UserClocks
{
    ...
    internal static UserClock Reading(DateTimeOffset instant, string zoneId = "UTC")
    {
        if (!UserTimeZone.TryRead(zoneId, out var zone))
        {
    ...
    private sealed class StatedTimeZones(UserTimeZone zone) : IUserTimeZones
    {
        public UserTimeZone ZoneOf(UserId user) => zone;

        public UserTimeZone? StatedZoneOf(UserId user) => zone;
    }
}
