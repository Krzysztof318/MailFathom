...
/// </summary>
public sealed class ServedUsersConvergenceTests
{
    ...
    private static readonly UserId Alex = UserId.Create(new Guid("33333333-3333-3333-3333-333333333333"));
    ...
    public async Task ConvergeAsync_EveryRecordAtTheVersionBound_ReadsNoRecord()
    {
        ...
        // Assert
        await documents.DidNotReceive().ReadAsync(Arg.Any<UserId>(), Arg.Any<CancellationToken>());
    }
    ...
    public async Task ConvergeAsync_ARecordThatDoesNotBind_KeepsTheVersionBoundAndReportsItOnce()
    {
        ...
        var log = new RecordingLogger<ServedUsersConvergence>();
    ...
    public async Task ConvergeAsync_BeforeTheStartupGateSettledARoster_ReadsNothing()
    {
        ...
        // Act
        await Convergence(documents, new ServedUsers()).ConvergeAsync(TestContext.Current.CancellationToken);

        // Assert
        await documents.DidNotReceive().ReadVersionsAsync(Arg.Any<int>(), Arg.Any<CancellationToken>());
    }

    private static ServedUsers ServingAlexAt(long version)
    {
        var roster = new ServedUsers();

        roster.Resolved([new ServedUser(Alex, "alex", [])], new Dictionary<UserId, long> { [Alex] = version });

        return roster;
    }

    /// <summary>The rows a deployment holds: each user's version, and the record a read of that user answers with.</summary>
    private static IUserSettingsDocumentReader Holding(params (UserId User, string Json, long Version)[] records) =>
    ...
    private static ServedUsersConvergence Convergence(
        IUserSettingsDocumentReader documents,
        ServedUsers roster,
        RecordingLogger<ServedUsersConvergence>? log = null,
        ...
        new(
            ...
            log ?? new RecordingLogger<ServedUsersConvergence>());
}
