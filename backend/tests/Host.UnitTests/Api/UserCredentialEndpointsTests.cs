...
/// answers with.
/// </remarks>
public sealed class UserCredentialEndpointsTests
{
    ...
    [Fact]
    public async Task ListAsync_ARequestNamingNoUser_IsRefusedWithoutReachingTheStore()
    {
        // Arrange
        var harness = new EndpointHarness(MailFathomPermission.AdminRead);
        ...
        await harness.Credentials.DidNotReceiveWithAnyArgs()
            .ReadForUserAsync(default, TestContext.Current.CancellationToken);
    }
    ...
    public async Task ProvisionAsync_AUsernameAndAPasswordTheDeploymentAccepts_ReportsTheIdentifierTheActMinted()
    {
        ...
        // Assert
        var provisioned = Assert.IsType<Ok<UserCredentialProvisionedResponse>>(result.Result).Value!;

        Assert.NotEqual(Guid.Empty, provisioned.CredentialId);
        Assert.Equal("user", provisioned.Lookup);
    ...
    public async Task ProvisionAsync_AValueAnotherCredentialIsAlreadyResolvedBy_IsAnsweredAsAConflict()
    {
        // Arrange
        var harness = new EndpointHarness(MailFathomPermission.AdminCredentialsWrite);
        harness.AnswerCreateWith(UserCredentialWriteOutcome.LookupTaken);
        ...
        Assert.Contains(UserCredentialMethod.Password.Name, refusal, StringComparison.Ordinal);
    }
    ...
    public async Task ProvisionAsync_AUserAlreadyHoldingAsManyCredentialsAsOneMay_IsAnsweredAsAConflict()
    {
        // Arrange
        var harness = new EndpointHarness(MailFathomPermission.AdminCredentialsWrite);
        harness.AnswerCreateWith(UserCredentialWriteOutcome.UserAtCredentialCeiling);
        ...
        Assert.Contains(
            UserCredential.MaximumListedPerUser.ToString(CultureInfo.InvariantCulture),
            refusal,
    ...
    public async Task ProvisionAsync_AUsernameOutsideTheAcceptedForm_IsRefusedNamingWhatIsAccepted(string? username)
    {
        ...
        // Assert
        var refusal = AssertRefusal(result.Result, StatusCodes.Status400BadRequest);
        Assert.Contains(UserCredentialUsername.DescribeAcceptedForm(), refusal, StringComparison.Ordinal);
    }
    ...
    public async Task ReplaceMaterialAsync_AnApiKey_AnswersWithTheKeyTheClientMustPresentFromNowOn()
    {
        ...
        // Assert
        var rotated = Assert.IsType<Ok<UserCredentialRotatedResponse>>(result.Result).Value!;
    ...
    public async Task ReplaceMaterialAsync_AMethodPresentingWhatThisDeploymentDidNotIssue_IsRefusedWithoutWriting()
    {
        ...
        Assert.Contains(UserCredentialMethod.OAuthSubject.Name, refusal, StringComparison.Ordinal);
    ...
    /// <summary>An identifier out of the wrong user's listing rotates nothing, and the answer says which pair the deployment could not find.</summary>
    [Fact]
    public async Task ReplaceMaterialAsync_ACredentialTheUserDoesNotHold_IsRefusedNamingBothIdentifiers()
    {
        // Arrange
        var harness = new EndpointHarness(MailFathomPermission.AdminCredentialsWrite);
    ...
    [Fact]
    public async Task DeleteAsync_ACredentialTheUserHolds_IsAnsweredWithNoBody()
    {
        // Arrange
        var harness = new EndpointHarness(MailFathomPermission.AdminCredentialsWrite);
    ...
    private static UserCredentialProvisioningRequest PasswordRequest(
        ...
        UserCredentialMethod.Password.Name,
        username,
        password,
        PublicKey: null,
        Issuer: null,
