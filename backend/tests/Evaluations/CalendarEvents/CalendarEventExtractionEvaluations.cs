...
/// <summary>Measures which calendar events the extraction agent reads out of a text, under every declared model.</summary>
/// <remarks>A theory over the cases for the reason <c>DiscoveryPlanningEvaluations</c> gives.</remarks>
public sealed class CalendarEventExtractionEvaluations
{
    ...
    public static TheoryData<string> Cases { get; } = new(CalendarEventExtractionCase.All.Select(static scenario => scenario.Name));

    [Theory(Skip = AiEvaluationRun.SkipReason, SkipUnless = nameof(EvaluationsRequested))]
    [MemberData(nameof(Cases))]
    public async Task ReadAsync_AMessageOrATypedSentence_EveryDeclaredModelWritesDownTheEventsItNames(string caseName)
    {
        // Arrange
        var request = CalendarEventExtractionScenario.RequestFor(CalendarEventExtractionCase.Named(caseName));

        // Act
        var shortfalls = await StructuredAnswerScenario.MeasureEveryDeclaredModelAsync(
            ChatCapability.CalendarEventExtraction,
            request,
