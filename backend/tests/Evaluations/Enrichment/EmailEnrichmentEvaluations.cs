...
/// <summary>Measures the marks the enrichment agent writes for each message, under every declared model.</summary>
...
public sealed class EmailEnrichmentEvaluations
{
    ...
    public static TheoryData<string> Cases { get; } = new(EmailEnrichmentCase.All.Select(static scenario => scenario.Name));

    [Theory(Skip = AiEvaluationRun.SkipReason, SkipUnless = nameof(EvaluationsRequested))]
    [MemberData(nameof(Cases))]
    public async Task DeriveAsync_AMessage_EveryDeclaredModelWritesTheMarksItSupports(string caseName)
    {
        // Arrange
        var request = EmailEnrichmentScenario.RequestFor(EmailEnrichmentCase.Named(caseName));

        // Act
        var shortfalls = await StructuredAnswerScenario.MeasureEveryDeclaredModelAsync(
            ChatCapability.Enrichment,
            request,
