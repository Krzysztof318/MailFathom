...
/// address under a reserved domain, nothing anybody received — and read through <see cref="CorpusMessage.Read" />, so its
...
internal static class WrittenCorpus
{
    ...
    private static IReadOnlyList<CorpusMessage> Exchange(int firstPosition, params WrittenMessage[] messages) =>
        [.. messages.Select((message, offset) => CorpusMessage.Read(message.Compose(), firstPosition + offset))];
