...
/// <summary>Proves what one user's record is read as, and what the schema refuses to hold two of.</summary>
/// <remarks>
...
public sealed class OrchestratedUserSettingsDocumentTests(MailFathomOrchestrationFixture orchestration)
{
    /// <summary>The label the migration chain backfills onto the user an upgraded deployment already served.</summary>
    /// <remarks>
    ...
    /// stopped being true.
    /// </remarks>
    private const string ProvisionedUserLabel = "user";

    /// <summary>The provisioned user is read as an envelope with the empty document their row was created with.</summary>
    [Fact]
    public async Task ReadAsync_TheUserTheDeploymentHolds_IsOneRowCarryingItsLabelVersionAndMarker()
    {
        // Arrange
        ...
        var record = await services.InScopeAsync(
            async (scope, token) =>
            {
                var user = await scope.GetRequiredService<MailFathomDbContext>()
                    .UserAccounts
                    .AsNoTracking()
    ...
    public async Task ReadAsync_AUserTheDeploymentHoldsNoRecordOf_IsAbsent()
    {
        ...
        // Act
        var record = await services.InScopeAsync(
            (scope, token) => scope.GetRequiredService<IUserSettingsDocumentReader>().ReadAsync(unheldUser, token),
            cancellationToken);
    ...
    /// <c>OrchestratedForeignUser</c> states: a deployment whose accounts come from configuration holds exactly one
    /// user record, and a second one left in <c>settings_accounts</c> refuses the folder bindings every later class in
    /// this collection arranges. It is what keeps the first test in this class reading the provisioned user too.
    /// </remarks>
    [Fact]
    public async Task Insert_ASecondUserUnderALabelAlreadyHeld_IsRefusedByTheDatabase()
    {
        // Arrange
        var cancellationToken = TestContext.Current.CancellationToken;
        await using var services = await OrchestratedMailFathomServices.StartAsync(orchestration, cancellationToken);
        var contested = $"user-{Guid.NewGuid():N}";
        ...
        try
        {
            await services.CommitAsync(
                async (_, session, token) =>
                {
                    ...
                    context.UserAccounts.Add(NewUser(holder, contested));
                },
                cancellationToken);
            ...
            var refused = await Record.ExceptionAsync(() => services.CommitAsync(
                async (_, session, token) =>
                {
                    ...
                    context.UserAccounts.Add(NewUser(contender, contested));
                },
                cancellationToken));
    ...
    /// refusal an operator is told to expect. The user is provisioned and erased here like every other one this
    /// class seeds.
    /// </remarks>
    [Fact]
    public async Task ReadAsync_ARecordPastWhatThisBuildBinds_IsRefusedRatherThanRead()
    {
        ...
        try
        {
            await OrchestratedForeignUser.ProvisionAsync(services, overfilled, cancellationToken);

            await services.CommitAsync(
                async (_, session, token) =>
                {
                    var context = await EfCorePersistenceSessionAccessor.JoinAsync(session, token);
                    var record = await context.UserAccounts.SingleAsync(user => user.Id == overfilled, token);
            ...
            // Assert
            var unreadable = Assert.IsType<UserSettingsUnreadableException>(refused);
            Assert.Equal(MailFathomErrorCode.UserSettingsUnreadable, unreadable.ErrorCode);
            Assert.Contains(overfilled.ToString(), unreadable.Message, StringComparison.Ordinal);
        }
        finally
        {
            await OrchestratedForeignUser.EraseAsync(services, overfilled);
        }
    }

    /// <summary>Composes one user document larger than the ceiling, as one setting rather than as many.</summary>
    /// <remarks>
    ...
    private static string DocumentPastTheCeiling() =>
        $$"""{"Filler":"{{new string('f', UserSettingsDocument.MaximumOctets + 1024)}}"}""";

    private static UserAccountEntity NewUser(Guid userId, string displayName) => new()
    {
        Id = userId,
