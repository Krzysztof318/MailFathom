...
/// <summary>Proves a stored message belongs to the user of the account it names, and to nobody else's read.</summary>
/// <remarks>
/// <para>
/// Three claims live here that no substitute can make. That the user reaches the row at all is what a real
/// <c>INSERT</c> leaves behind, and a column left at its default would be reported by nothing else. That a read
/// narrowed by user returns none of another user's mail is a predicate PostgreSQL evaluates, stated here as the two
/// counts of one identifier under two users so the user column is the only thing separating them. And that a
/// timeline naming one account is served from the index built for it is a plan, which needs enough rows under one
/// user for the account term to stop being the whole of the selectivity.
/// </para>
/// <para>
/// The second user is provisioned here and erased in a <c>finally</c>, for the reason
/// <see cref="OrchestratedForeignUser" /> gives. Two accounts are hung on them rather than one, because a corpus in
/// which the user and the account narrow to the same rows would let a plan chosen for either look like a plan chosen
/// for the account.
/// </para>
/// <para>
/// The strongest form of the claim is the second test here: two users each holding an account under one identifier,
/// which the composite key on <c>mailbox_accounts</c> is what permits. It is stated separately because the first test
/// gives every user identifiers of its own, so the account term could account for the separation there and only the
/// user column can account for it in the second.
/// </para>
/// </remarks>
...
public sealed class OrchestratedMailOwnershipTests(MailFathomOrchestrationFixture orchestration)
{
    ...
    /// <summary>
    /// Enough mail under one user for a sequential scan to be the more expensive plan, split over two accounts so the
    ...
    /// <summary>The UID of the one message the served user stores here, which is this class's control.</summary>
    /// <remarks>
    /// An absence proves nothing unless the same observation reports a presence, and the observation below is the
    /// served user's own timeline read over this alias. Without a message of theirs in it, a read that returned
    /// nothing because the folder was empty would look exactly like a read that returned nothing because the mail
    /// belonged to somebody else.
    /// </remarks>
    private const uint ServedUserControlUid = 60_900;
    ...
    /// <summary>
    /// The occurrence both users store, deliberately the same UID under the same alias and the same account
    /// identifier: what separates the two rows is the folder each user's account bound, and nothing else.
    /// </summary>
    private const uint SharedNameUid = 61_000;

    private const string ServedUserSharedNameSubject = "mail-ownership-shared-served";

    private const string ForeignUserSharedNameSubject = "mail-ownership-shared-foreign";

    private const int PageSize = 50;

    /// <summary>Counts one user's stored mail of one account within one folder alias, which is the claim's narrowing.</summary>
    /// <remarks>
    ...
    private const string OwnedFolderMailCountSql =
        ...
        WHERE stored_emails."UserId" = @userId
    ...
    private const string ServedUserControlSubject = "mail-ownership-served-control";

    /// <summary>Counts one account's stored mail under one user, which is the narrowing the claim turns on.</summary>
    private const string OwnedMailCountSql =
        """
        SELECT count(*)
        FROM stored_emails
        WHERE "UserId" = @userId AND "MailboxAccountId" = @accountId
        """;
    ...
    [Fact]
    public async Task StoredEmails_MailUnderASecondUser_HangOnThatUserAndAreServedToNoOtherUser()
    {
        // Arrange
        ...
        try
        {
            ...
            {
                await SeedForeignMailAsync(services, foreignUser, accountId, cancellationToken);
            }

            await AnalyzeAsync(services, cancellationToken);

            // Act
            var storedRows = await ReadForeignRowUsersAsync(services, cancellationToken);
            var accountRows = await ReadForeignAccountUsersAsync(services, cancellationToken);
            var folderRows = await ReadForeignFolderUsersAsync(services, cancellationToken);

            var underTheirUser = await CountAsync(services, foreignUserId, FirstForeignAccount, cancellationToken);
            var underOurs = await CountAsync(services, servedUser.Value, FirstForeignAccount, cancellationToken);

            var ourPage = await ReadServedUserPageAsync(services, FolderAlias, cancellationToken);

            var timelinePlan = await OrchestratedQueryPlans.ReadAsync(
                services,
                AccountTimelinePageSql,
                [UserParameter(foreignUserId), AccountParameter(FirstForeignAccount), PageSizeParameter()],
                cancellationToken);

            // Assert
            // Every row the write path left behind names the user of the account it was stored under, on the three
            // tables one binding and one upsert touch.
            Assert.Equal(SeededEmailsPerForeignAccount * 2, storedRows.Count);
            Assert.All(storedRows, userId => Assert.Equal(foreignUserId, userId));
            Assert.Equal(2, accountRows.Count);
            Assert.All(accountRows, userId => Assert.Equal(foreignUserId, userId));
            Assert.Equal(2, folderRows.Count);
            Assert.All(folderRows, userId => Assert.Equal(foreignUserId, userId));

            // The same identifier under two users, counted twice. The account term is identical in both reads, so the
            // zero is the user column and nothing else.
            Assert.Equal(SeededEmailsPerForeignAccount, underTheirUser);
            ...
            Assert.Contains(ourPage, summary => summary.Subject == ServedUserControlSubject);
    ...
    /// <summary>
    /// Two users each declaring an account under one identifier hold two mailboxes, and neither user's read reaches
    /// the other's mail.
    /// </summary>
    /// <remarks>
    /// <para>
    /// This is what keying the account by its user and its identifier is for. Under the single-column key the second
    /// insert violated the primary key, so the first user to claim a name claimed it for the deployment; here both
    /// rows exist, both bind the same alias, and both store an occurrence carrying the same UID.
    /// </para>
    /// <para>
    /// The account term is identical in every read below, which is what makes the separation attributable to the user
    /// column alone. The served user's page is read through the production timeline reader rather than by statement,
    /// because what a caller sees is what the claim is about.
    /// </para>
    /// </remarks>
    [Fact]
    public async Task MailboxAccounts_TwoUsersDeclaringOneIdentifier_AreTwoMailboxesNeitherUserReadsTheOtherOf()
    {
        // Arrange
        var cancellationToken = TestContext.Current.CancellationToken;
        await using var services = await OrchestratedMailFathomServices.StartAsync(orchestration, cancellationToken);
        ...
        try
        {
            ...
            await SeedSharedNameMailAsync(
                services,
                MailAccountIdentity.Create(servedUser, sharedAccountId),
                ServedUserSharedNameSubject,
                cancellationToken);

            await SeedSharedNameMailAsync(
                services,
                MailAccountIdentity.Create(foreignUser, sharedAccountId),
                ForeignUserSharedNameSubject,
                cancellationToken);

            // Act
            var accountUsers = await ReadAccountUsersOfAsync(services, sharedAccountId.Value, cancellationToken);
            ...
            var ourPage = await ReadServedUserPageAsync(services, SharedNameFolderAlias, cancellationToken);

            // Assert
            // One identifier, two accounts, one per user. This is the insert the previous key refused.
            Assert.Equal(2, accountUsers.Count);
            Assert.Contains(servedUser.Value, accountUsers);
            Assert.Contains(foreignUserId, accountUsers);

            // One message each, counted with the same account term. The difference is the user column and nothing else.
            Assert.Equal(1, underTheirs);
            Assert.Equal(1, underOurs);

            // The served user's own read reaches their message and not the other user's, over one alias of one
            // account identifier — so the absence is the user narrowing rather than an account term that missed.
            Assert.Contains(ourPage, summary => summary.Subject == ServedUserSharedNameSubject);
            Assert.DoesNotContain(ourPage, summary => summary.Subject == ForeignUserSharedNameSubject);
        }
        finally
        {
            await OrchestratedForeignUser.EraseAsync(services, foreignUserId);
    ...
    /// <summary>Binds the shared alias for one user's account of the shared identifier and stores one message in it.</summary>
    ...
    private static Task<List<Guid>> ReadAccountUsersOfAsync(
        OrchestratedMailFathomServices services,
        string accountId,
    ...
    /// <summary>Counts one user's mail of one account within one folder, through the connection the scoped context owns.</summary>
        ...
        CancellationToken cancellationToken) => OrchestratedQueryPlans.WithConnectionAsync(
            ...
            async (connection, token) =>
            {
                await using var command = OrchestratedQueryPlans.CreateCommand(
                    ...
                    [UserParameter(userId), AccountParameter(accountId), AliasParameter(folderAlias)]);
    ...
    /// <summary>Stores one message of the served user's own in this class's folder, as the control the absence needs.</summary>
    private static async Task SeedControlMessageAsync(
        OrchestratedMailFathomServices services,
        CancellationToken cancellationToken)
    {
        var binding = await OrchestratedFolderBinding.CommitAsync(services, FolderAlias, cancellationToken);
        var occurrence = SyntheticEmail.OccurrenceIn(binding, ServedUserControlUid);

        var commitResult = await services.CommitAsync(
            (scope, session, token) => scope.GetRequiredService<IEmailMetadataRepository>().UpsertMetadataAsync(
                session,
                SyntheticMailAccount.User,
                SyntheticEmail.RemoteMetadataOf(occurrence, ServedUserControlSubject),
                SyntheticEmail.ExtractionOf(
                    occurrence,
                    ServedUserControlSubject,
    ...
    private static async Task SeedForeignMailAsync(
        OrchestratedMailFathomServices services,
        MailUserId foreignUser,
        MailAccountId accountId,
        CancellationToken cancellationToken)
    {
        var binding = await OrchestratedFolderBinding.CommitAsync(
            services,
            MailAccountIdentity.Create(foreignUser, accountId),
            FolderAlias,
            FolderAlias,
            cancellationToken);
    ...
    private static Task<List<Guid>> ReadForeignRowUsersAsync(
        OrchestratedMailFathomServices services,
        CancellationToken cancellationToken) => services.InScopeAsync(
    ...
    private static Task<List<Guid>> ReadForeignAccountUsersAsync(
        OrchestratedMailFathomServices services,
        CancellationToken cancellationToken) => services.InScopeAsync(
            (scope, token) => scope.GetRequiredService<MailFathomDbContext>().MailboxAccounts
                .AsNoTracking()
    ...
    private static Task<List<Guid>> ReadForeignFolderUsersAsync(
        OrchestratedMailFathomServices services,
        CancellationToken cancellationToken) => services.InScopeAsync(
    ...
    /// <summary>Counts one account's stored mail under one user, through the connection the scoped context owns.</summary>
        ...
        CancellationToken cancellationToken) => OrchestratedQueryPlans.WithConnectionAsync(
            ...
            async (connection, token) =>
            {
                await using var command = OrchestratedQueryPlans.CreateCommand(
                    connection,
                    OwnedMailCountSql,
                    [UserParameter(userId), AccountParameter(accountId)]);

                return (long)(await command.ExecuteScalarAsync(token))!;
            },
            cancellationToken);

    /// <summary>Reads the served user's own timeline over one of this class's folders, the way a deployment resolves it.</summary>
    private static Task<IReadOnlyList<EmailSummary>> ReadServedUserPageAsync(
        OrchestratedMailFathomServices services,
        string folderAlias,
        CancellationToken cancellationToken) => services.InScopeAsync(
    ...
    private static NpgsqlParameter UserParameter(Guid userId) =>
