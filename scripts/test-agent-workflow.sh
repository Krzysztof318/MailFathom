...
stage_quick_start_checkout() {
  ...
  # This one is read: the script copies it rather than writing its own, so the deployment runs under the file this
  # repository reviews. The real one is staged rather than a stand-in, which is what makes the assertion below about
  # what it configures rather than about what the fixture happened to contain.
  cp "$source_repository_root/deploy/openssl/legacy-mail-server.cnf.example" "$checkout_root/deploy/openssl/"
...
# so the quick start reads its TLS parameters from the file this repository ships for exactly that server. It is the one
# place anything here turns that relaxation on, which is why what it configures is asserted rather than assumed.
quick_start_prepares_the_tls_policy_a_legacy_mail_server_needs() {
  local checkout_root compose_directory
  local strict_root
  local output_file="$test_directory/quick-start-tls-log"

  checkout_root="$(stage_quick_start_checkout 'quick-start-tls')"
  compose_directory="$checkout_root/deploy/compose"

  run_quick_start "$checkout_root" --provider fastmail > "$output_file" 2>&1

  assert_contains 'CipherString = DEFAULT@SECLEVEL=1' "$compose_directory/openssl-legacy.cnf"
  assert_contains 'OPENSSL_CONF: /etc/mailfathom/openssl-legacy.cnf' "$compose_directory/compose.override.yaml"
  assert_contains 'source: ./openssl-legacy.cnf' "$compose_directory/compose.override.yaml"

  # It reaches every TLS session the process makes, so a run that turned it on says so where the other postponed
  # decisions are listed rather than only in a comment nobody opens.
  assert_contains '  TLS policy ' "$output_file"

  # Declined by name, the platform's own policy is what stands, and nothing names a file that is not there.
  strict_root="$(stage_quick_start_checkout 'quick-start-tls-strict')"
  run_quick_start "$strict_root" --provider fastmail --no-legacy-tls > /dev/null 2>&1

  if [[ -e "$strict_root/deploy/compose/openssl-legacy.cnf" ]]; then
    printf 'A TLS policy nobody asked for was written.\n' >&2
    return 1
  fi
...
changes.ts signals - -
contacts.ts person GET /api/client/contacts
contacts.ts assertedContact GET /api/client/contacts/{contactId}
contacts.ts collectedContact GET /api/client/contacts/{contactId}
contacts.ts assertedContactPage GET /api/client/contacts
contacts.ts collectedContactPage GET /api/client/contacts/collected
contacts.ts emptyContactPage GET /api/client/contacts
contacts.ts contactWritten POST /api/client/contacts
contacts.ts contactAddressHeld POST /api/client/contacts
contacts.ts contactPromoted POST /api/client/contacts/{contactId}/promotion
contacts.ts contactErased DELETE /api/client/contacts/{contactId}
contacts.ts contactCorrespondence GET /api/client/contacts/{contactId}/correspondence
...
discovery.ts current GET /api/client/discovery/runs/{runId}
discovery.ts citations GET /api/client/discovery/runs/{runId}
discovery.ts retrieval GET /api/client/discovery/runs/{runId}
discovery.ts answerBlock GET /api/client/discovery/runs/{runId}
discovery.ts evidenceListBlock GET /api/client/discovery/runs/{runId}
discovery.ts timelineBlock GET /api/client/discovery/runs/{runId}
discovery.ts factTableBlock GET /api/client/discovery/runs/{runId}
discovery.ts suggestedActionBlock GET /api/client/discovery/runs/{runId}
discovery.ts completed GET /api/client/discovery/runs/{runId}
discovery.ts runTail GET /api/client/discovery/runs/{runId}
discovery.ts runWorkingTail GET /api/client/discovery/runs/{runId}
discovery.ts runStopped GET /api/client/discovery/runs/{runId}
discovery.ts runFailed GET /api/client/discovery/runs/{runId}
discovery.ts runPeriodSpent GET /api/client/discovery/runs/{runId}
discovery.ts runComposedNothing GET /api/client/discovery/runs/{runId}
discovery.ts runWithAnUnknownBlock GET /api/client/discovery/runs/{runId}
discovery.ts runAheadOfTheClient GET /api/client/discovery/runs/{runId}
