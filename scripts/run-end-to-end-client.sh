...
#   6. the user this run serves, then their mailbox, recorded through the administrative API
...
# The host started holding nobody and no mailbox: the database this run created records no user, and no configuration
# source declares a mailbox, because a mailbox is a row in a user's record. So the user is recorded first and the
# mailbox after, the way an operator writes them, through the administrative API. That ordering is the point rather than
# a detour around a missing setting — the write that commits a record publishes it to the running roster, so the user
