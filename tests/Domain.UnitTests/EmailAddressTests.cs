...
{
    /// <summary>Two addresses that differ only in case or in header folding must compare equal, which is what search and future deduplication depend on.</summary>
    [Theory]
    [InlineData("Anna.Kowalska@Example.Test")]
    [InlineData("anna.kowalska@example.test")]
    [InlineData("ANNA.KOWALSKA@EXAMPLE.TEST")]
    [InlineData("anna.kowalska@example.\r\n test")]
