...
- `SharedSources.UnitTests` and `TestDoubles.UnitTests` are the exception, because they cover the two directories no project owns: `src/shared` and `tests/shared`. Both directories are compiled into their consumers through `Compile Include` items, so each of these projects compiles the directory the same way a consumer does and asserts its contract in one place. The name is `SharedSources`, not `Shared`, because `Shared` is a reserved word in another .NET language and CA1716 rejects it as a namespace.
- Assert a shared type's own contract in the project that owns it, and keep an assertion that binds to a particular boundary in that boundary's project. The coverage marker's name and usage targets belong to `SharedSources.UnitTests`; whether `Infrastructure` applies it correctly belongs to `Infrastructure.UnitTests`.
...
- Put a test double that a second test project needs in `tests/shared` and compile it in with `<Compile Include="..\shared\*.cs" LinkBase="shared" />`, mirroring how `src/shared` is shared. Cover it in `TestDoubles.UnitTests`, because a fault in a shared double reports a false result in every suite that uses it rather than failing where the fault is.
