...
**No image carries MailFathom's own client.** The Uno Platform client whose bundle used to travel under `/app/wwwroot` was withdrawn and the client is being rebuilt in React, so `ClientEndpoint__Application__Enabled=true` fails at startup rather than serving a page. What the image does serve, once `ClientEndpoint__Enabled` is written, is the client **surface** under `/api/client` — an API a client of your own calls. [The client endpoint](https://krzysztof318.github.io/MailFathom/operations/client-endpoint.html) is the page.
