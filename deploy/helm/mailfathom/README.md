...
**And optionally the store itself.** `contentStorage.objectStorage.deploy.enabled` runs one [Silo](https://github.com/pgsty/silo) node — a maintained fork of the open-source MinIO server — on a retained claim beside MailFathom, for a deployment that wants payload bytes out of PostgreSQL without adopting a cloud provider. The endpoint is then derived from its Service rather than named, its console is not served by default, and its root credential lives in a Secret of its own that the application pod never mounts. By default it is one node and one volume: no erasure coding, no replication, and no failover. `deploy.replicas` between two and sixteen runs it as a distributed pool instead — one erasure set across the pods, one drive each, an object surviving the loss of as many pods as that set has parity — and the width is decided when the pool is created rather than raised afterwards. Because MailFathom refuses a plain `http` endpoint, the store terminates TLS with a certificate you supply for its Service name, and the authority that signed it is the `trustAnchorSecretKey` beside the credential; a pool needs that certificate to cover the peer names as well and needs its authority named in `deploy.tls.certificateAuthoritySecretKey`, which the chart refuses to install a pool without. Nothing of Silo is in this chart or in any MailFathom image — the cluster pulls it from PGSTY's own registry, under AGPL-3.0-or-later, which [THIRD_PARTY_LICENSES.md](https://github.com/Krzysztof318/MailFathom/blob/main/THIRD_PARTY_LICENSES.md) records. [Running the object store beside MailFathom](https://krzysztof318.github.io/MailFathom/operations/deployment-kubernetes.html#running-the-object-store-beside-mailfathom) and [running it as a pool](https://krzysztof318.github.io/MailFathom/operations/deployment-kubernetes.html#running-the-object-store-as-a-pool) are the pages.

**And a signal backplane, once you run more than one replica.** A client's live updates arrive over one connection held
by one replica, and a signal raised by whichever replica synchronized the account has to reach it. **The contract is
RESP rather than a product**: MailFathom asks the endpoint for `PUBLISH`, `SUBSCRIBE`, and `PSUBSCRIBE` and nothing
else, so Redis, Garnet, Valkey, or a managed equivalent all serve. `signalBackplane` either runs a
[Valkey](https://valkey.io/) StatefulSet beside MailFathom — one instance on no claim and nothing that outlives the
pod, because pub/sub keeps nothing — or is pointed at an endpoint you already operate with
`valkey.deploy: false`. **The chart refuses to render `replicaCount` above 1 with the client surface served and no
...
MailFathom is licensed under the [GNU Affero General Public License, Version 3](https://github.com/Krzysztof318/MailFathom/blob/main/LICENSE), SPDX identifier `AGPL-3.0-only`. Every third-party component it ships beside is registered in [THIRD_PARTY_LICENSES.md](https://github.com/Krzysztof318/MailFathom/blob/main/THIRD_PARTY_LICENSES.md). The software is provided without warranty and without liability, under sections 15 and 16 of that license.

**Section 13 is the part a Kubernetes operator reads.** Rendering this chart against a published image obliges nothing beyond the source this project already publishes. Build your own image from a **modified** MailFathom and let other people reach it over the network — through the ingress, the MCP surface, or the client endpoint — and those users must be offered the corresponding source of what they are talking to. Publishing the fork and pointing at it from where they reach the service is how that is done.
