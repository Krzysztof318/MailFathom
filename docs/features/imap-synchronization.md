...
- A stored email is identified by its own `StoredEmailId`, assigned once when synchronization first stores it and never changed, and everything local — reads, derived data, erasure, the client API, and the MCP tools — names it by that. `Domain` models where the server holds it as `EmailOccurrenceId`, keyed by `(account, folder, UIDVALIDITY, UID)`, which the stored row carries as an optional attribute. The folder component is a `MailFolderResolutionId` — an alias together with the generation it was bound under — rather than a folder name, for the reason [folder aliases and discovery](#folder-aliases-and-discovery) explains. `Email` is the repository-wide term for the mail artifact; `Message` is reserved so it stays unambiguous once AI conversation types exist.
...
and the derivation of what a message is about — and in what order. A run also drains
...
The repository treats `(account, folder, UIDVALIDITY, UID)` as the stable remote occurrence identity — the address a
stored email is synchronized and changed through, beside the local identity that names it everywhere else — and that tuple
...
Every replica of a deployment runs this worker, and the walk is one walk over one position, so a run takes the lease `stored-email-extraction` — the position row's own name — before it reads the position, and gives it back when the run ends, as [ADR 0031](https://github.com/Krzysztof318/MailFathom/blob/main/docs/decisions/0031-dividing-singleton-work-between-replicas-with-a-leased-scope.md) decides. A replica refused it reads nothing and asks again on its next `MailExtractionBackfill:Interval`. A run renews the lease every `MailExtractionBackfill:LeaseRenewalInterval` and stops on the first renewal it cannot complete, leaving every batch it committed durable for the next holder to resume past; a holder that crashed keeps the walk from the others for `MailExtractionBackfill:LeaseDuration` at most. A replica whose own run finds nothing left ends its worker exactly as a single replica does, having given the lease back first, so every other replica stays free to reach the same answer on its next interval and end itself too.
