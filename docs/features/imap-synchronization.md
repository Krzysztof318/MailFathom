...
- A write repository takes its EF Core context from the `IPersistenceSession` it is handed, and injects none of its own. A session created elsewhere is rejected rather than silently written around, so "this write joined the caller's transaction" is enforced instead of merely being the effect of both objects resolving from the same DI scope. Read methods take no session and use the scoped context, because a read joins no transaction.
