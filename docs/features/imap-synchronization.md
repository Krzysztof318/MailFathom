...
- `MailboxSynchronizer` opens folders through a read-only session port, requests bounded metadata batches, and skips messages above the configured raw MIME limit. It retains at most one fetched MIME payload at a time: each seen-preserving remote fetch finishes before a short local session atomically stores that occurrence's content and metadata, and that session commits and disposes before the next remote fetch starts. After the inspected window finishes, a separate short session advances the checkpoint only when the mailbox adapter reports a non-speculative UID cursor known safe from the opened folder state.
...
- `Infrastructure` provides EF Core PostgreSQL mappings for accounts, folders, message metadata, raw message content, and synchronization checkpoints. The MailKit adapter resolves folders asynchronously, caps UID progress with the opened folder UIDNEXT value, normalizes message sent dates to UTC before persistence, and rejects occurrence identities that do not belong to the open account, folder, and UIDVALIDITY scope.
