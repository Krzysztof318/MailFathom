...
That guard covers a reload, and only a reload. Across a **restart** the newly configured value is what the model is built from, while the generated column PostgreSQL already holds still carries the previous one, and nothing in the pre-migration world can tell the two apart — reading the column's actual definition needs a schema whose provenance is only knowable once specification 19's migration baseline exists. Until then, changing this setting is an operator action that must be paired with recreating the search documents, and the safe sequence is the same one any schema change needs.
