...
- **Decision 1 refined.** `MailAuthenticationMechanism` is a `readonly record struct` value object instead of an enum with a companion name table. The registered SASL name, the clear-text classification, and a `JsonConverterAttribute`-attached converter now live on the value, which removes the mapping table, the two exhaustive switches, and their unreachable `ArgumentOutOfRangeException` arms. The trade-off is that `[InlineData]` cannot carry the value, so the affected theories use `TheoryData`, and `default` is reachable — rejected in `MailAuthenticationPolicy.Create` and by the JSON converter.
