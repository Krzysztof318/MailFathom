...
| `MailSynchronization:MaxDrainedEmailsPerRun` | int | `200` | 1 – 10000; how many messages one run of an account [holding its mailbox](../features/held-mailboxes.md) takes off its source server, over both halves of the pass together — the source copies of mail already erased locally are spent first, and the stored mailbox is drained with whatever the budget has left. Raising it empties a source sooner and lengthens that account's runs. It bounds a mirrored account's run at nothing, since such an account has nothing to drain | reload |
| `MailSynchronization:MaxRestoredEmailsPerRun` | int | `100` | 1 - 10000; how many messages one run of an account [switching off](../features/held-mailboxes.md) puts back onto its source server, over both halves of the pass together — the held state written onto messages the source still holds is spent first, and the messages the drain took off are appended back with whatever the budget has left. Raising it fills a source sooner and lengthens that account's runs. It bounds a run at nothing unless the account is restoring | reload |
| `MailSynchronization:MaxDrainedEmailsPerCommand` | int | `50` | 1 – 1000; how many UIDs one `UID STORE` and `UID EXPUNGE` pair names. Lower it for a server that refuses a long UID set; it is not a rate and changes nothing about how much a run does | reload |
