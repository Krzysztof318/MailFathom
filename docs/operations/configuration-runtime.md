...
owners may name a mail account alike, and only one owner may be served while the administrative endpoint is enabled or
while the MCP or client endpoint is enabled with an empty `Authentication` list.
