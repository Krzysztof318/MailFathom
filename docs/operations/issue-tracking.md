...
## Five questions, five mechanisms
...
| Which stack does it land in? | A `backend` label, a `frontend` label, both, or neither | the rules under **Labels** |
...
## The order between issues

Some issues cannot start until another one lands, and that order is recorded twice: as prose in the body, where the reason lives, and as a GitHub issue dependency, where the fact is rendered on the issue and readable in one call. An issue carries a `blocked by` list and the mirror `blocking` list, written by the endpoint rather than by `gh issue edit`, and taking the blocker's `id` exactly as a sub-issue link takes a child's:

```bash
blocker_id=$(gh api repos/Krzysztof318/MailFathom/issues/<blocker-number> --jq .id)
gh api --method POST repos/Krzysztof318/MailFathom/issues/<blocked-number>/dependencies/blocked_by -F issue_id="$blocker_id"
```

`-F` sends the id as the number the endpoint takes, for the same reason the sub-issue call above uses it. Reading the list back is `gh api repos/Krzysztof318/MailFathom/issues/<number>/dependencies/blocked_by`, which is the call that answers *may this be started*; `.../dependencies/blocking` reads the mirror, and `DELETE .../dependencies/blocked_by/<blocker-id>` removes a relation that turned out wrong. The issue payload also carries an `issue_dependencies_summary`, which is the same answer as a count rather than as a list.

**This is not a sixth mechanism beside the five questions above**, for the same reason the sub-issue links are not one: it is a link between two issues rather than a field on one, and every question that table owns is about a single issue read alone. What it answers — what has to land before this one — no label, no milestone, and no board field answers.

**It is not the hierarchy either.** The sub-issue links remain the only source of truth for which issues deliver one feature between them, and **a parent is never blocked by its own child**: the sub-issue progress already says a parent closes when its children do, so the relation would record that a second time and bury the parent under a blocked-by list holding everything beneath it. What a parent does carry is its order against work outside itself — the parent it follows, the decision its whole feature waits on.

**And it is not the `blocked` label.** That label, with `Status: Blocked`, says the issue is stopped by something outside the project: an upstream outage, an incident, an answer owed by somebody who is not working here. An order between two issues in this repository is ordinary work in a known sequence, so it takes neither the label nor the status, however long the blocker stays open. **Labels** holds the narrow reading and this is the boundary it is narrow against.

**Only a blocker still open is written.** A relation to an issue that has already closed records what the body's prose says better. One already written is never removed when its blocker closes, though, and nothing has to be: the list keeps returning it with its closed state while `issue_dependencies_summary` counts only the blockers still open, so a closed one stops nothing and stays as the record of the order having been held to.

**The prose stays beside it.** The body's `## Dependencies` section says what is waited on and why, in the terms of the work — which half of a contract, which decision, which identifier — while the relation carries the bare fact. A relation without the sentence tells a reader deciding whether the order still holds nothing, and the sentence without the relation is what this section exists to end. Where the two disagree the body is corrected first, because that is where the reason is, and a relation written from a sentence nobody re-read is how a stale order outlives the reason for it.

### A parent's delivery order

**Every parent carries a `## Delivery order` table.** It is what a parent is for: **Issue content** opens one only where the parts of a feature have an order between them, and this is where that order is written rather than left to be assembled from the children. Four columns, none of which says what another one says:

| Column | What it says |
|---|---|
| `Wave` | The earliest point this child can start. It is derived from `Waits on` rather than decided beside it, and everything in one wave may be taken at the same time |
| `Child` | The issue, by number |
| `Waits on` | What has to land first — the real constraint, and what the relations above are written from. A number from outside this parent is a dependency on other work rather than on a sibling |
| `Runs beside` | The children this one has no order against, which may therefore be taken at the same moment even from another wave |

A wave is not a schedule and carries no date: nothing on the board schedules work, and the table says what may start rather than when it will. The table opens with the sentence that defines those four columns, so a reader who has met no other parent can read it — `#1152` is the short worked example and `#980` the long one.

Three conventions keep the eighteen tables one table. A wave is derived from the constraints between siblings alone, so a child held only by work outside this parent still takes the wave its siblings give it and the `Waits on` cell is what says what actually holds it. `Runs beside` is symmetric — where one child names another, that other names it back, and `every other child here`, or `every other child here but #x, #y`, stands in wherever it reads shorter than the list. `—` in either column means none, and a child already delivered takes `—` in `Wave` and says so in `Waits on`, because the rows below it are written against what it committed and a reader needs that as much as the pending ones. Under the table, prose names where the parallelism actually is, which line is the narrow one, and what closes the feature.

The table is written when the parent is opened, and rewritten in the same pass as whatever moved it — an issue split into two, a child added, an order discovered while implementing one. It and the dependency relations are one decision recorded twice, the table being the form a reviewer reads and the relations the form a session acts on, so a row that changes is a relation to write or delete before the pass is done.

## Labels

Every issue carries exactly one `type:*` label, and a stack label where the work lands in one of the two stacks. Nothing else is required. The type names what the work produces, which is a property of the work itself, so it is chosen when the issue is opened and then left alone; it does not track progress and it never changes because circumstances did.
...
The remaining labels are flags, applied only when they are true: `blocked` when an issue waits on something outside the project, applied together with `Status: Blocked` for the reason **Status transitions** gives and never for an order against another issue here, which **The order between issues** records instead, `security` when a change needs a security review before it merges, `parent` on an issue whose sub-issues deliver one feature between them, and `good first issue` or `help wanted` on work the project would rather someone else took. `shipped` is historical, marking the six issues written retrospectively for work that predates the roadmap; never apply it to new work. `backend` and `frontend` are not among the flags: they answer a question of their own and the subsection below holds their rule.

`security` is the one flag among those that decides something beyond how the issue reads. `Apply pull request rules` carries it onto every pull request whose body refers to the issue — the one that closes it, and equally one that only names it as related work — and `Fathom review` reads it there: the security rubric is applied to the whole change for the first three passes, and the costlier model performs every pass. [Labels the change earns](agent-workflow.md#labels-the-change-earns) and [What the security label decides](agent-workflow.md#what-the-security-label-decides) carry both halves. So applying it to an issue is a decision about how the work will be reviewed, not only about where it sits on the board.
...
**A parent carries the stacks its children land in.** It delivers nothing itself, so the only thing its labels can describe is the feature beneath it, and a feature whose children are split across the two stacks is exactly the case both labels are for. That is the one place both sit on an issue whose own diff is empty, and it is why a child still takes only the label its own work lands under.

**Nothing audits the pair, and nothing can.** `Area`, `Queue`, and `Size` are audited by their absence because every issue takes a value; here the absence is itself an answer, so an issue that belongs to neither stack and an issue nobody classified read identically. Making the absence provable would take a third label meaning *neither*, carried by the majority of issues here to buy one query, which is a value on every issue in exchange for a check. The rule is applied when the issue is opened instead, in the same pass as the `type:*` label, and triage applies it to an arrival for the same reason it applies the rest.

`Area` does not answer this question and is not made to. It groups by the part of the system a change belongs to, and each stack reaches several of its values: a `frontend` issue sits in `Client` where it is client behavior, in `Platform` where it is that stack's build tooling, and in `Release` where it is packaging. `Client` is the area most client work takes, and it is still a different statement from the label — neither implies the other.

### `agent:claude` and `agent:codex`

`agent:claude` or `agent:codex` says a session in that runtime has this issue in hand. These are the only labels that describe the session rather than the work, and the matching one is applied at the moment work on the issue genuinely begins — the worktree is being made for it, the implementation is starting — by `$start-task`, at the step named there. Reading an issue is not taking it, so triage does not apply either, and neither does planning, estimating, or answering a question about one; a marker that meant *somebody looked at this* would be worth nothing to the reader it exists for.

The marker is never removed. A session that ended is not a reason to clear it, and it stays through the close, so it reads as *a session in this runtime has had this in hand* rather than as *a session is running now*. That is the weaker claim and deliberately so: a label meaning the stronger one would be wrong from every session that stopped without clearing it, and nothing here would notice, whereas the weaker claim cannot go stale because what it records already happened.

Neither marker answers any of the five questions above, which is what lets it stand beside them rather than duplicating one. `Status` is the lifecycle fact, the built-in workflows own it, and it moves when a pull request does; the marker moves hours earlier, when the work starts. The board is also private, so `Status` answers *where is this* for one reader while the marker answers *is anyone on this and in which runtime* on a public issue list, without opening the issue and without the board. Nothing reads either marker in return — no workflow, script, or skill branches on it — so applying one by hand starts nothing and removing one stops nothing.
...
The board carries three single-select fields beyond `Status`. Set `Area` on every issue. Set `Queue` on every open issue. `Size` is written twice: a placeholder when the issue is opened, and the value the pull request's own diff measured when `$finish-change` runs.

- **`Area`** groups every item by the part of the system it belongs to. Ten values, and each is a place rather than a phase, which is what lets the grouping survive the release that produced the work in it:
  ...
  Three of those boundaries are decisions rather than descriptions. Retrieval and answering are separate because the two parent issues that own them draw that line already, so a parent's children never land in two areas. `Client` is a part of the system rather than a second spelling of the `frontend` label: it holds what the client does, while that stack's build tooling sits in `Platform` and its packaging in `Release` beside the backend's. And there is no security area, because `security` is a label: encoding one state in two places is exactly the duplication **Five questions, five mechanisms** exists to prevent.
...
- **`Queue`** is the ordering signal, and a new issue takes one of the values below `Next` without asking. `Later` is the default: accepted scope not yet started. `Needs decision` says this issue waits on an answer rather than on effort; name the `type:decision` issue that produces the answer, or state that none exists yet. `Parked` records a review outcome or a side question that carries no commitment to act — something the project decided about and may return to, which is why it never stands in for work the project has declined or for an issue nobody has read yet. A parent issue takes one of those values by the same rules as any other issue, including a parent whose children span releases. That it groups other work rather than doing any is said twice already — by the `parent` label and by the `[P] ` prefix its title carries — and a `Queue` value saying it a third time answers none of the five questions this field owns while making every reader of the field learn a value that orders nothing. So the field says of a parent exactly what it says of anything else: `Later` while its children are in flight, `Needs decision` where the feature waits on an answer, `Parked` where the project is not committed to it, and `Next` where the owner wants the whole feature in front of them, which spends one of the five slots below in the same way the issue it groups would. `Next` means in the owner's field of view now, and it has three writers. The owner sets it to mean ready to start, and at most five **open** issues hold it that way; the cap is what keeps the value a decision rather than a copy of everything already accepted. `$finish-change` sets it as well, on the issue its pull request closes, and `$prepare-release` on the issue tracking the release it just opened two pull requests for — that skill never invokes `$finish-change`, so the write is its own — both so that work already in flight is legible in the view the owner reads instead of only in the pull request list. Those sit outside the cap: an agent opening a pull request is not choosing what to start next and must never spend one of the five slots that decision uses. A closed issue keeps whatever `Queue` value it had and stops counting, which is why neither kind has to be cleared on merge and why every view that reads `Queue` filters `is:open`.
- **`Size`** measures the pull request in changed lines, additions plus deletions, including tests and documentation. The ranges are contiguous and leave no gap: `S` under 1000, `M` from 1000 to 2499, `L` from 2500 to 4999, `XL` from 5000 up. `L` is the normal size of a substantial unit of work here, so an `XL` is a genuine outlier rather than a large-sounding label.

  **The value is a measurement, and it is taken from the diff.** `$finish-change` reads the pull request's own `additions + deletions` and writes the band, inside the board write it already makes, so the field states what the change was rather than what somebody expected it to be. Nothing is estimated there and nothing is corrected: a review round landing after that write can carry a change across a boundary, which leaves the field one band out and is still a measurement rather than a guess to argue with.

  What an issue is opened with is a placeholder the board needs before any diff exists, and it is one rule rather than a judgement:

  | The issue | Placeholder |
  |---|---|
  | Anything that is not `type:feature` | `S` |
  | `type:feature` with six or fewer acceptance items | `S` |
  | `type:feature` with seven or more | `L` |

  That rule is worth 63% exact bands across the 643 merged pull requests whose titles name the issue they closed, against 52% for writing `S` on every issue and 63% for the best constant obtainable in any cell of `type:*` against acceptance-item count. That third figure is the point of quoting it: it is a ceiling rather than a score, so no finer table beats this one, and the six-row table with two surcharges and a round-up rule that stood here scored 62%. What defeats every version of it is the spread inside one cell — a `type:feature` with seven or eight acceptance items landed `S` 16 times, `M` 31, `L` 42, and `XL` 21, which is nearly flat across the whole scale.

  **Do not add a signal to it.** Every other one an issue body already carries was measured against the same population and none of them survives: Spearman against changed lines is `+0.62` for the acceptance-item count itself, `+0.55` for the acceptance section's word count, `+0.41` for the body's length, `-0.30` for the number of paths named in it, and near zero for code blocks, user stories, and issue references — while a model reading the stack label beside the count returns the same 63% under cross-validation. Asking an issue's author for a signal the body does not carry is refused for the same reason a capacity field is refused under **Issue content**: it is a question on every issue, bought in exchange for a number nothing has shown can be produced at that point.

  **The split warning is not this field's, and cannot be.** `XL` once meant split this before it starts, and read off an issue body that warning finds a third of the changes that land there while being wrong about half of the ones it names — 21 of 64 found, 18 of the 39 named wrongly — and no rule over these signals holds both halves up. What the warning needs is the file list a plan has and an issue body does not, so `$start-task` raises it in its brief once the work has been read against the repository, and this field records where the change came out.

  Re-measure when the placeholder stops matching what pull requests land at, which is a question about this project's habits rather than about the scale: the bands measure a diff and cannot be wrong about one, so a recalibration moves the rule and never the boundaries. The first half of that measurement is one call, and the second joins its issue numbers to the `type:*` label and the acceptance-item count of each body. The limit covers every merged pull request rather than a recent window, because a window silently measures a different sample than the last recalibration did:

  ```bash
  gh pr list --state merged --limit 2000 --json title,additions,deletions \
    | jq -r '.[] | select(.title | test("^\\[#[0-9]+\\]"))
             | [(.title | capture("^\\[#(?<n>[0-9]+)\\]").n), .additions + .deletions] | @tsv'
  ```

  A parent issue takes `XL` and keeps it. Its size is the sum of its children rather than a diff of its own, and no pull request closes a parent, so nothing later writes over the value — which is what makes it the one place on this field that is still an estimate, and it reads as *this is delivered in pieces* on exactly the issues that already are.
...
A view holds no state. Every one of them is a filter over fields that already exist, which is why the set below adds nothing to the five mechanisms and why no view can be left out of date by an agent forgetting a step.
...
- **The two review workflows fire on a review's state, and neither reaches an item here.** `Code review approved` reads `APPROVED` and `Code changes requested` reads `REQUEST_CHANGES`; `Fathom review` submits `COMMENT` under a `NEEDS CHANGES` heading whenever it withholds approval, deliberately, so that a reviewer reporting no status check cannot block a merge, and GitHub does not let the author of a pull request review their own — which is every pull request the project opens. It does submit `APPROVED`. What stops `Code review approved` writing `Ready to merge` from that is the board's own contents: a built-in workflow moves the item the event is about, and the item a review event is about is the pull request, which this board does not hold — it holds issues, and nothing adds a pull request to it. They stay enabled because they cost nothing and would be correct on a board that carried pull requests, but nothing in the ordinary flow reaches them.
- So `Fathom review` writes `Changes requested` itself, in a job of its own after it has published a review, on every issue the pull request's body closes. It follows a review that withheld approval — the verdict rather than the presence of findings, which stopped being the same question once a review carrying only P3 findings began publishing them under an approval. That makes the value the newest verdict rather than the first one, and makes it a verdict a reader can go and look at rather than a state nothing submitted. `docs/operations/agent-workflow.md` § *What the verdict moves on the board* holds the mechanism.
- **`Ready to merge` is not the review's to write**, and that is the one place the two verdicts part company. The column says the change is waiting on nothing but the owner pressing the button, and a reviewer reads the diff rather than the pipeline: an approval is published while `Required CI` is still running as often as not, and a pipeline can go red minutes afterwards. So `Apply pull request rules` writes it, from both halves at once — the reviewer approved the head in front of it, and every check on that head outside `CodeQL` finished without failing. A check that failed earns `Changes requested` from the same pipeline, whatever the review said, because a red build is owed an answer exactly as a finding is. [The board status the state earns](agent-workflow.md#the-board-status-the-state-earns) holds the rules; `CodeQL` is outside them because it is not a required check on `main`.
- **`In review` is the review workflow saying a review is running**, written on the same issues as the review starts. It writes over every other status, including one a built-in workflow set, because what the board said before the review started is what the review has now replaced. What replaces it depends on the verdict: a withheld approval is written minutes later by the review itself, and an approval leaves the item here until the pipelines agree with it. A run that publishes no verdict leaves it here too, which reads as a review asked for and answered by nothing. `Done` and `Blocked` are the exception at both ends of the review, for the reason the entry below gives.
- `Ready to merge` and `Changes requested` are verdicts on the change rather than further phases of the work, which is why they are a pair: a change everything agrees on moves into the first and a change a reader or a pipeline objected to moves into the second. Together they separate a pull request waiting on the owner's merge from one waiting on the agent that has to answer it, and both from work still being written — a distinction `Queue` cannot draw, because every one of those items is legitimately `Next`. `In review` separates the fourth case from all three: waiting on the reviewer itself.
- **`Conflicts` says the change no longer merges**, and it is written by `Apply pull request rules` rather than by a review. It sits beside `Changes requested` because it says the same thing about who acts next — the agent owes a rebase rather than an answer — and it is written from `Ready to merge` and from nowhere else. That column is the one claiming a change is waiting on nothing but the owner pressing the button, so a conflict is news there and only there; an item still being written, blocked, or done says nothing about whether the conflict is new. That rule runs on a push to `main`, because a merge is what makes another branch stop merging and GitHub raises no event on the branch it happened to. Nothing writes the reverse transition: the rebase that resolves the conflict is a push, the push starts a review, and the verdict leaves `Conflicts` the same way it leaves `Changes requested`. Where no review runs — a draft, or the automatic ceiling reached — the item stays there until one does.
