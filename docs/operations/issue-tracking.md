...
A wave is not a schedule and carries no date: nothing on the board schedules work, and the table says what may start rather than when it will. A child already delivered keeps its row with `—` in `Wave` and says so, because the rows below it are written against what it committed and a reader needs that as much as the pending ones.
