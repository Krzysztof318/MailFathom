...
- **`Size`** measures the pull request in changed lines, additions plus deletions, including tests and documentation. The ranges are contiguous and leave no gap: `S` under 1000, `M` from 1000 to 2499, `L` from 2500 to 4999, `XL` from 5000 up and to be split before it starts. `L` is the normal size of a substantial unit of work here, so an `XL` is a genuine warning rather than a large-sounding label.
  ...
  The estimate is read off the issue rather than guessed at, from the two things every issue already carries: its `type:*` label, and the number of items in the `Acceptance` section the work item template makes mandatory. Those two place a new issue in one row of the table below. Each row is what the issues already delivered measured, 171 of them, every one joined to the changed lines of the pull requests that closed it; the count is there because the thin rows are worth less trust than the others.

  | `type:*` | Acceptance items | Issues measured | Median changed lines | Estimate |
  |---|---|---|---|---|
  | `feature` | 3 to 6 | 7 | 1086 | `M` |
  | `feature` | 7 or 8 | 32 | 2832 | `L` |
  | `feature` | 9 or more | 47 | 3076 | `L`, or `XL` where a surcharge below applies |
  | any other | 3 to 6 | 48 | 203 | `S` |
  | any other | 7 or 8 | 27 | 534 | `S` |
  | any other | 9 or more | 10 | 1516 | `M` |

  Two surcharges carry a large feature the last step, and they are the only two the measurement separated from the spread around it: within `type:feature` at nine acceptance items or more, a change reaching `mfctl` landed `XL` in 45% of cases against 16% for one that did not, and a change carrying a migration in 36% against 19%. Such a feature doing either is estimated `XL` rather than `L`. A third signal that looks like it belongs there does not: a feature publishing a new MCP tool landed `XL` in 25% of cases against 23% for one that did not, which is no separation at all, so it is not a surcharge.
  ...
  The table is re-measured rather than re-argued, from the pull requests themselves rather than from what the board recorded — a revised `Size` overwrites the estimate it corrected, so the board keeps no record of having been wrong and only the diffs do. The first half of that measurement is one call, and the second joins its issue numbers to the `type:*` label and the acceptance-item count of each body:

  ```bash
  gh pr list --state merged --limit 400 --json title,additions,deletions \
