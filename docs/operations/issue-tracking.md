...
Writing it from the skill is not a shortcut past the automation; it is the only place the write can happen. The board's built-in workflows set `Status` and nothing else, so no project automation reaches a custom single-select field. A GitHub Actions workflow could, but this board belongs to a user rather than to an organization, and nothing scoped can write to a user's Projects v2 — a GitHub App has no permission that covers one, and a fine-grained token has no `Projects` scope at the account level. Only a classic token with the `project` scope can, and that scope is account-wide: storing one as a repository secret would give every workflow run write access to all of the owner's projects, to save a step in a skill that already holds such a token and already talks to this board. The cost is that a pull request opened by neither `$finish-change` nor `$prepare-release` moves nothing, which for a repository whose pull requests are all opened by agents is a smaller gap than the credential would be.
