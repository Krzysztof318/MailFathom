...
- **`In review` is that same workflow saying a review is running**, written on the same issues as the review starts and replaced by the verdict minutes later. It preserves nothing, unlike the verdict: a review that has started is true of the item whatever it was doing before, so a re-review asked for on a `Done` or `Blocked` item moves it too, and a run that publishes no verdict leaves the item there — which reads as a review asked for and answered by nothing.
...
- **`Blocked` is the one status a hand writes**, and the exception is narrow: it says the issue is stopped by something outside the project — an upstream outage, a GitHub Actions incident, an answer owed by somebody else — which is a fact no event on the pull request carries and therefore one no automation can derive. A `Fathom review` verdict refuses to write over it for that reason, exactly as it refuses to write over `Done`; the `In review` write does not, because it says only that a review is running now. A built-in workflow will still replace it on the next event, which is correct: a push, a link, or a merge is proof that whatever stopped the work has stopped stopping it.
