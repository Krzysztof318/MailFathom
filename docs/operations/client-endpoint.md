...
that carries the act to the server, and `changes` names it. So `applied` there means the mail has changed and a record
may still be converging, which is what keeps a message acted on during the restore from arriving back in the state the
source last had; a message the drain has already taken off the source carries no record and is answered as a held
account's act is, the restore writing the act onto it when it appends the message. A delete answered `applied` on such
an account moves the message into the local trash and records the delete the source has not heard about; the erasure a
second delete asks for is `recorded` and reaches no server, exactly as on a held account.
