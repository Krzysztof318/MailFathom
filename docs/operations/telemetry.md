...
the retry policy could not resolve, and an unexpected failure. So an ordinary restart leaves the one turn that was in
flight marked as an error, which is what an abandoned unit of work is. A pass stopped from outside is the exception: one
the host stopped, or one whose hold on the sweep was lost to another replica — which
[happens in the ordinary course](../features/embedding-backfill.md#which-replica-sweeps) — carries no outcome and leaves
its status unset, because a handover between replicas is not a failure and a rolling restart would otherwise read as one.
...
| --- | --- |
| `read_account_directory` | Which accounts the caller is assigned, and how current the local copy of each is |
...
| `scan_sensitive_content` | Wherever a read guards a payload before publishing it | `mailfathom.sensitive_content.egress_point`, `…texts` as how many texts the operation scanned, `…outcome` as `succeeded`, `refused`, `cancelled`, or `failed`, and either `mailfathom.mail.account` as the mailbox whose mail was being published or `mailfathom.user` as the user whose whole mail was |
...
`client_thread_state`, `client_reply_draft`, `client_contact_correspondence`,
`client_contact_relationship`, or `attachment_download`. The
...
**Whose mail an operation published is on the span alone**, as `mailfathom.mail.account` or as `mailfathom.user`.
Exactly one of the two is written, and which it is says what the flow resolved: a pass holding one mailbox names the
mailbox, and a read that hands out mail from whichever of a user's accounts matched names the user. A posture is the
mailbox's own, so a scan that cannot be attributed to one cannot be read against what was asked for — and either
identifier on a counter incremented once per guarded text would be an unbounded dimension, which is what every closed
tag here exists to avoid. Both attributes are absent rather than zero where a flow resolved neither.
...
which would put the credential in the telemetry written to prove it never left. The two identifiers on the span are
generated by this deployment and name a person no more than a mail account alias does.
