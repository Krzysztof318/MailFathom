...
**This is what a client's live updates need above one replica, and nothing else here needs affinity.** A signed-in
session lives in PostgreSQL, so every replica accepts one any other replica minted and honours its revocation, and the
session routes need no affinity at any replica count. A Discover run is a row beside it: the question may be asked of
one replica, the answer read from a second, and the run stopped through a third, and the run survives the pod that
composed it. The chart says nothing about either, because there is nothing in either for an operator to configure.
[Sessions live in PostgreSQL](client-endpoint.md#the-session-token-routes) and
[the Discover routes](client-endpoint.md#the-discover-routes) are where those two halves are stated.
