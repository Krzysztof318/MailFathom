...
| `Chat:ReasoningEffort` | enum | *(unset)* | `None`, `Low`, `Medium`, `High`, or `ExtraHigh`; unset sends no reasoning parameter at all, which a model that does not reason requires. `None` is not the same as unset — it states an effort of none and sends it, which is what a provider refusing tools beside an unstated effort asks for. `ExtraHigh` reaches the provider as `xhigh`, and a model that does not accept a level refuses the request rather than falling back | restart |
