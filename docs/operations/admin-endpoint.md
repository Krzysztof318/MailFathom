...
| `with something that is not a token response` | The authorization server's token endpoint was reached and what came back is not a token response, with the status it answered named. A proxy, a login page, or an error page in front of the server looks like this, and so does an answer naming a character set this machine does not carry. Nothing was signed in to and nothing was stored. |
