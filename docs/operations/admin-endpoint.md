...
| `GET /api/admin/accounts/custody` | `mailfathom.admin.read` | Reports which copy of one account's mailbox is the truth, whether a switch is still under way, and — for an account [holding its mailbox](../features/held-mailboxes.md) — how much of its source server is still to be emptied, or, for one switching off, how much is still to be put back and which appends an operator has still to settle. This is what `mfctl account custody show` asks. |
| `POST /api/admin/accounts/custody/switch` | `mailfathom.admin.custody.write` | Asks for one account's custody to become `HoldMailbox` or `MirrorSource`. **Switching on ends with a mail server no longer holding a copy of the mailbox**, message by message, as the account's own runs empty it. A refusal is answered `200` with one sentence per reason rather than an error status, because each names something about the deployment an operator corrects — a replica holding work under a build that does not know the mode, a synchronized folder playing a virtual role, an account whose deletes only flag a message `\Deleted` and leave it on the server, or, switching off, a folder mapping naming nothing the source advertises. Asking for the custody the account already has is accepted and changes nothing. This is what `mfctl account custody switch` sends. |
| `POST /api/admin/accounts/custody/restore/settle` | `mailfathom.admin.custody.write` | Records what an operator found in the folder an unanswered `APPEND` was issued against. An append whose answer never came back leaves a folder that may hold the copy and may not, and MailFathom reissues nothing on its own — so this is the one act of the mode only a person can perform, and the account stays `Restoring` until every such record is settled. *Found* keeps the record for good, so the message is never appended a second time; *not there* deletes it, and the next run appends the message as it would any other. A record nothing is standing for is answered `200` reporting that nothing was settled. This is what `mfctl account custody settle` sends. |
...
**Nothing any of the three routes answers with is mail.** A job's payload names a stored email or, for a classification
an earlier build queued, a message occurrence, and it is never read: the reading projects the identity, the kind of work, the attempts spent, the failure classification and its
...
**`mfctl folder erase --account <id> --folder <alias>` is the only thing an operator has that takes a folder's local
mail away.** Nothing else an operator does does, deliberately: switching a folder's `Synchronize` off keeps what it
stored, and removing its mapping leaves the rows where they are, so that [editing a configuration
...
The new label lasts, for every user this deployment serves. No configuration source names a user, so no start reads
...
somebody's password. Material supplied through these routes is sealed under the deployment's
...
| `mfctl account custody switch --account <id> --to <custody>` | Asks for `HoldMailbox` or `MirrorSource`, after a confirmation naming what happens to the mail, and prints every reason a refusal gave |
| `mfctl account custody settle --account <id> --record <id> --found\|--missing` | Records whether an unanswered append's copy is in the folder, which is what releases an account held in `Restoring` |
...
**Both also remove the client sessions that credential minted**, on every replica and in the act itself rather than at
the next request each of them makes, so closing a way into somebody's mail closes the clients already signed in through
it and reaches an abandoned session as readily as an active one. A rotation does not: it changes what may be presented at
[the exchange](client-endpoint.md#the-session-token-routes) and says nothing about sessions already exchanged, so an
operator ending somebody's sessions disables the credential rather than rotating it.
Ending them is an act rather than a race, and PostgreSQL is what makes it one: a disable clears the flag and removes
that credential's sessions in one transaction, while a sign-in holds the user record and then the enabled credential
row until it commits. So an exchange that authenticated just before the act either matches no enabled credential and
refuses, or commits first and has its session removed by the act that waited for it.
