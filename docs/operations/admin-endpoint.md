...
| `POST /api/admin/owners/{ownerId}/credentials` | `mailfathom.admin.credentials.write` | Provisions one, from a username and a password the body carries. **This is one of the two routes that mint a way into somebody's mail.** |
...
| `DELETE /api/admin/owners/{ownerId}/credentials/{credentialId}` | `mailfathom.admin.credentials.write` | Removes the record and frees the username. **This cannot be undone**, and it is the reason `mfctl credential delete` shows the credential and asks before sending it. |
