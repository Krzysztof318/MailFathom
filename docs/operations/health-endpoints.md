...
|---|---|---|
| Several user records while `MailSynchronization:Accounts` still supplies mailboxes | That section names no user, so with more than one user held there is no sole user for its accounts to belong to | Record each of those mailboxes against the user who owns it with `mfctl user account add`, then clear the section, so every mailbox says whose it is |
