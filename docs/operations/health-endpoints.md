...
| Several owners while `McpEndpoint` or `ClientEndpoint` admits a caller naming no owner | An owner-facing surface that authenticates nobody, or whose credentials are the deployment's own key rather than a person's, could not say whose mail an act is about, so every caller reaching it is composed against the single owner the deployment holds | Configure an owner's username and password as the only credential on those two surfaces, or switch them off. `AdminEndpoint` is not among them: an administrator acts for the deployment, and every owner-scoped route there names the owner it is for |
