...
| `…:MaxOutputTokens` | int | `4096` | 1 – 200000; what one answer may occupy. Reaching it is not a failure — the answer arrives marked as cut short. A reasoning model spends part of the same budget on reasoning before it writes, which is why the default leaves it that much room; declare a smaller figure to bound what one call can cost | reload |
...
| `…:StickySessions` | bool | `false` | that every request of one Agent conversation asks the gateway to stay on the upstream provider whose prompt cache already holds that conversation. **Honoured by OpenRouter alone today**: it is sent only when `…:Address` is an `openrouter.ai` host, and on any other address nothing is sent and startup logs a warning naming the model. No other operation sends a session. [Chat generation § A conversation can stay on one provider](../features/chat-generation.md#a-conversation-can-stay-on-one-provider) says what is grouped and why | reload |
...
| Refused names | any member the deployment writes or has a key for — `model`, `messages`, `input`, `instructions`, `tools`, `tool_choice`, `parallel_tool_calls`, `functions`, `function_call`, `response_format`, `text`, `stream`, `stream_options`, `store`, `include`, `previous_response_id`, `conversation`, `background`, `n`, `max_tokens`, `max_completion_tokens`, `max_output_tokens`, `temperature`, `top_p`, `reasoning`, `reasoning_effort`, `session_id`. Use `MaxOutputTokens`, `Temperature`, `TopP`, `ReasoningEffort`, or `StickySessions` for the ones that have a key. The refusal is on the top-level name alone, so a nested member of the same name is an ordinary value |
...
| `Chat:ConversationCompaction:Model` | Summarising the earlier part of an Agent conversation that has outgrown [`MailAnswering:MaxConversationContextTokens`](#mailanswering), so a turn sends the summary in its place |
...
| `Chat:RelevanceFilter:MinimumRelevance` | int | `70` | 1 – 100, on the scale the model answers a judgement on. A threshold of 0 is refused: it would pay for a judgement that can drop nothing. What each value keeps and drops for a given model is measured against a labelled set by the [agent evaluation suite](local-development.md#measuring-the-relevance-filter), which is also where the default comes from: it is a threshold at which no measured model dropped a passage answering a labelled lookup | reload |
...
| `MailAnswering:MaxConversationContextTokens` | int | `64000` | 1000 – 10000000; what one turn of an Agent conversation may send, in tokens, before its earlier part is compacted. One number for the whole deployment rather than per person or per model: a larger model is used by raising it. A conversation past it is summarised by the model `Chat:ConversationCompaction:Model` routes to, the summary is recorded beside everything it covers — nothing is removed and a person still reads every message — and the turn sends the summary plus every later turn verbatim. The summary's call is charged to the same period ceilings a question is. A summariser that fails does not fail the turn: it sends as much recent history as fits, records nothing, and logs a warning. Refused above `MaxTokensPerRun`, since the first call of a long conversation's turn sends up to this much and the run ceiling counts it | restart |
