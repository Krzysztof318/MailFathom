...
| Host layered provisioned configuration files | `Information` | `ServiceName`, `FileCount`, `ProvisionedFiles` |
...
The provisioned-file record carries the count and, in `ProvisionedFiles`, each file's path with the format it was read in, in the order they were layered. The paths are the deployment's own mount points and name no person's data; they are recorded because the extension decides how a file is read, so a file read in a format nobody intended is otherwise invisible. It is written here rather than through the container pipeline for the same reason the others are: it describes what the host read before a container existed to log it. A `0` against a deployment that mounts a ConfigMap is how an empty or misplaced mount becomes visible; [configuration sources](configuration-sources.md) states what is counted.
...
The replica is on the resource as well, as `service.instance.id`: `<host name>:<process id>`, composed by the same code that gives the container pipeline its value and the host its `work_leases` rows and administrative answers, so a startup record — including the `Critical` one a crashing replica leaves behind — is filtered to its replica by the value everything it exports afterwards carries. It names a machine and a process and nothing else. Unlike the build, a `service.instance.id` supplied through `OTEL_RESOURCE_ATTRIBUTES` wins; [the replica every record names](telemetry.md#the-replica-every-record-names) holds that rule and what overriding it gives up.

The application name stays a property of the startup records rather than a resource attribute, which is where it is useful in any case, including on a console with no resource attached at all. The version and the revision are both: properties of these five records and attributes on the resource they are exported with.
