...
One attribute is added on top of what the SDK resolved, and for exactly the same reason: `service.version`, read from the assembly's own stamped version, which is what the container pipeline puts on the resource of everything it exports. Both pipelines therefore name one build, and a startup record and a span from the same process cannot claim different versions. [The build every record names](telemetry.md#the-build-every-record-names) holds that rule in full, including why a `service.version` supplied through `OTEL_RESOURCE_ATTRIBUTES` does not override the stamped one.

The application name stays a property of the startup records rather than a resource attribute, which is where it is useful in any case, including on a console with no resource attached at all. The version is both: a property of these four records and an attribute on the resource they are exported with.
