...
- Everything before `builder.Build()` — configuration loading, options binding, secret-resolution registration, and the whole of `HostComposition.Compose`, which is where all three happen — runs with no logger in scope at all. `CreateBuilder` is where a malformed `appsettings.json` or a failing configuration provider throws, and it is the very first statement of the program.
