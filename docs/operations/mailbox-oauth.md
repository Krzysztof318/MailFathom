...
| `non_json_response_http_…` | The token endpoint answered something that is not a token response, and the number is the HTTP status it answered with. A proxy, a login page, or an error page in front of the authorization server looks like this, and so does an answer naming a character set this platform does not carry. Check that the endpoints came from the provider's own discovery document rather than being typed. |
