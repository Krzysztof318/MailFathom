...
token is held per account and no user is beside it — the credential is the mailbox's, whoever is assigned it — so the
account identifier is the whole of the statement:
...
| A token request failing with a cryptographic error, and no `invalid_grant` | The stored token does not open. A sealed token is bound to the account it was stored for — the generated identifier alone, no user beside it — so this is what a row copied between accounts, restored from another deployment, or altered in place meets, and it is also what a token sealed before the release that keyed the mail graph by that identifier meets. Nothing falls back to the configured reference. Authorize the account again with `--account`, which replaces what is stored. |
