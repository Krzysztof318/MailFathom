...
Five workflows run for every pull request targeting `main`. Three of them always run; `Typo check` and `CodeQL` always run except on a draft. `CI` also runs on every push to `main`, which [`CI` after a merge to `main`](#ci-after-a-merge-to-main) below describes. It carries seven jobs:
