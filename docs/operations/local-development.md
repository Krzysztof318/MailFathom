...
neither verification script builds or tests it: both build `backend/MailFathom.slnx` alone, and the formatting pass
described further down is the only thing either of them does to this solution. What gates the rest is the `Frontend`
job of `CI`, which runs exactly the three commands below on a change that reaches the client. Build and test it
directly:
...
Formatting is the one part of the client's verification that is not a command to run by hand, and it comes in the two
halves the service's does. The repairing half is `scripts/verify-fast.sh`, which formats the client C# files the branch
changed against `frontend/MailFathom.Client.slnx` and restores that solution to do it — only when the branch changed
one such file, so a branch that stayed in `backend/` pays nothing, and the `wasm-tools` workload above is what lets the
pass load the browser head when it does run. The verifying half is the `Verify formatting` step of the `Frontend` job.
Neither verification script verifies the client, so a client change that never went through the fast loop is reported
by `CI` rather than before the commit; [What gates the client stack](agent-workflow.md#what-gates-the-client-stack)
carries the whole split, and `dotnet format` is never invoked by hand here for the reason it is never invoked by hand
for the server.
