...
|---|---|---|
| Thread state | Four corpus conversations, recorded as `State as expected` | An agreed meeting states an agreement; an owner's undertaking to pay by a named day is a commitment due that day; a question the last message asks and nothing answers is an open question resting on that message; and a settled conversation, where both sides say nothing more is needed, carries no open question and no commitment |
| Contact relationship | Four correspondences built from the corpus as the correspondence index answers them, recorded as `Card as expected` | Nine conversations across tickets, invoices, and travel give a card naming its cases; a correspondence with a conversation named for an outstanding balance gives a next action or an open item citing it; a settled one gives no next action; and a single conversation is too thin for a card at all |
