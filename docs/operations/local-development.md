...
One workflow, `CI`, runs for every pull request targeting `main`, and it always runs. It carries four jobs:
...
Both expensive jobs restore from a cached `~/.nuget/packages` keyed on `Directory.Packages.props`, `global.json`, and `.config/dotnet-tools.json`. Because every version is pinned centrally, those three files decide what restore downloads, so a changed pin misses the cache rather than resolving against a stale package set.
