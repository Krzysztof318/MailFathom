...
`/.github/`, `/.config/`, `/.agents/`, and `/.claude/` already do. The entries assign no ownership the repository-wide line does not assign today; they state it deliberately, so that whoever adds a rule under one of those directories later meets the requirement in the file rather than discovering it from a review that has quietly stopped asking for the owner. They are the review-time half of the protection the `Protected paths` check applies at push time, and the two are written to be read together: one decides whose approval merges the change, the other decides whether the change belongs in that pull request at all. The two lists are the same list, and a directory added to one belongs in the other.
