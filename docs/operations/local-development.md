...
`/.github/` is the one directory that already does. The entry assigns no ownership the repository-wide line does not assign today; it states it deliberately, so that whoever adds a rule under `.github/` later meets the requirement in the file rather than discovering it from a review that has quietly stopped asking for the owner. It is the review-time half of the protection the `Protected paths` check applies at push time, and the two are written to be read together: one decides whose approval merges the change, the other decides whether the change belongs in that pull request at all.
