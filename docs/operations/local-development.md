...
`CI` runs on every push to `main` as well, and runs very nearly what a pull request gets — with one difference in the other direction: nothing here is narrowed by what the merge touched. Six of its eight jobs execute, and all six execute unconditionally: `Detect changes`, then `Build and unit test`, `Pending model changes`, the client build behind `Frontend`, and both jobs behind `Repository contracts`. `dotnet format` and `Required CI` are the two that never run here, and the `Verify formatting` step inside `Frontend` is skipped with them.
