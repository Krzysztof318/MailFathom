...
reaching a reader as a 404. [The documentation site](documentation-site.md) records what the site carries, which
versions it publishes, and what a new page under `docs/` owes it.
...
- `Workflow contracts` runs `scripts/test-agent-workflow.sh`, and it is the only job that reads the part of the repository the build, formatting, and model jobs cannot: the workflows, the verification scripts, the fathom-review helpers, the skills, the licensing header outside the solution, and the `describes:` marker on every page under `docs/`. It has no trigger condition of any kind — no path filter, no draft exemption, no dependency — so it starts with `Detect changes` and runs beside the expensive jobs rather than after them. It checks the repository out shallowly with `persist-credentials: false` and runs the suite; nothing else is provisioned, because the suite fakes `dotnet` with a symlink to itself, builds the Git repositories it tests under a temporary directory, and reaches no network. The whole job takes about twenty seconds on a GitHub-hosted runner, of which the suite itself is fifteen. It takes longer than that under `scripts/verify-full.sh` on a developer machine, which is a different measurement rather than a contradiction: there it competes with whatever else that machine is doing, and it is one step of a gate whose build and coverage run dominate it either way. [Entry points](agent-workflow.md#entry-points) describes the suite itself and the local gate that runs it as well.
- `Required CI` is this workflow's one required status check, and the only conclusion the ruleset reads from it. It depends on the other five, runs under `if: always()` so a cancelled or skipped dependency cannot skip it in turn, and reads their results: `Detect changes` and `Workflow contracts` must have succeeded, and each of the other three must have either succeeded or been skipped. `failure` and `cancelled` fail it. The contract job is held to the stricter rule because it has no way to skip on the events this job runs for: a path filter and a draft exemption are what produce a legitimate `skipped`, and it has neither, so the conclusion could only come from a job that failed to start. This job itself does not run on a push to `main`, which is the one event that skips the contract job outright.
...
`CI` runs on every push to `main` as well, and runs deliberately less there than on a pull request. Two of its six jobs execute: `Build and unit test` and `Pending model changes`, both unconditionally, over the whole solution. `Detect changes`, `dotnet format`, `Workflow contracts`, and `Required CI` are all skipped.

Each absence has its own reason, and none of them is cost.

- `Detect changes` is a pull-request question. On `main` the trigger has already answered the only version of it that is asked there, so a second and finer decision inside the run would reproduce the first one less legibly.
- `dotnet format` is a property of the files a change wrote, which is exactly what the pull request that wrote them answered. It is also the one verdict here that two changes merging without seeing each other cannot break. The model check is not: one pull request can move the EF Core model while another adds a migration, and neither run sees both, which is why that job stays.
- `Workflow contracts` asserts properties of the repository rather than of a change, so its answer on `main` is the answer the pull request already got over the same tree.
- `Required CI` exists to be the `main` ruleset's one required check, and a ruleset evaluates a pull request. On a push nothing waits for a conclusion, and two of the three results it reads are skipped by design, so it would have to be taught to forgive them one event at a time. The run's own conclusion is what a push reports through, and `Verify` is the whole of it. On every event the ruleset does read, the job still runs under exactly that name.

The trigger carries a `paths-ignore` list, and the reason the `pull_request` trigger cannot have one does not apply here. That reason protects a required check: a run GitHub never instantiated reports no conclusion, so a pull request would wait on it forever. Nothing waits on a push run, so a merge that touched only `docs/`, `specs/`, `assets/`, `deploy/`, `scripts/`, `.agents/`, `.claude/`, or a repository-root Markdown file starts no run at all, and `main` keeps the last verdict that meant something instead of gaining one that means less. `.github/**` is deliberately not on that list, because this workflow and the one it calls decide what a run does.
...
Draft pull requests skip the build, formatting, model-check, typo-check, and code-scanning jobs without allocating a runner; only the seconds-long `Detect changes`, `Required CI`, and `Protected paths` jobs and the twenty-second `Workflow contracts` do any work, and `Required CI` clears the ones that skipped because a skipped job is a valid outcome.
