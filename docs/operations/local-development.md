...
Four workflows run for every pull request targeting `main`. Two of them always run; `Typo check` and `CodeQL` always run except on a draft. `CI` carries six jobs:
...
`Workflow contracts` is the one job in `CI` that allocates a runner for a draft, and its reason is the opposite of the typo check's. Every pull request here opens as a draft and is marked ready only when the owner asks for a review, so a draft-exempt contract check would stay silent through the whole interval in which the change is being written and report for the first time at the end of it. What it verifies is also what a draft is as likely to have broken as anything else, since a workflow, a script, or a skill is edited while the pull request is still a draft like any other file. The cost that justifies the exemption elsewhere is not there either: about twenty seconds, against the SDK, restore, build, and analysis each exempt job pays for. A draft can therefore show a red `Required CI` while every other job in the workflow skipped, which is the signal working rather than a fault to chase; a draft is unmergeable either way.
