...
agent drives a running app through, and the settings channel between them — and the Uno SDK adds them to any project
it builds, the last two to any head whose `OutputType` is `Exe`. The first carries an end-user licence agreement
between Uno Platform and whoever restores it, inside the package's own `LICENSE.md`; the other three declare no licence
at all. The decision is taken and they stay: they run on a developer's machine, no artifact carries them — a Release
