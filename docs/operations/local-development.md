...
The Discover planning agent is measured over a set of written questions rather than one message: one stating its own scope and one stating none, one naming a person, one naming a period, one asking for something no run can do, and two reading equally well two ways. Its answer is a plan rather than prose, so each case records a `Plan as expected` metric — the answer was read as a plan rather than fallen back from, and the plan says what the question stated, such as a lookup narrowed to the sender it named or bounded to the month it named, and a request to send mail read as `unclassified` rather than as a question it could answer. The judge is asked only about the two ambiguous questions, where no single plan is right, and grades them on `Intent Resolution` against a threshold of three out of five that `DiscoveryPlanningScenario` records — three rather than four because a judge that does not know both readings are acceptable grades whichever one a plan chose as a partial resolution, and has graded the same plan four on one run and three on the next.
