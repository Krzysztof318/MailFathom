...
<!-- describes: scripts/**, global.json, MailFathom.code-workspace, .config/dotnet-tools.json, .config/typos.toml, .config/CodeCoverage.proj, .config/testconfig.json, backend/src/AppHost/**, backend/src/Infrastructure/Persistence/MailFathomDbContextDesignTimeFactory.cs, .github/workflows/**, backend/tests/IntegrationTests/ProviderAdapters/**, backend/tests/IntegrationTests/ObjectStorage/**, backend/tests/Evaluations/**, backend/tests/Evaluations.UnitTests/**, backend/tools/**, frontend/package.json, frontend/pnpm-workspace.yaml, frontend/.npmrc, frontend/tsconfig.base.json, frontend/tsconfig.json, frontend/eslint.config.ts, frontend/vitest.config.ts, frontend/playwright.config.ts, frontend/src-tauri/** -->
...
The script keeps everything under `artifacts/ai-evaluations/` — the store, the TRX results, and `report.html`, which `dotnet aieval report` renders from the store with the newest ten runs side by side. The store holds two halves. Its results are what the report compares against, and the script keeps the newest thirty runs of them. Its cache holds every answer the run paid for, keyed by prompt, model, and parameters, so a run that changed neither costs nothing; an entry lives a year before the script prunes it. A case keeps its answers cached whatever its verdict, so a repeated run replays the sample an earlier run drew, passed or failed, and reports the same rate for nothing. Forgetting a case that fell short would re-ask it until one sample passed and then keep that one, so a run read from the cache would report nearly every case passing whatever the model's own rate is: over the same code, 0.4% of the answers read from the cache failed against 12% of the answers asked afresh. A new sample is drawn where the key changes — a prompt, a tool, a model, or a parameter — and a case frozen on a failing sample stays failed until then, or until a run discards the cache. Because a replayed rate and a measured one read the same, every repetition's result carries an `Answers read from the cache` and an `Answers asked afresh` metric counting where the model under test's answers came from, and the script ends the run with one line summing both over the run. A comparison between two runs is between measurements only where both asked their answers afresh. The tools a model is offered are part of the key as well — each one's name, description, and parameter schema — because `Microsoft.Extensions.AI` leaves them out of the key it computes, and a changed tool definition would otherwise read back the answer given to the old one; a request offered no tools keeps the key it always had. The rest of the key is the library's own, and the library does not promise it stays the same across its releases, so a bump of `Microsoft.Extensions.AI` may re-ask every case once. Deleting the store's `cache` directory makes a local run ask every model and the judge again while its results are kept. `MAILFATHOM_AI_EVALUATIONS_STORE` points the script at another store.
