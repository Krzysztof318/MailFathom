...
`AgentConversationScenario` puts seven questions to the Agent a conversation is held with, composed as a deployment composes it and over the same in-memory search, with the conversation it writes into held in memory so every proposal can be read back. The corpus is search passages and nothing more, so the search tool, the three tools proposing a new message, a calendar event, and a task, and the tool suggesting what to ask next are offered — proposing and suggesting read nothing — while the tools opening a stored message, a conversation's state, the calendar, and the task list have nothing to read here. The cases are one answer from the English mailbox, the same question asked by a person reading Polish, a Polish request to quote an English error message exactly, a request to propose a message to a named recipient, a request to put a call on the calendar at a stated hour, a request to add a task due on a stated day, and a question [a message written to steer the agent](#mail-written-to-steer-the-agent) answers. Checked without a judge: that the run searched where the answer rests on mail, that the answer carries the evidence as the mail states it — which is what holds a quotation untranslated — that it proposed exactly the one thing asked for, read as the message's recipient, the event's starting instant, or the task's due day, and nothing where nothing was, whichever tool a proposal came through, that it stayed inside its bounds, that it carried out nothing a message asked, that every question it suggests asking next names something the conversation is about — a word from a list each case carries, so a suggestion drawn from nowhere or out of the mail fails it — and that it is written in the person's language. The judge grades intent resolution and task adherence.

`AgentConversationSummaryScenario` puts three stretches of an Agent conversation to the agent that compacts one, composed as a deployment composes it. The cases are a comparison of two quotes ending in a decision and a proposal, a stretch that has to fold an earlier summary in, and a stretch whose agent turn quotes mail asking to be obeyed. Checked without a judge: that the summary keeps every fact a follow-up would need — each name, amount, date, and decision the case lists — and that it is not the code the quoted mail asked for.
...
A new scenario starts from `StructuredAnswerScenario` where the agent answers with a structure, or from `ReplyDraftScenario` and the test beside it where it answers in prose, and decides nothing about judges, credentials, or storage: it composes the agent through the agent's own composition, sends the model's call through the run's response cache, and hands its answer to `ScenarioRun.EvaluateAsync`. `Microsoft.Extensions.AI.Evaluation.Quality` supplies the judged evaluators and `Microsoft.Extensions.AI.Evaluation.NLP` the ones that compare an answer with a reference text and need no judge at all.
...
|---|---|
| Mail answering | Four Polish questions — one a single message answers, one a promised payment day, one a later message corrects, and one nothing answers — then a Polish question about English mail and an English question about Polish mail |
...
| Discover composition | Three Polish questions — one a single extract answers, one tracking a change, and one the extracts do not answer — then a Polish question over English extracts and the reverse |
...
The models are named by `MAILFATHOM_EVALUATION_EMBEDDING_MODELS` and reached over `MAILFATHOM_EMBEDDING_ADDRESS` with `MAILFATHOM_EMBEDDING_API_KEY` — the endpoint and key the embedding provider-contract tests read, apart from the chat endpoint, because a deployment declares the two providers apart. `MAILFATHOM_EVALUATION_EMBEDDING_DIMENSION` asks every model for vectors of that width and cuts a wider answer to it, renormalized as a deployment trimming its vectors cuts it, and a model answering narrower than asked fails the run naming both widths. The width joins the name a result is filed under, as `<model>@<width>`, so one model measured at its own width and at a narrower one is two columns of the report. Every vector goes through the store's cache, one entry per text, so a run over an unchanged mailbox costs nothing, and `Cost (USD)` carries what the provider charged.

What it leaves out is the lexical half of a hybrid search: fusing with the in-memory word match would measure that stand-in rather than PostgreSQL's full-text ranking, which is what a deployment fuses with.

### Continuous integration

The `AI evaluations` workflow runs the script on dispatch alone — with an optional `ref`, an optional `models` list, an optional `repetitions` count, an optional `reasoning_effort`, an optional `embedding_models` list, and an optional `embedding_dimension` — and on no schedule and no pull request, because what a run spends is asked for rather than arriving on a timer. It declares the models under test through the dispatch's `models` input, falling back to `vars.EVALUATION_MODELS` where it names none and to `vars.CHAT_PROVIDER_MODEL`, the model the `Integration tests` workflow reaches, where neither does, and how many times each case is asked through the `repetitions` input, falling back to `vars.EVALUATION_REPETITIONS` and then to once, reaching them through the `vars.CHAT_PROVIDER_ADDRESS` and `secrets.CHAT_PROVIDER_API_KEY` the provider-contract tests already use, and the judge through `vars.JUDGE_PROVIDER_MODEL`, with its reasoning effort from `vars.JUDGE_REASONING_EFFORT` where that is set, reached over that same address and key. It declares the embedding models through the `embedding_models` input, falling back to the model the `Integration tests` workflow already reaches — `vars.EMBEDDING_PROVIDER_ROUTED_MODEL`, or `vars.EMBEDDING_PROVIDER_MODEL` where that is unset — over `vars.EMBEDDING_PROVIDER_ADDRESS` and `secrets.EMBEDDING_PROVIDER_API_KEY`, and passes `reasoning_effort` to every chat model under test and `embedding_dimension` to every embedding model, each only where a dispatch states it. The `ref` input may name a branch, a tag, or a commit SHA, full or abbreviated: the workflow resolves it to the full SHA of the commit it names before checking out, because the checkout action reads anything shorter than a full SHA as a branch or tag name, and a ref naming no commit fails that step naming the ref. A step's log header prints every `env:` value a workflow hands it and masks only the secrets, so the judge's model and effort are visible in that one place.
