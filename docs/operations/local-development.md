...
`backend/tests/Evaluations` measures what an agent answers rather than whether its code runs: each scenario puts a message from the committed synthetic corpus, `backend/tools/SyntheticMail/corpora/office-en.zip`, or a question about it, to an agent under every declared model, asserts what the answer's structure can settle, and has one pinned judge model grade what it cannot. It is run by one script, and neither verification script runs it:

```bash
bash scripts/run-ai-evaluations.sh
```

A scenario calls real models, so it skips unless `MAILFATHOM_AI_EVALUATIONS` is `true`, and nothing sets it. What runs without it is the free half: tests that drive the scenarios through scripted providers and prove that the store holds no value of the judge and no address outside a reserved domain, that a repeated run asks neither the model nor the judge again, that what a run reports spending is what the provider said it charged, and that each structural check of a mailbox answer fails on the answer it exists to catch. A requested run that finds a variable missing fails naming it:
