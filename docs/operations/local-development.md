...
- **`CI`'s `Frontend` job runs the full gate's client flow and both browser suites.** It is gated on the `frontend` path
  ...
  `pnpm test:desktop`; it shares nothing with the first but the checkout, which is why it is a job rather than four more
  steps.
...
- **`pnpm test:desktop` needs more than a browser, and neither verification gate runs it either.** It builds the desktop
  ...
  for that WebView, which is `webkit2gtk-driver` on Debian and Ubuntu, and a display, which on a headless machine means
  running it under `xvfb-run`. It is what proves the two rules a WebView answers rather than the client: that an instant
