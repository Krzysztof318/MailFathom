...
The script keeps everything under `artifacts/ai-evaluations/` — the store, the TRX results, and `report.html`, which `dotnet aieval report` renders from the store with the newest ten runs side by side. The store holds two halves. Its results are what the report compares against, and the script keeps the newest thirty runs of them. Its cache holds every answer the run paid for, keyed by prompt, model, and parameters, so a run that changed neither costs nothing; an entry lives a year before the script prunes it. The exception is an answer that fell short: when a model falls short on a scenario, that scenario's answers and the judge's verdicts on them leave the cache under that model, so a retry asks the model again and so does every later run. Otherwise one miss would be read back from the cache by every attempt after it, and the scenario would fail however the model answered when asked again. The verdict already filed stays in the results, so the report still shows the attempt that fell short. `MAILFATHOM_AI_EVALUATIONS_STORE` points the script at another store.
