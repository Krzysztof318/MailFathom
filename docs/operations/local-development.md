...
| Contact relationship | Three Polish correspondences — several kinds of matter, a quotation still being chased, and a settled itinerary — then an English correspondence under a Polish mailbox and the reverse |
...
Each case is held to what its English counterpart is held to, and also to the language it has to write in, which `WrittenLanguage` decides without a judge. It counts Polish diacritics and function words against English function words, calls a text one language only where it outweighs the other twice over, so a subject quoted in its own language does not change the verdict, and leaves a text too short to hold two telling words undecided rather than guessing. The structured agents write in the mailbox's language and fail their one expectation metric where they do not; the prose calls record a separate `Written in the expected language` metric. What the prose has to be written in follows the agent's own instruction rather than the mailbox: an answer and a composed result are written in the question's language, and a reply draft in the conversation's language unless the ask names another, the mailbox's language deciding a reply only where there is no conversation to follow — so an English ask on a Polish conversation is drafted in Polish, and a Polish ask on an English one in English.
