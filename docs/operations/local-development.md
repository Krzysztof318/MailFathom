...
nothing in the two verification scripts touches it: both build `backend/MailFathom.slnx` alone. What gates it is the
`Frontend` job of `CI`, which runs exactly the three commands below on a change that reaches the client. Build and test
it directly:
