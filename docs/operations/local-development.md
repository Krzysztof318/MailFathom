...
`CI` runs on every push to `main` as well, and runs very nearly what a pull request gets. Four of its six jobs execute: `Detect changes`, then `Build and unit test` and `Pending model changes` when the merge could have moved them, and `Workflow contracts` unconditionally. `dotnet format` and `Required CI` are the two that never run here.
