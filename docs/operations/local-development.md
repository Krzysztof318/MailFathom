...
Draft pull requests skip the build, formatting, model-check, typo-check, and code-scanning jobs without allocating a runner; only the seconds-long `Detect changes`, `Required CI`, and `Protected paths` jobs and the minute-long `Workflow contracts` do any work, and `Required CI` clears the ones that skipped because a skipped job is a valid outcome.
