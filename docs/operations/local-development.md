...
`THIRD_PARTY_LICENSES.md` records it as content an external model produced, under the terms it was produced under. `hard-shapes-en.zip` beside it is 15 messages in two exchanges written by hand in the same archive shape, for the evaluation suite: the shapes of real mail a model-written corpus did not produce, with every participant under `.test` like the first. It replays the same way, and its `corpus.json` records that nothing generated it.
