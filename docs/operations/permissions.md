...
| `mailfathom.mail.delete` | mail | Deleting the user's mail off their mail server outright: [`POST /api/client/mutations/deletes`](client-endpoint.md#the-mutation-routes). It is separate from `mailfathom.mail.move` because filing mail elsewhere leaves it there to be found and this does not — the occurrence is expunged, and what becomes of the local copy is the account's configured disposition rather than the caller's to name. There is no withdrawal route beneath it: a withdrawal takes back a change nothing has been asked of a server for, and this one is not offered a way back for the same reason the client asks about it twice. Which folder the message is in decides nothing here — the trash-only rule the client draws is a sentence on a screen rather than a policy of the deployment's, so the grant is the whole of the boundary. Reading where a delete has got to is `mailfathom.mail.read`, like every other read here. It publishes no MCP tool |
...
server to expunge the occurrence, and nothing on this surface puts it back. So a deployment that lets a client file
mail has not thereby let it destroy any. It is also the one grant here whose act the client confirms twice rather than
offering a way back afterwards, which is the client's answer to the same fact rather than a second rule.
