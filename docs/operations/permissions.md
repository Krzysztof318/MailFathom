...
| `mailfathom.mail.accounts.write` | mail | Maintaining the signed-in user's own record: [`POST /api/client/record`](client-endpoint.md#the-record-routes), [`POST /api/client/record/mail-accounts`](client-endpoint.md#the-record-routes), [`POST /api/client/record/mail-accounts/removal`](client-endpoint.md#the-record-routes), [`POST /api/client/record/mail-accounts/folders`](client-endpoint.md#the-record-routes), [`POST /api/client/record/mail-accounts/folders/replacement`](client-endpoint.md#the-record-routes), and [`POST /api/client/record/mail-accounts/folders/removal`](client-endpoint.md#the-record-routes), which save the whole record, declare one more mailbox in it, stop it declaring one, and do the same three things to one folder of one mailbox. The three folder routes reach no mail server, which is [ADR 0007](https://github.com/Krzysztof318/MailFathom/blob/main/docs/decisions/0007-remote-mailbox-mutation-boundary-and-write-session.md): a folder here is a mapping this deployment reads under, so declaring one says which remote path to read and withdrawing one says to stop reading it, and neither renames nor deletes anything on the server. It also maintains the envelope that record hangs on: [`POST /api/client/display-name`](client-endpoint.md#the-name-routes), which records that person under the name they correct theirs to. It is separate from `mailfathom.mail.read` for the reason every write here is: reading somebody's mail is not deciding which mailboxes this deployment reads for them. It reaches that user's record and no other — the routes name nobody, so the caller's own identity is the whole of whose record is changed — and it withdraws no mail: a mailbox a record stops declaring keeps everything already stored for it. Nothing under this name reaches anybody else's row: recording a user, relabelling another one, and erasing one are administrative, and no user-facing grant lists who else this deployment serves. The name route is the one place it writes the envelope, and it writes only the caller's own — refused outright while a configuration source still declares that person, because a start would write the declared name back over it. It publishes no MCP tool |
