...
administrative half. `mailfathom.mail.flags.write`, `mailfathom.mail.move`, and `mailfathom.mail.send` are the three
worth naming rather than leaving to the ungoverned default: the first two write to the user's real mail server, the
second of them being the one that can lose a message if it stops half way, and the third is the only name in either half
whose effect leaves the deployment and cannot be recalled.
