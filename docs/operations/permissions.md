...
| --- | --- | --- |
| `mailfathom.mail.read` | mail | The tools that read the local mailbox copy: `list_accounts`, `list_emails`, `get_email_content`, `search_emails`. On the client endpoint it also reaches [`GET /api/client/accounts`](client-endpoint.md#the-accounts-route) and [`GET /api/client/folders`](client-endpoint.md#the-folders-route) with [`GET /api/client/managed-folders`](client-endpoint.md#the-folder-management-routes), which name the signed-in user's own accounts and folders and how current each of them is, and [`GET /api/client/emails`](client-endpoint.md#the-mail-list-route) with [`GET /api/client/emails/search`](client-endpoint.md#the-mail-search-route) [`GET /api/client/threads/{threadId}`](client-endpoint.md#the-conversation-route), [`GET /api/client/messages/{storedEmailId}`](client-endpoint.md#the-message-route), [`GET /api/client/messages/{storedEmailId}/body`](client-endpoint.md#the-message-body-route), and [`GET /api/client/messages/{storedEmailId}/attachments/{position}`](client-endpoint.md#the-attachment-route), which are the user's mail itself — subjects, correspondents, the opening of each message's text, the extracts a search cuts around what matched, one whole conversation at a time, one message's headers with what was established about its author and what it carries, one message's whole body as the document a reading pane draws and as the words beside it, and the octets of any file it carries. It also reaches [`POST /api/client/citations/resolution`](client-endpoint.md#the-citation-route), which follows the citations of a presentation plan to the mail behind them: a message, a passage of one with the offsets it was cut from, or a file it carries — all of it mail this grant already reads, published under the scope the routes above are read under, so a source outside it is reported as private rather than served. It is also half of what [`GET /api/client/contacts/{contactId}/correspondence`](client-endpoint.md#the-contact-correlation-route) asks — the conversations one contact's addresses appear in and the documents they sent, which is this grant's own mail read back against a person rather than a folder, and the other half is [`mailfathom.mail.contacts.read`](#the-published-set) below. It also reaches [`GET /api/client/record`](client-endpoint.md#the-record-routes), which is that user's own record as the deployment holds it, secrets redacted — reading which mailboxes are read for you is part of reading your mail, and changing them is [`mailfathom.mail.accounts.write`](#the-published-set) below. It also reaches [`GET /api/client/display-name`](client-endpoint.md#the-name-routes), which is the name this deployment records that person under, with whether it would accept a change of it beside it — somebody whose mailboxes an administrator maintains is shown their own name whether or not they may correct it. It also reaches the two [`/api/client/time-zone`](client-endpoint.md#the-time-zone-a-persons-days-are-read-in) routes, the write included: which zone somebody's own days are read in decides nothing about whose mailboxes this deployment connects to, and it is what every relative period this grant already answers — “this week”, “since Tuesday” — is resolved against, so reading it and stating it sit on the same reading the bell and the preferences do. It is also the whole of what the five [notification routes](client-endpoint.md#the-notification-routes) ask, the three writes included: a person's notification centre is what this deployment drew for them about mail they can already see, so reading it, counting what is unread, marking one or all of it read, and taking rows out of it reach no mail server and move nothing in a mailbox — the message a notification points at stays exactly where it is, which is why erasing one is not [`mailfathom.mail.delete`](#the-published-set) — and somebody has to be able to clear their own bell whatever grant they hold over their mail configuration. It is also the whole of what the eight [task routes](client-endpoint.md#the-task-routes) ask, the five writes included: a person's task list is this deployment's own record of what they owe, so reading either half of it, recording a task, editing one, completing one, accepting what mail proposed, and erasing one — which is also how a proposal is dismissed — reach no mail server and move nothing in a mailbox. The message a task cites is a value the task carries rather than mail those routes read, which is why erasing one is not [`mailfathom.mail.delete`](#the-published-set) either, and somebody has to be able to keep their own list on the same reading the bell is cleared on. It is also the whole of what the six [calendar routes](client-endpoint.md#the-calendar-routes) ask, the four writes included, on that same reading: an event is this deployment's own record of when one person is committed, so reading a window of it, reading one event, putting one on the calendar, amending one, accepting a date their mail proposed, and removing one — which is also how a proposal is dismissed — reach no mail server and no calendar server, and move nothing in a mailbox. What each read answers with is bounded by how many events it may carry rather than by how long the window is, and a window outside that bound is refused rather than answered short. It is also the whole of what the two [calendar import routes](client-endpoint.md#the-calendar-import-routes) ask, on the same reading again: a file somebody was handed reaches this person's own calendar and nothing else, the summary route writes nothing at all, and confirming it creates the events that summary named — which is the same act as putting one there by hand, performed over a file rather than a form. Nothing an entry in such a file names is fetched, so neither route reaches a mail server, a calendar server, or any other address. It is also what [`POST /api/client/signals/ticket`](client-endpoint.md#the-signal-channel) asks, which mints the single-use ticket a client opens [the signal channel](client-endpoint.md#the-signal-channel) against: what that channel says is that mail arrived, that stored mail moved, that a folder set moved, that a notification was written, or that a run finished — each of them a statement about what this grant already reads, and each of them an instruction to read it again over the routes above rather than an answer of its own. It is also the whole of what the two [`/api/client/preferences`](client-endpoint.md#the-preferences-routes) routes ask, the write included: those hold what a person set about their own client rather than what this deployment reads for them, so somebody whose mail accounts an administrator maintains — and who therefore does not hold `mailfathom.mail.accounts.write` — still turns their own telemetry off. Where semantic retrieval is configured, searching places the caller's own query text with the embedding provider, so this is not an egress-free grant |
| `mailfathom.mail.ask` | mail | `ask_mail`, which answers from mail content by sending it to a model provider. On the client endpoint it also reaches the three [Discover routes](client-endpoint.md#the-discover-routes) — [`POST /api/client/discovery/runs`](client-endpoint.md#the-discover-routes), which asks a question of the user's mail, [`GET /api/client/discovery/runs/{runId}`](client-endpoint.md#the-discover-routes), which reads the run answering it, and [`DELETE /api/client/discovery/runs/{runId}`](client-endpoint.md#the-discover-routes), which stops it. All three are the same act as the tool and are therefore published under the same grant, the reading and the stopping routes included: what each of them touches is a question that was asked under this one, so a run belongs to the user who asked for it and somebody else's reads as no such run whether it is being read or stopped, on whichever replica answers the request. Stopping is published here rather than under a write grant because it ends work this deployment was doing and changes nothing in a mailbox. It also reaches the two [phrasing routes](client-endpoint.md#the-phrasing-routes) — [`GET /api/client/emails/search/phrasing`](client-endpoint.md#the-phrasing-routes), which says whether this deployment reads a typed sentence at all, and [`POST /api/client/emails/search/phrasing`](client-endpoint.md#the-phrasing-routes), which reads one into filters somebody can see and correct. Neither searches anything, and what makes them this grant rather than the reading one is where the sentence goes: it leaves the deployment for a chat provider and is charged to the same allowance a question is. A caller holding `mailfathom.mail.read` alone is answered `403` and searches by the words they typed, which is the same search a deployment with no provider serves. It reaches the two [reply drafting routes](client-endpoint.md#the-reply-drafting-routes) on the same reading — [`GET /api/client/replies/drafting`](client-endpoint.md#the-reply-drafting-routes), which says whether this deployment drafts a reply at all, and [`POST /api/client/replies/drafting`](client-endpoint.md#the-reply-drafting-routes), which drafts one out of the conversation being answered. Neither sends, queues, or stores anything, and what makes them this grant rather than a writing one is where the mail goes: the conversation and a sample of the account's own sent mail leave the deployment for a chat provider, charged to the same allowance a question is. It reaches [`GET /api/client/contacts/{contactId}/relationship`](client-endpoint.md#the-contact-relationship-route) on the same reading: it reads the correlation an opened contact is drawn beside into a short note about where that correspondence stands, and the subjects and file names of it leave the deployment for a chat provider, charged to the same allowance a question is. It asks for the two grants that correlation needs as well — [`mailfathom.mail.read`](#the-published-set) and [`mailfathom.mail.contacts.read`](#the-published-set) — so a caller holding this one alone is refused naming the one they are missing, and a deployment that derives no card answers that it derived none rather than refusing. It reaches the two [calendar event drafting routes](client-endpoint.md#the-calendar-event-drafting-routes) on the same reading — [`GET /api/client/calendar/drafts`](client-endpoint.md#the-calendar-event-drafting-routes), which says whether this deployment reads a typed description at all, and [`POST /api/client/calendar/drafts`](client-endpoint.md#the-calendar-event-drafting-routes), which reads one sentence into the event it describes. Neither writes to a calendar and neither stores anything: what makes them this grant rather than a writing one is that the sentence leaves the deployment for a chat provider and is charged to the same allowance a question is, and a caller holding the reading grant alone fills the dialog's fields in themselves. It reaches the two [day-layout routes](client-endpoint.md#the-day-layout-routes) on the same reading — [`GET /api/client/tasks/today/layout`](client-endpoint.md#the-day-layout-routes), which says whether this deployment arranges a day at all, and [`POST /api/client/tasks/today/layout`](client-endpoint.md#the-day-layout-routes), which suggests how the tasks owed today could sit around what is already on the calendar. Neither writes anything — no task is moved, no proposal accepted, and no event created — and what makes them this grant rather than the reading grant the rest of the list is served under is where the lines go: the task titles and the titles of the day's commitments leave the deployment for a chat provider, charged to the same allowance a question is. The arrangement asks for [`mailfathom.mail.read`](#the-published-set) beneath it as well, since the tasks owed and the day's commitments are that grant's own records, so a caller holding this one alone is refused naming the one they are missing — while the route that only says whether a day is arranged reads neither and answers on this grant alone. It reaches the ten [Agent conversation routes](client-endpoint.md#the-agent-conversation-routes) on the same reading — the history, searching it by words and by meaning, one conversation read from a cursor, asking, steering and stopping a running answer, accepting or declining a proposal, archiving a conversation and restoring it, and deleting one. A conversation is the person's own questions about their mail and the answers quoting it, so reading it is this grant rather than the reading one, and nothing but accepting a proposal reaches a mailbox: accepting carries out the act the Agent proposed, so it asks for that act's own grants beside this one — [`mailfathom.mail.drafts.write`](#the-published-set) and [`mailfathom.mail.send`](#the-published-set) for a new message, and [`mailfathom.mail.read`](#the-published-set) as well for an answer to one — and a caller missing any of them is refused before the decision is recorded. The answer the Agent composes reads mail, the calendar, and tasks under the grant the question was asked with, and offers a proposing tool only where that grant could carry the proposal out. It does not imply `mailfathom.mail.read`, and granting it is granting access to mail |
| `mailfathom.mail.flags.write` | mail | `set_mail_flags`, which marks mail read or unread, stars or unstars it, and writes its keywords. On the client endpoint it also reaches [`POST /api/client/mutations/flags`](client-endpoint.md#the-mutation-routes), which is the same act asked for one batch at a time, and [`POST /api/client/mutations/flags/withdrawals`](client-endpoint.md#the-mutation-routes), which takes back one that has not reached the server yet — withdrawing needs authority over the same kind of change rather than authority of its own, because the worst it does is stop one. It is the one MCP grant whose effect reaches the user's mail server, and it does not follow from reading mail: a deployment that lets an agent read has not thereby let it change anything. It does not reach placement: moving mail is `mailfathom.mail.move` below |
| `mailfathom.mail.move` | mail | Filing the user's mail into another folder: [`POST /api/client/mutations/moves`](client-endpoint.md#the-mutation-routes) and [`POST /api/client/mutations/moves/withdrawals`](client-endpoint.md#the-mutation-routes). It is separate from `mailfathom.mail.flags.write` because the two acts fail differently — a flag misdescribes mail the user can still find, while a move puts the mail somewhere else and on a server without `MOVE` is a copy followed by a delete. Reading where a move has got to is `mailfathom.mail.read`, like every other read here. It publishes no MCP tool |
| `mailfathom.mail.delete` | mail | Deleting the user's mail off their mail server outright: [`POST /api/client/mutations/deletes`](client-endpoint.md#the-mutation-routes), [`POST /api/client/mutations/deletes/withdrawals`](client-endpoint.md#the-mutation-routes), and [`POST /api/client/mutations/deletes/releases`](client-endpoint.md#the-mutation-routes). It is separate from `mailfathom.mail.move` because filing mail elsewhere leaves it there to be found and this does not — the occurrence is expunged, and what becomes of the local copy is the account's configured disposition rather than the caller's to name. A delete written there waits out the person's own notification time and a short grace before anything reaches the server; the withdrawal route takes it back while it waits, under this grant because the grant that wrote a change is the grant that withdraws it, and the release route ends the wait early. Which folder the message is in decides nothing here — the trash-only rule the client draws is a sentence on a screen rather than a policy of the deployment's, so the grant is the whole of the boundary. Reading where a delete has got to is `mailfathom.mail.read`, like every other read here. It publishes no MCP tool |
| `mailfathom.mail.drafts.write` | mail | `save_draft`, `update_draft`, and `delete_draft`, which write a message into the user's own drafts folder, replace it, and take it back out. It is the safe half of authoring mail and is its own name because that half is worth granting on its own: a draft is delivered to nobody, is withdrawn by deleting it, and lands in a folder the user already reads, so an agent holding this and nothing else can prepare mail whose worst failure is a message in Drafts. It does not imply `mailfathom.mail.send` and is not implied by it — `send_draft` is admitted under the sending grant, so a caller holding this name alone cannot make a draft leave. Its effect does reach the user's own mail server, which is `mailfathom.mail.flags.write`'s reach rather than a send's. A draft answering stored mail needs `mailfathom.mail.read` beneath it as well, because an answer is derived from the message it answers. On the client endpoint it reaches [the drafts routes](client-endpoint.md#the-drafts-routes) — listing, opening, composing, revising, giving up, and staging or removing a file — which are the same acts on the same drafts in the same folder, so a person's client and an agent draw on one name rather than two |
| `mailfathom.mail.send` | mail | Asking this deployment to send mail from an account it holds. It is the one grant here whose effect leaves the deployment and cannot be recalled, which is why it follows from nothing: reading a mailbox is not writing from it, and marking mail reaches the user's own server rather than a stranger's. It covers `send_email`, which queues a message for a mailbox this deployment holds, and `reply_to_email` and `forward_email`, which queue one anchored to mail it already holds — those two also need `mailfathom.mail.read`, because an answer is derived from the message it answers. It covers `get_outgoing_email` and `cancel_outgoing_email` as well, which report what became of a queued send and stop one that is still waiting: a caller may read back and withdraw exactly what it was allowed to queue, and what this mailbox has written to whom is not something the reading grant confers. It covers `send_draft`, which queues the message a draft holds, and the promotion beneath it, because promoting a draft is asking for mail to leave. On the client endpoint it reaches [`POST /api/client/drafts/{draftId}/send`](client-endpoint.md#the-drafts-routes) and [the outbox routes](client-endpoint.md#the-outbox-routes), which are the same act and the same reading back and withdrawal the tools above are, plus a paging over the caller's own queued sends that no tool offers — an enumeration is a person's screen rather than an agent's. It is asked for again by the use cases behind all six and by the outbox beneath the four that send, so it governs the act rather than only the tool |
| `mailfathom.mail.accounts.write` | mail | Maintaining the signed-in user's own record: [`POST /api/client/record`](client-endpoint.md#the-record-routes), [`POST /api/client/record/mail-accounts`](client-endpoint.md#the-record-routes), [`POST /api/client/record/mail-accounts/removal`](client-endpoint.md#the-record-routes), [`POST /api/client/record/mail-accounts/folders`](client-endpoint.md#the-record-routes), [`POST /api/client/record/mail-accounts/folders/replacement`](client-endpoint.md#the-record-routes), and [`POST /api/client/record/mail-accounts/folders/removal`](client-endpoint.md#the-record-routes), which save the whole record, create one more mailbox assigned to that user alone, end their assignment to one, and do the same three things to one folder of one mailbox. The three folder routes reach no mail server, which is [ADR 0007](https://github.com/Krzysztof318/MailFathom/blob/main/docs/decisions/0007-remote-mailbox-mutation-boundary-and-write-session.md): a folder here is a mapping this deployment reads under, so declaring one says which remote path to read and withdrawing one says to stop reading it, and neither renames nor deletes anything on the server. It also maintains the envelope that record hangs on: [`POST /api/client/display-name`](client-endpoint.md#the-name-routes), which records that person under the name they correct theirs to. It is separate from `mailfathom.mail.read` for the reason every write here is: reading somebody's mail is not deciding which mailboxes this deployment reads for them. It reaches that user's record and no other — the routes name nobody, so the caller's own identity is the whole of whose record is changed — and it assigns no account to anybody else. Withdrawing a mailbox ends that person's assignment to it and erases what they authored there; where it was the last assignment the account goes too, with every message, folder, and attachment this deployment stored for it, because an account nobody is assigned to serves nobody. The person disposes of their own hold on a mailbox under this grant, where an administrator ending somebody else's assignment takes `mailfathom.admin.erase`. Nothing under this name reaches anybody else's row: recording a user, relabelling another one, and erasing one are administrative, and no user-facing grant lists who else this deployment serves. The name route is the one place it writes the envelope, and it writes only the caller's own — refused outright while a configuration source still declares that person, because a start would write the declared name back over it. It publishes no MCP tool |
| `mailfathom.mail.folders.write` | mail | Creating, renaming, moving, and deleting the folders of any account the signed-in user holds: [`POST /api/client/managed-folders`](client-endpoint.md#the-folder-management-routes), [`POST /api/client/managed-folders/renames`](client-endpoint.md#the-folder-management-routes), [`POST /api/client/managed-folders/moves`](client-endpoint.md#the-folder-management-routes), and [`POST /api/client/managed-folders/deletions`](client-endpoint.md#the-folder-management-routes). Where the mailbox is MailFathom's own the act is a local commit, and where it is mirrored from a source server the same act runs on that server — so this grant does reach a mail server, and on a mirrored account it is what lets somebody issue `CREATE`, `RENAME`, and `DELETE` there. It is separate from `mailfathom.mail.move` because what it changes is where mail can be filed rather than where one message is, and a deletion takes the mail with it — inside the trash of a held account it erases the folder with every message in it, and on a mirrored account the [account's own deletion setting](configuration-mail.md#one-account--a-mailbox-in-a-users-record) decides whether the folder goes off the server as well. Reading the folders is `mailfathom.mail.read`, like every other read here. Each change is written to the service log as an audit record naming the account, the folder's identity, and the kind of change, never the folder's name. It publishes no MCP tool |
| `mailfathom.mail.contacts.read` | mail | `list_contacts` and `get_contact`, which read the books the caller's user reads — their own, and the collected book of each mail account assigned to them: names, addresses, and the notes a user wrote about identified third parties. On the client endpoint it also reaches [the contact routes](client-endpoint.md#the-contact-routes) that read — [`GET /api/client/contacts`](client-endpoint.md#the-contact-routes), which pages the people the signed-in user wrote down, [`GET /api/client/contacts/collected`](client-endpoint.md#the-contact-routes), which pages the people their own mailboxes picked up, and [`GET /api/client/contacts/{contactId}`](client-endpoint.md#the-contact-routes), which reads one of them. They are the same reading the two tools are, split into two pages because an address book and a list of everybody ever written by are different things on a screen. There is no export here: that stays administrative, because it is what a data-subject request is answered from It is also half of what [`GET /api/client/contacts/{contactId}/correspondence`](client-endpoint.md#the-contact-correlation-route) asks: that route reads one contact out of those books and then correlates their addresses with the mail this caller already holds, so it is published under `mailfathom.mail.read` and refused for a caller missing either — reading somebody's mail is not reading who their correspondents are, and the answer carries both |
| `mailfathom.mail.contacts.write` | mail | `create_contact`, `update_contact`, `delete_contact`, and `promote_contact`, which record, amend, erase, and take on a person across the books the caller's user reads. Recording and amending reach that user's own book alone, and promotion writes the caller's own copy of a record one of their mail accounts collected. **Erasing does not**: `delete_contact` reaches whichever of those books holds the person, so erasing a collected record takes it out of the mail account's book for every user assigned that account. The erasure is here rather than apart, because a grant that cannot edit the book cannot be trusted to take somebody out of it — and that is what this one grant reaches beyond the caller's own book. On the client endpoint it also reaches the four [contact routes](client-endpoint.md#the-contact-routes) that write — [`POST /api/client/contacts`](client-endpoint.md#the-contact-routes), [`PUT /api/client/contacts/{contactId}`](client-endpoint.md#the-contact-routes), [`POST /api/client/contacts/{contactId}/promotion`](client-endpoint.md#the-contact-routes), and [`DELETE /api/client/contacts/{contactId}`](client-endpoint.md#the-contact-routes) — which are the same four acts the tools are, performed by the person rather than for them. The administrative surface's bulk erase of a whole collected book is not among them: emptying what one mailbox picked up is an act about the mailbox that an operator takes |
| `mailfathom.admin.read` | administrative | The reads reporting the deployment's own state and no mail: what synchronization is doing per account and per folder, embedding status and the activation preview, the loaded rules, a run's progress, what a rewind would cost, where a re-derivation has got to, the stopped-job list, the outbox counted by stage and listed without naming anybody, and which users this deployment holds records for, what each of them signs in with, and what each one's record declares, beside the user, mail account, and organization records this deployment [will not read](admin-endpoint.md#records-this-deployment-will-not-read). The roster and the user records are here and nowhere else: no user-facing grant reads them, so a person signed in to this deployment cannot learn who else it serves. It also reads the deployment's **own configuration** — the three routes under `/api/admin/configuration`, which report every setting the deployment composed with the layer that decided each one, the persisted document an editing session opens, and what an adoption would copy. Read without a prefix that is the whole composed configuration: authorization-entry names, OAuth issuers and authorized subjects, provisioned file paths, mail account addresses, and folder mappings. A secret-bearing setting and a bootstrap-only one report the redaction marker rather than their value, and so does a variable of the host process that no MailFathom section names — but the rest is read in full, so a credential holding this name is one that may read how the deployment is put together |
...
| `mailfathom.admin.erase` | administrative | Disposing of what this deployment holds: the mail stored for a folder an account no longer mirrors, one person and everything the contact book derived from them, the database copies a finished move left beside the objects it verified, and **a user together with every message, folder, attachment, and derived index this deployment holds for them**. That last one is why erasing a user is not `mailfathom.admin.configuration.write`: recording somebody decides what the deployment reads next, and removing them destroys what it already read |
...
different consequences, and only the second is visible to the user in the client they open. It carries no read of its
own either — a credential granted it and nothing else can change an email it can name and cannot list one.

`mailfathom.mail.move` is separate from `mailfathom.mail.flags.write` because those two writes fail differently. A wrong
flag misdescribes mail the user can still find in the folder they left it in; a wrong move puts the mail somewhere
else, and against a server without the `MOVE` extension it is a copy followed by a delete, which is the one act on this
surface that can lose a message outright if it stops half way. A deployment that lets a client mark mail read has
therefore not thereby let it refile the mailbox.

`mailfathom.mail.delete` is separate from `mailfathom.mail.move` on the same reading carried one step further. A
move the user did not want is undone by moving the mail back, because the mail is still somewhere; a delete asks the
server to expunge the occurrence, and nothing on this surface puts it back once it has. So a deployment that lets a
client file mail has not thereby let it destroy any. It is also the one grant here whose act the client both confirms
beforehand and holds for a moment afterwards: a delete written on the client surface waits out the person's own
notification before anything reaches the server, which is the only way back an act that cannot be reversed can have.

`mailfathom.mail.send` is separate from all three for a stronger version of the same reason. A flag change reaches the
user's own mail server and is undone in their own client with the gesture that would have made it; a send reaches
...
authoring mail and is the half that reaches nobody: the message goes into the user's own drafts folder, the user
...
**A command acting for a user makes one more request when the invocation names none.** Every `mfctl user` command
but `list` and `add` takes `--user`, and does not need it on a deployment holding one person: without it the command
reads the roster and acts for the sole user it finds, which is one `mailfathom.admin.read` request beside the
...
| `mfctl account edit` | the declaration the editing session opens, and the version what you saved is committed against |
...
Recording, amending, promoting, and erasing a person are performed by the contact tools and by the signed-in user's own
[contact routes](client-endpoint.md#the-contact-routes) as well as by those routes, so the use case behind them admits
the administrative name *or* `mailfathom.mail.contacts.write`. The two halves are disjoint, so requiring one of them
would leave the act reachable from `mfctl` and refused from every agent and every client. It is an alternative rather
than a widening, and it stops where the act does: **exporting somebody is named for the administrative surface alone**,
because that is what a data-subject request is answered from, and so is emptying a whole mailbox's collected book.
[MCP tools](../features/mcp-tools.md#the-contact-book-on-this-surface) and
[the client endpoint](client-endpoint.md#the-contact-routes) are what the other half publishes.
...
**Which mailboxes a caller reaches.** Every tool call resolves the accounts the credential's own user controls and
refuses anything outside them, whichever method got the caller in, and no permission narrows or widens that. Which user
...
surface or the client surface is admitted to act for one user, and every mailbox read is resolved against the accounts
