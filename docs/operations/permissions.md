...
| `mailfathom.mail.ask` | mail | `ask_mail`, which answers from mail content by sending it to a model provider. On the client endpoint it also reaches the two [Discover routes](client-endpoint.md#the-discover-routes) — [`POST /api/client/discovery/runs`](client-endpoint.md#the-discover-routes), which asks a question of the owner's mail, and [`GET /api/client/discovery/runs/{runId}/events`](client-endpoint.md#the-discover-routes), which reads the run answering it. Both are the same act as the tool and are therefore published under the same grant, the reading route included: what it hands back is the answer to a question that was asked under this one, so a run belongs to the owner who asked for it and somebody else's reads as no such run. It does not imply `mailfathom.mail.read`, and granting it is granting access to mail |
