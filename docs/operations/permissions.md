...
| `mailfathom.mail.contacts.read` | mail | `list_contacts` and `get_contact`, which read the books the caller's user reads — their own, and the collected book of each mail account assigned to them: names, addresses, and the notes a user wrote about identified third parties |
...
the administrative surface alone, because taking a record on is the user's judgement about somebody collection
