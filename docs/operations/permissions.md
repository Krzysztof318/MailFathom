...
| `mailfathom.mail.folders.write` | mail | Creating, renaming, moving, and deleting the folders MailFathom keeps for an account whose mailbox it holds: [`POST /api/client/local-folders`](client-endpoint.md#the-local-folder-routes), [`POST /api/client/local-folders/renames`](client-endpoint.md#the-local-folder-routes), [`POST /api/client/local-folders/moves`](client-endpoint.md#the-local-folder-routes), and [`POST /api/client/local-folders/deletions`](client-endpoint.md#the-local-folder-routes). Every one of them is refused on an account whose mailbox is mirrored from its source server, whatever this grant says, and none of them reaches a mail server. It is separate from `mailfathom.mail.move` because what it changes is where mail can be filed rather than where one message is, and a deletion inside the trash erases the folder with every message in it. Reading the folders is `mailfathom.mail.read`, like every other read here. Each change is written to the service log as an audit record naming the account, the folder's identity, and the kind of change, never the folder's name. It publishes no MCP tool |
