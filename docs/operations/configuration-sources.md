...
Two further bounds hold while owners are declared. Only one owner may be served whenever the administrative endpoint is enabled, or whenever the MCP or client endpoint is enabled with an empty `Authentication` list: those are the two shapes that admit a caller carrying no owner — an administrator acts for the deployment rather than for a person, so the acts of theirs that need an owner resolve the sole one, and a surface requiring no credential has nothing to resolve one from. A mail-serving surface that requires a credential serves any number of owners, because every method it accepts resolves the owner record the credential belongs to. A deployment serving several in one of the two refused shapes is refused, and the message says which of them it was. And no two owners may name a mail account alike — this release resolves an account's settings by its identifier alone, so a name two owners shared would reach whichever declaration the lookup met first. Give each mailbox a name no other owner uses.
