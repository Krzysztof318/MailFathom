...
The same section is **refused** once nobody reads it. An adoption copies the accounts into the owner's own record and leaves the section where it was, so a deployment whose every owner reads a record of their own still binds a section that belongs to nobody — and an account's settings are resolved from that section before any record, which would leave each mailbox synchronized under the file the adoption said had stopped being read. Clear `MailSynchronization:Accounts` after adopting; a start that meets it says so and names the section.
