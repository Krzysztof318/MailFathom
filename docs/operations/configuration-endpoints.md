...
### The accepted methods — `McpEndpoint:Authentication:<n>`

Each entry names one method the endpoint accepts and carries whatever that method needs from a deployment. The
credentials themselves are records beside the user each resolves, provisioned over
...
| `…:<n>:Basic:AttemptsPerMinute` | int | `10` | Wrong passwords one source and one login each get per minute — the login being the organization's short name in front of the username where it names one — spent only by a wrong password and returned a minute after it; the source axis is skipped for a peer `ReverseProxy:TrustedProxies` names, whose address reports the proxy rather than the client; `1` to `600` | restart |
| `…:<n>:Basic:MaxConcurrentVerifications` | int | `128` | Password verifications the surface may have in flight at once, whatever logins they name — what a caller varying the login can make one replica derive together; `32`, which is what one source or one login may already hold, to `512`; one replica's, so *n* replicas derive up to *n* × this | restart |
...
| `ClientEndpoint:TelemetryLevel` | enum | `Info` | `Trace`, `Debug`, `Info`, `Warn`, `Error`, `Fatal` — the lowest severity a client is asked to record about itself, published on [the session route](client-endpoint.md#the-session-route) and applied by the client before a record is written. The default for every client rather than the only answer: a person whose record states a [`ClientTelemetryLevel`](configuration-sources.md#the-level-this-persons-client-records-at--clienttelemetrylevel) is served theirs instead, without a restart and without changing what anybody else is served. Read only where a collector is named; a deployment forwarding nothing publishes `off` whatever this and every record says. `Debug` is what a defect report is worth turning on — for one person on their record rather than here, see [what the client publishes](telemetry.md#what-the-client-publishes-about-itself) | restart |
