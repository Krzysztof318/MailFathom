...
| `…:<n>:Permissions` | string list | absent = everything this surface publishes | The [permissions](permissions.md#the-published-set) every credential this entry admits may hold; an empty list grants nothing. A value ending in `.*` grants [the subtree beneath that prefix](permissions.md#writing-a-grant), resolved against the published set on every start. A name nothing publishes, a name belonging to the other surface, a repeated name, a pattern matching nothing or matching only the other surface, a pattern overlapping what the grant already carries, and a bare `*` or `mailfathom.*` each fail startup naming the entry's index | restart |
