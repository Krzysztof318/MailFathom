...
credentials themselves are records beside the owner each resolves, provisioned over
[the administrative endpoint](admin-endpoint.md#owner-credentials), so nothing here names a key, a public key, a subject,
...
| `…:<n>:Basic` | block | — | Written only on an entry accepting `password`. Startup refuses it on a surface that answers its routes over clear text without being declared to sit behind a proxy that terminates TLS — which `HttpAndHttps` does too, unless its clear-text socket redirects | restart |
