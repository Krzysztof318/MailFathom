...
| `…:<n>:Permissions` | string list | absent = everything this surface publishes | The [permissions](permissions.md#the-published-set) every credential this entry admits may hold; an empty list grants nothing. A name nothing publishes, a name belonging to the other surface, and a repeated name each fail startup naming the entry's index | restart |
