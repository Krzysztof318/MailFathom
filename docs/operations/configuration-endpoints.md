...
| `…:<n>:Basic` | block | — | Accepts [an owner's username and password](mcp-endpoint.md#passwords). Carries no credential: the records are provisioned over [the administrative endpoint](admin-endpoint.md#owner-credentials). At most one entry may carry it, and startup refuses it on `AdminEndpoint` and on a surface that answers its routes over clear text without being declared to sit behind a proxy that terminates TLS — which `HttpAndHttps` does too, unless its clear-text socket redirects | restart |
| `…:<n>:Basic:AttemptsPerMinute` | int | `10` | Password attempts one source and one username each get per minute, spent only by a wrong password and returned a minute after it; the source axis is skipped where a reverse proxy is named, since every request then reports the proxy; `1` to `600` | restart |
