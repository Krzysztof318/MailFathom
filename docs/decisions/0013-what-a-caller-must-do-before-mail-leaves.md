...
Two readings follow, and the second is the one this record is built on. Client-side consent is real and worth advertising for: all five ask before a write by default, and two of them decide from what the descriptor says — Visual Studio Code explicitly, and the ChatGPT application by its own warning that a write can happen when a tool is mis-tagged as read-only. And client-side consent is never a guarantee: every product above documents the setting that removes it, and Claude Code's elicitation hook removes it from elicitation too. A confirmation the client performs is a confirmation the client's operator can switch off, whichever protocol feature carries it.
