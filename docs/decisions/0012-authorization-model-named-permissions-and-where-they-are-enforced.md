...
Issue 588 held the axis as future work behind a multi-user or multi-tenant decision, which ADR 0014 has since taken; what remains on that issue is the narrower axis 0014 deliberately did not take, bounding a credential *within* its own user's accounts. It carries the two things a later design starts from rather than rediscovers: that accounts are not permissions, since folding them together would put the operator's own account aliases into a vocabulary this repository publishes; and that a bound has to be applied before ranking rather than after it, so that mail the caller may not see never influences the results it does.
...
- Issue 590 is the parent this record gates. Issue 584 writes the grant in configuration, 585 carries the caller into the application layer, 586 and 587 enforce on the two surfaces, and 589 records the refusals. Issue 745 allocates `mailfathom.mail.send` when there is a sending tool. Issue 588 is not one of them: it holds the account bound this record excludes, outside issue 590's group and behind a multi-user decision that ADR 0014 has since made.
