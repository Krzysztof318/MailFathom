...
`Accounts` is read exactly as `Permissions` is, and for the same reasons: no `Accounts` key means every configured account, now and as accounts are added, while the key carried with an empty list means none. One rule for both settings rather than two, so an operator learns the absent-versus-emptied reading once and issue 584 implements it once.

The cost of the absent case is that configuring a second mailbox widens every credential whose entry never named accounts. That is what the deployment already does today, and it is stated on the page describing the setting rather than met by making the operator enumerate their accounts on every entry — which would silently exclude a new account from every credential until each was edited, and is the failure this reading is chosen against.
