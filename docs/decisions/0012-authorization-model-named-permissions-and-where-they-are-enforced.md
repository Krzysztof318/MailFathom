...
So no entry carries an account list, the contract the application layer receives says nothing about accounts, and no path narrows what it returns by the caller's identity. Every admitted caller reaches every account the deployment configures, which is what it does today.
