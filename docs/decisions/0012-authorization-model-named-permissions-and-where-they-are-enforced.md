...
<!-- describes: backend/src/Domain/Access/**, backend/src/Application/Access/**, backend/src/Host/Security/**, backend/src/Host/Configuration/Access/**, backend/src/Host/Api/**, backend/src/Host/Mcp/McpAttachmentDownloadEndpoint.cs, backend/src/Mcp/Tools/** -->
...
**The MCP protocol has no place on a tool descriptor to state a required permission.** Checked on 2026-08-15 against the current draft specification: a `Tool` carries `name`, `title`, `description`, `icons`, `inputSchema`, `outputSchema` and `annotations`, and `ToolAnnotations` carries `title`, `readOnlyHint`, `destructiveHint`, `idempotentHint` and `openWorldHint`. Per-tool scope advertisement is an open item of the Authentication Interest Group's Tool Scopes working group and is not in the protocol. An earlier design had each tool declare an OAuth security scheme and a required `mail.read` scope; that predates the settled protocol, and this record supersedes it on the point.
...
- **The deployment serves one user's mail.** This is not a multi-tenant service, so the model does not have to express delegation, group mapping, or a policy language, and inventing them would be scaffolding for a shape nobody has. **ADR 0014 has since decided that one deployment serves several users**, so this driver no longer describes the shape being built. What it decided about *this* record survives it unchanged: MailFathom is still not multi-tenant, ownership is not a permission, and the closed set below names no account. What ownership adds is a scoping axis beside the vocabulary rather than inside it, resolved before a query rather than granted on a credential.
...
| --- | --- |
| `mailfathom.mail.read` | The MCP tools that read the local mailbox copy: `list_accounts`, `list_emails`, `get_email_content`, `search_emails`. Not an egress-free grant — see below. |
...
**`mailfathom.mail.read` still reaches a provider, and the record says so rather than letting the driver above be read as a promise.** Where semantic retrieval is configured, `search_emails` places the caller's own query text with the embedding provider — `SemanticEmailSearch` asks `ITextEmbeddingGenerator` to embed it, which this repository classifies as `SensitiveContentEgressPoint.HostedEmbeddingInput` and scans before it leaves. So withholding `mailfathom.mail.ask` stops mail content going to a *chat* provider on a caller's behalf and does not make the grant egress-free. Splitting a third permission out of search was rejected: what leaves is the caller's own query rather than mail, the deployment's own switch already decides whether semantic retrieval runs at all, and a permission for it would name a capability an operator turns off in the place it is configured. The documentation states the egress where the grant is described, and the two pages that own the boundary keep stating it.
...
An `Authentication[]` entry states what it grants, on both endpoints, as `Permissions` — a list of published names, which is the ceiling on what any credential the entry admits may do. An entry carrying no `Permissions` key at all takes the whole of its surface as its ceiling, for the reason *An unstated grant is the whole surface* gives below — which is not what an entry carrying the key with an empty list takes. A second setting decides whether the credential itself may be granted less than that ceiling:
...
**`Permissions` written as an empty list grants nothing**, and is the one way to say that in configuration. Absence and emptiness are opposite statements: an operator who wrote nothing has not reached this feature yet, and one who wrote `[]` has reached it and narrowed all the way. Reading them the same would give the maximal grant to the single arrangement in which the operator did the narrowing, which is the wrong way round on least privilege. A credential granted nothing authenticates and is refused every operation — on the administrative surface, every route but the session read, which sits outside the permission model for the reason given with the table above. So emptying a grant is a way to retire a credential without deleting its entry, and not a way to make the entry inert: what it still answers is the caller's own identity and the deployment's version. An operator who wants nothing answered at all removes the entry.

The binder does not distinguish the two on its own, and this repository already knows how to. `McpCorsOptions.AllowedOrigins` is declared exactly as `Permissions` will be, and `McpEndpointOptions.ReadFrom` asks `section.GetSection(...).Exists()` to tell an absent list from an emptied one — so the property's own default is the restrictive value and the permissive default belongs to the read. Issue 584 follows that arrangement rather than inventing a second one, which also means an entry constructed anywhere but from configuration starts from the grant that reaches nothing.
...
### On the administrative surface, the grant belongs to a named administrator

Issue 2029 amends the section above for the administrative surface, and this record states the result in place because it is still `proposed`. An `AdminEndpoint:Authentication[]` entry admitted credentials and nobody: its grant was the entry's, and the principal it produced named whichever key or token had been presented. Nothing said who performed an administrative act, and nothing could confine a credential to the networks it was meant to be used from — which a deployment behind split-horizon DNS needs, because DNS is not a control and a key meant for the internal network works from the internet once it leaks.

`AdminEndpoint:Administrators[]` replaces that list. **An administrator is the unit a grant belongs to on this surface.** Each one carries a `Name`, the `Credentials` it signs in with — one or more `ApiKey`, `PublicKey`, or `OAuth` blocks — and the `Permissions`, `PermissionsFromTokenScopes`, and `AllowedSourceNetworks` that hold on every one of them. Every rule the section above states about a grant carries over with the administrator in the entry's place: an absent `Permissions` key is the whole surface, an empty list is nothing, and `PermissionsFromTokenScopes` is refused on an administrator that also carries a key or a public key. Administrators stay in configuration: no database record, no provisioning route, and no personal-data lifecycle, so the surface keeps answering for the deployment rather than for a mail user.

- **The admitted principal is named by the administrator.** Its identity is the administrator's `Name`, so the session read, the refusal record, and every log scope name the person or system rather than the credential. A name is required, unique ignoring case, and refused where it would be indistinguishable from an unauthenticated caller.
- **A token binds to exactly one administrator by its issuer and subject.** The subject is what `AuthorizedSubjects` already lists, and a client-credentials token carries the service account's subject, so one claim serves both a person and a system. The client identifier was the alternative and was not chosen: authorization servers name it in different claims — `azp`, `client_id`, `appid` — so binding on it would be a per-vendor mapping, and it would be a second axis beside the subject for the same question. Startup refuses a subject two administrators claim at one issuer, and a validated token naming no administrator fails authentication and is answered `401`, exactly as a token that did not validate.
- **`AllowedSourceNetworks` confines an administrator to the addresses it may act from.** Entries are addresses or CIDR networks validated as `ReverseProxy:TrustedProxies` is, the host-bits refusal included, and an empty list is no restriction. A request presenting a valid credential from elsewhere fails authentication with the same answer a wrong credential gets, so the refusal discloses nothing about the credential, and a warning names the administrator and the source address for the operator. The check runs in this process, where both the source and the credential are known; the reverse proxy supplies only the network fact, so no proxy has to know which key belongs to whom.
- **The source address is the client's, and only a named proxy is believed about it.** The forwarded-headers policy applies `X-Forwarded-For` beside the scheme and the host when `TrustedProxies` names a proxy narrower than a whole address family, and never otherwise. Startup therefore refuses `AllowedSourceNetworks` while no such proxy is named, since a restriction compared against an address any client could write would be no restriction. IPv4-mapped addresses are compared in their IPv4 form, as the proxy match already is.

The break is recorded as ADR 0004 requires: the configuration schema moves, and an operator moves each former entry's credential blocks under `AdminEndpoint:Administrators:<n>:Credentials`, gives the administrator a `Name`, and moves `Permissions` and `PermissionsFromTokenScopes` onto it.
...
Such an entry that also carries no `Permissions` key therefore advertises its surface's whole half, because that is genuinely what a token there may bring. One carrying the key with an empty list advertises nothing, for the same reason: it grants nothing, so there is nothing a client should ask for. An operator who wants a shorter list in the document writes the ceiling they meant, which is the same act as narrowing the grant.
...
A credential whose entry carries no `Permissions` key at all holds every permission that entry's surface publishes. Nothing about this feature has to be configured before a deployment works, and nothing an operator has already written stops working when they upgrade into it. This is about the key being absent and never about the list being empty: an operator who wrote `[]` reached this feature and narrowed all the way, and *A grant belongs to the credential's configuration entry* above gives them nothing.
...
That reading is already this repository's on the neighbouring question. An `Authentication[]` or `Administrators[]` list with no entries at all accepts no credential, and neither `UserFacingAuthenticationConfiguration` nor `AdministratorConfiguration` refuses it: a posture is reported by the startup warning each endpoint carries rather than treated as a mistake. A grant nobody narrowed is the same kind of fact, and it is reported the same way.

Three things carry the cost of the permissive reading, and none of them asks the operator to configure anything:

- **The startup record states what every entry and administrator resolved to**, naming the ones whose grant was not written down and what they therefore hold, and for an administrator the networks it may act from. An operator meets their own posture in the log on the first run rather than inferring it later from what a credential turned out to be able to do.
...
One consequence reaches the children directly and is stated here so none of them goes looking for it: **nothing in this model breaks a public surface.** `Permissions` and `PermissionsFromTokenScopes` are settings an entry may carry, and every existing entry is valid without them; a listing narrows only for a caller whose operator narrowed it, so the tool contract a shipped deployment serves does not move either. Issue 584's and issue 586's acceptance items about recording a break therefore have nothing to record, and what issue 590 predicted — that the configuration schema and part of the tool contract would break — is what this record decided against. A child that does introduce a break records it as any change would.

### The accounts a caller may reach are not part of this model

A permission says what a caller may do. Which mailbox it may do it to is a different question, and this record does not answer it.

Not because the two would be hard to combine, but because bounding a caller to particular accounts presupposes a deployment in which more than one caller has a legitimate claim to a different part of the mail — several users, or one user deliberately confining an agent to a corner of their own mailbox. This deployment serves one user's mail, which is a decision driver above rather than a fact about the current release, and *Revisit* below already names a deployment serving more than one user as what reopens the question. Settling the bound now would fix its shape — a list of aliases on a credential's entry — before the question that shapes it has been asked, which is the same mistake as inventing a policy language for a tenant nobody has. **That question has since been asked and answered by ADR 0014**, and the answer vindicates the exclusion rather than reversing it: the bound is not a list of aliases on a credential's entry and not a permission at all, but a narrowing of the account resolution that runs before a mail query, so the shape this record declined to fix is not the shape the answer took.

So no entry carries an account list, the contract the application layer receives says nothing about accounts, and no path narrows the accounts it returns by the caller's identity. Every admitted caller reaches every account the deployment configures, which is what it does today.

Issue 588 held the axis as future work behind a multi-user or multi-tenant decision, which ADR 0014 has since taken. **Nothing remains on it.** The narrower axis that record was read as leaving there — bounding a credential *within* its own user's accounts — is refused by it, and the issue is closed as not planned: a credential reaches every account the user it resolved to owns, and nothing narrows it further. So this record's exclusion is permanent rather than pending, and the two things it preserved stay true of anything that proposes the bound again: that accounts are not permissions, since folding them together would put the operator's own account aliases into a vocabulary this repository publishes; and that a scope has to be applied before ranking rather than after it, so that mail the caller may not see never influences the results it does.
...
What the application layer receives is an application-owned contract describing the caller: the configured identity it was admitted under and the permissions it holds. Nothing from `System.Security.Claims`, ASP.NET Core, or the MCP SDK crosses that boundary, and no use case learns which credential admitted the caller. A host adapter populates it per request scope.
...
No user directory, no group or tenant claim mapping, no delegation or impersonation, no policy language, no per-message rules, no bound on which of the configured mail accounts a caller may reach, and no permission that varies with the content of a request. A later issue that wants one of these is proposing a decision this record did not take, and says so.
...
- Neutral, because every admitted caller goes on reaching every account the deployment configures. That is unchanged rather than conceded, and it stays that way until a multi-user decision gives the bound a shape to take.
...
- Startup refusals are unit-tested per arrangement: an unknown name, a cross-surface name, a permission name written into `RequiredScopes`, a permission name written into `AdvertisedScopes`, and `PermissionsFromTokenScopes` on an entry carrying a key or a public key. Each asserts the configuration path in the message, including the entry's index. Beside them a test asserts the arrangements that are *not* refusals and what each resolves to: an absent `Permissions` key and a surface with no entry at all reach the whole surface, and an explicitly emptied `Permissions` reaches nothing. Those are the cases worth pinning, because absence and emptiness arrive from the binder identically and only the read tells them apart.
...
- Issue 590 is the parent this record gates. Issue 584 writes the grant in configuration, 585 carries the caller into the application layer, 586 and 587 enforce on the two surfaces, and 589 records the refusals. Issue 745 allocates `mailfathom.mail.send` when there is a sending tool. Issue 588 is not one of them: it held the account bound this record excludes, outside issue 590's group and behind a multi-user decision that ADR 0014 has since made and then used to refuse the bound outright, which is why that issue is closed as not planned. Issue 1313 recorded the refusal here and in ADR 0014.
...
- Revisit when the Tool Scopes working group settles per-tool scope advertisement in the protocol. **The second trigger has fired**: a deployment serving more than one user's mail was decided by ADR 0014 (`0014-single-tenant-multi-user-ownership-on-the-mail-account.md`), which reopened the accounts axis and answered it outside this vocabulary — ownership hangs on the mail account and narrows the resolution, so the closed set here is unchanged and no permission names an account. That record also adds a deployment administrator distinct from a user: a principal holding administrative capabilities from this same set rather than a new name in it. Issue 2029 gave that administrator a name and a network restriction in configuration, which *On the administrative surface, the grant belongs to a named administrator* records.
