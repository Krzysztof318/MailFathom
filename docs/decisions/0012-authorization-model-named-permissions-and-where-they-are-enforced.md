...
- Startup refusals are unit-tested per arrangement: an unknown name, a cross-surface name, a name repeated between `RequiredScopes` and a grant, and `PermissionsFromTokenScopes` on an entry carrying a key or a public key. Each asserts the configuration path in the message, including the entry's index. Beside them a test asserts the arrangements that are *not* refusals and what each resolves to: an absent grant and a surface with no entry at all reach the whole surface, and an explicitly emptied grant reaches nothing. That pair is the one worth a test of its own, because the two arrive from the binder identically and only the read tells them apart.
