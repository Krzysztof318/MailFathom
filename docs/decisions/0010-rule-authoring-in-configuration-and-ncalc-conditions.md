...
- **Whatever holds a rule has to answer what a rule's version is.** The architecture draft's section 21.5 derives an execution's idempotency identity from the message occurrence, the rule version, the trigger generation, and the action, and issue 458 has to explain a past run afterwards. Both need a revision to name, so the identity of a revision is part of this decision rather than downstream of it.
...
- Section 21.5 of the architecture draft derives an execution's idempotency identity from the message occurrence, the rule version, the trigger generation, and the action, and requires rule ordering, stop-or-continue behavior, and conflict resolution to be deterministic and reviewable without invoking a model.
