...
**A folder name is validated where it is written**: it is non-empty, carries no control character and no hierarchy delimiter, is bounded in length, differs without regard to case from every sibling, and is not `INBOX` at the top level. The local hierarchy delimiter is `/`, fixed, because it is MailFathom's own; the restore rejoins a local path's segments with the delimiter the source reports. **A segment carrying that delimiter is never rejoined.** Before its first `CREATE` the restore reads the source's delimiter, and where any local folder name carries it — `2026.Q1` against a server whose delimiter is `.` — it issues nothing, names those folders in its report, and waits in `Restoring` until they are renamed, because a rejoined path would name folders on somebody's mail server that nobody created. Escaping is not available instead: IMAP has no quoting for a mailbox name's own hierarchy delimiter.
