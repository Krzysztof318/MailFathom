...
The answer is that the two markup surfaces carry two different `sandbox` values, because they have two different problems — **which the fourth question below revises: they carry the same value now, and what separates them is only which script the client prepends.** The dialog keeps `sandbox=""` until then. The embedded frame carries `sandbox="allow-scripts"` and nothing else — no `allow-same-origin`, so the framed document holds an opaque origin and reaches nothing of the page — and the client prepends its own measuring script to the `srcdoc`, ahead of the message markup, which reports the height by `postMessage`. What then holds *nothing in the message runs* on that surface is no longer the sandbox but the representation: [#1484](https://github.com/Krzysztof318/MailFathom/issues/1484) serves markup with nothing executable in it. That is a property moving off the platform and onto a build, and the tables and consequences below count it as one rather than presenting it as free.

**A fourth question was asked on [#1797](https://github.com/Krzysztof318/MailFathom/issues/1797), and this record answers it in place for the same reason it answered the previous three.** A reader who opens the full-markup surface cannot follow a link on it. `sandbox=""` sets the sandboxed navigation browsing context flag along with every other flag, so an anchor in the sender's markup does nothing whatever when it is pressed: no navigation, no new context, and no failure the reader can read as anything but the application being broken. That was never chosen — it is what granting nothing happens to mean — and it went unnoticed because the surface was reasoned about as something to look at rather than as something to read. A message's links are most of what a message is for, and *show me what actually arrived* is not answered by a picture of it.

The answer is that the dialog joins the embedded view. Both markup surfaces carry `sandbox="allow-scripts"` and nothing else, and the client prepends a second script of its own to each `srcdoc` — a capture-phase click listener that reports the pressed anchor's address by `postMessage`, which the parent validates and hands to the application's own link opener, exactly as the reading pane's links have always been handed to it. So the two surfaces stop carrying two different values, the inertness property comes off the platform on the dialog as well as on the embedded view, and following a link out of the application is one operation resolved at the composition root on every surface that draws a message.
...
- **A surface a reader cannot act on is a surface they leave.** A message's links are much of what the message is; a rendering that draws them and then swallows every press is not a safer reading of that message but a broken one, and the reader's next move is the mail client they came from. So *what does this surface cost* is asked beside *what can a reader do on it*, and a property held by making the surface useless is not held at an acceptable price.
- **Following a link is one operation, wherever the link was drawn.** [ADR 0023](0023-where-the-client-keeps-the-credential-it-signs-in-with.md) put the application's outward acts behind a shell operation resolved once at the composition root, and a link is one of them — the desktop head has to reach the system browser rather than navigate its own WebView. A surface that opened links by its own route would be a second answer to that question, and the head-specific half of it would live in a component instead of in the shell.
...
**I — how a link inside a markup surface is followed, which is a question neither B4 nor B5 asked:**

- I1 — `allow-popups` and `allow-popups-to-escape-sandbox`, so an anchor with `target="_blank"` opens a context of its own from inside the framed document.
- I2 — `allow-top-navigation-by-user-activation`, so a pressed link navigates the top-level context.
- I3 — `allow-scripts` with no navigation flag at all, and a capture-phase click listener the client prepends to the `srcdoc` ahead of the message markup, reporting the pressed anchor's address by `postMessage` for the parent to open through the application's own link opener.
- I4 — nothing: links stay inert on the markup surfaces, and a reader who wants to follow one goes back to the reduced tree, where the same link is already followable.
...
On the fourth question: **I3, and both markup surfaces therefore carry `sandbox="allow-scripts"`.** The dialog stops carrying `sandbox=""`; the paragraph above that said it keeps that value, and the row of the table below that rested the dialog's inertness on the sandbox, are corrected here rather than left standing. Neither surface is granted a navigation flag — not `allow-popups`, not `allow-popups-to-escape-sandbox`, not `allow-top-navigation` in any of its forms — so the framed document still has no way out of the frame on its own. What it has instead is the client's own listener, prepended to the `srcdoc` ahead of the message markup: it reads the pressed anchor's `href`, refuses the press, and reports the address by `postMessage`. The parent accepts that report only from a frame this component created, admits only `http`, `https`, `mailto`, and `tel`, bounds its length, and hands what survives to the same link opener the reading pane uses — one browsing context on the web head, the system browser on the desktop head, decided at the composition root and by nothing in the component.

**What that costs on the dialog is the same trade the embedded surface already made, and it is now made on both.** *Nothing in the message runs* was held there by the sandbox with [#1484](https://github.com/Krzysztof318/MailFathom/issues/1484)'s strip behind it; granting `allow-scripts` takes it off the platform on the dialog too, so the strip is the whole of that property on every surface that draws markup. The record does not present that as free, and the tables and consequences below carry it. What it buys is that the surface whose promise is *this is what arrived* stops being the one surface where a reader cannot act on what arrived.

**One value on both surfaces is a simplification rather than a loss.** *Two surfaces, two values* was a fact a reader had to hold; what remains is one value and two prepended scripts, both written in the same one component, which is a difference a reader can see in the source they are already reading.
...
### The embedded surface: the same frame, the same representation, and — since the fourth question — the same flag
...
|---|---|---|---|
| No script, embedded object, form, or event handler executes | The document contract has nowhere to express one | **The representation**, since the fourth question. `allow-scripts` is granted here too, so what the sandbox held is now held by the same strip the embedded view rests on; no `allow-forms`, so a form still submits nowhere | **The representation.** `allow-scripts` is granted, so the strip is the whole of it: nothing executable survives [#1484](https://github.com/Krzysztof318/MailFathom/issues/1484)'s pass, and the only script in the frame is the client's own. Off the platform and onto a build |
...
| A link cannot navigate the application | A scheme allow-list, and the client opens it out of the application | **The sandbox, the allow-list, and the client's own opener.** No `allow-top-navigation` and no `allow-popups`, so the frame reaches nothing by itself; a pressed link is reported to the parent, filtered to the four admitted schemes, and opened out of the application | The same three, unchanged. `allow-top-navigation` and `allow-popups` are as absent here as there, so script buys the markup no way out of the frame, and a link leaves through the parent's opener |

**On the dialog, three of those five depended on the browser where none did before**, and that was the honest cost of that surface as the second amendment left it. The fourth question moves row one off the platform there as well, so the dialog's count is now two rather than three and reads exactly like the embedded view's — with the same sentence attached, below, about what that number actually means. Row five still rests partly on the flags that are withheld, and row three still rests on the frame being its own browsing context, which is as much the engine's guarantee as the flags are and is counted with them rather than treated as free because a frame feels structural.

**On both markup surfaces the count is two rather than three, and that is not an improvement.** Row one came off the platform, which is what moved the number — and it landed on a filter, which is the thing this record's own drivers say a safety property should not rest on: *a rendering path whose safety rests on the completeness of a filter is one whose safety is only as good as this year's bypass.* Trading a browser guarantee for a filter is a worse position than the one the count reports, so the count is given with the sentence rather than on its own. What makes it acceptable rather than a reversal of the record is that the filter is a removal over the message's own parse in the service, exercised by its own tests, asserted as *nothing executable survives* rather than as a list of forms — and that everything it might miss is confined to an opaque origin that can reach nothing of the page. It is the weakest of the three surfaces and this record says so plainly.
...
**On the embedded surface the same footer is carried by the representation alone, and since the fourth question the dialog's is too.** Both frames grant `allow-scripts`, so *scripts are blocked* is true on either because nothing executable reached the frame — the same words, one mechanism short of what the dialog had behind them until [#1797](https://github.com/Krzysztof318/MailFathom/issues/1797). **That corrects the paragraph above rather than standing beside it**: *drop the frame and scripts run* described the dialog as the second amendment left it, and what keeps the promise there now is [#1484](https://github.com/Krzysztof318/MailFathom/issues/1484)'s pass, with the client's own prepended listener as the only script the frame carries. The copy is still right and still stays as it is on both surfaces, because what a reader is owed is what is true of the message in front of them rather than an account of which layer secured it; what changes is that the footer is now a claim about that pass and about nothing else. It is also not the client's sentence to rewrite — the wording is the design project's own, so a footer that needed different words would be a correction handed to the owner rather than a catalogue entry edited here.
...
**The pane does not linkify plain text, and that rule is unchanged by the service having started to.** A link is a `MailDocumentLink` or it is words. Finding addresses in the text and making them clickable would be the client deciding what a link is, which is what `MailLinkDeception` exists to stop two clients from disagreeing about — so where an address a message wrote as words is worth following, [#1797](https://github.com/Krzysztof318/MailFathom/issues/1797) has the reduction find it and hand it over as an ordinary `MailDocumentLink`, carrying the same target, host, and deception verdict as one the sender wrote as an anchor. The client's half of the rule is what it always was: draw the link the document carries, and never derive one.
...
- Neutral, because the two markup surfaces carried two different `sandbox` values between the second amendment and the fourth, which was a difference a reader had to hold rather than one rule to remember. It was deliberate and stated with its reason, and it cost nothing in the code because both surfaces are the same component; the fourth question removes it, leaving one value and two prepended scripts.
...
- Good, on the fourth question, because the surface whose whole promise is *this is what arrived* is one a reader can act on: a link in the sender's markup opens where every other link in this client opens, rather than swallowing the press and reading as a defect.
- Good, because a link leaves through one operation on all three surfaces, so the desktop head reaches the system browser from the markup surfaces by the same route it already reached it from the pane, and no component learns which head it is running on.
- **Bad, because the dialog now grants `allow-scripts` as well, so *nothing in the message runs* is a property of a build on every surface that draws markup rather than on one.** The second amendment paid that price on the embedded view and called it the weakest of the three; the fourth pays it on the dialog too, which is the honest reading of the change rather than a flag granted for a convenience.
- Bad, because the anti-tracking property and the inertness property are now coupled on both markup surfaces rather than on one, for the reason the embedded view's own consequence gives: a construct the strip misses could construct an address the sender never wrote.
- Neutral, because the click listener is a second script the client prepends to the same `srcdoc`, in the same one component, so it adds a script rather than a surface — but it is a second thing that has to be right about a stranger's document, and the address it reports is a value the parent acts on.
- Bad, because the parent's `postMessage` handler now carries two kinds of report rather than one, so the source check and the shape check it already needed for a height are load-bearing for an address as well, where the failure is opening a target a message chose rather than drawing a frame at the wrong height.
- Good, because no navigation flag is granted to reach it: `allow-popups`, `allow-popups-to-escape-sandbox`, and every form of `allow-top-navigation` stay refused, so the framed document still cannot reach anything on its own initiative and the opener remains the only way out.
...
- **The browser suite proves the frame, against the built bundle.** Opening the surface on a message whose markup would fetch from another host issues no request to any host but the page's own origin; a message carrying a script does not run it; the frame carries `sandbox` without `allow-same-origin` and without any navigation flag; and the reading pane's own document still holds no `iframe`, so the surface is where it is meant to be and nowhere else.
...
- **A message carrying a script does not run one, and that is the service's suite to prove rather than the browser's.** The frame permits script on both markup surfaces, so the property is entirely [#1484](https://github.com/Krzysztof318/MailFathom/issues/1484)'s pass over the message's own parse — and the browser suite draws a corpus rather than a served representation, so a corpus writing a script would prove only that a granted flag runs one. What the browser suite carries instead is the shape of the promise: the corpus serves what the service serves, nothing executable in it, and the surface reaches no host it was not asked to.
...
Following a link is validated on both markup surfaces, since the fourth question gives them the same value and the same listener:

- **A pressed link opens out of the application and navigates nothing.** The unit suite asserts that a click reported from a frame reaches the application's link opener with the address the anchor carried, and the browser suite asserts that the page itself did not navigate and that the framed document did not either — which is what the absent navigation flags are there to make true and what an assertion on the attribute alone would not establish.
- **The parent refuses a report it should not act on.** An address in any scheme but `http`, `https`, `mailto`, or `tel`, a relative address, one past the stated length bound, a payload that is not the shape the listener sends, and a report from a window that is not a frame this component created each open nothing at all. These are unit assertions, one per refusal, for the reason the parser's own refusals are.
- **The frame carries no navigation flag.** Its `sandbox` value is asserted to hold `allow-scripts` and nothing else on both surfaces — no `allow-same-origin`, no `allow-popups`, no `allow-popups-to-escape-sandbox`, and no `allow-top-navigation` in any form — which is the assertion that the way out of the frame is the parent's opener and not the framed document's own initiative.
- **The service's own suite proves an address written as words is a link like any other.** A reduction over text carrying an absolute address, a bare host, and a mail address produces `MailDocumentLink` values with the targets, hosts, and deception verdicts an anchor carrying the same address would produce, and the punctuation a sentence wrapped around one is not part of what a reader follows.
...
### I1 — `allow-popups` and `allow-popups-to-escape-sandbox`

- Good, because it needs no script and no protocol between the two documents: an anchor with `target="_blank"` opens a context, and the browser does the rest.
- Bad, because `allow-popups` is not conditioned on a user gesture, so the framed document may open contexts nobody pressed anything for — which is the behaviour a message with an advertising payload wants and a reader never does.
- Bad, because `allow-popups-to-escape-sandbox` hands what is opened a context that is not sandboxed at all, so a stranger's markup is granted the power to place an unsandboxed document in front of the reader.
- Bad, because it routes around the shell operation entirely: on the desktop head the framed document's popup is the WebView's problem rather than the system browser's, so the one difference between the heads would stop living in the shell, which is exactly what [ADR 0023](0023-where-the-client-keeps-the-credential-it-signs-in-with.md)'s shape exists to prevent.

### I2 — `allow-top-navigation-by-user-activation`

- Good, because the gesture requirement is real: nothing navigates unless the reader pressed something.
- Bad, because what it navigates is the application. The client is a single page holding a session, and a link that replaces it with a sender's page has discarded what the reader was doing, which is the same failure the pane's own scheme allow-list is half written against.
- Bad, because it would grant the frame a way out of itself for a job the parent can do without granting one, which is a flag paid for nothing.

### I3 — the client's own listener, and the parent's opener

- Good, because the frame is granted no navigation of any kind: the way out is the parent's, on an address the parent read, filtered, and chose to open.
- Good, because a link leaves through the same operation on every surface in the client, so the desktop head reaches the system browser by the route [ADR 0023](0023-where-the-client-keeps-the-credential-it-signs-in-with.md) established rather than by a second one written for a frame.
- Neutral, because the mechanism is the one the embedded surface already carries — a script prepended ahead of the message markup and a `postMessage` the parent matches by source — so it is a second use of a boundary rather than a new one.
- Bad, because it needs `allow-scripts` on the dialog, which is what takes *nothing in the message runs* off the platform there and puts it on the strip, and that is the whole price of the option.
- Bad, because the parent now acts on an address a document sent it, so the scheme filter, the length bound, and the source check are load-bearing rather than tidy — a handler that trusted the payload would open whatever a message chose.

### I4 — links stay inert on the markup surfaces

- Good, because it grants no flag, adds no script, and keeps *nothing in the message runs* on the platform where the dialog had it.
- Neutral, because the same link is followable in the reduced tree, so nothing is unreachable — only unreachable *here*.
- Bad, because the surface that promises fidelity is the one where the message does not work, and the reader who opened it did so precisely because the reduction was not enough for them. Sending them back to the pane to press the link answers their complaint with the thing they complained about.
- Bad, because a press that does nothing is indistinguishable from a defect, which is the state the UX contract refuses everywhere else in this record.
...
- The design project drew both surfaces first and is right about the dialog, the footer included: *isolated, scripts and remote content blocked* is true of what this record decides, and it is right not to name which mechanism holds which. On the embedded view it is not right yet — it draws `allow-same-origin` and a `contentDocument` read, which this record refuses — and [#1507](https://github.com/Krzysztof318/MailFathom/issues/1507) is the correction. That is the one design change this record produces, and it is the user's to make in the project rather than something the client closes by building around it.
- [#1797](https://github.com/Krzysztof318/MailFathom/issues/1797) is where the fourth question was asked and where this third amendment was written, and it is the one amendment whose issue also implements what it decides: the dialog's `sandbox` value, the click listener both surfaces prepend, the parent's filtering and its use of the link opener, and — on the service side, and governed by *The pane does not linkify plain text* above rather than by anything about a frame — the reduction finding the addresses a message wrote as words. What the issues around it keep is unchanged: [#1484](https://github.com/Krzysztof318/MailFathom/issues/1484)'s representation and its acceptance that *nothing executable survives* become load-bearing on the dialog as well as on the embedded view, and [#1485](https://github.com/Krzysztof318/MailFathom/issues/1485)'s single named component is where both values and both scripts live.
