...
| Message style cannot escape | Nothing crosses the wire that could escape | **The frame.** A document in its own browsing context cannot style the application around it, which is what a frame is for and is stronger here than on the pane |
...
Two of those five now depend on the browser where none did before, and that is the honest cost of the surface. It is bounded: `sandbox` is not a recently shipped capability whose support diverges between WebView2 and WebKitGTK, which is the divergence [ADR 0021](0021-client-stack-react-typescript-tailwind-tauri-and-pnpm.md) makes this record wary of. The property that would have diverged — a policy delivered through markup — is the one deliberately not relied on.
...
- Bad, because two of the five properties now rest on the browser's sandbox where none rested on the browser before. `sandbox` is old and universally implemented rather than a recently shipped capability, which bounds the risk, but the sentence *nothing about the browser is relied on* is no longer true of the whole client.
