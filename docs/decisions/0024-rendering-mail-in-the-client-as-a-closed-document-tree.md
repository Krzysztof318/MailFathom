...
- Bad, because the parent's `postMessage` handler now carries two kinds of report rather than one, so the source check and the shape check it already needed for a height are load-bearing for an address as well, where the failure is opening a target a message chose rather than mis-sizing a frame.
