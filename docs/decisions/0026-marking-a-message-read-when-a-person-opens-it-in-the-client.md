...
<!-- describes: frontend/src/Client.App/src/readingPane/**, frontend/src/Client.App/src/messageBody/**, frontend/src/Client.App/src/thread/**, frontend/src/Client.App/src/messageRows/**, backend/src/Application/Mail/Mutations/**, backend/src/Infrastructure/Persistence/Users/** -->
...
- **The mailbox user's own act is the authorship this write needs.** ADR 0007's whole driver for the permitted set is *a change the mailbox user authored, carried by MailFathom on their behalf, undone in their own client with the gesture that would have made it*. A person opening their own mail is the plainest instance of that the record has met.
...
- **A2** Always, for every user and every account, with no way to turn it off.
- **A3** By default, and the user may turn it off.
- **A4** Only where the user has turned it on, off by default.
...
- **B2** A local read state per user, stored beside the remote flag and rendered in preference to it.
...
**E — where the user's choice lives:**

- **E1** A configuration key per mail account, written by the operator.
- **E2** One user-level setting in the user document, covering every account that user reads.
- **E3** A client-local setting, held on each device the user signs in from.

## Decision Outcome

Chosen options: **A3**, **B1**, **C2**, **D1**, and **E2**.

**Opening a message in the client marks it read on the user's own mail server, by default, and the user may turn that off.** The write is an ordinary flag mutation the person's act authors, submitted by the client over the mutation surface and carried to IMAP by the write session ADR 0007 already defines. The route that served the body sets nothing, and where the write does not happen the client shows what the server last reported and remembers no reading of its own.

### The act is the authorship, so nothing is confirmed

Opening a message is not a mutation somebody stumbles into. It is a deliberate gesture, aimed at one message the person picked, whose entire purpose is to read it — and marking read is what every mail client they have does with that gesture. ADR 0007 admits a change *the mailbox user authored*, and every requester it has admitted so far acts for the user in their absence — a rule they configured once, a classification they enabled, an agent holding a grant. This is the first where the user is themselves present, acting on their own mail, in a surface only they sign into. There is nothing left to confirm, and a prompt asking whether opening a message counts as having opened it would be the clearest possible statement that the application does not understand what it is.
...
That is also why this is not the exception to [ADR 0013](0013-what-a-caller-must-do-before-mail-leaves.md)'s confirmation discipline that it might look like. That record governs mail *leaving* — an act that reaches a stranger and cannot be recalled. A `\Seen` change reaches nobody but the user, is visible to them in every client they own, and is undone by the same gesture in any of them.
...
**What may be marked is what may be read**, which is ADR 0007's rule rather than a second one: the same scope answers both, so an account this deployment no longer serves and a folder withheld from the signed-in user are mail the client can no more mark than open. A write surface reaching past what the read surface withholds would be the way round the withholding rather than a capability of its own.

### A conversation marks read what it drew, which is one message until the reader asks for the rest

This is the question ADR 0020 never had to answer, and the screen that raises it is `thread/Thread.tsx`. A conversation draws its latest message and hides every earlier one behind a single control naming how many there are; `thread/threadOpening.ts` answers only *where the reader arrived* — the message somebody named where the conversation holds it, and the latest otherwise — and the history opens where arriving inside it would otherwise leave that message hidden. So a reader who opens a conversation from a search result or from a restored workspace, having named no message, gets one body on the screen rather than several they expanded with no gesture of their own.

**Every body the conversation drew is marked read (D1)**, and the reason is that the trigger stays one rule. What marks a message read is that its body was drawn, wherever it was drawn — the reading pane and a thread message mount the same `messageBody/Message.tsx` and put the same words in front of the same person. D2 would make the trigger depend on *why* a component mounted rather than on what the reader is looking at, which is the distinction that produces two mechanisms where the section above has just argued for one, and it is not one a person could state: they would be told that the messages they just read are read, unread, and unread, according to which of them the screen chose to open.

What bounds it is the control rather than a number. Nothing the screen decides for itself draws more than one body; every further body is behind *show earlier messages*, which is a gesture the reader makes knowing how many are behind it, and pressing it draws all of them — up to the five hundred a conversation assembles. So the worst an *opening* produces is one record, and the worst a *press* produces is a conversation's length, which is the reader having asked to see the conversation. Whether the screen opens at one message or several is that screen's decision and is revisited there; what this record fixes is that the flag follows what was drawn.
...
**And it would be personal data with no purpose left to justify it.** A per-user record of which message was read at which moment is a behavioural profile of somebody's correspondence, retained for as long as the mail is. Where the flag *is* written, the mailbox already holds that fact and MailFathom stores none of it; where it is not, storing it would be collecting something the user has just declined.

### The choice is the user's, and it is one value

**It is one user-level setting, held as typed content of the user settings document.** [ADR 0002](0002-configuration-reading-mapping-and-reload-boundary.md) settled that a user's settings are typed content of that document rather than a configuration layer, and this is a user's setting in the plainest sense: it describes how *this person* reads mail, in a surface only they sign into, across every account they own. It is not an operator's key (E1), because an operator deciding whether somebody's mail client marks mail read is deciding something about that person's reading rather than about the deployment. It is not a device setting (E3), because read state is exactly what must not fragment per head: a phone that marks read and a desktop that does not is the divergence problem reintroduced inside one product, and `frontend/src/AGENTS.md` forbids the branch that would express it anyway.

**It covers every account that user reads, deliberately.** A per-account switch would be right if the accounts differed in kind, and the case where they do is a shared mailbox whose unread state is how a team tracks work. That case is thin here — [ADR 0014](0014-single-tenant-multi-user-ownership-on-the-mail-account.md) hangs ownership on the mail account and a shared mailbox is one user's account like any other — and it is the stated criterion for revisiting: an operator serving a mailbox where unread is somebody else's state is the report that moves this value onto the account.

**The setting governs opening, and nothing else.** A user who turns it off keeps every deliberate marking the client offers — marking a message read without opening it, and marking one unread again — because what they declined is reading being a write, not the ability to say a message is read. Reading it as a switch over the whole capability would leave somebody unable to clear an unread count they can clear from any other client they own.

**The default is on, for the reason ADR 0007 gave the reversible actions.** That record defaults `MarkAsFlagged` and `WriteKeywords` to `true` because they are undone from any mail client, and keeps `Delete` the one action every account opts into. Marking read is in the first group by every measure: it destroys nothing, displaces nothing, and is undone with one gesture in any client the user has. A default of off (**A4**) would additionally make the application wrong out of the box in the way people notice fastest, which is the opposite of the easy first run this project asks of its defaults.

### What an operator can still withhold, and what they cannot

**The lever is the permission, not a new key.** `mailfathom.mail.flags.write` is the grant under [ADR 0012](0012-authorization-model-named-permissions-and-where-they-are-enforced.md) that reaches the user's mail server, and [it does not follow from `mailfathom.mail.read`](../operations/permissions.md#the-published-set). A client credential granted reading and not that name signs in, reads mail, and marks nothing: the client discovers this from the `permissions` list [the sign-in route already returns](../operations/client-endpoint.md), offers no marking, and shows the remote flag alone — which is the same behaviour as the user having turned the setting off, reached from the other side. So a deployment that wants a client incapable of touching a mailbox has that today and needs nothing added.
...
`backend/AGENTS.md` keeps its guarantee and gains the author it was missing. *Synchronization and content retrieval must never set the remote IMAP `\Seen` flag* is exactly as true after this decision as before it — no read path acquires a write, and the route that serves a body still reaches no mail server. What the sentence listed was three authors, all of them acting for the user in their absence; the fourth is the user themselves, present, opening their own mail in the client. The bullet now names it and points here.
...
- Good, because the client agrees with every other mail client the user uses, which is the one thing about read state a person notices and the one thing they cannot be argued out of.
...
- Neutral, because the user's choice is one value across their accounts, which is right until somebody serves a shared mailbox in this client and is the stated trigger for moving it.
...
- Bad, because *show earlier messages* marks a conversation's whole history read for one gesture, however long it is. What bounds it is that nothing but that gesture draws more than the latest message, so the reader chose it knowing the count the control names; it is reversible, and it is the price of the trigger being one rule rather than two.
...
- A unit test over the conversation asserts that a conversation drawn with its history hidden submits one marking and no more, and that showing the history submits one for each message it draws.
- A unit test over the user setting asserts that the default is on for a user who has written nothing, and that turning it off leaves the client rendering the remote flag with no second state consulted.
...
- Good, because there is no setting, no user document field, and no branch in the reading pane.
...
- Bad, because it removes the user from a decision about their own mailbox, which is the opposite of what every other permitted mutation in ADR 0007 does.

### A4 — off by default, the user may turn it on
...
- Good, because the user who turned the write off still gets a client that remembers what they have read, which is what they would probably say they wanted.
- Neutral, because the storage itself is small — a row per message per user, or a bit on the row.
...
- Bad, because it stores a behavioural record of somebody's reading that the mailbox is not holding, retained as long as the mail — collected at the moment the user declined to have it written down anywhere.
...
- Bad, because the write becomes unavoidable rather than a choice: a user who turned marking off would need the read route to branch on their setting, so the read path would carry the setting as well as the write.
...
- Bad, because it misapplies ADR 0013's confirmation discipline, which exists for acts that reach a stranger and cannot be recalled. This one reaches the user alone and is undone with one gesture.
...
- Bad, because the trigger stops being *the body was drawn* and becomes *the body was drawn for a reason of this kind*, so one screen has two markings rules and a reader is told that the messages they just read are read, unread, and unread.
...
- Good, because it needs no schema, no user document field, and no round trip.
...
- Bad, because it is lost with the device and invisible to the user from anywhere else, so *why is my mail being marked read* has an answer only on the machine causing it.
...
- [ADR 0002](0002-configuration-reading-mapping-and-reload-boundary.md) is why a user's setting is typed content of the user settings document rather than a configuration layer.
...
- Revisit this record if a deployment serves a mailbox in the client whose unread state belongs to somebody other than the user, which is the case that moves the setting from the user onto the account.
