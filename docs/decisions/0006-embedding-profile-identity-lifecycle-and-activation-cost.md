...
- Changing model must not be **an outage**. The guarantee is that a new generation is built in the background and the old vectors keep serving until it is complete.
...
pgvector 0.8.2 stores a `vector` of up to 16000 dimensions but indexes only 2000 of them with HNSW; `halfvec` raises the indexable ceiling to 4000 at half precision. So a model is not merely large or small — it is indexable, or it is stored and searched exactly, which is correct but slower.
...
- The column shape and the chunk coordinates this builds on were settled before this record. That the embedding model is a configuration value is upheld here, and narrowed: a configuration value declares the model, and an activation is what makes it the one vectors are produced under.
