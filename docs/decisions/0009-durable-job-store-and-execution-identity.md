...
- **A licence obligation is inherited by everyone who runs MailFathom, not only by the person who added the package.** The product is self-hosted and redistributed as an image operators copy into their own registries, and it has to stay distributable under commercial closed-source terms beside its own open-source ones. A dependency whose terms are copyleft or commercial does not stop at this repository.
...
- Bad, because a per-organization subscription cannot be passed to the people who self-host the product. A revenue-based free tier does not fix it: MailFathom would still be requiring each operator to qualify for another vendor's programme in order to run it.
...
- [ADR 0031](0031-dividing-singleton-work-between-replicas-with-a-leased-scope.md) extends this record rather than replacing it. It carries the lease shape above to work nothing enqueued — an account's synchronization, a derived-data sweep, a stored-content move — restates the ordering rule in the form an open-ended loop needs, and answers the revisit trigger below: a deployment-wide bound was asked for, and the answer is a shared spend ledger and a shared rate while job concurrency stays per process.
- [ADR 0032](0032-reaching-a-client-from-any-replica-over-websockets-and-a-resp-backplane.md) makes the one exception to the refusal of a message broker stated above. It adds an optional RESP pub/sub backplane that carries client signals between replicas, because no first-party SignalR fan-out runs over PostgreSQL. That backplane coordinates no work and holds nothing any part of MailFathom reads back, and the refusal stands for everything this record governs.
- Revisit when the volume makes a single claim query a measured bottleneck rather than a suspected one, or when what an operator needs to see outgrows what MailFathom is willing to build — a dashboard is the one thing a library here would have given away, and wanting it back is a legitimate reason to reopen this. A condition this line once carried has already fired and is not among them any more: a deployment-wide concurrency bound was asked for, and ADR 0031 above answered it by leaving job concurrency per process.
