...
<!-- describes: backend/src/AI/Chunking/**, backend/src/Application/Emails/AttachmentText/**, backend/src/Application/Emails/Chunking/**, backend/src/Application/Emails/Extraction/**, backend/src/Host/Configuration/Embeddings/** -->
...
That implication is load-bearing and is worth deciding rather than inheriting, because an attachment is frequently where the substance of a message is. A contract, an invoice, a scanned letter, an exported table: a mailbox user asking what was agreed about the roof is usually asking about a PDF, and a semantic search answering only from covering notes reads as broken rather than as bounded.
...
- Bad, because semantic search answers from covering notes while the substance sits in a file nobody can find, which reads to a mailbox user as the feature not working.
...
- **A document attachment's text joins the lexical index, as a document of its own.** This amends what this record first stated. The earlier text left the `tsvector` untouched and named the resulting asymmetry a stated consequence; implementing the decision showed it to be the wrong half of it, because a mailbox user searching for a word that occurs only in a contract is asking a lexical question and gets nothing back. What joins the index is a second document per attachment rather than more text on the message's own — `email_attachment_texts` carries the vector beside the text it is built from, and `email_search_documents` is byte for byte what it was. That separation is the whole of what the asymmetry was protecting: a message's own headline still quotes only what the message said, so an attachment never puts words into a snippet as though the covering note had carried them, and a lexical match on an attachment names the file and the coordinate the citation resolves through. **An image description is embedded and never lexically indexed**, which [ADR 0030](0030-describing-an-image-attachment-in-words-and-ranking-a-depicted-match-below-a-written-one.md) decides and the database enforces rather than a writer: the generated column produces a vector for a document's text and nothing at all for a description, so a word a model chose cannot be matched as though somebody had written it.
- Revisit this decision if optical character recognition becomes worth its cost and its accuracy floor, which is the largest gap it leaves; or if a retrieval evaluation shows attachment passages crowding out body passages badly enough that ranking needs to know the difference beyond citing it.
