...
### Amendment 2: a configuration change is announced over the backplane

*2026-09-13. Issue 1957.*

The section above limited the backplane to client signals and made any other use a decision of its own. Issue 1957 is that decision, and the owner took it on 2026-09-13: **one exception, and only one.**

**A committed configuration change is announced over the backplane.** A write to the deployment's persisted document, or to a user's record, republishes it in the process that committed it and in no other, so every other replica went on serving what it bound when it started. Where a deployment declares a backplane, the committing replica now publishes one message on one channel beneath the deployment's own prefix, and every replica that hears it compares what it bound with the versions PostgreSQL holds and republishes whatever is newer through the same path a write republishes by.

**It is held to the boundary above in every other respect.**

- **It carries nothing.** The message is empty. No setting, no user identifier, and no version crosses the endpoint, so nothing about a deployment's configuration or its users can be read there, and a message published by anything else able to reach the endpoint costs every replica one round of reads rather than a change.
- **It coordinates no work, and nothing reads back from it.** The rows are the only source a replica takes its answer from; the message says only when to look.
- **It is not the guarantee.** Every replica compares the stored versions on a thirty-second interval whether or not anything was announced, so a replica that missed the message across a gap in the backplane, and every replica of a deployment that declares none, still converges within that interval. The announcement shortens the wait, exactly as a client signal shortens a client's wait for its own re-read.

**What it costs a deployment.** Every replica of a deployment that declares a backplane opens a connection to it, including a replica serving no client surface, because several replicas answering agents alone read one set of settings as much as several serving a client do. That connection is separate from the hub's, and losing the endpoint on either is reported by the same transition counter.
