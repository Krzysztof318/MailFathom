...
- **A dependency is inherited by everyone who runs MailFathom.** The product is self-hosted and redistributed as an image operators copy into their own registries, and it has to stay distributable under commercial closed-source terms beside its own open-source ones, so an SDK taken here is an SDK taken on their behalf.
