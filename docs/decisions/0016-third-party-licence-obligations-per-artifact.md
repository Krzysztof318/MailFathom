...
The first application of this record was LibVLCSharp 3.7.0, reached transitively by the Uno Platform client's Skia desktop head and excluded from that head's publish, so it was resolved and undistributed rather than shipped under LGPL section 6 — the state this record calls latent. Issue [#1091](https://github.com/Krzysztof318/MailFathom/issues/1091) holds the evidence that verdict was read against, and [#1222](https://github.com/Krzysztof318/MailFathom/issues/1222) the evidence that established the exclusion. The client stack was withdrawn afterwards and the component left `THIRD_PARTY_LICENSES.md` with it, so the register carries no latent row today; the rules below are what the next one is written against.
