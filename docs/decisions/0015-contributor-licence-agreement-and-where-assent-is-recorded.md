...
Acceptance is one comment on the pull request the contributor already opened, from the account that authored it. The `Contributor licence` workflow reads the comment, appends an entry to `signatures.json` on the `cla-signatures` branch, and publishes a `license/cla` commit status. Two kinds of author are passed without being asked, on two different fields and for two different reasons. An `author_association` of `OWNER`, `COLLABORATOR`, or `MEMBER` is an account that already holds write access here, the third of which arises only if this repository ever moves under an organisation. A `user.type` of `Bot` is not an account at all in the sense the agreement needs: a bot's output is not a work of authorship anybody could grant rights in, so there is nothing for it to grant and nobody to ask.
...
**Concurrent acceptances are resolved by compare-and-swap rather than by serialising the workflow.** Two pull requests can accept in the same minute, and each run reads the register before it writes one. The ref is updated with `force=false`, so a register that moved under a run produces a rejected call rather than a lost signature, and the run reads again — five bounded attempts with jittered backoff. That rejection is the only one retried: the call's output is kept and its status classified, so a refusal the register will never accept — a permission the App does not hold, a ruleset that does not admit it — stops at the first attempt carrying the reason, rather than exhausting the budget on a failure no retry addresses. A GitHub Actions concurrency group was rejected for the opposite reason it looks right for: with `cancel-in-progress: false` the platform keeps one run queued and cancels older pending ones, which would silently discard an acceptance.
