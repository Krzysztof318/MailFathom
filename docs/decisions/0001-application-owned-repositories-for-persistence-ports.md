...
Therefore a write repository must obtain its persistence context *through* the supplied session, and must fail loudly when handed a session it cannot write through. In MailFathom, `EfCorePersistenceSessionAccessor.JoinAsync` performs that resolution and write repositories consequently do not inject `DbContext` at all.

Joining is also what opens the session's transaction. `BeginSessionAsync` opens none, so a session covers the writes that joined it and nothing that ran before the first of them — which is how a caller that must finish work outside a transaction, such as handing an object to a remote content endpoint, does so without the transaction being removed from the writes that depend on it. Those writes are real: a session issues set-based statements and a row lock beside the changes it stages, and only the transaction makes the two one fact.
...
- An attempt that met a database failure which can clear on its own is replayed by the same loop, wherever before the commit round trip it was raised: by a statement a write issued while staging, or by the save. The unit of work is the only thing that can be repeated — a dropped connection takes its transaction with it, so nothing below this can repeat a statement — and the body a caller already declared safe to repeat is exactly that unit. The session classifies the provider failure, because that is where the provider's exception is, and either raises `PersistenceTransientFailureException` from the commit or reports the classification through `TryEndOnTransientFailure` for a failure that never reached one; the policy acts on either, because that is where the staging body is.
- A connection lost during the commit round trip itself is not replayed. The server may have made the write durable and the client will never hear which, so staging the unit of work again would apply an accumulating write twice; the session raises `PersistenceCommitOutcomeUnknownException` and the policy lets it through to the caller, which is the only layer that knows whether its own write may be offered a second time. This is the same distinction the outgoing send already draws for a connection lost after the message data.
- The attempt count is bounded; exhaustion throws `PersistenceConcurrencyConflictException`, or `PersistenceTransientFailureException` where the last attempt met a transient failure, rather than converting the operation to last-writer-wins.
...
- Cancellation, shutdown, or a failure that is neither a conflict nor transient stops the loop immediately.
...
- This ADR refines the boundary rule root `AGENTS.md` states, that `Application` owns ports and `Infrastructure` owns persistence, EF Core, PostgreSQL, `bytea`, pgvector, and provider-specific mapping details.
