...
- **A key the document does not carry is inherited from below.** The document is sparse, and the merge is the .NET provider merge and no other: objects compose by child key and an array element overrides by its own numeric index — written as an object keyed by that index, `{ "Rules": { "1": … } }`, because the parser renumbers a JSON array's elements from `0`. No second merge vocabulary is introduced, and no whole-array replacement rule.
...
- **The document carries no secret material under the default interpretation**, for the reason ordinary configuration does not: under `ReferenceOnly` a secret-bearing setting holds a reference whichever source supplied it, and [ADR 0005](0005-data-encryption-key-ring-and-provisioning.md) owns the sealing of anything stored beneath this one. Under `ReferenceOrInline` or `InlineOnly` a configured value is the material, and this layer inspects nothing beyond the refused bootstrap keys — so a deployment on an inline mode persists material into an unsealed column exactly as it writes material into a file. That is the interpretation's cost rather than this layer's, and it is one more reason `ReferenceOnly` is the default.
