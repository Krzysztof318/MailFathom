...
An image attachment becomes searchable by being put into ordinary text — a transcription of a scanned page, a caption, an account of what a chart shows — by a chat provider reached exactly the way every other chat call in this system is reached: the declared endpoint chain, `ProviderChatModelClient`, the resilience pipeline, and the ceilings issue 1561 bounds. What reading it costs is one chat call per image, once, and what it produces is text. The model first decides which of two things the picture is — a document to transcribe, such as a scanned invoice, a receipt, a handwritten note, or a form filled in by hand, or a picture to describe, such as a photograph, a bare logo, or a chart — and its answer opens with a line naming which, so the rest of the pipeline knows whose words it holds. From that point nothing in the pipeline knows an image was involved except the one column the ranking rule below needs: the description is chunked by the same chunker under the same boundary rules, embedded under the same profile, stored in the same table, and compared with the same distance operator against the same query vector.
...
A transcription is not a description. Its words were written by somebody and the model only read them out, so it is stored as a document's text and joins both indexes exactly as a parsed document does — this record's amendment of 2026-09-24 ([#2205](https://github.com/Krzysztof318/MailFathom/issues/2205)), which first stated every image reading as a description. What follows concerns a description alone.
...
- the **written ranking** — messages ordered by their nearest passage derived from something a person wrote: message text, a document attachment's own text where ADR 0029's switch admits it, and a picture of a document the model transcribed — so a scanned invoice ranks as the words on it rather than below a paragraph that mentions it;
- the **depicted ranking** — messages ordered by their nearest passage derived from an image description.

The written ranking is what enters reciprocal rank fusion with the lexical ranking, exactly as the single semantic ranking does today, so nothing about fusion changes and no constant is introduced. The depicted ranking is then reduced to the messages the fused result does not already carry, and appended after it whole.

**The two sections share one depth, and it is the depth the surface already had.** Where a caller states a limit, as `search_emails` does, that limit is applied to the concatenation. Where the depth is fixed rather than requested — `MailSearchBrowser` always ranks to `RankedSearchList.MaximumRankedDepth` and the page size reaches only the walk, never the ranking — the concatenation is cut to that same bound, so the depicted section occupies what the written results leave of it and nothing more. Giving the tail a depth of its own would publish a list past `MaximumRankedDepth`, and that number is not a page size to extend: its own remarks call it a bound on how much of a mailbox one query can walk out of the deployment, which is an egress limit rather than an ergonomic one.
...
The same ordering governs every surface, and today that is not one place. `ReciprocalRankFusion.Fuse` has two callers: `MailboxSearchReader`, which `search_emails` is handed directly and which `ask_mail`'s retrieval reaches through `MailboxKnowledgeSearch` — a wrapper that sits beside the tool rather than under it, over the same reader — and `MailSearchBrowser`, which `/api/client`'s search route reaches directly and which reads both rankings and fuses them itself. So a floor written where fusion happens would be written twice.
...
Monotonicity is not the only thing the boundary demands, and the second constraint is the one an obvious implementation violates. `RankedSearchCursor.After` refuses a score that is not finite or is negative, on the stated ground that "every ranking here scores finitely and never below zero" and a value outside that "says the boundary was composed rather than ranked" — and `TryReadScore` refuses the same on decode. A fused score is a small positive sum of reciprocals, so subtracting to land beneath it yields a negative number and the client route would throw on any page ending in the depicted section: the mapping written to keep the cursor working would be what breaks it. **The mapped value is therefore finite and strictly between zero and the least fused score**, which is always available because a fused score is positive. Where the fused result is empty the depicted section is the whole list and carries its reciprocal ranks unmapped, those being positive and descending already.
...
### A scanned page is transcribed, and that is not the recognition ADR 0029 refused

This has to be said outright rather than left to be inferred from the word "transcription" above, because ADR 0029 refused an optical-character-recognition engine by name and a reader is owed the boundary.

A scanned page that arrives *as an image attachment* is read by the chat provider, and the model transcribes it rather than describing it. So this record closes the gap ADR 0029 named as its own worst consequence — the scanned letter, which is exactly the document a person most expects to be found. The amendment of 2026-09-24 carries the same reading one step inward: where this switch is on, a PDF page with no text layer, and the pictures a Word document or a presentation carries, are read the same way, bounded by a count per document, and ADR 0029's decision 4 records that half. What stays refused is what ADR 0029 actually refused: a recognition dependency, with its own licence review and its own price per page.
...
The third ground does reach it, and this record answers it rather than passing over it. ADR 0029 refused recognition partly on "an accuracy floor that would put invented words into a vector space presented as the mailbox's meaning", and a chat model transcribing a page can misread it exactly as a recognition engine can. The amendment weighed that against what the floor was costing: a scanned invoice ranked below every message that merely mentioned an invoice, which is the wrong way round for the document a person is looking for. A transcription is therefore ranked as written text, and what contains a misreading is no longer the floor. The transcription is stored as readable text, so a wrong word is visible to the person reading the result rather than buried in a vector nobody can inspect; the model is told to transcribe exactly what the page says and to say where a word is illegible rather than invent one; and the evaluation suite holds it to the characters of degraded, handwritten, and mixed scans. A description, which is a machine's vocabulary rather than anybody's words, stays under the floor.
...
- Bad, because a depicted result is only visible where the written results leave room, and how much room that is differs by surface. On `search_emails` it is whatever the caller's limit leaves, which for a small limit is often nothing. On `/api/client` it is whatever the written results leave of `MaximumRankedDepth`, so a person can page to the depicted section whenever fewer than two hundred messages matched — more reachable than on the tool, and still never past the deployment's own walk bound. That is what a strict floor means rather than an oversight, and a caller-set filter raising depicted results explicitly is the first thing to revisit if it proves too blunt; it is deliberately not built now.
...
- A unit test proves that every mapped depicted score is finite and above zero, so `RankedSearchCursor.After` mints a boundary on a depicted result rather than refusing one, including where the fused result was empty.
- A unit test proves that the concatenation on a fixed-depth ranking never exceeds `RankedSearchList.MaximumRankedDepth`, and that a fused result filling that depth leaves no depicted section rather than extending the list.
