...
- [0018: Keep the client's sign-in credential only where the operating system holds a secret for one user, persist nothing on the browser head, and store the owner's password rather than anything derived from it](0018-where-the-client-keeps-its-sign-in-credential.md)
