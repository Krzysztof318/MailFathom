...
- [0032: Serve the signal hub over WebSockets alone with negotiation skipped, redeem its ticket from PostgreSQL on any replica, carry signals between replicas over an optional RESP backplane with Garnet as the deployed default, and make the client's own re-read the guarantee a signal is not](0032-reaching-a-client-from-any-replica-over-websockets-and-a-resp-backplane.md)
- [0033: Keep a signed-in client's session in one PostgreSQL table every replica reads, verify it with one indexed read and no cache, refuse an unreachable store rather than signing anybody out, and let a foreign key and a share lock on the credential row carry the revocation the mint barrier carried](0033-where-a-signed-in-session-lives-so-every-replica-accepts-it.md)
