...
- The release workflow refuses to publish when `CHANGELOG.md` carries no section whose heading matches the tag, or when that section is empty. This is the only mechanical check on the changelog, deliberately: a rule requiring every pull request touching `backend/src/` to touch the file would be satisfied by filler.
