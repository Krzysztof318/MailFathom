...
The ordering works out without further rules. SemVer compares alphanumeric prerelease identifiers in ASCII order, so `0.2.0-nightly.41` precedes `0.2.0-rc.1`, which precedes `0.2.0`. The nightly stream is therefore a preview of the release it will become, and no nightly identifier can ever sort above a release.
...
An annotated tag `v<major>.<minor>.<patch>` pushed on a commit reachable from `main`. It is preceded by one reviewed pull request and followed by one generated one:
...
Two moving tags exist in each registry and are the only mutable references: `latest` follows the highest stable release and never a prerelease, and `nightly` follows the newest nightly. Every other tag is immutable.

The `io.mailfathom.release-channel` label is carried on every image and states `release` or `nightly` explicitly, so the channel survives a reference that has been rewritten, re-tagged, or reduced to a digest.
...
- Its release trigger is written as "an accepted version bump reaches the protected default branch", deferring to whatever this issue decided. The answer is the tag, so that clause becomes an annotated `v<x.y.z>` tag on a commit reachable from `main`.

There is a consequence for the deployment assets, which currently encode the old premise as enforcement. `deploy/compose/compose.nightly.yaml` hard-codes `ghcr.io` for its nightly image, and the chart's values schema forces `ghcr.io` on the nightly channel and rejects any other registry. Under this decision those checks are wrong in both directions: they would refuse a nightly pulled from Docker Hub and accept a release pulled from GHCR. Both move to the check the identifier already supports — the nightly channel requires a `-nightly.` reference and the release channel rejects one — which is a stricter guard than the hostname it replaces. The acknowledgement gates in front of the nightly channel are untouched; nothing about reaching a nightly gets easier.
...
- moves the nightly guard in `deploy/compose/compose.nightly.yaml` and in the chart's values schema off the registry hostname and onto the prerelease identifier;
...
- adds the release and nightly workflows that assert the prefix, the ordering, and the immutability before publishing anything.
...
- The Helm chart's existing drift check — it compares `image.tag` against `Chart.appVersion` on the release channel and refuses an install where the two disagree — activates on its own the moment a real `appVersion` replaces `0.0.0-unreleased`. Nothing has to be written for it; stamping the version is what switches it on.
