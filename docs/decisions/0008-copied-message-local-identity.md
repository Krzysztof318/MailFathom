...
- [ADR 0034](0034-holding-a-mailbox-mailfathom-alone-keeps.md) keeps this record's decision that a copy is a second stored message, and replaces its second clause for every account: the row's own identifier is the identity of a stored message, and the occurrence is an optional attribute of it.
