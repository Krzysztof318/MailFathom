...
**That outcome is not a fourth arrangement, and it needs no new sentence.** It is `untilTheClientCloses` — the credential held for the run and no longer, which is exactly what `CredentialLifetime` already publishes and what the sign-in screen already says for a desktop machine offering no keychain. So the store reports that value and the screen renders the sentence it already has, and what the person is told is what 0023 requires them to be told: that they will be asked again, and why. `untilTheTabCloses` is the one arrangement Android never reports, because it is a browser's sentence and the page's storage is the store this amendment refuses.
