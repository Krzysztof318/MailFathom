...
- **A debug build, signed with the debug certificate.** Those are two properties rather than one, and the record fixes both, because Tauri signs a debug build automatically and signs a release build only from a keystore and a signing configuration — and no release signing key, keystore, `keystore.properties`, or store credential exists in this repository or in its secrets, nor is one created for this. Taking the debug build rather than a release build signed with a debug certificate follows from that: the release path needs the keystore this record refuses.

  **A debug build is `android:debuggable`, and that has a consequence worth stating rather than discovering.** On a device with USB debugging enabled, `run-as` gives a shell running as the application's own user, which reads its private data directory and exercises the Keystore keys held for it. So on this artifact the credential is protected from *other applications* and from a stolen device's file system, and is not protected from somebody with the device unlocked, developer options on, and a cable. That is acceptable precisely because this is a build somebody tries rather than a client they live in; a supported head would be a release build, which is why the checklist below carries the signing identity.
- **Built from the same source tree as the other two heads**, over the same bundle, through `frontend/src-tauri/`. There is no Android-only *application* source and no Android-only screen: what is Android's is inside the shell, under `frontend/src-tauri/`, where the credential store's third implementation lands and where the rule already permits a difference between the heads to live.
...
### The no-platform-branch rule holds, and the shell is the only seam a head operation crosses

Adding a third head changes nothing about the rule in `frontend/src/AGENTS.md` § *The two heads*. No component, hook, or screen asks which head it is running on, and no module is chosen by target. That rule permits two kinds of difference and this section keeps both of them where it found them: a CSS one, and a shell concern.

- **Every composition comes from the questions a screen may ask about itself** — the width it has been given, what the pointer can do, and what the viewport says its own safe area is. The bottom navigation, the single pane, the drawers, the touch sizing, and the swipe on a message row are all answers to the first two, which is why a narrow desktop window gets them too and why nothing about them is Android's.
- **`env(safe-area-inset-*)` is the third, and it is the CSS seam `frontend/src/AGENTS.md` § *The two heads* permits beside the shell one.** That rule allows two kinds of head difference — a CSS one, naming the safe-area inset and the pointer or hover query, or a shell concern — and this record's *only seam* claim is about the second kind: shell **operations** go through `shellOperations/` and nowhere else. A safe-area inset is not a branch at all and needs no permission from that claim, because the code asks nothing about which head it is on; the viewport supplies a value and the stylesheet reads it, and it reads zero where there is nothing to avoid. It matters here because an Android device with a gesture bar and a cutout is where those values stop being zero, and ADR 0021 found exactly that: the platform-shaped part of its proof of concept was two CSS lines and `env(safe-area-inset-bottom)`. [#1616](https://github.com/Krzysztof318/MailFathom/issues/1616) is the change that adds them, and it is read against this paragraph rather than against a record that says the seam does not exist.
...
[ADR 0023](0023-where-the-client-keeps-the-credential-it-signs-in-with.md) decided this question against a desktop keychain and a browser page. Most of what it decided is about the credential rather than about a head, and all of that holds here unchanged: what is kept is the session the deployment minted, as one entry, and nothing beyond the fields that entry holds; it is bound to the address it was given for and discarded where that address changed; sign-out, an `unauthenticated` refusal, and a changed address are what clear it, and an `unavailable` read does not; nothing outside `Client.Backend` composes or inspects it; and the sign-in screen says which arrangement a person is getting, in a sentence, chosen by a value the store reported rather than by a platform. There are three of those arrangements rather than two, which is what `CredentialLifetime` in `Client.App/src/signIn/credentialStore.ts` already publishes — kept until signed out, until the tab closes, or until the client closes — each with its own sentence on the screen. This record adds one sentence to that set and no new rule: the fallback below says which.

Two parts of it were written for two heads and are amended here.

**The mechanism.** 0023 says the keychain is reached *through the `keyring` crate*, which is the desktop answer rather than the general one — the crate has no Android backend. The Android shell answers the same three operations — keep, read back, forget — with the key material in **the Android Keystore**, which is the platform's own protected storage: the operating system holds the key, backs it by hardware where the device has it, and hands it to no other application.

The shape that reaches it is **`EncryptedSharedPreferences`, which is not a platform API** — it is `androidx.security:security-crypto`, a Jetpack library layered over the Keystore through Tink, Apache-2.0, and one whose every API Google deprecated at 1.1.0-beta01 in favour of platform APIs and direct Keystore use. Two things follow and neither is left implicit. **It is a third-party component this decision introduces**, not one the Android toolchain brings along, so it takes a `THIRD_PARTY_LICENSES.md` row and an [ADR 0016](0016-third-party-licence-obligations-per-artifact.md) review of its own against the APK as the artifact, and the register is where its exact version is recorded. And **the durable half of this decision is the Keystore rather than the wrapper**: the library is where the credential goes today because it is the shortest correct route to that store, and replacing it with direct Keystore use — which is what its deprecation notice points at — changes the implementation of an operation the application already declares and changes nothing decided here. [#1619](https://github.com/Krzysztof318/MailFathom/issues/1619) is where that choice is made against the library's state at the time it is built.

Either way it is a second implementation of an operation the application already declares, so it lands inside the seam above and nothing over that seam changes.

**The fallback.** This is where the amendment actually bites, and it is the reason a record was needed rather than a note. 0023's rule is that a shell offering no keychain keeps the credential for the run in `sessionStorage`, and it argues that outcome carefully for a browser tab and for a Linux machine with no Secret Service. Applied to Android unread, that same rule resolves a phone to the page's own storage — and a phone kills background applications constantly, so it is a password typed several times a day. That is not a stated outcome anybody chose; it is what a rule written for two heads does when a third arrives. **On the Android head, protected storage that cannot be reached — or a key the operating system has invalidated, as a screen-lock change does — keeps nothing and asks the person to sign in again.** It never falls back to the page's own storage.

**What is decided is the outcome rather than the member that names it: the credential is held for the run and no longer.** That is the same duration `untilTheClientCloses` already publishes, and whether the store reports that member or one of its own follows from the shape below rather than from this paragraph. What the record does fix is the other end: `untilTheTabCloses` is the one value Android never reports, because it is a browser's sentence and the page's storage is the store this amendment refuses.

**The sentence, though, is Android's own, and this head owes a catalogue entry rather than reusing the desktop one.** `signIn.keptUntilTheClientCloses` is wrong here in both halves. Its reason — that this machine offers no keychain to keep a sign-in in safely — is false on a device that offers exactly that and could not be reached, or whose key a screen-lock change invalidated; and its retention overstates, because a phone kills a background process constantly, so nothing is held until the person closes anything. ADR 0023 § *What the person is told* makes the *why* the load-bearing half and gives each case a reason of its own, which is the rule this follows rather than an exception to it: the entry says that nothing is kept, that they will be asked again the next time the client starts, and which of the two reasons it was. `signIn.notKept` is not that entry either — it is the notice for a `keep` that answered `false` at the moment somebody signed in, a different moment from the sentence read before anybody types.

How that entry is selected is [#1619](https://github.com/Krzysztof318/MailFathom/issues/1619)'s to shape and is not free: `lifetimeMessages` in `Client.App/src/signIn/SignIn.tsx` keys the sentence off `CredentialLifetime` alone, so a fourth sentence is either a fourth member of that type — reported in place of `untilTheClientCloses`, for the same duration — or a change to how the screen chooses. Both satisfy what is decided above, because what is decided is the duration and the sentence rather than the member; what this record fixes is that a person is not told something false about their own device.

One constraint is added that 0023 had no reason to state, because neither an operating-system keychain nor a browser tab is copied off the machine by default. Android's is: automatic backup is on unless an application turns it off, and it copies what the application wrote under its own data directory to a vendor's cloud. A credential synchronised there is a copy of a live credential in a place nobody chose, and one that renews has an expiry nobody is there to let run out.

**So the constraint is the head's data rather than the credential entry alone: automatic backup is off for this artifact.** The entry's own exclusion follows from that rather than standing as a special case. The reason it is written this wide is the driver above — a device is a place mail now rests, and the credential is not the only thing the head leaves there. `deployment/adoptedDeployment.ts` and `device/deviceStore.ts` keep the deployment address, the theme, the language, and how the Mail space divides its width in the WebView's `localStorage`; the WebView keeps its own storage and cache for the pages that rendered mail. None of that is promised to survive anything on a nightly — there is no restore story to protect, because there is no update path and no install this artifact carries forward — so an off-device copy of it buys a person nothing and costs them mail metadata somewhere they did not put it.

**Turning it off is two things rather than one, and the second is the one easy to miss.** `android:allowBackup="false"` governs the cloud backup; from API 31 it does not dependably govern device-to-device transfer, and Android's own documentation says that behaviour varies by manufacturer — so a person migrating to a new phone carries the head's state across while the manifest says backup is off. The head therefore also carries an `android:dataExtractionRules` file whose `<device-transfer>` section excludes its data, beside the `<cloud-backup>` section that says the same thing twice. Both halves are the decision; neither alone is.

What stays out of scope is the finer answer a restore needs — which paths are worth carrying and which are not — because a nightly carries nothing forward and has no restore to shape. That belongs to a supported head, beside the privacy statement checklist item 6 already names.

0023 is not superseded by it, and its reasoning is not rewritten: its text stays the record of what was decided for two heads, and this is where a reader goes for the third. The one edit it takes is a pointer to this record, added in the same change — a record whose decision has moved says where it moved, which is what `docs/decisions/README.md` item 6 requires of a supersession and is worth no less for an amendment. Without it, a reader arriving at 0023 from the citation in `Client.App/src/signIn/credentialStore.ts` would read its fallback rule as governing every head, which on Android is the rule this record decides the opposite of.
...
8. **The Android artifact reviewed as an artifact under [ADR 0016](0016-third-party-licence-obligations-per-artifact.md)**, whose unit is `(component, version, artifact)` — the APK is a third one beside the web bundle and the desktop application. Two things reach it: the Android toolchain's own components, and the library this record's credential amendment introduces, which is a row this decision owes rather than one the toolchain brings.
...
- Good, because the credential on the head most likely to be lost or stolen is in the Android Keystore rather than in a page, and is kept off both copies the platform would otherwise take, the cloud backup and the device-to-device transfer — against another application, and against a stolen device's file system. On this artifact it is *not* protected against somebody with the device unlocked and developer options on, because a debug build is `android:debuggable` and `run-as` reaches it; the bullet above says so and a supported head's release build is what closes it.
...
- Neutral, because ADR 0023 keeps its reasoning and its status and gains only a pointer here, so a reader following the credential question now reads two records rather than one.
...
- Bad, because it leaves the credential question unanswered on a head the tree still reaches, so somebody building an APK by hand gets 0023's browser path and a sign-in on every kill, with nothing saying that was decided.
