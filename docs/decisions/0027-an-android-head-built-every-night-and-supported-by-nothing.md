...
**The store reports `untilTheClientCloses` for it** — the credential held for the run and no longer, which is what `CredentialLifetime` already publishes and is the truthful answer to how long it lasts. `untilTheTabCloses` is the one value Android never reports, because it is a browser's sentence and the page's storage is the store this amendment refuses.
...
How that entry is selected is [#1619](https://github.com/Krzysztof318/MailFathom/issues/1619)'s to shape and is not free: `lifetimeMessages` in `Client.App/src/signIn/SignIn.tsx` keys the sentence off `CredentialLifetime` alone, so a fourth sentence is either a fourth member of that type or a change to how the screen chooses. What this record fixes is that a person is not told something false about their own device; which of those two shapes carries it is the implementer's.
