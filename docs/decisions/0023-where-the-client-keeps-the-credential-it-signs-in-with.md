...
The client signs in with HTTP Basic and holds a session afterwards. The password reaches one route — the exchange — which derives a key once and answers with a token, and every request after it presents that token; [#1746](https://github.com/Krzysztof318/MailFathom/issues/1746) is what made it so. What the client holds between restarts is therefore a credential that expires on the deployment's clock, that signing out revokes, and that names one client rather than the person's password. What has not changed is that it holds something: [#1422](https://github.com/Krzysztof318/MailFathom/issues/1422) requires _open it and already be signed in_ as one of its user stories, and [#1419](https://github.com/Krzysztof318/MailFathom/issues/1419) is titled for keeping somebody signed in across restarts.

This record was first written when the client presented a password on every request and kept the finished Basic header between starts. It is amended rather than replaced because the question it answers is unchanged — _where does each head put the credential it holds_ — and both answers survived the exchange: what moved is what a leaked copy is worth. Every paragraph below is written against the session.
...
- **What is kept is a session, not a password.** The token expires on the deployment's own clock, signing out revokes it, and an operator disabling or deleting the credential behind it revokes every session that credential minted. A copy that leaks is therefore worth what is left of one session against one deployment, rather than a password somebody may have reused elsewhere and that nothing in the client could revoke. That is a smaller loss than this record was first written against and it is not a small one: a live token reads that person's whole mailbox for as long as it lasts, and a script holding one can renew it for as long as it keeps holding it. Every storage option is still judged on what reaches the copy.
- **The password is still typed and still travels — once.** `base64(user:password)` reverses in one line, so the exchange is a request carrying a reversible password and is treated as one. What the exchange changed is how many requests that is: one per sign-in rather than one per read. No head keeps it afterwards, because after the exchange no head has it.
...
`ClientSession` carries `authorization` as a finished header value and `Client.Backend` composes none. What is kept is that same string, the instant the deployment said the session stops working, and the name the deployment knows the person by — written as one document under one store entry, so one act writes it and one act removes it.
...
### An OAuth sign-in keeps a grant beside the session, and it is kept where the head keeps things durably

[#2017](https://github.com/Krzysztof318/MailFathom/issues/2017) gave the client a second way to sign in: a deployment may publish authorization servers, and a person who picks one comes back holding a token that server issued rather than one this deployment minted. This section is that amendment, and it is an addition rather than a reversal — every paragraph above goes on describing the session, which is what a password sign-in still produces, and this describes the value beside it.

**What is kept is one document under one entry of its own**, named apart from the session so that neither read can ever answer with the other, and bound to the same deployment address by the same rule. It carries seven fields: the finished `Authorization` header value, the instant the access token stops working, the refresh token, the issuer that issued both, the client identifier the deployment published for that server, the resource the token was issued for, and the name the deployment knows the person by. The four past the header value and the instant are what renewing it needs and what nothing can recompute — a deployment that withdraws a provider must not strand a grant in a state where nothing can even revoke it, so which server to present a refresh token to is kept rather than looked up.

**A grant goes in the durable half alone, whatever the checkbox holds**, and that is not an exemption from what the checkbox states. What the checkbox decides is how long a *password* sign-in lasts on this device, which is a question this client is entitled to answer because the deployment minted that session against a password the person typed here. An OAuth sign-in's life belongs to the authorization server: it ends when that server stops renewing it, when somebody signs out, or when an administrator withdraws the grant. A client that dropped it with the tab would be overruling the server about a decision the deployment does not own, and would ask somebody to authorize again at a server that never stopped saying yes. So the sign-in screen goes on offering the checkbox for the password form it governs, and a provider control is not what it is about.

**An expired access token is not nothing kept**, which is the one place this read differs from the session's and the difference that matters most. An expired session is over and the person signs in again; an expired access token is the ordinary state of a grant nobody used yesterday, and the refresh token beside it is what turns it back into a working one without anybody typing. So the instant is read for *being* an instant and never for being in the future.

**The refresh token is the part worth naming plainly.** It is a longer-lived credential than the session above, it renews itself, and a server that rotates them replaces it on every renewal — so the client presents the one it is holding, takes whichever one comes back, and never presents a rotated token twice. Where it is kept is the head's own durable store: the keychain on the desktop, `localStorage` in a browser, and nothing at all on a head that has neither, which keeps the grant for the run exactly as it keeps a session for the run. The web head's exposure is the one § *`localStorage` on the web head* already states in full, and it is larger here rather than different: a script on the origin that reads a grant can renew it for as long as it keeps reading. What bounds it is the same thing that bounds the session — signing out, an administrator withdrawing the grant at the server — plus one this decision adds: **signing out asks the authorization server to revoke the refresh token**, asked and not waited on, exactly as signing out already tells the deployment about a session.

**Signing out is one act however somebody signed in.** It clears the session, the grant, and everything derived from either, on the head's own terms — the keychain entry deleted rather than emptied — and a grant left behind would sign the next start back in. Every other clause in § *What clears it* holds unchanged, with one reading for the grant: a refused *refresh* is what ends it, because a server refusing the refresh token has said the grant is over in a way no retry undoes, while a server that could not be reached at all is retried on the next tick and clears nothing.

**A token whose subject the deployment maps to no user is not a sign-in.** The client asks the authorization server for no identity token, so the one party that knows what a token means here is the deployment, which answers an empty `401` where the subject belongs to nobody. That is refused on the screen rather than kept, however good the token is — and a deployment that merely could not be reached is not that answer, so the provider's own name is shown and the grant is kept.

### The web head keeps it for the tab, and beyond it only where somebody asked

`sessionStorage`, holding that one value, written by the single module that signs in and read by nothing else in `Client.App` — and `localStorage`, holding the same value under the same name, where the person ticked _Keep me signed in_ on the sign-in screen. A session is in exactly one of the two, so writing it to one removes it from the other, and signing out removes it from both.
...
- **It reaches the web head and the desktop head identically**, because it is a choice about where a session goes rather than a fact about a platform. Ticked is the head's own durable place — the keychain on the desktop, `localStorage` in a browser — and unticked is the tab on both. `frontend/src/AGENTS.md` § _The two heads_ is unaffected: the screen renders one control and one value, and which store is behind it is resolved at the composition root exactly as it was.
- **A head that offers nowhere durable offers no checkbox**, and says why in the sentence it already said. That covers the two arrangements below and the shell that answers _keep it for the run_, which is [ADR 0027](0027-an-android-head-built-every-night-and-supported-by-nothing.md)'s answer for a device that kills the client all day — reading a ticked box there as permission to use the page's own storage would let a screen overrule the shell that knows the device.
- **Nothing else about a kept session moves.** It is the same document with the same three fields, bound to the same address, cleared by the same five things — and the _remember me_ marker this record refused stays refused: what the person ticked is not written down, because which store the session turned up in already answers it.
...
- **A screen renders what the store reported**, in the same way it renders a `ClientFailureReason`: one of two sentences, chosen by a value, not by a platform. That is not a head branch, and a store that answers _this does not outlive the application_ is answered identically whether it is a browser tab or a Linux machine with no keyring.
...
- **Sign-out clears the stored session**, everything held in memory, and everything derived from it. On the desktop head that is the keychain entry deleted, not overwritten with an empty value. It also tells the deployment, which stops the token working before it would have expired — asked and not waited on, because signing out of the client is what the person pressed and a deployment that never heard still expires the token on its own.
- **A renewal replaces it.** The client renews before the session ends, so a client left open stays signed in without anybody typing anything, and what is stored is the newest session rather than a growing list of them.
...
Neither sentence says _your password is kept_, because none is.
...
- Good, because the head that is an application keeps somebody signed in, and the head that is a web page never leaves a renewable credential where a later script can find it.
- Good, because what is stored is a session rather than a password: it expires, signing out revokes it, an operator revokes it by disabling or deleting the credential behind it, and none of those was possible when the stored value was a password.
- Good, because one entry is stored, in one place, bound to one address, and one act removes it — so what the client holds about a person is answerable in a sentence, which is what storage limitation looks like from the outside.
...
- Neutral, because a second kind of credential now sits under this record. What is kept for an OAuth sign-in is a grant rather than a session, and the two are separate documents under separate entries — so the rules above did not have to be widened to cover a value they were not written for, and a read for one can never answer with the other.
- Bad, because a refresh token is a longer-lived credential than anything this record kept before, and on the web head it sits in `localStorage` for every deployment that publishes a provider, rather than only where somebody ticked a box. That is the authorization server's decision rather than this client's to take back, and what answers it is that signing out asks the server to revoke it — which is the one mitigation a client can offer and is weaker than the checkbox, since it needs somebody to press it.
- Bad, because the client now depends on a second party staying reachable to keep somebody signed in. A deployment that is up while its authorization server is not leaves a grant that cannot be renewed, which the client meets as a failure to retry rather than as a sign-out — so somebody sees a client that works until the access token runs out and then asks them to sign in again for a reason neither party can state.
...
- What the amendment adds is proven where the rest of this record is: `oauthGrant.test.ts` holds the read, the rotation, and the refusal that ends a grant; `useGrantRenewal.test.ts` holds that a refresh token is presented once and that a renewal answering after somebody signed out applies nothing; `oauthFlow.test.ts` holds that the verifier is written down and never stated in the address, that an attempt is taken once, and that a token the deployment maps to nobody is refused rather than kept.
...
Refused as the web head's storage and later adopted as the web head's _offer_, which is the amendment above rather than a second reading of this list. Every line below is still what it says; what changed is that the last one stopped being true.
...
- Bad, because it is unlocked by a password, so it answers _where does the password live_ with _behind another password_.
...
- [ADR 0021](0021-client-stack-react-typescript-tailwind-tauri-and-pnpm.md) is what makes this two heads rather than four, and it named _what a session stores client-side_ as one of the privacy questions the stack made reachable rather than answered. This is that answer.
...
- [ADR 0012](0012-authorization-model-named-permissions-and-where-they-are-enforced.md) is why a refused credential and a missing grant are two different outcomes here rather than one: a permission is enforced in the use case as well as at the transport, so a 403 is an answer about what this user may do rather than about who they are.
- [ADR 0027](0027-an-android-head-built-every-night-and-supported-by-nothing.md) amends this record for a third head. Two things below are the desktop's and the web's rather than every head's once Android exists: the keychain reached _through the `keyring` crate_, which has no Android backend, and the rule that a shell offering no keychain keeps the credential for the run in `sessionStorage`, which on Android is decided the other way — protected storage that cannot be reached keeps nothing rather than falling back to the page's own storage, on a sentence of its own rather than this record's. One count above is also superseded, twice over: § _The application asks the shell_ and § _What the person is told_ each say the screen renders one of **two** sentences, and Android made a third. The amendment above then replaced the question those sentences answer — how long what is kept survives — with where a session somebody asked to keep would actually go, which is what `KeptBeyondTheTab` publishes: the device's own store, this browser, the run a shell keeps, protected storage that could not be reached, and a key the device threw away. The first two are where the screen offers the choice and the last three are where it states an outcome, so what a reader should take from this bullet is that the count is the type's rather than either section's. Everything else here holds on all three heads unchanged.
- [#1746](https://github.com/Krzysztof318/MailFathom/issues/1746) is what this record was amended for, and it is the revisit the first version of this paragraph asked for: the client gained a way in that issues something expiring, so what is kept stopped being a password. `docs/operations/client-endpoint.md` is where the exchange, the lifetime, and how a client renews are described for an operator.
- [ADR 0033](0033-where-a-signed-in-session-lives-so-every-replica-accepts-it.md) is the answer to the third revisit trigger this record carried: it decides that the service keeps a signed-in session in PostgreSQL rather than in the process, so that any replica accepts it and any replica honours its revocation. It changes nothing a head keeps and nothing in this record's decision, exactly as that trigger predicted. What it removes is the last consequence above, and it removes it when issue 1900 lands rather than now — so that consequence is stated here as what a deployment does today, and the record to read for what replaces it is ADR 0033.
- [#2017](https://github.com/Krzysztof318/MailFathom/issues/2017) is what this record was amended for the second time, and § *An OAuth sign-in keeps a grant beside the session* is that amendment. `docs/operations/client-endpoint.md` is where the published document, the configuration an operator writes, and the redirect address they register are described for them.
- Revisit this decision again if a supported desktop target appears whose keychain the `keyring` crate does not reach; or if the web head acquires an origin it does not share with the deployment; or if a head gains somewhere durable that an ordinary script on the origin cannot read, which is the one change that would let a refresh token be kept on the web head without the exposure stated above. The third trigger this paragraph carried — the service holding sessions somewhere that survives a restart — has fired, and the bullet above is where it went.
