...
The item is keyed by the deployment address that was signed into, so one deployment's password is never presented to another. **At most one item is held at any moment**, and two things enforce that rather than one, because the address can change without the running process seeing it move:
