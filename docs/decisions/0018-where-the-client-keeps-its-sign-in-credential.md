...
**Signing out clears both halves**, and an entry that cannot be cleared is reported rather than left behind: the profile is genuinely forgotten either way, and only the operator can open a keyring that has since locked.
...
- [`operations/admin-endpoint.md`](https://github.com/Krzysztof318/MailFathom/blob/main/docs/operations/admin-endpoint.md) states where the command keeps a credential on each platform and what the fallback means, for the operator rather than for a reader of this record.
