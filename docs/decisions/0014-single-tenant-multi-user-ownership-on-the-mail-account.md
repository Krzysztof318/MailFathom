...
### Ownership is a column on the account and on nothing else in the mail graph

`mailbox_accounts` gains the user that owns it. No other table of the mail graph gains an owner column. The one record outside that graph does: the contact book is assembled rather than brought in by an account, so it hangs on nothing that could carry an owner and both its tables name one — `contacts` because there is nothing above it to read an owner from, and `contact_addresses` because the reads that would otherwise walk the table lead with the owner in an index and an index cannot reach through the contact to find it. Step 4 of the delivery order below is where that lands.
