...
<!-- describes: src/Application/Access/**, src/Application/Accounts/**, src/Application/Emails/Mailboxes/**, src/Infrastructure/Persistence/Entities/**, src/Infrastructure/Persistence/MailFathomDbContext.cs -->
...
**`mailbox_accounts` is a one-column table.** It holds an identifier and nothing else, created lazily by whichever synchronization run first binds one of the account's folders. Every table holding mail either carries `MailboxAccountId` or reaches one through a foreign key, and `mail_folders` cascades from the account while `stored_emails` cascades from the folder. An owner hung on that row therefore reaches the whole graph without a column on the table that will hold the mail.
...
- **Erasure has to be dischargeable and provable.** Dropping a database discharges a deletion request completely; a shared database has to reach mail, embeddings, and the content store and then show that nothing was left.
...
What stands in its place is that the predicate is written once. `StoredEmailSelectionPredicate.Matching` is the only place a mailbox read is narrowed, both read models compose it, and the tombstone exclusion already leads it precisely so no caller can opt out. The owner narrowing joins it there.
...
The identifier becomes opaque and generated. The display name becomes unique **within a user** rather than across the deployment, and it is what a caller writes: `MailAccountSelector` already resolves an account by its identifier *or* by the name it is published under, so the human-facing half of this survives the change unaltered.

Per ADR 0004 the breaks are named against their surfaces. **MCP tool contract**: `list_accounts` publishes a different identifier and `MailboxScopeArguments` accepts one, so a client holding a configured account name keeps working while a client holding a stored identifier does not. **Configuration schema**: the account identifier is no longer written by an operator, because the account is no longer configured. The operator's action is to re-point any client that stored an identifier rather than a name, and to provision the accounts that were configured.
...
A shared database, so erasure is a cascade rather than a `DROP DATABASE`, and the cascade is the one the model already has: deleting a user's `mailbox_accounts` rows takes their folders, and the folders take their stored mail, and the mail takes its content, its search documents, its chunks, its chunks' embeddings, its classifications and their signals, and its repair requests. Threads, mutations, executions, audit entries, and outgoing mail hang from the account directly.

That is what ownership on the account buys over a discriminator repeated on every table: the deletion is one statement against one table and the graph follows, so the evidence that nothing was left is a property of the foreign keys rather than a checklist somebody keeps complete. Two things stand outside it and are named as work rather than assumed: the contact book, which today is deployment-wide, and any ledger row recording spend a user incurred, which outlives the vectors it paid for by design and is retained as a cost record rather than erased with them.

### The administrative surface gains a deployment administrator

Every authenticated caller on the administrative endpoint may perform every administrative operation. Several users require an administrator distinct from a user — issue 889 calls it a tenant administrator, and since this record refuses tenants it is a **deployment administrator**.
...
- Good, because erasing a user is one delete against `mailbox_accounts` and the foreign keys discharge the rest, which is evidence rather than a checklist.
...
- Bad, because the MCP tool contract and the configuration schema both break, and an operator has to provision what they had configured and re-point any client holding a stored account identifier.
...
- A unit test asserts that every mailbox read model composes `StoredEmailSelectionPredicate.Matching`, which is what makes the owner narrowing a property of the system rather than of each query.
...
- The breaks named against the MCP tool contract and the configuration schema are carried into `CHANGELOG.md` by the release pull request, per ADR 0004.
