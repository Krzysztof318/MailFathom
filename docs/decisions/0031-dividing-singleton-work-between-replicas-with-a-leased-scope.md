...
<!-- describes: backend/src/Host/Hosting/Workers/**, backend/src/Application/AiProviders/**, backend/src/Application/EmailContent/Storage/**, backend/src/Infrastructure/Observability/MailAnsweringSpendTracker.cs, backend/src/Host/Security/ClientAssertions/** -->
...
- The `describes:` marker names the code this decision is *about* rather than the code that implements it, because none of the latter exists: the workers that will take a lease, the pacer whose marker moves, the ceilings that were already shared, the answering tracker that becomes one, and the replay store. It gains the lease's own paths when issue 1289 lands them, which is one of the two edits an accepted record is permitted.
