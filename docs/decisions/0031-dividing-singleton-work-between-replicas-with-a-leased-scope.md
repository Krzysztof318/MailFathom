...
<!-- describes: backend/src/Host/Hosting/Workers/AccountPushNotificationWatch.cs, backend/src/Host/Hosting/Workers/AccountSynchronizationSupervisor.cs, backend/src/Host/Hosting/Workers/MailSynchronizationCoordinator.cs, backend/src/Host/Hosting/Workers/MailEmbeddingBackfillWorker.cs, backend/src/Host/Hosting/Workers/MailExtractionBackfillWorker.cs, backend/src/Host/Hosting/Workers/StoredContentMoveWorker.cs, backend/src/Application/AiProviders/ProviderRequestPacer.cs, backend/src/Application/EmailContent/Storage/StoredContentCeiling.cs, backend/src/Application/Emails/Embeddings/Limits/IEmbeddingSpendLedger.cs, backend/src/Infrastructure/Observability/MailAnsweringSpendTracker.cs, backend/src/Host/Security/ClientAssertions/ClientAssertionReplayStore.cs, backend/src/Host/Security/ClientAssertions/ClientAssertionAuthenticator.cs, backend/src/Host/Security/ClientAssertions/UserClientAssertionAuthenticator.cs, backend/src/Application/Access/Credentials/IClientAssertionSpendStore.cs, backend/src/Infrastructure/Persistence/ClientAssertions/ClientAssertionSpendStore.cs, backend/src/Infrastructure/Persistence/ClientAssertions/Configurations/SpentClientAssertionConfiguration.cs, backend/src/Infrastructure/Persistence/Entities/SpentClientAssertionEntity.cs, backend/src/Application/Coordination/IWorkLeaseStore.cs, backend/src/Application/Coordination/WorkLease.cs, backend/src/Application/Coordination/WorkLeaseHolder.cs, backend/src/Application/Coordination/WorkScope.cs, backend/src/Infrastructure/Persistence/Coordination/WorkLeaseStore.cs, backend/src/Infrastructure/Persistence/Coordination/WorkLeaseStatements.cs, backend/src/Infrastructure/Persistence/Coordination/Configurations/WorkLeaseConfiguration.cs, backend/src/Infrastructure/Persistence/Entities/WorkLeaseEntity.cs, backend/src/Infrastructure/Observability/WorkLeaseTelemetry.cs -->
...
`ClientAssertionReplayStore` was in memory and per process, and it is consulted from the authentication handler on every request that presents a client assertion. Its own remarks recorded that choice and its reasoning, and both were right at one replica.
...
**So the answer is a change to the store**, and issue 1835 is where it was made. The spend is an insert of the verifying credential and the identifier under a unique constraint, where whether a row was inserted is the answer, beside a sweep of entries past the point at which an assertion can no longer be presented. It is one insert on a path that has already verified a signature and is about to serve a request that reaches the database anyway — under a millisecond, measured against PostgreSQL 17 rather than assumed, and unmoved by a hundred thousand rows already in the table. The table's size is bounded by the same two facts the in-memory store rested on, with one correction the implementation forced: only a verified assertion is remembered, and an entry outlives its assertion by at most one further permitted lifetime, because it is dropped by the first sweep past that point rather than at the instant itself. The sweep therefore judges expiry against the instant validation stops accepting an assertion — its own expiry plus the tolerated clock skew — since dropping a record while the assertion carrying it is still served is the replay this exists to refuse.

**It gated the chart.** Issue 1287's acceptance says `replicaCount` is safe to raise; while the record was a process's, raising it silently weakened authentication, so this was a precondition of that claim. Issue 1835 discharged it, outside issue 1287's own children.
...
- Bad, because this record opened a change to an authentication path rather than closing one. The chart's `replicaCount` could not honestly be called safe until the replay store was shared, and that work became issue 1835 rather than a child of issue 1287.
...
- [ADR 0032](0032-reaching-a-client-from-any-replica-over-websockets-and-a-resp-backplane.md) answers the one question this record left open: how a replica reaches something it does not hold, which is a running client's connection. It answers with an optional RESP backplane that carries client signals and coordinates no work, and a signal ticket any replica redeems from PostgreSQL. Every kind of work this record divides stays coordinated through PostgreSQL alone.
- **The replay store is issue 1835.** It is a consequence of the replica count rather than of anything issue 1287's children build, it is a change to an authentication path, and it is a precondition of calling `replicaCount` safe — so it took an issue outside that parent, and that issue landed the table, the port, and the adapter this record asked for.
- The `describes:` marker names the code this decision is *about* rather than the code that implements it, and it gains the implementing paths as they land: the workers that will take a lease, the pacer whose marker moves, the two ceilings that were already shared, the answering tracker that becomes one, and the replay store with the two authenticators that spend into it. It names those files one by one rather than the directories holding them, because `Host/Hosting/Workers/` also holds the job and outbox workers that run under ADR 0009's own lease and are not this decision's subject at all. It gained the lease's own paths with issue 1289 and the replay store's port, adapter, and entity with issue 1835, which is one of the two edits an accepted record is permitted.
