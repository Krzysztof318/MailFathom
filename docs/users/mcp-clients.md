...
on the mail server rather than MailFathom's copy of it, and it advertises itself as destructive too — a keyword
replacement states the whole set, so a label the caller did not list comes off. Where
