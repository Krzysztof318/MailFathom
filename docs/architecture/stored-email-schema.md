...
| `ix_contacts_book_holder_display_name_sort_key_id` | `(BookHolderId, DisplayNameSortKey, Id)` | The one order a contact book is listed in and the one a keyset page continues from. The holder leads it because a page is read over a handful of named books, so each is seeked into rather than the table scanned and narrowed, and the identity settles two people whose names compare equal, which is what makes the order total within a book and the walk terminate |
