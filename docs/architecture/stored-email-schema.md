...
| `DocumentWrittenAtRuntime` | Whether anything has written the document while the deployment was running. A user recorded and never given a mailbox carries the empty object the row was provisioned with, and a user whose document was written and then emptied carries the same octets: the two are different facts and this is what tells them apart. Nothing reads it in this release — every user is served from their own record whatever it says — and it goes with the refusals written against it, in [issue 1829](https://github.com/Krzysztof318/MailFathom/issues/1829) |
