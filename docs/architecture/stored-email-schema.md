...
| `ix_outgoing_email_filings_placement` | `(UserId, MailboxAccountId, FolderPath, PlacementUidValidity, PlacementUid)` where `ObservedAt` is null and the stage is `Confirmed` | The question every synchronized batch asks: is one of these UIDs a copy this deployment filed. The filter is both halves of what that join can match, which is what keeps the index the size of the copies not yet seen coming back rather than of everything ever sent — a mirror withdrawn before any run saw it, and an append the server never answered, are rows nothing will ever match again |
| `ix_outgoing_email_filings_message_id` | `(UserId, MailboxAccountId, InternetMessageId)` where `ObservedAt` is null and the stage is `Confirmed` | The same question on a server that advertises no `UIDPLUS` and therefore named no placement, answered by the identity in the appended bytes, and bounded the same way |
