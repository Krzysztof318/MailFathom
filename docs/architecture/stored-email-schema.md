...
`mailbox_accounts`. `mailbox_accounts` keeps its single-column key here, so no identifier that was legal before this is
refused after it; putting the owner into that key is a change of its own.
...
| `OwnerId` | The owner that account belongs to. Beside the key rather than in it, because the key is what makes one outstanding run per account a property of the schema and the identifier already names one account within its owner |
...
|---|---|
| `mailbox_account_id` | The account whose mail the run walks, and the primary key |
| `OwnerId` | The owner that account belongs to, beside the key for the reason the rule run's is |
| `requested_at` | When the run was asked for |
...
| `email_thread_identifiers.MailboxAccountId`, `IdentifierHash` | The primary key, with `OwnerId` beside it rather than in it: the key is what two messages binding one identifier collide on, and the identifier is already scoped to one account. The hash is the lower-case hexadecimal SHA-256 digest of the identifier as it was stored, `character varying(64)` |
...
| `OwnerId` | The owner that account belongs to, and the only account reference in the schema that is nullable — it is absent for exactly the rows the account is, because work belonging to no account belongs to no owner either. It is what the turn below is read over, and `GREATEST` ignoring a null is what keeps such a row claimable rather than stranded |
...
| `OwnerId` | The owner that account belongs to, beside the key rather than in it for the reason every keyed account reference here has one beside it |
...
| `OwnerId` | The owner that account belongs to, beside the key as the position row's is |
...
| `IX_jobs_MailboxAccountId` | `(MailboxAccountId)` | The foreign key back to the account, which the constraint is checked from both sides of. EF Core indexes it because the composite index above no longer leads with that column |
