...
| `McpEndpointEnabled`, `ClientEndpointEnabled` | Whether the user is served on the MCP endpoint and on the client endpoint, `boolean NOT NULL DEFAULT TRUE`. The default is what a row recorded before the columns existed reads, and what a provisioning insert naming neither writes. Every credential resolution and every session read joins them in the statement it already runs, so a switch turned off refuses that user's next request on every replica with no cache to expire |
