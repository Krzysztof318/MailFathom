...
| `State` | `Pending` while the job is claimable, `Claimed` while an attempt holds it, `Succeeded` once it is done, and `DeadLettered` once nothing will attempt it again. Text for the reason every other bounded value here is: it stays readable in an ad-hoc query and survives a reordering of the enum |
