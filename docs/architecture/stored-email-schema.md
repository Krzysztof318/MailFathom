...
| `SecretDigest` | The SHA-256 digest of the token's secret half, 32 bytes. The secret itself is never stored, so a row read out of the database or out of a backup authenticates nothing; the presented secret is hashed and the two digests compared in constant time |
