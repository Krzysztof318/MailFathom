...
`SensitiveContentStamp` records the configuration these words were derived under, exactly as the search document's does, because a description and an extracted text are derived mail content like any other. Deleting a message cascades to these rows, so nothing read out of a file outlives the message it arrived on.
