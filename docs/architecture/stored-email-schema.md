...
| `ix_mail_drafts_promoted` | `(PromotedToOutgoingEmailId)` where it is not null | The one question a finished delivery asks of this table: which draft, if any, became that send. The filter is what keeps the index the size of the drafts that have been promoted rather than of every draft ever written, since a draft that was never promoted can never answer it |
