...
however many people read it. [ADR 0014](https://github.com/Krzysztof318/MailFathom/blob/main/docs/decisions/0014-single-tenant-multi-user-ownership-on-the-mail-account.md) is the decision. Both tables of the [contact book](#the-contact-book) carry a user of their own, for two different reasons: it is an assembled record rather than mail an account brought in, so `contacts` hangs on nothing that could carry a user and names one directly, and `contact_addresses` repeats it despite hanging on `contacts` through `(ContactId, UserId)`, because the reads it answers lead with the user in an index and an index cannot reach through the contact to find one.
...
`contacts` holds one row per person the user's book knows, and `contact_addresses` one row per address that person
uses. What reaches it comes from two places, and the `Origin` column below is which: a row the user wrote down is the
...
| `UserId` | Whose book holds this person, keyed onto `settings_accounts` and cascading from it. Every read of the book carries it, and it leads the two indexes a read would otherwise walk the table for — the listing and the address uniqueness below — while the identity lookups seek on the key and carry the user as a predicate beside it. It never changes: a contact is written in a book rather than moved between them |
...
`contact_addresses` carries the address as written beside its comparison form, the contact it belongs to, and the user
that contact belongs to. It is rows rather than an array column, and that is what makes both rules over it structural.
The comparison form is unique **within one user's book** rather than across the table, so one address stays in one
person's hands however many callers claim it at once — the loser of that race is recognized by the constraint it
violated and is answered with which contact holds the address — while two users who each correspond with the same
person each hold their own record of them. Uniqueness across the table would instead let one user's book decide what
...
The user is repeated on the address row rather than read through the contact, because an index cannot reach through a
foreign key to a column on the parent and the uniqueness above is over the user and the address. What keeps the
repetition honest is the key itself: the foreign key is `(ContactId, UserId)` onto the alternate key `(Id, UserId)` on
`contacts`, so an address row can only carry the user of the contact it hangs on — no other pair exists to point at.
...
| `ix_contacts_user_display_name_sort_key_id` | `(UserId, DisplayNameSortKey, Id)` | The one order the contact book is listed in and the one a keyset page continues from. The user leads it because a page is always of one person's book, and the identity settles two people whose names compare equal, which is what makes the order total within a book and the walk terminate |
| `ix_contact_addresses_user_normalized_address` | `(UserId, NormalizedAddress)`, unique | One address in one person's hands within one user's book. It is also what the lookup from an address to a person is served from, rather than a scan, and leading with the user is what makes that lookup read one book rather than the table |
| `IX_contact_addresses_ContactId_UserId` | `(ContactId, UserId)` | The foreign key back to the person, which is what erasing one reaches their addresses by. It carries the user because the key does |
...
`settings_accounts` is personal data of a kind nothing else on this page holds: it is the record of a person rather than of their mail, and its document is what they configured MailFathom to do on their behalf. It is therefore the row a data-subject erasure is aimed at, and the cascade beneath it is what discharges the rest — which is also why the tables that record a mail account without keying onto one are taken by the same operation rather than left for somebody to remember. The contact book is reached by the cascade rather than by that derived list: `contacts` and `contact_addresses` record no mail account, but `contacts` keys onto the user directly and `contact_addresses` keys onto `contacts` through `(ContactId, UserId)`, so erasing the user takes the whole book with them in two hops — which is what an erasure request owes about an assembled record of third parties this user wrote down. `stored_secrets` keys onto the user directly as well, so the sealed material behind every reference in their document leaves through that cascade. The refresh token of an account that was authorized but never synchronized is reached by the explicit account predicate over `mailbox_refresh_tokens`, over the accounts this user was the last one assigned; it no longer survives for lack of a `mailbox_accounts` row. Nothing in the user row reaches a log, a metric, a trace, or an exception message: a failure names the user's identifier, which is a value MailFathom generated, and never the document beside it.
