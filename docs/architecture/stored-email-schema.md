...
| `ix_outgoing_email_filings_message_id` | `(UserId, MailboxAccountId, InternetMessageId)` where the stage is `Confirmed` | The same question on a server that advertises no `UIDPLUS` and therefore named no placement, answered by the identity in the appended bytes, and bounded the same way. It also narrows the outer scan of the duplicate withdrawal, which reads this deployment's own confirmed sent filings; the provider's copy is not in this table at all and is found by a correlated match against `stored_emails` on the folder, the UID space, and that same identity, at a UID the placement does not name |
