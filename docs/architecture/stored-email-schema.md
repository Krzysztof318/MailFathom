...
| `DocumentWrittenAtRuntime` | Whether anything has written the document while the deployment was running. The sole user `MailSynchronization:Accounts` belongs to carries the envelope alone and the empty object the row was provisioned with, and a user whose document was written and then emptied carries the same octets: the two are different facts and this is what tells them apart, so nothing has to read a deployment's files to find out which of them a row is |

**The document has a typed record to bind to, and the envelope beside it never depends on reading one.** What the
column holds is one user's configurable record — their mail-account declarations today, and the settings that are
their own rather than the deployment's as each moves out of the deployment's section. A start binds it for a user
whose marker below is set and for no other, so the document of the sole user `MailSynchronization:Accounts` belongs to
is not read at all and a row edited by hand for them changes nothing. What the binding does is bind strictly — a
...
unique *within the user*, which is the naming space this binder applies. A deployment-wide bound narrows it — no two
users this deployment serves may name a mail account alike, because the settings read in front of persistence still
resolves an account by its identifier alone — so two people each naming `work` is refused today, wherever each of the
two is read from, and is what that bound's removal delivers. The record may carry no secret material at
