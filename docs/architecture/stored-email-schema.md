...
**Occurrence identity.** `mail_folder_id`, `uid_validity`, and `uid` are the stable remote identity of one message in one folder, and `id` is the local UUIDv7 that every other table references. `mailbox_account_id` and `OwnerId` are a copy of the owning folder's account and of the owner that account belongs to: every timeline index leads with them, and an index cannot span a join. Nothing repoints a folder at another account and nothing moves an account between owners, so both are written with the row and never revised.
...
`stored_secrets` holds the material an owner document refers to through `database:<uuid>`. The document carries only
the reference; a database reader therefore sees no mailbox password in the JSONB record. The material is sealed under
the ADR 0005 key ring before PostgreSQL sees it, and the authenticated binding contains the owner, the row identifier,
...
| `OwnerId` | The owner whose record may carry the reference, a required foreign key onto `settings_accounts` with `ON DELETE CASCADE` |
| `Name` | The bounded declared `ConfiguredSecret.Name`, unique within the owner and safe for rotation and audit records |
...
identifier is the string whoever declared the account wrote, and it names one account *within its owner* rather than
...
owners' rows under one key. Reads follow the same order: a mail-reading or mail-writing query narrows on the owner and
...
binding's `(owner, account, alias, generation)`. Such a read carries the owner because the key does, and adding a term
beside the key would narrow nothing.

`mailbox_accounts` carries `Id` and `OwnerId` and nothing else, and its primary key is both of them in that order. The mailbox identifier is the string whoever declared the account wrote; the owner beside it is a `uuid` foreign key onto `settings_accounts` that cascades. Both are relational columns rather than values in a document, which is the point: ownership, lookup, uniqueness, and cascade erasure are then guarantees PostgreSQL gives rather than predicates somebody remembered to write. The key is what scopes the name — a second owner declaring `work` gets a second mailbox rather than a refusal — and the owner leads it rather than following it, which is what leaves the table without an index of its own: the read that used to need one, which mail accounts one owner owns, is a range of the key and is what the erasure below asks.
...
| `OwnerId`, `MailboxAccountId`, `StoredEmailId` | Whose mailbox and which local email, as values that survive both being removed. The account is named as the pair, so an entry that outlives the account row still says whose it was |
...
| Column | What it records |
|---|---|
| `RunId`, `OwnerId`, `MailboxAccountId`, unique together | Which question this was and whose mailbox it reached, the account named as the pair; the uniqueness is what makes a repeated append leave one entry, and the run identifier is what joins the entries a multi-account question left |
...
| Column | What it records |
|---|---|
| `OwnerId` | The owner whose mail the run walks, and the first half of the primary key |
| `mailbox_account_id` | The account, and the second half of the key. It follows the owner because the key is what makes one outstanding run per account a property of the schema, and an identifier names one account only within its owner — keyed alone it would have made one run per *name*, shared by everyone who chose it |
...
| `email_threads.OwnerId`, `email_threads.MailboxAccountId` | The account the conversation belongs to, as the pair. Membership never spans two accounts, because a message identifier is only unique in the sense the sender's own system gave it, and one account's correspondence must not be assembled out of another's |
...
| `Payload` | The references the work is described by, as one `jsonb` document. Nothing queries into it — the key, the type, the account, and the available instant are all columns — so it is a document rather than a schema, and it is bounded at the enqueue boundary so a payload that copied content into it is refused instead of stored. The one statement that has ever written into it is the migration that added the owner beside every account reference, for the reason below |
| `MailboxAccountId` | The account the work belongs to, null when it belongs to none. With the owner beside it, a foreign key onto `mailbox_accounts` that cascades, so removing an account takes its queued work with it |
| `OwnerId` | The owner that account belongs to, and the only account reference in the schema that is nullable — it is absent for exactly the rows the account is, because work belonging to no account belongs to no owner either. `ck_jobs_account_owner` is what makes *exactly* true rather than customary: a composite foreign key with one half null is not checked at all, so a row naming an identifier and no owner would reference a mailbox nothing resolved, and the check refuses it. The column is also what the turn below is read over, and `GREATEST` ignoring a null is what keeps a row belonging to no account claimable rather than stranded |
...
**The owner is in the document as well as on the row, and the migration is what put it there.** Every payload that names an account names its owner beside it and declares the property required, so a document the previous release wrote no longer parses. A claim reads a batch of rows rather than one, so a single unparseable document would take every job claimed beside it with it and the queue would never drain — which is why the migration that filled the column writes the same value into the document, and why an upgrade needs no queue drained first. A payload naming no account has no owner to write and is left as it was.
...
| Column | What it records |
|---|---|
| `OwnerId` | The owner whose mail is being walked, and the first of the three columns the primary key is over |
| `MailboxAccountId` | The account, and the second, following the owner because an identifier names one account only within it |
...
| Column | What it records |
|---|---|
| `OwnerId` | The owner whose mail the run walks, and the first of the three columns the primary key is over |
...
| `OwnerId`, `MailboxAccountId` | The account the message is submitted through and sent as, named as the pair. Plain columns rather than a foreign key: the account row is created by the first folder binding synchronization writes, and an account configured only to send need never have synchronized anything |
...
| `OwnerId`, `MailboxAccountId` | The account whose mailbox holds the copy, as the pair, carried here so the read that recognizes a copy coming back is answered without joining the record |
...
| `OwnerId`, `MailboxAccountId` | The account every occurrence is submitted through and sent as, as the pair, plain columns for the reason the outgoing record's are |
...
| `ix_stored_emails_owner_account_timeline` | `(OwnerId, mailbox_account_id, received_at DESC NULLS LAST, id DESC)` | The timeline of one of an owner's accounts, and of every account one owner holds. [ADR 0014](https://github.com/Krzysztof318/MailFathom/blob/main/docs/decisions/0014-single-tenant-multi-user-ownership-on-the-mail-account.md) names an owner-led index beside it as the candidate for *all of my mail*, on the reading that a request naming no account would otherwise be planned as a scan followed by a top-N sort. No such read exists: a timeline over several accounts is walked one account at a time and the walks are merged, for the reason [the ordering contract](#the-timeline-ordering-contract) gives, so every timeline query carries an equality on the account and an owner-led index would be one nothing reads and every stored message paid for |
...
| `ix_stored_emails_owner_account_identity` | `(OwnerId, mailbox_account_id, id)` | The order a whole-mailbox rule run walks an account's mail in. The identity rather than the timeline, because a walk that has to resume needs a total order no later write disturbs and a position that is one column rather than a nullable timestamp paired with a tie-breaker |
| `ix_stored_emails_awaiting_rule_evaluation` | `(OwnerId, mailbox_account_id, id)` over the rows whose `rules_evaluated_at` **and** `FiledFromOutgoingEmailId` are both null | The queue of mail no rule pass has evaluated, read once per account run. The filter is the point: in steady state almost every row of an account has been evaluated, so without it the read would walk the account's whole index to find the handful that qualify, on every run of every account. The second clause is what keeps a copy of this deployment's own outgoing mail out of that queue permanently — such a row is never stamped as evaluated, because it never was, so excluding it anywhere else would leave it at the head of the queue for good |
| `ix_stored_emails_awaiting_attachment_text` | `(OwnerId, mailbox_account_id, id)` over the rows whose `AttachmentTextDerivedAt` is null **and** whose `AttachmentCount` is above zero | The queue of mail whose attachments no pass has read, in the same identity order the rule queue walks and for the same reason. Both clauses of the filter earn their place: the timestamp is what takes a message out of the queue once its attachments have been settled, and the count is what keeps the mail carrying no attachment at all — most of a mailbox — out of an index that would otherwise be as large as the account |
...
| `pk_email_thread_identifiers` | `(OwnerId, MailboxAccountId, IdentifierHash)`, unique | Which conversation an identifier belongs to, which is the question every arriving message asks once per identifier it names. Uniqueness is also the race: two messages binding one identifier at once leave one writer to retry against what the other wrote |
| `ix_email_thread_identifiers_thread` | `(EmailThreadId)` | The identifiers a merge has to repoint at the surviving conversation |
| `pk_mailbox_accounts` | `(OwnerId, Id)`, unique | Which mailbox an owner's identifier names, and the uniqueness that scopes the name to its owner. The owner leads it, so it is also the read that used to need an index of its own — which mail accounts one owner owns, asked by an erasure before it takes the rows no cascade reaches |
| `IX_email_threads_OwnerId_MailboxAccountId` | `(OwnerId, MailboxAccountId)` | The key back to the owning account, which is what erasing one reaches its conversations by |
...
| `ix_mailbox_mutations_outstanding` | `(OwnerId, MailboxAccountId, RecordedAt)` where the stage is not `Completed` | The changes an operator asks about: those in flight and those given up on |
| `ix_mailbox_mutations_placement` | `(OwnerId, MailboxAccountId, DestinationFolderPath, PlacementUidValidity, PlacementUid)` where `PlacementObservedAt` is null | The question the forward pass asks of every batch it discovers: is one of these UIDs where a relocation or a copy put an email |
...
| `ix_outgoing_emails_identity` | `(OwnerId, MailboxAccountId, RequesterOrigin, RequesterIdentity)`, unique | An outgoing message's idempotency identity, which is what makes the same authored request twice one delivery. It spans terminal rows deliberately: a row that was sent is what stops the same request asking again |
| `ix_outgoing_emails_claimable` | `(OwnerId, MailboxAccountId, AvailableAt, Id)` where the stage is `Recorded` | The batch a delivery pass claims, oldest first. The filter is what keeps it proportionate to the outbox rather than to everything the deployment has ever sent: a claim reads only rows nothing has transmitted for, and in steady state that is almost none of the table. The identity is in the key because two sends recorded in one instant need a total order for the claim to be deterministic |
| `ix_outgoing_email_filings_placement` | `(OwnerId, MailboxAccountId, FolderPath, PlacementUidValidity, PlacementUid)` where `ObservedAt` is null and the stage is `Confirmed` | The question every synchronized batch asks: is one of these UIDs a copy this deployment filed. The filter is both halves of what that join can match, which is what keeps the index the size of the copies not yet seen coming back rather than of everything ever sent — a mirror withdrawn before any run saw it, and an append the server never answered, are rows nothing will ever match again |
| `ix_outgoing_email_filings_message_id` | `(OwnerId, MailboxAccountId, InternetMessageId)` where `ObservedAt` is null and the stage is `Confirmed` | The same question on a server that advertises no `UIDPLUS` and therefore named no placement, answered by the identity in the appended bytes, and bounded the same way |
| `ix_outgoing_emails_outstanding` | `(OwnerId, MailboxAccountId, RecordedAt)` where the stage is none of `Sent`, `Refused`, or `Cancelled` | The outbox a restart reads and an operator asks about: what is queued, what is in flight, and what has stopped. The filter names the three terminal stages rather than the successful one alone, so a refused send stays visible while the deployment's whole sending history does not |
| `ix_outgoing_emails_period_usage` | `(RecordedAt, OwnerId, MailboxAccountId)` | What a send ceiling counts: the messages one period was asked for, for one account and for the whole deployment. It carries no filter, because a period counts every message it was asked for whatever became of it, and the account is behind the instant rather than in front of it so one index answers both questions — the deployment's count reads a range of it and an account's count reads the same range narrowed. The deployment's half names no owner either, and that is the ceiling it belongs to rather than a narrowing left out: a deployment ceiling bounds what the process puts on the network, which is one budget however many owners it serves |
| `ix_recurring_sends_identity` | `(OwnerId, MailboxAccountId, RequesterOrigin, RequesterIdentity)`, unique | A declaration's idempotency identity, which is what makes the same authored act twice one declaration. It spans stopped rows deliberately: a declaration that was stopped is what keeps the act that made it from quietly declaring a second one |
...
| `ix_mailbox_mutation_audit_entries_owner_account_completed` | `(OwnerId, MailboxAccountId, CompletedAt, Id)` | The two ways the trail is worked: a keyset-paginated page of an account's history, and the retention pass that erases what ended before a cutoff |
| `ix_mail_answering_audit_entries_run_owner_account` | `(RunId, OwnerId, MailboxAccountId)`, unique | One entry per run per account, whatever a repeated append attempts |
| `ix_mail_answering_audit_entries_owner_account_completed` | `(OwnerId, MailboxAccountId, CompletedAt, Id)` | The same two readers the trail above has: a keyset-paginated page of an account's runs, and the retention pass beside it |
...
A timeline over several accounts is walked one account at a time and the walks are merged, rather than issued as one query naming the account list. An index leading with the owner and the account serves one account as an ordered walk, and PostgreSQL derives an ordered path from appended paths rather than from `= ANY`, so a single predicate over the list would be planned as a scan of everything that matched followed by a top-N sort — and the keyset walk would stop being bounded on exactly the mailbox that is large enough for it to matter. Each walk is bounded to the page size and the walks are ordered together, so what the outer ordering sorts is at most one page per account rather than the whole mailbox. Every timeline query therefore carries an equality on the account, which is why the index table above holds no owner-led timeline index.
