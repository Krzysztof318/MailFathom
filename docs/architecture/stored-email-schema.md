...
## The user a mailbox belongs to

`settings_accounts` holds one row per user, and `mailbox_accounts` gains the user every mailbox belongs to. This is the axis the whole graph above hangs on: a folder cascades from the account, a message from the folder, and everything derived from a message from the message, so a user on the account row is a user on all of it. [ADR 0014](https://github.com/Krzysztof318/MailFathom/blob/main/docs/decisions/0014-single-tenant-multi-user-ownership-on-the-mail-account.md) is the decision. Both tables of the [contact book](#the-contact-book) carry a user of their own, for two different reasons: it is an assembled record rather than mail an account brought in, so `contacts` hangs on nothing that could carry a user and names one directly, and `contact_addresses` repeats it despite hanging on `contacts` through `(ContactId, UserId)`, because the reads it answers lead with the user in an index and an index cannot reach through the contact to find one.
...
The one exception is a lookup by a key that already names one row — a primary key, or an alternate key such as a folder
...
| `UserId`, `MailboxAccountId` | The account, a foreign key onto `mailbox_accounts` that cascades, which is what takes the whole hierarchy with the account |
...
| `Name`, `NameKey` | The name as written, and its upper-cased comparison key. The unique index `ix_local_mail_folders_user_account_parent_name` over the account, the parent — with nulls not distinct, so the top is one set of siblings — and the key, filtered to live folders, is what refuses two siblings one name however many writers reach it at once |
| `Role` | `Inbox`, `Drafts`, `Sent`, `Junk`, or `Trash` on the five protected folders, `null` on every other; `ix_local_mail_folders_user_account_role` keeps one live folder per role |
...
## What one user's stored content holds

`user_stored_content` holds one row per user, keyed by `UserId`, with a `bigint` `StoredContentByteCount`. It is the only figure on this page that duplicates something derivable from the rows beneath it, and the duplication is the point: the derivation is a sum over one person's whole mailbox, and the per-user storage ceiling it serves is consulted before every message. What it counts is the payload bytes, which is what a re-derivation produces — not what the table occupies on disk, an answer that exists only for the table as a whole and is what the deployment-wide ceiling reads instead.

Every movement is issued inside the transaction that stores or removes the payload, as a composed `INSERT … ON CONFLICT DO UPDATE` that adds a difference rather than writing a total, so a crash cannot leave a message stored and uncounted and two runs storing at once add to each other. Each statement resolves the user from the message itself, which carries `UserId` beside the account it names, so no caller passes one and nothing here reads the account table to find out whose the mail is. A user with no row has never had one written — a deployment upgraded before their first message, or a user provisioned since — and their first read derives the figure once and adopts it. That derivation is the one operation here that writes a total rather than a difference, so it is also the only one that takes a transaction of its own: it claims the user's row before it measures and holds it until it has written, and a store committing meanwhile is either counted by it or applied on top of it.
...
| `UserId`, `MailboxAccountId`, `StoredEmailId` | Whose mailbox and which local email, as values that survive both being removed. The account is named as the pair, so an entry that outlives the account row still says whose it was |
...
| Column | What it records |
|---|---|
| `RunId`, `UserId`, `MailboxAccountId`, unique together | Which question this was and whose mailbox it reached, the account named as the pair; the uniqueness is what makes a repeated append leave one entry, and the run identifier is what joins the entries a multi-account question left |
...
| Column | What it records |
|---|---|
| `UserId` | The user whose mail the run walks, and the first half of the primary key |
...
| `UserId`, `MailboxAccountId`, `StoredEmailId` | Whose mail was evaluated, and which message, the account named as the pair. The message is a foreign key onto `stored_emails` that cascades, which is the whole erasure story here: what a rule concluded about a message goes when the message does, without a rule anybody has to remember |
...
| `email_threads.UserId`, `email_threads.MailboxAccountId` | The account the conversation belongs to, as the pair. Membership never spans two accounts, because a message identifier is only unique in the sense the sender's own system gave it, and one account's correspondence must not be assembled out of another's |
...
| `Payload` | The references the work is described by, as one `jsonb` document. Nothing queries into it — the key, the type, the account, and the available instant are all columns — so it is a document rather than a schema, and it is bounded at the enqueue boundary so a payload that copied content into it is refused instead of stored. The one statement that has ever written into it is the migration that added the user beside every account reference, for the reason below |
| `MailboxAccountId` | The account the work belongs to, null when it belongs to none. With the user beside it, a foreign key onto `mailbox_accounts` that cascades, so removing an account takes its queued work with it |
| `UserId` | The user that account belongs to, and the only account reference in the schema that is nullable — it is absent for exactly the rows the account is, because work belonging to no account belongs to no user either. `ck_jobs_account_user` is what makes *exactly* true rather than customary: a composite foreign key with one half null is not checked at all, so a row naming an identifier and no user would reference a mailbox nothing resolved, and the check refuses it. The column is also what the turn below is read over, and `GREATEST` ignoring a null is what keeps a row belonging to no account claimable rather than stranded |
...
| `TurnAt` | When this job's turn comes once its user's queue is shared with everybody else's, and what the claim orders by. A virtual instant rather than a real one: the enqueue reads how far its user's waiting work has already reached, adds a second, and takes the later of that and the instant the job becomes available — so a user with nothing waiting lands on the moment its work is due, and a user queueing a thousand jobs at once holds turns spread over the next thousand seconds instead of a thousand turns at one instant. That is the whole of what stops one person's backlog postponing another's due work, and it is decided here rather than in the claim so the claim stays one indexed statement. Never null, and never earlier than `AvailableAt` as written: the enqueue floors it there, and a retry and a returned dead letter each carry it forward to the instant they name, so no write leaves a job holding a turn it could not take. The two do diverge afterwards, and a release is where — it moves `AvailableAt` to now and leaves the turn alone, because the attempt gave the work back rather than failing at it, so a released job resumes the place it already had rather than going to the end of its user's queue. Nothing reading a row may assume `TurnAt >= AvailableAt`. Rows written before the column existed carry the available instant they already held, which is the order the claim had been draining them in |
...
**The user is in the document as well as on the row, and the migration is what put it there.** Every payload that names an account names its user beside it and declares the property required, so a document the previous release wrote no longer parses. A claim reads a batch of rows rather than one, so a single unparseable document would take every job claimed beside it with it and the queue would never drain — which is why the migration that filled the column writes the same value into the document, and why an upgrade needs no queue drained first. A payload naming no account has no user to write and is left as it was.
...
| Column | What it records |
|---|---|
| `UserId` | The user whose mail is being walked, and the first of the three columns the primary key is over |
...
| Column | What it records |
|---|---|
| `UserId` | The user whose mail the run walks, and the first of the three columns the primary key is over |
...
| `UserId`, `MailboxAccountId` | The account the message is submitted through and sent as, named as the pair. Plain columns rather than a foreign key: the account row is created by the first folder binding synchronization writes, and an account configured only to send need never have synchronized anything |
...
| `UserId`, `MailboxAccountId` | The account whose mailbox holds the copy, as the pair, carried here so the read that recognizes a copy coming back is answered without joining the record |
...
| `UserId`, `MailboxAccountId` | The account every occurrence is submitted through and sent as, as the pair, plain columns for the reason the outgoing record's are |
...
| `UserId`, `MailboxAccountId` | The account the draft belongs to and whose drafts folder holds its copy, as the pair. Plain columns for the reason an outgoing record's are: an account configured only to send need never have synchronized anything |
...
| `ix_stored_emails_user_account_timeline` | `(UserId, mailbox_account_id, received_at DESC NULLS LAST, id DESC)` | The timeline of one of a user's accounts, and of every account one user holds. [ADR 0014](https://github.com/Krzysztof318/MailFathom/blob/main/docs/decisions/0014-single-tenant-multi-user-ownership-on-the-mail-account.md) names a user-led index beside it as the candidate for *all of my mail*, on the reading that a request naming no account would otherwise be planned as a scan followed by a top-N sort. No such read exists: a timeline over several accounts is walked one account at a time and the walks are merged, for the reason [the ordering contract](#the-timeline-ordering-contract) gives, so every timeline query carries an equality on the account and a user-led index would be one nothing reads and every stored message paid for |
...
| `pk_email_thread_identifiers` | `(UserId, MailboxAccountId, IdentifierHash)`, unique | Which conversation an identifier belongs to, which is the question every arriving message asks once per identifier it names. Uniqueness is also the race: two messages binding one identifier at once leave one writer to retry against what the other wrote |
| `ix_email_thread_identifiers_thread` | `(EmailThreadId)` | The identifiers a merge has to repoint at the surviving conversation |
| `pk_mailbox_accounts` | `(UserId, Id)`, unique | Which mailbox a user's identifier names, and the uniqueness that scopes the name to its user. The user leads it, so it is also the read that used to need an index of its own — which mail accounts one user owns, asked by an erasure before it takes the rows no cascade reaches |
| `IX_email_threads_UserId_MailboxAccountId` | `(UserId, MailboxAccountId)` | The key back to the owning account, which is what erasing one reaches its conversations by |
...
| `ix_mailbox_mutations_outstanding` | `(UserId, MailboxAccountId, RecordedAt)` where the stage is not `Completed` | The changes an operator asks about: those in flight and those given up on |
| `ix_mailbox_mutations_placement` | `(UserId, MailboxAccountId, DestinationFolderPath, PlacementUidValidity, PlacementUid)` where `PlacementObservedAt` is null | The question the forward pass asks of every batch it discovers: is one of these UIDs where a relocation or a copy put an email |
...
| `ix_outgoing_emails_identity` | `(UserId, MailboxAccountId, RequesterOrigin, RequesterIdentity)`, unique | An outgoing message's idempotency identity, which is what makes the same authored request twice one delivery. It spans terminal rows deliberately: a row that was sent is what stops the same request asking again |
| `ix_outgoing_emails_claimable` | `(UserId, MailboxAccountId, AvailableAt, Id)` where the stage is `Recorded` | The batch a delivery pass claims, oldest first. The filter is what keeps it proportionate to the outbox rather than to everything the deployment has ever sent: a claim reads only rows nothing has transmitted for, and in steady state that is almost none of the table. The identity is in the key because two sends recorded in one instant need a total order for the claim to be deterministic |
| `ix_outgoing_email_filings_placement` | `(UserId, MailboxAccountId, FolderPath, PlacementUidValidity, PlacementUid)` where the stage is `Confirmed` | The question every synchronized batch asks: is one of these UIDs a copy this deployment filed. The stage is the whole filter, so the index is the size of the copies still standing rather than of everything ever sent — a mirror withdrawn before any run saw it, and an append the server never answered, are rows nothing will ever match again. Having been met is deliberately not part of it, because a provider that files the sent copy itself puts a second occurrence of the same message in the same folder and both have to be recognized |
| `ix_outgoing_email_filings_message_id` | `(UserId, MailboxAccountId, InternetMessageId)` where the stage is `Confirmed` | The same question on a server that advertises no `UIDPLUS` and therefore named no placement, answered by the identity in the appended bytes, and bounded the same way |
| `ix_outgoing_email_filings_recent_by_filing` | `(UserId, MailboxAccountId, Filing, AppendedAt, OutgoingEmailId)` where the stage is `Confirmed` | The same rows read the other way round, which is what the sweep for a copy the provider duplicated asks: which of this account's standing copies of one kind were appended recently enough to still be acted on. The two above lead with what a discovery is recognized by and answer nothing about when a copy was filed, so without this one that sweep reads every confirmed filing the account has ever kept — on every outbox pass — to find the few inside a fifteen-minute window. The identity is behind the instant for the reason the claim's is, and it is what lets the ordering be read rather than sorted. The provider's own copy is in none of these three: it has no row in this table at all, and is found by a correlated match against `stored_emails` on the folder, the UID space, and the identity the append carried, at a UID the placement does not name |
| `ix_outgoing_emails_outstanding` | `(UserId, MailboxAccountId, RecordedAt)` where the stage is none of `Sent`, `Refused`, or `Cancelled` | The outbox a restart reads and an operator asks about: what is queued, what is in flight, and what has stopped. The filter names the three terminal stages rather than the successful one alone, so a refused send stays visible while the deployment's whole sending history does not |
| `ix_outgoing_emails_period_usage` | `(RecordedAt, UserId, MailboxAccountId)` | What a send ceiling counts: the messages one period was asked for, for one account and for the whole deployment. It carries no filter, because a period counts every message it was asked for whatever became of it, and the account is behind the instant rather than in front of it so one index answers both questions — the deployment's count reads a range of it and an account's count reads the same range narrowed. The deployment's half names no user either, and that is the ceiling it belongs to rather than a narrowing left out: a deployment ceiling bounds what the process puts on the network, which is one budget however many users it serves |
| `ix_recurring_sends_identity` | `(UserId, MailboxAccountId, RequesterOrigin, RequesterIdentity)`, unique | A declaration's idempotency identity, which is what makes the same authored act twice one declaration. It spans stopped rows deliberately: a declaration that was stopped is what keeps the act that made it from quietly declaring a second one |
...
| `ix_mail_drafts_user_account_revised` | `(UserId, MailboxAccountId, RevisedAt)` | An account's drafts in the order the pass settles them, oldest change first. No filter, because the read that uses it is already narrowed by the copy predicates beneath it and a deployment's whole set of held drafts is what a user is editing rather than a history that accumulates |
...
| `ix_mail_rule_executions_user_account_evaluated` | `(UserId, MailboxAccountId, EvaluatedAt, Id)` | An account's rule history newest first, and the retention pass that erases what has outlived its window. The identifier is in the key because two executions of one batch share an instant and a keyset page needs a total order to continue from |
| `ix_mail_rule_executions_user_account_rule_evaluated` | `(UserId, MailboxAccountId, RuleName, EvaluatedAt, Id)` | What one rule has been concluding, which is the question a rule that never seems to fire is investigated with |
...
| `ix_mailbox_mutation_audit_entries_user_account_completed` | `(UserId, MailboxAccountId, CompletedAt, Id)` | The two ways the trail is worked: a keyset-paginated page of an account's history, and the retention pass that erases what ended before a cutoff |
| `ix_mail_answering_audit_entries_run_user_account` | `(RunId, UserId, MailboxAccountId)`, unique | One entry per run per account, whatever a repeated append attempts |
| `ix_mail_answering_audit_entries_user_account_completed` | `(UserId, MailboxAccountId, CompletedAt, Id)` | The same two readers the trail above has: a keyset-paginated page of an account's runs, and the retention pass beside it |
...
| `ix_jobs_user_account` | `(UserId, MailboxAccountId, EnqueuedAt)` | An account's queued work, which is what erasure and any per-account bound reach a job by |
...
`user_stored_content` is read the other way round, and the difference is what its cascade records. A byte count against a user names no message either, but what it describes is how much mail that person holds rather than what this deployment spent, so it carries the classification of the mail it counts: it cascades from `settings_accounts`, and erasing a user takes the figure with everything else derived from their mail. Nothing in it reaches a log, a metric, a trace, or an exception message beyond the user's own generated identifier.
...
- Erasing a user leaves no row naming a mailbox of theirs, including the refresh token of an account that has only ever been authorized and therefore holds no account row at all — that row is arranged by the test deliberately, because it is the case the user column beside every account reference closed and the one a subquery over the account table left behind. The claim is made against every table the model says records a mail account rather than against a list written beside it, so a table added later is counted by the same assertion, and the count taken before the erasure is asserted to hold no zero — a table nothing seeded would otherwise let the test pass while proving nothing about it. What survives is asserted the same way: a second mailbox is stored under the user the rest of the suite's mail hangs on, with rows on both sides of the erasure — three tables the cascade reaches and three it does not — and every count beneath it is unchanged afterwards, down to the raw MIME of its message. Both users are given a contact as well, which is the one part of the record that names no mail account at all and is therefore invisible to every count above: the erased user's person and address row are gone, and the other user's are where they were.
- The account row a first folder binding creates, and the folder row beside it, carry the user the caller resolved the account through. What refuses a user the deployment holds no record of is the foreign key onto that record rather than a decision in the write path — the run takes the user from the identity it was handed — so the refusal is proved as the account row being absent afterwards. Which user a configured account belongs to, and the refusals a deployment holding none or several meets, are settled once while the host starts and are asserted there instead.
- Mail stored under a second user hangs on that user on all three tables one binding and one upsert touch, and no read of another user's reaches it. The narrowing is stated as the same account identifier counted under two users, so the user column is the only thing separating the two answers, and the served user's own timeline read over the same folder alias reports a message of theirs beside none of the other user's — the presence is what makes the absence an absence rather than an empty folder. The corpus is six hundred messages split over two of that user's accounts, which is what leaves the planner a choice to make, and a timeline naming one of them is asserted to be served from `ix_stored_emails_user_account_timeline`.
