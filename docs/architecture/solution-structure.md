...
Two projects are named after a shared directory rather than a boundary, because the two linked-source directories otherwise have no project that owns their contract: `SharedSources.UnitTests` compiles `src/shared/` and `TestDoubles.UnitTests` compiles `tests/shared/`, each the same way a consumer does. The split is by what an assertion binds to — the coverage marker's name and usage targets are properties of the marker and belong to `SharedSources.UnitTests`, while whether `Infrastructure` applies it correctly stays in `Infrastructure.UnitTests`. `SharedSources` carries the longer name because CA1716 rejects `Shared` as a namespace, which is a reserved word in another .NET language.
