...
`Boundaries.UnitTests` is named after no boundary at all, because what it covers is what holds *between* them. Its rules read the compiled intermediate language of every assembly a release ships, through ArchUnitNET, and that is the whole reason it exists beside the reference-list assertions the boundary suites already carry: reflection reports what a signature names, so a capability obtained inside a method body and never stored is invisible to it, and a library confined to one namespace of an assembly is a claim a reference list cannot make at all. It reaches every assembly through `Host` and `Cli`, the two executables, and asserts five things — that nothing outside `Application` and `Infrastructure` can obtain a mailbox write session, that Entity Framework Core, Npgsql, pgvector, and the entities they map stay inside the persistence adapter, that MailKit, MimeKit, and AngleSharp stay inside the mail adapter, that PdfPig stays inside the document adapter, and that Ical.Net stays inside the calendar adapter — the last three because a parser's own model of somebody else's file is what those adapters exist to convert: a message's markup is parsed in the first and leaves as the closed document tree a client draws, an attachment is read in the second and leaves as characters and a closed set of reasons, and an offered `.ics` file is read in the third and leaves as MailFathom's own entries and a closed set of reasons an entry became none, so a type from any of those libraries reaching a boundary above would mean somebody had been handed a stranger's file to interpret. Two exceptions are named in the rules themselves rather than worked around: `ServiceCollectionExtensions` composes every adapter and therefore names every library one is built on, and `TransientFailureClassifier` reads MailKit's exceptions to answer whether a protocol failure is worth retrying.

`PublicSurfaces.UnitTests` is named after no boundary either, and for a related reason: its subject is the contracts a release breaks rather than any one project, and each of them is composed somewhere else. It renders every registered MCP tool descriptor, every bindable configuration key, the OpenAPI document the host generates for its whole HTTP API, and the presentation plan contract a Discover run produces into a golden file committed beside its sources, and holds the next build's rendering against it. Two of the four are surfaces [ADR 0004](https://github.com/Krzysztof318/MailFathom/blob/main/docs/decisions/0004-versioning-and-release-policy.md) governs; the HTTP API and the presentation plan are not among that ADR's four and are recorded for the same reason, because an endpoint, a verb, a request shape, a status code, a security declaration, or a block a client has no renderer for changes a client exactly as a renamed tool does. The point is not to judge a change — a break is permitted below `1.0.0` — but to make one impossible to take unnoticed, so the changelog entry a release depends on stops resting on the author's memory. It references `Host`, `Mcp`, and `Application` so a renamed options class, a renamed tool, or a renamed part of the plan breaks the build here instead of quietly emptying a record, and it reaches the bound options classes, the route composition, and the document's own registration through an `InternalsVisibleTo` grant, since each of those is internal. The HTTP record is generated rather than described: the suite composes the real service graph for a deployment serving every surface, maps every route the host maps, and reads the document back — so what a client would bind to is recorded, and the routes that are not operations are absent because the document's allow-list left them out rather than because nothing mapped them. The presentation plan is recorded here rather than in the HTTP document because that document is generated from the endpoints the host maps and no route serves a plan yet, while what a client keys its renderers by — the plan's schema version, each block type's own version, and the shape of every block — is already fixed. The two surfaces it does not cover are covered elsewhere: the database schema by `OrchestratedSchemaArtifactTests` against a real server, and the deployment contract by the assets that render it.
...
`Benchmarks` is the one project under `backend/tests/` that is not a suite. It measures how long chunking, ranking, and MIME extraction take with BenchmarkDotNet and runs from `diagnostics.yml`, when somebody dispatches it, as a report that gates nothing — no job waits on it and a failure there is swallowed — because throughput on a shared, virtualized runner is not reproducible to a precision any threshold could read. It sits here rather than under `backend/tools/` because what it measures is the other half of what `AllocationBudgets.UnitTests` measures and the two share the message they measure, and because a benchmark is something a runner executes rather than a command a person types. `.config/CodeCoverage.proj` names it as the exception to the glob that finds test projects, and `.config/testconfig.json` excludes its assembly the way `SyntheticMail`'s is excluded.

`Evaluations` measures what the agents answer rather than whether their code runs, against the models a run declares and graded by one pinned judge model. It calls the production code it measures rather than a copy of it, which is what each reference is for. It references `AI` for the agents' own compositions, turns, instructions, and readings, and reaches `Application` for the join a Discover run makes of its stages, for the use cases the Agent's tools read through — the content reader, the state browser, the calendar, the task list, and the authoring of an answer — composed over the corpus in place of a database, and for the selection, bounds, and history composition a deployment applies before an agent is asked. It references `SyntheticMail` for the archive shape of the committed corpora its scenarios are fed from — the images the describer is shown, which the corpus carries none of, are committed in the project itself. It references `Infrastructure` for how a deployment reads a message — the attachment walk and the body-text extraction a corpus message is stored through, and the renderer the Agent's tools read a whole message through — and for the store's own queries, which the scenarios run over in-memory rows where a deployment runs them in SQL — except in the one scenario that measures retrieval over PostgreSQL, which migrates a database of its own and stores and ranks the corpus through the writers and index readers a deployment uses. It references `Host` for the defaults a deployment's options classes declare, so no bound or default is restated in the suite. Its scenarios call real providers, so every one skips unless a run asks for it. It opts out of test discovery the way `IntegrationTests` does, which keeps both gates and the coverage run from starting it, and `.config/testconfig.json` excludes its assembly all the same; `scripts/run-ai-evaluations.sh` runs it, and [Local development](../operations/local-development.md#agent-evaluations) holds what a run needs and keeps.

`Evaluations.UnitTests` is the evaluation suite's free half, and like `SyntheticMail.UnitTests` it is named after a project that is not a production boundary. Its tests drive the scenarios through scripted providers and prove each check catches the answer it exists for, so it asks no model and runs with every other unit suite. It is a project apart from `Evaluations` because xUnit sets parallelism for a whole assembly: the paid scenarios run sixty-four at once, a level set by what the providers accept, and these tests run under the unit suites' defaults. It references `Evaluations` alone, which brings it the scenarios, the corpora, and the images, and `Evaluations`, `AI`, and `Application` grant it their internals.
