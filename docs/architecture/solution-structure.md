...
The directory exists because that project is neither of the other two things. It is not `src/`: it ships in no artifact, it is not a command of `mfctl`, and the container image's build context is an allow-list that admits `src/` and never sees it. It is not `tests/` either, because it is a command a person types rather than something a runner executes. That is the whole of the reason: a project under `tests/` is discovered by the coverage gate's own glob unless `.config/CodeCoverage.proj` names it as an exception, which `tests/Benchmarks` is, so being outside the gate is no longer a reason to be outside `tests/`.
