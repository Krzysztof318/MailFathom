...
`PublicSurfaces.UnitTests` is named after no boundary either, and for a related reason: its subject is the contracts a release breaks rather than any one project, and each of them is composed somewhere else. It renders every registered MCP tool descriptor, every bindable configuration key, and the OpenAPI document the host generates for its whole HTTP API into a golden file committed beside its sources, and holds the next build's rendering against it. Two of the three are surfaces [ADR 0004](https://github.com/Krzysztof318/MailFathom/blob/main/docs/decisions/0004-versioning-and-release-policy.md) governs; the HTTP API is not one of that ADR's four and is recorded for the same reason, because an endpoint, a verb, a request shape, a status code, or a security declaration changes a client exactly as a renamed tool does. The point is not to judge a change — a break is permitted below `1.0.0` — but to make one impossible to take unnoticed, so the changelog entry a release depends on stops resting on the author's memory. It references `Host` and `Mcp` so a renamed options class or a renamed tool breaks the build here instead of quietly emptying a record, and it reaches the bound options classes, the route composition, and the document's own registration through an `InternalsVisibleTo` grant, since each of those is internal. The HTTP record is generated rather than described: the suite composes the real service graph for a deployment serving every surface, maps every route the host maps, and reads the document back — so what a client would bind to is recorded, and the routes that are not operations are absent because the document's allow-list left them out rather than because nothing mapped them. The two surfaces it does not cover are covered elsewhere: the database schema by `OrchestratedSchemaArtifactTests` against a real server, and the deployment contract by the assets that render it.
