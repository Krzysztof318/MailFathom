...
`Evaluations` measures what the agents answer rather than whether their code runs, against the models a run declares and graded by one pinned judge model. It references `AI` for the agents' own compositions, instructions, and readings, and `SyntheticMail` for the committed corpus its scenarios are fed from, and its scenarios call real providers, so every one skips unless a run asks for it. It opts out of test discovery the way `IntegrationTests` does, which keeps both gates and the coverage run from starting it, and `.config/testconfig.json` excludes its assembly all the same; `scripts/run-ai-evaluations.sh` runs it, and [Local development](../operations/local-development.md#agent-evaluations) holds what a run needs and keeps.
