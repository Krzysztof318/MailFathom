...
<!-- describes: MailFathom.slnx, Directory.Build.props, backend/src/*/*.csproj, backend/tests/*/*.csproj, tools/*/*.csproj, frontend/*/README.md, backend/src/Host/ServiceDefaultsExtensions.cs -->
...
The build contract stays at the repository root rather than moving down with the solution: `MailFathom.slnx`,
`Directory.Build.props`, `Directory.Packages.props`, `global.json`, and `NuGet.config` are found by MSBuild walking up
from a project file, so a copy under `backend/` would govern nothing a root one does not while a frontend build would
still need its own manifest. `tools/` stays at the root for the same reason it is not a boundary: it is development
tooling the solution references from its tests and ships nothing.
...
Being outside `backend/src/` is what makes the exclusion true today — the release publishes `backend/src/Cli/Cli.csproj` by name — and `the_development_tooling_never_reaches_a_published_artifact` in `scripts/test-agent-workflow.sh` is what keeps it true: it refuses a project under `backend/src/` that references one under `tools/`, a workflow step that names a path there, and a line re-admitting the directory to the image's build context. What it deliberately permits is the `paths-filter` entry in `ci.yml`, which decides which jobs a change starts and is the opposite of publishing.
...
`SyntheticMail.UnitTests` is named after a boundary that is not a production one, because the project it covers is under `tools/` rather than `backend/src/`. Its assertions are as binding as any other suite's — a generator that stopped producing the same corpus from the same seed would be useless and would say nothing about it — and only the coverage denominator treats it differently, for the reason [Development tooling](#development-tooling) gives.
