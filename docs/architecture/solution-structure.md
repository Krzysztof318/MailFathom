...
`Evaluations` measures what the agents answer rather than whether their code runs, against the models a run declares and graded by one pinned judge model. It references `AI` for the agents' own compositions, instructions, and readings, and reaches `Application` for the join a Discover run makes of its stages, `SyntheticMail` for the archive shape of the committed corpora its scenarios are fed from — the images the describer is shown, which the corpus carries none of, are committed in the project itself — and `Infrastructure` for how a deployment reads a body — the quoted-history trimmer, so a corpus message is cut into passages from the trimmed reading a deployment chunks rather than from the whole history a reply carries, and the reader that derives displayed text from a body carried as markup alone. Its scenarios call real providers, so every one skips unless a run asks for it. It opts out of test discovery the way `IntegrationTests` does, which keeps both gates and the coverage run from starting it, and `.config/testconfig.json` excludes its assembly all the same; `scripts/run-ai-evaluations.sh` runs it, and [Local development](../operations/local-development.md#agent-evaluations) holds what a run needs and keeps.
