...
- [Signing the Windows CLI binaries](operations/windows-code-signing.md) documents the SignPath Foundation signature the released `mfctl` binaries for Windows are to carry, why no release carries one yet, where signing sits in the release, the variables and secret it needs, and what a signing failure withholds.
