...
- [Connecting an MCP client](operations/connecting-an-mcp-client.md) is the end-to-end sequence for signing a client in through your own identity provider: which steps are once per deployment and which repeat per client, the three client-registration shapes, one worked provider, a verification recipe to run before touching a client, and what each failed sign-in means.
