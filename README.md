...
Four are the only things on this surface whose effect reaches a mail server at all, and they hold different grants:
...
### Four tools reach a mail server, each behind a grant of its own

- Four tools on the MCP surface reach a mail server. `set_mail_flags` marks a message read or unread, stars or unstars it, and changes its keywords; `send_email` sends one message as an account you configured to send; `reply_to_email` and `forward_email` answer one message you already hold, deriving the addressing, the subject, the threading, the quotation, and a forward's attachments from your stored copy rather than from anything a client supplies. Nothing a client sends can delete or move your mail, nothing a client sends decides who a message is from, and each of the four is offered only to a credential granted its own permission — `mailfathom.mail.flags.write` and `mailfathom.mail.send` — neither of which reading mail carries, neither of which implies the other, and both of which narrowing a credential's entry withholds. A reply and a forward need `mailfathom.mail.read` beside the sending grant, because an answer is derived from the message it answers. What a client can write besides is MailFathom's own contact book, which is a table in your database rather than anything at your mail provider.
...
- **Acting on mail, not only reading it.** `set_mail_flags` was the first capability that writes to a mailbox from the protocol side, and `send_email` is the first whose effect reaches somebody who is not this mailbox's owner: it composes and records a message behind a grant of its own, and the account's delivery pass offers it to a submission server rather than the caller waiting on one. What remains is the rest of writing to a mailbox — moving, filing, and deleting — and answering an existing message rather than starting a new one, which needs the answered message's own identifiers rather than a value a caller supplies. Every later capability takes the route these two took: a named permission, a recorded change, and a pass of its own that owns the mail server.
