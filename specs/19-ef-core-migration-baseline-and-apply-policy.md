...
The baseline is extended rather than regenerated. A model change appends a migration and never deletes, renames, reorders, or regenerates an existing one, because a migration identifier that a database has recorded in `__EFMigrationsHistory` can never be reached again once it is regenerated — that database can only be recreated, destroying what it held. The `add-migration` skill is that workflow, and nothing in the repository deletes a migration.

The additive workflow arrived before the first release rather than with it, which is a departure from what this specification originally scheduled. The reason is that the guarantee has to exist before anything is published, not once something already has been: ADR 0004 establishes a nightly channel built from `main`, and a nightly whose baseline is regenerated between builds cannot be upgraded to the next one at all, because the host fails fast on a migration history it does not recognize. The same argument applies to the local database that #222 made persistent. The cost is that the first release may ship `Initial` plus a small number of incremental migrations rather than exactly one, which is a weaker property than a single reviewed baseline and a much weaker requirement than an upgrade path that works.
