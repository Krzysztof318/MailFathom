...
- Allocate the code as five digits, `C S NNN`: category, subcategory, then the failure's number within that subcategory. Categories are 1 configuration and transport security, 2 mail protocol, 3 persistence, 4 outbound resilience, 5 the MCP boundary, 6 embedding providers, 7 chat providers. Allocate a number once and never reuse or renumber it, the same way an enum member's value is never reordered.
