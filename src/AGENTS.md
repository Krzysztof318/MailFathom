...
These govern every C# file in the repository, not only the ones under `src/`. `tests/AGENTS.md` points here for that reason: a test is C# and is read by the same analyzers, so a rule that held only under `src/` would be a rule the test suite was exempt from.
