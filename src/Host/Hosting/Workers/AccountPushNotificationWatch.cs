...
/// </para>
/// <para>
/// Each watched folder holds a session, and each session holds its own dependency-injection scope for as long as it
/// lives. That is what makes the rotation boundary work: the scope pins the settings snapshot the session connected
/// under, so a newly published snapshot is a session to recycle rather than a value the running session would ignore.
/// A long-lived connection is the one place a rotated password or a revoked token could otherwise stay in use until the
/// process ends.
/// </para>
/// <para>
...
internal sealed partial class AccountPushNotificationWatch : IAsyncDisposable
{
    ...
    /// <remarks>
    /// An account with nothing watched simply waits, which is what every account did before push existed. Otherwise
    /// each watched folder waits in renewal-sized commands until the delay is spent, and the first change ends all of
    /// them — the pass that follows covers every folder of the account, so there is nothing to gain by letting the
    /// others keep waiting.
    /// </remarks>
        ...
        CancellationToken cancellationToken)
    {
        ...
        var watching = this.watchedFolders
        ...
        await Task.WhenAll(watching.Select(entry => this.WaitOnFolderAsync(
            runSettings,
            entry.Key,
            entry.Value,
            delay,
    ...
    public async ValueTask DisposeAsync() => await this.StopWatchingAsync(this.watchedFolders.Keys.ToArray());
    ...
    /// <remarks>
    /// The transition is what an operator needs; repeating the current mode once per interval would bury it. The two
