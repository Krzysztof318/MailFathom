...
/// <remarks>
/// The marker is carried rather than re-derived, because the query that selected the message already answered it and a
/// second question per message would be a second round trip for something the first one knew. Nothing else about the
/// message travels: its subject, its participants, and its text are mail content, and a walk reports counts.
/// </remarks>
public sealed record StoredEmailAwaitingEmbedding(StoredEmailId StoredEmailId, bool RequiresChunking);
