...
/// <summary>States whether a request named a page of the rule history, and what stopped it when it did not.</summary>
public enum MailRuleExecutionQueryOutcome
{
    /// <summary>The request names a page, and the query is present.</summary>
    Accepted = 0,

    /// <summary>The page size asked for lies outside the range the history serves.</summary>
    PageSizeOutOfRange = 1,

    /// <summary>The time range ends at or before it begins, so it names no executions.</summary>
    TimeRangeEmpty = 2,
    ...
    /// <summary>A rule filter was supplied and holds no name.</summary>
    RuleNameBlank = 4,
}
...
/// </remarks>
public sealed record MailRuleExecutionQueryResult
{
    private MailRuleExecutionQueryResult(MailRuleExecutionQueryOutcome outcome, MailRuleExecutionQuery? query)
    {
    ...
    public MailRuleExecutionQueryOutcome Outcome { get; }

    /// <summary>Gets the query, which is present exactly when <see cref="Outcome" /> is <see cref="MailRuleExecutionQueryOutcome.Accepted" />.</summary>
    public MailRuleExecutionQuery? Query { get; }
    ...
    public static MailRuleExecutionQueryResult Accepted(MailRuleExecutionQuery query)
    {
        ...
        return new MailRuleExecutionQueryResult(MailRuleExecutionQueryOutcome.Accepted, query);
    }
    ...
    {
        if (outcome == MailRuleExecutionQueryOutcome.Accepted || !Enum.IsDefined(outcome))
        {
            throw new ArgumentOutOfRangeException(
                nameof(outcome),
                outcome,
                "A refused rule history query must name a declared refusal.");
        }

        return new MailRuleExecutionQueryResult(outcome, query: null);
    }
}
