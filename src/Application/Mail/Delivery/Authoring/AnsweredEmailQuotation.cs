...
internal static class AnsweredEmailQuotation
{
    ...
    /// <see cref="MaximumAttributionCharacters" /> plus the fixed markup, which is what lets the markup alternative be
    /// composed without a cut: a bound applied to markup after it was sanitized would hand back an element somebody
    /// else opened and this system closed nowhere.
    /// </remarks>
    public const int QuotationOverheadReserve = 320;

    /// <summary>The greatest number of characters the attribution line carries.</summary>
    /// <remarks>
    ...
    /// <returns>The author's markup with the quoted message beneath it.</returns>
    ...
    /// structure of somebody else's reply.
    /// </para>
    /// </remarks>
    public static string HtmlBody(string authored, string attribution, string? quotedHtml, string quotedText)
    {
        ArgumentNullException.ThrowIfNull(authored);
        ArgumentNullException.ThrowIfNull(attribution);
        ArgumentNullException.ThrowIfNull(quotedText);

        var quotation = quotedHtml ?? string.Create(
            CultureInfo.InvariantCulture,
            $"<pre>{WebUtility.HtmlEncode(quotedText)}</pre>");

        return string.Create(
            CultureInfo.InvariantCulture,
            $"{authored}<p>{WebUtility.HtmlEncode(attribution)}</p><blockquote>{quotation}</blockquote>");
    }
    ...
    private static string? AuthorOf(EmailContentHeaders headers)
    {
        ...
        return MailTextBounds.TruncateAtTextElementBoundary(
            described,
            MaximumAttributionCharacters - AttributionSentenceReserve);
    }
}
