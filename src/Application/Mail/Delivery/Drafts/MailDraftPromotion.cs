...
public sealed class MailDraftPromotion
{
    ...
    /// <param name="requester">The authored act asking, which is what makes the same promotion twice one delivery.</param>
    ...
    /// one, so a caller that asked twice queues one message. That is a property of the draft rather than of the
    /// idempotency key: the key protects two callers asking together, and this protects one caller asking again after
    /// the first answer never reached it.
    /// </remarks>
        ...
        CancellationToken cancellationToken)
    {
        ...
        var request = OutgoingEmailRequest.Create(draft.AccountId, requester, draft.Recipients);
