...
/// <summary>States whether a request named a page of dead letters, and what stopped it when it did not.</summary>
public enum DeadLetteredJobQueryOutcome
{
    ...
    /// <summary>The type filter names no job type this build declares.</summary>
    JobTypeUnknown = 2,
...
/// </remarks>
public sealed record DeadLetteredJobQueryResult
{
    private DeadLetteredJobQueryResult(DeadLetteredJobQueryOutcome outcome, DeadLetteredJobQuery? query)
    {
    ...
    public DeadLetteredJobQueryOutcome Outcome { get; }

    /// <summary>Gets the query, present exactly when <see cref="Outcome" /> is <see cref="DeadLetteredJobQueryOutcome.Accepted" />.</summary>
    public DeadLetteredJobQuery? Query { get; }
    ...
    public static DeadLetteredJobQueryResult Accepted(DeadLetteredJobQuery query)
    {
        ArgumentNullException.ThrowIfNull(query);

        return new DeadLetteredJobQueryResult(DeadLetteredJobQueryOutcome.Accepted, query);
    }
    ...
    {
        if (outcome == DeadLetteredJobQueryOutcome.Accepted || !Enum.IsDefined(outcome))
        {
            throw new ArgumentOutOfRangeException(
                nameof(outcome),
                outcome,
                "A refused dead-letter query must name a declared refusal.");
        }

        return new DeadLetteredJobQueryResult(outcome, query: null);
    }
}
