...
/// <summary>Decides how long a job waits before it becomes claimable again after a transient failure.</summary>
/// <remarks>
/// <para>
/// This is the queue's own backoff, and it is a different decision from the one an adapter's resilience pipeline makes.
/// The pipeline decides whether one call to a dependency is worth repeating within a single attempt and has already
/// spent that budget by the time a handler raises; this decides when the whole job is worth attempting again, minutes
/// later and in a process that need not be the one that failed. The two never wrap each other, which is what keeps the
/// attempt counts from multiplying into a retry storm.
/// </para>
/// <para>
/// The delay doubles per attempt and is drawn from a range rather than computed exactly. Jobs that failed together
...
public static class JobRetryBackoff
{
    ...
    /// <summary>Computes how long a job waits before its next attempt.</summary>
    ...
    /// <param name="attemptCount">How many attempts the job has already been handed out for, counting from one.</param>
