...
/// <summary>States whether a request named a page of an account's classifications, and what stopped it when it did not.</summary>
public enum SpamClassificationHistoryQueryOutcome
{
    ...
    /// <summary>The time range ends at or before it begins, so it names no classifications.</summary>
    TimeRangeEmpty = 2,
    ...
    /// <summary>The verdict filter names no verdict this system reaches.</summary>
    VerdictUnknown = 4,
}
...
/// </remarks>
public sealed record SpamClassificationHistoryQueryResult
{
    private SpamClassificationHistoryQueryResult(
        SpamClassificationHistoryQueryOutcome outcome,
    ...
    public SpamClassificationHistoryQueryOutcome Outcome { get; }

    /// <summary>Gets the query, present exactly when <see cref="Outcome" /> is <see cref="SpamClassificationHistoryQueryOutcome.Accepted" />.</summary>
    public SpamClassificationHistoryQuery? Query { get; }
    ...
    public static SpamClassificationHistoryQueryResult Accepted(SpamClassificationHistoryQuery query)
    {
        ArgumentNullException.ThrowIfNull(query);

        return new SpamClassificationHistoryQueryResult(SpamClassificationHistoryQueryOutcome.Accepted, query);
    }
    ...
    {
        if (outcome == SpamClassificationHistoryQueryOutcome.Accepted || !Enum.IsDefined(outcome))
        {
            throw new ArgumentOutOfRangeException(
                nameof(outcome),
                outcome,
                "A refused spam classification query must name a declared refusal.");
        }

        return new SpamClassificationHistoryQueryResult(outcome, query: null);
    }
}
