...
// Which of the three reading surfaces a message is drawn on, reached by the components that draw one.
//
...
/**
 * Which rendering an open message is drawn on, which a tree with no provider above it reads as the reduced document.
 *
 * The reduced document is the default for the reason the deployment's own unset answer is: it is what this client has
 * always drawn, and a test or a screen mounted without the frame draws what somebody who has set nothing would see.
 */
export const MessageViewContext = createContext<ClientMessageView>('reduced');

/** Which of the three renderings the message being drawn is shown as. */
export function useMessageView(): ClientMessageView {
