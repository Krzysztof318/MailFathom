...
//
// The four commands below are what ADR 0023 decided this shell would offer, and they are the only reach the application
// has into Rust. They are commands this application defines rather than a plugin's, so the capability system does not
// gate them and no `capabilities/` file exists to add; what does reach them is the Tauri API the WebView is given
// through `withGlobalTauri` in `tauri.conf.json`, which is why this shell pins no JavaScript binding of its own.
//
