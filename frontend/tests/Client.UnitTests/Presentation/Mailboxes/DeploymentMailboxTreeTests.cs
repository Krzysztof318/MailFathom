...
public sealed class DeploymentMailboxTreeTests
{
    ...
    {
        // Arrange
        using var over = await TreeOver.CreateAsync(
            TwoMailboxes,
    ...
    {
        // Arrange
        using var paused = await TreeOver.CreateAsync(
            """{"synchronizationEnabled":false,"accounts":[]}""",
            cancellationToken: TestContext.Current.CancellationToken);
        using var refreshing = await TreeOver.CreateAsync(
            TwoMailboxes,
            cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task Rows_AnOwnerWhoOwnsNoMailbox_IsEmptyRatherThanAFailure()
    {
        // Arrange
        using var over = await TreeOver.CreateAsync(
            """{"synchronizationEnabled":true,"accounts":[]}""",
            cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task Constructor_AMissingService_IsRefused()
    {
        ...
        using var harness = await DeploymentHarness.CreateAsync(
            _ => StubTransport.JsonResponse(TwoMailboxes),
            cancellationToken: TestContext.Current.CancellationToken);
    ...
    private sealed class TreeOver : IDisposable
    {
        ...
        internal static ValueTask<TreeOver> CreateAsync(
            string answered,
            RememberedMailboxes? remembered = null,
            CancellationToken cancellationToken = default) =>
            CreateAsync(_ => StubTransport.JsonResponse(answered), remembered, cancellationToken);
