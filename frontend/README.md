...
question, their scope, and their selection across. **Mail**, **Calendar**, and **People** are built —
`src/routing/spaces.ts` names the three in `implementedSpaces` — and the other four are present, named as placeholders,
and say in a sentence that there is nothing behind them yet. They are drawn rather than hidden because the design
project is what the client is measured against and it shows all seven; a rail with three destinations would be a
different product from the one that was designed. A built space is also the only kind that is reached under a grant:
**People** is offered to a credential that may read the address book, **Calendar** to one that may read mail — which is
the grant the calendar routes themselves are published under — and a placeholder is reached by anybody, which is why a
screen is not.
