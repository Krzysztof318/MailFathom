...
| What the webview may load, connect to, and submit to                | `app.security.csp` merged in by `src-tauri/run-tauri.ts`: the directives both heads share, from `src/Client.App/contentSecurityPolicy.ts`, and the head's own `connect-src`                            |
...
| What the system bars leave the page | The window-insets listener in `MainActivity.kt`                                                         | The status bar, the navigation bar, a display cutout and the keyboard are padded off the content view. Android's WebView answers `env(safe-area-inset-*)` for a cutout alone, so the tokens the page already reads report nothing for the bars a full-bleed window puts it under |
...
states, a conversation, a search, a notification centre, and a question answered on the Discover screen — the run is
started, followed and stopped against the corpus exactly as against a deployment — with no database, no mail server and
no service running.
...
mailfathomFixtures.emptyCollections = true; // the empty folder, the search that found nothing, the empty centre, the run that composed nothing
...
question, their scope, and their selection across. **Discover**, **Mail**, **Tasks**, **Calendar**, and **People** are
built — `src/routing/spaces.ts` names the five in `implementedSpaces` — and the other two are present, named as
placeholders, and say in a sentence that there is nothing behind them yet. They are drawn rather than hidden because the
design project is what the client is measured against and it shows all seven; a rail with five destinations would be a
...
mail — which is the grant the calendar and task routes themselves are published under — **Discover** to one that may ask
questions of it, and a placeholder is reached by anybody, which is why a screen is not. **Tasks** asks a second question of its own rather than being reached under a
