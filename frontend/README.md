...
| What the webview may load, connect to, and submit to                | `desktopHeadDirectives` in `src/Client.App/contentSecurityPolicy.ts`, beside the web head's, merged in by `src-tauri/run-tauri.ts` as `app.security.csp`                                               |
