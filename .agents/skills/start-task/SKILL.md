...
8. Place the issue. It carries exactly one `type:*` label, a `Track` and a `Queue` value on the board, a milestone when the rule in root `AGENTS.md` assigns one, and a `Size` value once the work is planned. Decide each from the rules rather than asking. `Queue: Next` is never one of them: the owner chooses it, and `$finish-change` writes it once the pull request exists, so a new issue takes `Later`, `Needs decision`, or `Parked`. Verify the values landed, because the built-in workflows set `Status` and nothing else, and an unplaced issue disappears from the views the owner reads.
