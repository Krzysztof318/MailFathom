...
- **Client UI evidence.** A change that edits XAML or styles under `frontend/` carries a Uno App MCP screenshot and a visual-tree snapshot of the affected screen, taken in the session that wrote the change. That evidence is not a test and does not stand in for the authored-view binding assertions in `Client.UnitTests`. A change that only moves a model, a string, or a converter owes none.
