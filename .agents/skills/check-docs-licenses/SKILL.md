...
- `.editorconfig` holds the three-line `file_header_template`: the copyright, the grant, and the repository URL that tells a reader who meets one file outside this checkout where the project lives. IDE0073 enforces it on every C# source file and reaches nothing else, so the same three lines are written by hand into every `.yml`, `.yaml`, and `.sh` file, into the Helm templates as a `{{- /* ... */ -}}` comment that renders to nothing, and into each `SKILL.md` as `license` and a `metadata` block. `scripts/test-agent-workflow.sh` compares all four against this template, which is what keeps them one header rather than four that merely resemble each other. Changing the template therefore rewrites the whole repository twice over — one whole-solution `dotnet format` pass for the C# files and a hand pass over the rest — so treat it as a repository-wide change rather than a formatting tweak, and never hand-edit a C# header.
