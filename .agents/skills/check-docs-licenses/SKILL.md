...
MailFathom is licensed under the GNU Affero General Public License, Version 3, SPDX identifier `AGPL-3.0-only`. It carried Apache-2.0 until 2026 and the move is recorded in ADR 0025, which is also where the reason the `-only` arm was taken over `-or-later` lives. That decision is recorded in five places and each one has a single job, so a change that touches licensing keeps them consistent rather than picking whichever is nearest.

- The root `LICENSE` is the unmodified official AGPL-3.0 text as the Free Software Foundation publishes it at `https://www.gnu.org/licenses/agpl-3.0.txt`. Never edit it, never append attribution, third-party terms, or commentary to it: GitHub detects the license by matching that file against the known text, and an edit turns a detected `AGPL-3.0` into `NOASSERTION`.
- The root `NOTICE` names Krzysztof Kasprowicz as the original author, which AGPL-3.0 section 7(b) permits a distribution to require be preserved, and the repository the project lives in, for the reason the file header below carries the same URL — and which is also where a recipient of a binary goes for the source section 13 talks about: `NOTICE` travels beside the binaries into the publish output and the container image, so it is often the only thing a reader has to work back from. It stays informational — it adds no use restriction, changes no license term, and claims nothing about contributions by other copyright holders. Third-party notice text may join the distributed bundle beside it, never inside it.
...
- The deployment assets state the same identifier where their own ecosystems read it: `org.opencontainers.image.licenses` in `deploy/docker/Dockerfile` and `artifacthub.io/license` in the chart's `Chart.yaml`. Those are claims about terms rather than the terms themselves, and nothing asserts either one mechanically, so a change touching either is read: check that both still name `AGPL-3.0-only` and that the build context still admits `LICENSE` and `NOTICE`. A label that outlived the files it names is the failure worth catching, and the publish check above is what catches the files' half of it.

`README.md` and `CONTRIBUTING.md` carry none of those jobs and are still written to match them. Each explains the decision to a reader rather than recording it — that `CLA.md` is what places a contribution under the license, the AGPL carrying no clause of its own that does, that contributors keep their copyright either way, that the header the `.editorconfig` template applies is one project's mark rather than a claim about who wrote a line, that section 13 asks an operator running a *modified* version over a network to offer its source, and that sections 15 and 16 give the software with no warranty and no liability. The last two are there because `README.md`, `deploy/docker/README.md`, and `deploy/helm/mailfathom/README.md` are the files rendered outside the repository, where a reader who never opens `LICENSE` would otherwise meet the grant without the obligation or the disclaimer; it summarizes and points at the text, and never restates a term as though the summary were operative. A change to the five above therefore reads both, because prose that contradicts the record is what a contributor will act on.
...
**MailFathom being AGPL-3.0-only relaxes none of that**, and the reading that it does is the one this paragraph exists to refuse. What the project grants for its own code is the owner's to change — the contributor licence agreement is what keeps it changeable — and what a dependency grants is not. A copyleft dependency would end the closed-source arm above permanently and could not be undone by a later decision, so the acceptance policy is read against the closed-source model rather than against the licence this repository currently carries.
