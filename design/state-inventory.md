...
**Mirror stamp `0137d9bdb81209a7`** — the etag set this was extracted from. Run
...
|---|---|
| `MailFathom Prototype.dc.html` | The whole signed-in application: seven screens, every overlay, every composition. 9 450 lines, and the only file that carries navigation |
...
`FRAMES` (line 3588) is the design's own four sizes: `phone` 390 × 844, `fold` 884 × 832,
...
Which blocks a run produces is the plan, and there are three (`PLANS`, line 3162): `contract`
...
| Side list resizer | `showSideResizer` | `!drawer && !sideCollapsed && !isMobile` (line 7764) — drag widens the folder pane between 210 and 420 px, double-click returns it to 210 |
...
| Menu on a folder row | `folderMenu` (line 5359) | right-click a folder, or hold it — *Mark all as read* always, *New folder inside* while the path is under three segments, and *Edit folder* and *Delete folder* only on a folder the user made; **All accounts carries none** |
| New folder row | `g.canAdd` (line 7785) | drawn at the end of every account group as a `create_new_folder` row reading *New folder* — `g.key !== "all"`, so the unified view carries none, having no mail server of its own; a collapsed side list draws the glyph alone (`g.expanded`), and the row takes the phone's taller padding under `isMobile` |
...
(`STD_FOLDERS`, line 7014) — and a count only where the mock data gives one. **The unified view
...
| State | Flag | Reveal |
|---|---|---|
| The folder tree | `st.folderOpen[<mailbox·path>]` | a folder with children draws a twisty; each level is indented 15 px, and **three path segments is the ceiling** (`MAX_DEPTH`, line 5372) |
...
| New folder dialog | `newFolderOpen` (line 8594) | the *New folder* row at the end of an account group, *New folder inside* on a folder, or *New folder here* in the move sheet |
...
| Where it may sit | `nfLocations` (line 8628), from `folderLocations` (line 5378) | the select's options: *Top level of the account* first, then the account's folders drawn as `Parent / Child`. Editing one drops the folder itself, everything under it, and any parent too shallow to hold its subtree |
...
| The name is taken | the `warning` notification *Pick another name* (line 5414) | saving an edit onto a name already used in that place — *There is already a folder called “<name>” inside …* |
| Delete confirmation | `delFolderOpen` (line 8598) / `delFolderText` (line 8602) | *Delete folder* — four outcomes computed from the state rather than one sentence |
...
| State | Flag | Reveal |
|---|---|---|
| AI summary | `aiSumOpen` (line 8555) | the `auto_awesome` control on a message row, or *AI summary* at the top of that row's context menu |
| Its cards | `aiSumCards` (line 5585) | **three at most**, each a heading, the claim, a *Why:* line, a model chip reading *Model — mailfathom-email-enrichment*, and the fragment of the message it rests on |
...
| A thread being dragged | `st.dragIds` (`dragThreadsStart`, line 5519) | a row is `draggable` and starts a drag; the dragged rows go to `opacity:0.45` while it lasts, and a drag started on a row inside a selection of more than one carries **the whole selection** rather than that row |
| A folder under the drag | `st.dragOverKey` (`dragOverFolder`, line 5530) | the pointer over a folder row in the side list while `dragIds` is set — that row draws the accent-soft background, the accent text and an `inset 0 0 0 2px var(--accent)` ring, which is the only affordance saying a drop lands here |

Dropping is a move and nothing new: `dropOnFolder` (line 5536) hands the ids to `moveThreads`,
which now takes them as an argument (line 5507) rather than reading `moveFor`, so a drag and the
...
| Context chip | `agentCtxShown` (line 8795) | the agent was opened from a screen that carries context, **or** the open conversation carries its own `ctx` and it was not cleared |
...
| An answer still being worked | `m.working` (line 4633) | the message carries `working` — it draws an `autorenew` glyph and *The agent is going through your mail…* instead of prose |
| An answer carrying blocks | `m.hasBlocks` (line 7460) | the message carries `blocks` |
| Back to the conversation | `convReturnShown` (line 8799) | a block's `nav` row was pressed, which is what `goLook` (line 6482) records in `agentReturn`; the pill stands fixed over whatever screen was opened until `backToConv` (line 6487) is pressed |
...
An answer is not prose with buttons under it. `bkView(b, key, isMobile)` (line 6227) draws one block,
...
| Accepted | `accepted` | `acceptBk` (line 6451) — the controls go, a `check_circle` chip reads *done*, and one result line states what happened and at what time |
| Accepted but not done | `failed` | the block carries `failed` — an `error` chip reading *not done*, and *Try again* as the only control |
| Declined | `isDeclined` | `declineBk` (line 6462) — the card collapses to a dashed outline holding one struck line, *Declined — nothing was done*, and its `nav` rows go with it |
...
| Error | `isError` | `state` is `error` — `errorText` and a *Try again* control that `retryBk` (line 6463) puts back to `ready` |
| Editing a draft | `editing` | *Edit here* — `editBk` (line 6465) swaps the paragraphs for a `textarea` inside the conversation, and the controls become *Send* and *Stop editing* |
...
`<conversation>#<message>#<block>` (`setBk`, line 6445), which is what lets the seeded histories in
`SEED_CONVS()` (line 4486) stay immutable and lets the same block object be proposed twice.
*Another time* is the case that shows why: `anotherTimeBk` (line 6471) declines the block, writes the
...
line 7428). Seven are seeded, and two exist for states nothing else reaches: `c-states`, whose blocks
came back loading, error, empty, partial and failed, and `c-invoice`, which is archived.

### States the blocks screen holds and the prototype does not

`MailFathom AI Blocks.dc.html` draws the catalogue rather than a conversation, so it is where a
phase is read against its siblings. Thirteen `PROPOSAL_SPECS` cards (line 417 of that file) stand
under the heading *Proposal blocks — the Agent's three phases* (line 319), two to a row, one per
...
| Settings: its five tabs | `tabProfil` / `tabAccounts` / `tabAi` / `tabNotif` / `tabApp` | the tab strip — *Profile*, *Accounts*, *AI*, *Notifications*, *Application* (`settingsTabs`, line 9156), `tabProfil` being the default |
...
| Unread notifications | `notifHasUnread` | the unread count; it gates three regions including the rail badge. The panel's own badge is one composed string, `notifNewLabel` (line 8655), reading *1 new* or *N new* |
| **No notifications** | `notifEmpty` | the shown list is empty — reachable by filtering to unread after reading them all |
| Refresh | `refreshAll` (line 6061) / `refreshBusy` | the `refresh` control in the rail, titled *Refresh*, and its floating copy at the phone's bottom-left (line 209) — it re-reads the mail in front of the reader and the notification centre, pulses its glyph (`mfpulse`, 1 s) rather than spinning while it waits, and refuses a second press until the re-read lands |
| Move dialog | `moveOpen` | the move action on a selection — grouped by mailbox (`moveGroups`, line 8570), each group ending in *New folder here*, and the toast it produces names the mailbox |
| Permanent delete | `confirmDel` under `inTrash` | a delete action while the open folder is a Trash (line 5311) — *Delete permanently*, no undo offered, and the toast reads *Permanently deleted* rather than *Moved to trash* |
| The rail's nav scrolls | `railNavStyle` (line 7233) | a viewport too short for the seven destinations — the nav scrolls and the bottom cluster stays pinned |
...
*Aplikacja* (line 9156, now *Profile* and *Application*, with *Accounts*, *AI* and *Notifications*
standing between them), the user menu's sign-out item *Wyloguj* (line 143, now *Sign out*), and the
single-pane state toggle beside the thread head, *ukryj* (line 8538, now *Hide panels —
...
(`settingsResizable` / `settingsResizeDown`, line 9134) that drags it between 560 × 420 and the
viewport less 40 px. **The grip exists on two panes only**, so a phone or fold capture draws none.

The card also **moves**. Its header is the handle: `settingsHeadDown` (line 9105) is `null` on one
...
| State | Flag | Reveal |
|---|---|---|
| The account list | `acctListShown` (line 9264) | the *Accounts* tab with no draft open — one row per account carrying a colour dot, the display name, the address, `IMAP host:port · SMTP host:port` and `chevron_right`, under a `MAIL ACCOUNTS` caption beside `acctCount`, and under the rows the sentence that settles what this tab is: *Accounts are set up by your administrator — here you can open one and change its settings.* |
| The account editor | `acctEditShown` | a row. `acctTitle` (line 9275) reads *Edit account* and nothing else, there being no second way in |
| The address is fixed | the `E-MAIL ADDRESS` input | it is `readOnly` with `tabIndex` `-1`, drawn on `--bg` rather than `--sub`, under *The address identifies the account and cannot be changed here.* (line 2624) |
...
| Connection test refused | `acctTestBad` (line 9302) | *Test connection* while a host, a port, a login or a password is missing — it names every field still empty |
| Connection test passed | `acctTestOk` | *Test connection* with all six filled — it names the two hosts |
| An earliest date set | `acctEarliestClearShown` / `acctEarliestHint` (line 9312) | the date field — the hint states that older mail stays on the mail server, and reverts to *No limit* when cleared |
| The folder mapping | `acctFolders` (line 9319) | six rows pairing Inbox, Sent, Drafts, Archive, Spam and Trash with `INBOX`, `Sent`, `Drafts`, `Archive`, `Junk` and `Trash` |
| The three switches over what it deletes and sends | `acctHardDelete` / `acctPurgeGone` / `acctSaveSent` (line 9324) | each is on by default and each carries a hint that changes with its position — hard delete states that a message goes from both servers with nothing left to restore |
| The secret check | `acctSecretScan` / `acctSecretScanWarn` (line 9287) | the account editor's own switch, under `BEFORE SENDING TO THE MODEL — THIS ACCOUNT ONLY` (line 2743), on by default; while it is on, a warning states the check is best-effort and can both miss a secret and hide ordinary text |
| The AI language | `aiLangOptions` (line 9253) / `aiMatchReply` | *AI* — English or Polski, **English first and English selected** (`st.aiLang` falls back to `"en"`), separate from the interface language, with a note that a change applies only to content written from now on, and *Match the language of the message* beneath it. `langOptions` (line 9365), the interface language in the account menu, is drawn and defaulted the same way |
| Notifications cleared on read | `notifAutoClear` (line 9247) | the *Notifications* tab's switch; while it is on, a `REMOVE` select offers Right away, After 1 hour, After 24 hours and After 7 days |
| Notified on this device | `notifDevice` (line 9124) | the *Notifications* tab's `THIS DEVICE` group (line 2807), off by default — *When the window is not in front of you, this device tells you how many things arrived and of what kind — never from whom, never what it is about.* |
| How long one stays up | `notifDuration` / `notifDurationLabel` (line 9127) | the range beneath it, 1 to 30 whole seconds and 5 by default, drawn as `<n> s` in the accent; its copy ties the value to the undo window, which *waits exactly as long as its notification is up* |
| The message view | `viewAiStyle` / `viewSimpleStyle` / `viewHtmlStyle` (line 9185) | *Application* — a `MESSAGE VIEW` strip of three (line 2836), *AI simplified*, *Simplified* and *Original* in that order, each drawn by one `segStyle` helper (line 3625) that is the only place the segment's own styling lives |
| Images loaded automatically | `autoImages` (line 9165) | *Application* — while it is on, a warning states the image is fetched from the sender's server and is how tracking pixels work |
| Layout reset | `resetLayout` / `resetLayoutDone` (line 9240) | *Reset layout* under `LAYOUT ON THIS DEVICE`; the confirmation *Layout restored to the defaults.* stands for 3 200 ms and then goes |
...
`shield` note (line 2777) reading that the check *is set separately for each mail account — open
...
scripted batch out of three (`DEMO_BATCHES`, line 4887) — a message lands at the top of the list and
...
| A row moved to a folder | `rowAnim[<id>] === "move"` | `mfrowmoved` — 900 ms, a warning wash that comes up and fades back out; the row stays, so nothing collapses (line 5514) |
...
the wash has finished (`animateRowsOut`, line 6048). The wash is drawn on the row rather than on the
wrapper that collapses (`rowWash`, line 6805), which is what lets the two run at different lengths.
...
warning colours behind the row as it is dragged (`swipeBgStyle`, line 8863) rather than the success
...
- **The application with no mailbox at all.** `const emptyApp = false;` (line 7008) is a constant, and
