...
| `docs/operations/issue-tracking.md` | `$start-task` opens or places an issue, `$finish-change` links one | Which work needs an issue, what its body contains, the one `type:*` label, the milestone, the board's `Area`, `Queue`, and `Size` fields and its views, and how an arrival from outside the project is triaged |
...
**The owner's checkout** is where `origin` is `Krzysztof318/MailFathom` and project `4` answers. Everything here applies, including the rules that exist nowhere else: the `agent/<short-description>` branch name, the linked worktree, the `type:*` label and the milestone on an issue, and pushing to this repository. The board's `Area`, `Queue`, and `Size` fields are always written here and sometimes elsewhere, for the reason the next paragraph gives.
