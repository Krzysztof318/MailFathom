...
- Use `scripts/verify-deployment-assets.sh` for any change to `Dockerfile`, `.dockerignore`, or `deploy/`. It reads the files rather than running anything, so it costs seconds: it rejects a floating base-image tag, an image stage that never drops to an unprivileged account, a Compose file that reaches the nightly registry, a chart that renders differently twice, and a values schema that has stopped rejecting what must never install. It needs a Docker daemon and takes `helm` from a pinned container image when none is installed.
- Use `scripts/smoke-deployment.sh compose` or `scripts/smoke-deployment.sh kubernetes` when a deployment change has to be proven end to end. It starts the real deployment, so it costs minutes; the Kubernetes half additionally needs `kind`, `kubectl`, and `helm`. Both are absent from `verify-fast.sh` and `verify-full.sh` on purpose, and the `Deployment assets` job of the CI workflow runs them.
