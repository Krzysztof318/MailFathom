...
| `docs/operations/issue-tracking.md` | `$start-task` opens, places, or claims an issue, `$finish-change` links one | Which work needs an issue, what its body contains, the one `type:*` label, the `agent:claimed` marker a session applies when it takes the issue, the milestone, the board's `Area`, `Queue`, and `Size` fields and its views, and how an arrival from outside the project is triaged |
...
**The owner's checkout** is where `origin` is `Krzysztof318/MailFathom`. Everything here applies, including the rules that exist nowhere else: the `agent/<short-description>` branch name, the linked worktree, the `type:*` label, the `agent:claimed` marker and the milestone on an issue, and pushing to this repository. The board's `Area`, `Queue`, and `Size` fields are always written here and sometimes elsewhere, for the reason the next paragraph gives.
...
- Write the licensing header by hand in every file that is not C#. IDE0073 reads C# only, so `dotnet format` and `scripts/verify-fast.sh` apply `.editorconfig`'s `file_header_template` to `.cs` files and to nothing else — a new workflow, script, Helm template, Quadlet unit source, skill, or documentation-site asset gets no header from either, and neither says so. A new file carries the same three lines in the form its own readers parse: `# ` comment lines opening a `.yml` or `.yaml` file or a systemd unit source under `deploy/quadlet/`, the same comment lines under the shebang of a `.sh` file, a `{{- /* ... */ -}}` comment in a Helm template so the header stays out of the rendered manifest, `license` plus a `metadata` block naming the author and the repository in a `SKILL.md` frontmatter, `// ` lines opening a `.js` module, and one `/* ... */` block opening a `.css` file, which has no line comment to use instead. `scripts/test-agent-workflow.sh` fails when one is missing and reads the expected text out of `.editorconfig`, so the header is one decision recorded in one place and applied in six forms.
...
- Never invoke `dotnet format` by hand. Both of its modes already run where they belong — the fast loop repairs the files the branch changed, and the full gate verifies them without touching anything. Neither one has to *report* a style defect, because the Release build in front of both already does: `EnforceCodeStyleInBuild` and `TreatWarningsAsErrors` turn every IDE rule `.editorconfig` sets to `warning` into an error naming its file and line, so a diagnostic with no code fix fails the build rather than surviving to a formatting pass. What the repairing pass adds is the part no build reports and only a rewrite fixes: the ordering of using directives and a missing final newline. Fix what the build named, then run `scripts/verify-fast.sh` again.
...
- Review a change under `deploy/` by reading it. No script here deploys anything, and `helm lint` and `helm template` against `deploy/helm/mailfathom/ci/*-values.yaml` are the useful local reading, as is `QUADLET_UNIT_DIRS=deploy/quadlet /usr/lib/systemd/system-generators/podman-system-generator --user --dryrun`, which prints the units Quadlet would generate from `deploy/quadlet/` and reports a key it does not recognize; nothing gates on any of them.
