...
- The explicit deployment step is one idempotent SQL file per release, `mailfathom-schema-<version>.sql`, and `scripts/build-schema-artifact.sh` is what produces it. It comes from `aspire publish`, which reads the `PublishAsMigrationScript` declaration in the app model — never from a `dotnet ef migrations script` invocation written into a pipeline, which would be a second statement of which context, which migrations project, and which options, able to drift from the first without anything noticing. The release workflow blocks the image push when the artifact cannot be generated. `docs/operations/database-schema.md` records the apply path and why a command in the service image, a migration bundle, a Helm hook Job, and an `initContainer` were each refused; do not reopen one of those without answering the reason recorded there.
...
- Read `docs/AGENTS.md` before changing documentation. Create or modify ADRs only with explicit owner approval; `docs/decisions/` is a protected path, so a pull request that touches an ADR is refused unless the owner authored it.
- An ADR whose `status` is `accepted` is closed, and owner approval does not reopen it. Its text is never edited — not to correct it, not to extend it, not to bring it up to date with what the code now does. The record states what was decided and on what reasoning, and every change made since was written against that text; rewriting it replaces the reasoning retroactively and leaves nothing saying it ever changed. A decision that turns out to be wrong is replaced by a **new** ADR, and the old one is marked `superseded` with a pointer to its replacement. `proposed` is the status in which an ADR is still being written and is editable; `accepted` ends that.
- Exactly two edits are permitted on an accepted ADR. The `status` transition to `superseded` and the pointer that goes with it, written by the change that supersedes it — that is how superseding is recorded rather than an exception to the rule. And its `<!-- describes: ... -->` marker, which declares where the code the ADR decided about now lives; it says nothing about the decision, and letting it rot would mean an ADR describing a path that no longer exists, which is the failure `scripts/test-agent-workflow.sh` refuses.
- Every link in the repository-root `README.md` is absolute, including the ones that point back into this repository and the two that point at its own headings. That file is the one piece of documentation rendered outside the repository — a container registry description, a chart listing, a package page, anything that copies the Markdown — and a relative path resolves against the wrong root everywhere except GitHub's own blob view, so it arrives as a broken link rather than as a link to somewhere else. Repository files link as `https://github.com/Krzysztof318/MailFathom/blob/main/<path>`, a directory as `.../tree/main/<path>`, an image at its `raw.githubusercontent.com` URL, and a heading on the README itself as `https://github.com/Krzysztof318/MailFathom#<anchor>`. This applies to the root `README.md` alone: pages under `docs/` are read inside the repository and keep relative links, which is what lets a directory be moved without rewriting every reference into it.
...
- Use `scripts/review-obligations.sh` to see what a change obliges elsewhere in the repository: the tests naming each changed type, the pages whose `describes:` marker covers each changed path, and the registers whose trigger moved. It is the same index `Fathom review` runs on a pull request, reached through an adapter that hands it a local diff rather than a second implementation, so a rule cannot hold in review and lapse in the pipeline. It reports and never gates, and nothing it prints is a finding until it is confirmed in the file it points at — the behavior no test reaches, the sentence that stopped being true, the row that is missing. `$review-change` and `$check-docs-licenses` both work from it.
...
- The `Typo check` workflow spell-checks the files a pull request changes and annotates each finding in the Files changed view. It runs on every pull request that is not a draft, gates nothing, and is not in the `main` ruleset. What it flags is usually a typo to fix, and the exception is what `_typos.toml` at the repository root exists to record: that file separates accepted vocabulary — `unparseable`, `requeueing`, `HashiCorp` — from the deliberate misspellings the strict-binding tests and their documentation are about, where `Directroy`, `Enabeld`, `Authentcation`, `Passwrod`, and `MaxAttemps` are the misspelling the rule catches rather than a mistake to correct. Never "fix" one of those: it deletes the example and, in a test, the assertion with it. Adding a row means saying which of the two kinds it is and why, and a genuine typo that is merely inconvenient to fix belongs in neither.
- The `Fathom review` workflow reviews a published pull request on its own and spends Claude subscription usage doing it. It reports no status check and gates nothing, so it is never something to trigger as part of a task: `$review-change` is the review an agent performs, and a comment line beginning with `fathom-review` or `@fathom-review` is the owner's way of asking for another. The phrase only triggers a run when it leads a line, so writing it mid-sentence in prose about the workflow is safe and is how to refer to it. A published pull request is reviewed again on every push, so the verdict describes the head that will merge, and merging or closing it cancels a review still in flight rather than paying for a verdict on a change that has landed; a draft is skipped by design, and a comment is what reviews one anyway. A re-review waits for the pull request's conversation to go quiet before it reads anything, so replying to the previous pass's threads and pushing the fix in the same minute is safe in either order — the run collects the answers rather than reporting the finding again as unanswered. Those automatic re-reviews stop at ten on one pull request, past which a label or a comment is what asks for another. What it posts arrives as the `Fathom reviewer` GitHub App rather than as `github-actions`, and a run that found nothing approves the pull request with `APPROVED` and its summary as the body — so an approval from that App is a clean pass rather than a merge decision. A run that found something opens its body with `NEEDS CHANGES` instead, which is a heading and not a review state: it never satisfies the `main` ruleset, which requires an approving review from a code owner, and it never requests changes. The review reads what the change says about itself as well as what it does: the pull request body and every issue that body closes are judged against the diff the way a line of documentation is, because the body becomes the merge commit's message and merging closes each of those issues whether or not the change finished them. A body claiming behavior the diff does not have, a body claiming verification that did not happen, a diff doing something substantial the body never mentions, and an acceptance item of a closed issue the change does not meet are each a finding; how the body was written is not one. The review is not confined to the diff in the other direction either. A step indexes what the change obliges the rest of the repository to do — the tests that name a changed type, the pages whose `describes:` marker covers a changed path, the registers whose trigger moved — and the reviewer confirms each row against the file it points at before it becomes a `P2` anchored to the changed line that created the obligation. That index is where a page under `docs/` earns its marker: `docs/AGENTS.md` states the convention, and `scripts/test-agent-workflow.sh` fails a page carrying none and a marker naming a pattern that matches nothing, so a documented class that is deleted fails the build rather than leaving the page to be found stale. `docs/operations/agent-workflow.md` describes what the run may touch, why it never checks out the branch it reviews, why its `pull_request_target` trigger is the one granted exception to the rule that forbids it everywhere else, what the index does and does not assert, what bounds the subscription usage it spends, and how the App is provisioned.
