...
- The `Claude review` workflow is the owner's to start, for the same reason. It is a manual dispatch that spends Claude subscription usage to post a second review on one pull request, next to the Codex review that runs on its own. It reports no status check and gates nothing, so never dispatch it as part of a task; `$review-change` is the review an agent performs. `docs/operations/agent-workflow.md` describes what it does and what it refuses to do.
