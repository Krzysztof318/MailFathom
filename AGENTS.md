...
- The `Claude review` workflow reviews a published pull request on its own and spends Claude subscription usage doing it. It reports no status check and gates nothing, so it is never something to trigger as part of a task: `$review-change` is the review an agent performs, and a `claude-review` comment is the owner's way of asking for another. A draft is skipped by design, which is why marking a pull request ready is what starts the review, and a later push starts nothing: pushes run the required checks only. `docs/operations/agent-workflow.md` describes what the run may touch and why it never checks out the branch it reviews.
