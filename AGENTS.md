...
- Record a user-visible change in `CHANGELOG.md` in the change set that causes it. What earns an entry is what a consumer of a release would notice: anything reaching one of the four surfaces [ADR 0004](docs/decisions/0004-versioning-and-release-policy.md) versions — the MCP tool contract, the configuration schema, the database schema, the deployment contract — plus a defect that was observable from outside and anything with a security consequence. A refactor, a test, a continuous-integration adjustment, a documentation edit, and an internal rename earn none, and adding a line for one is the failure the rule guards against: a file recording them stops being read and then stops being written. `$check-docs-licenses` carries the obligation, because no gate can tell a user-visible change from a refactor.
...
- Read the version with `scripts/read-declared-version.sh` rather than retyping it. `<VersionPrefix>` in `Directory.Build.props` is the only place a version number is written, every project derives its four version properties from it centrally, and it names the *next* release rather than the last one. Anything that has to put that number somewhere — an image tag, an OCI label, the chart's `appVersion` — reads it from there, and `scripts/verify-deployment-assets.sh` fails a copy that drifted. `docs/operations/release-procedure.md` records the whole scheme.
- Use `$prepare-release` to cut a release. It reads the version rather than asking, refuses a dirty tree, an empty `Unreleased`, and a version already tagged, opens the changelog and version-bump pull requests as drafts, and prints the order the two merges and the tag land in. It pushes no tag: tagging is what makes a release real and stays the owner's act.
...
- Use `scripts/verify-deployment-assets.sh` for any change under `deploy/`. It reads the files rather than running anything, so it costs seconds: it rejects a floating base-image tag, an image that never drops to an unprivileged account or that carries a schema tool, a Compose file that reaches the nightly registry, a chart that renders differently twice, a values schema that has stopped rejecting what must never install, an image or chart that has stopped stating the project's license or carrying the files that hold it, and a version that has drifted between the declared prefix, the chart's `appVersion`, and the build paths that name the image. It needs a Docker daemon and takes `helm` from a pinned container image when none is installed.
- Use `scripts/smoke-deployment.sh compose` or `scripts/smoke-deployment.sh kubernetes` when a deployment change has to be proven against a running deployment. It starts the real thing, so it costs minutes; the Kubernetes half additionally needs `kind`, `kubectl`, and `helm`.
- Neither deployment script belongs to a pull-request check. Both are absent from `verify-fast.sh` and `verify-full.sh`, and the `Deployment assets` workflow that runs them — together with the two-architecture image build — is manual dispatch only, like the integration suite. Nothing in it publishes: the release channels are #156's.
...
- The `Fathom review` workflow reviews a published pull request on its own and spends Claude subscription usage doing it. It reports no status check and gates nothing, so it is never something to trigger as part of a task: `$review-change` is the review an agent performs, and a comment line beginning with `fathom-review` or `@fathom-review` is the owner's way of asking for another. The phrase only triggers a run when it leads a line, so writing it mid-sentence in prose about the workflow is safe and is how to refer to it. A published pull request is reviewed again on every push, so the verdict describes the head that will merge; a draft is skipped by design, and a comment is what reviews one anyway. What it posts arrives as the `Fathom reviewer` GitHub App rather than as `github-actions`, and a run that found nothing approves the pull request with `APPROVED` and its summary as the body — so an approval from that App is a clean pass rather than a merge decision. A run that found something opens its body with `NEEDS CHANGES` instead, which is a heading and not a review state: it never satisfies the `main` ruleset, which requires an approving review from a code owner, and it never requests changes. `docs/operations/agent-workflow.md` describes what the run may touch, why it never checks out the branch it reviews, and how the App is provisioned.
